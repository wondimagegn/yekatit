...
class ExamResultsController extends AppController {
	...
	var $menuOptions = array(
			'parent' => 'grades',
			'exclude' => array('index', 'edit', 'delete', 'view', 'get_exam_result_entry_form', 'get_exam_result_view_page', 'cancel_grade_change_request',
	...
	function beforeFilter() {
		parent::beforeFilter();
		$this->Auth->Allow('get_exam_result_view_page',
	...
	function beforeRender() {
		if($this->role_id==ROLE_INSTRUCTOR){
			$acyear_array_data = $this->ExamResult->ExamType->PublishedCourse->CourseInstructorAssignment->find('list',array('conditions'=>array(
		...
		if(isset($this->request->data['ExamResult']['published_course_id'])) {
			...
			for($i = 0; $i <= (count($students) + count($student_adds) + count($student_makeup)); $i++) {
				if(isset($this->request->data['grade_change_request_'.$i])) {
					$exam_grade_detail = $this->ExamResult->CourseRegistration->ExamGrade->find('first', 
						array(
							'conditions' => 
								array(
					...
					*/
					...
					$published_course_department = $this->ExamResult->CourseRegistration->PublishedCourse->find('first',
						array(
					...
					$makeup_exam_detail = $this->ExamResult->ExamType->PublishedCourse->CourseRegistration->MakeupExam->find('first',
						array(
							'conditions' => 
							array(
								'OR' => 
					...
					debug($exam_grade_detail);
				        if ((!isset($exam_grade_detail['CourseRegistration']) || empty($exam_grade_detail['CourseRegistration']) || $exam_grade_detail['CourseRegistration']['id'] == "" || $published_course_id == $exam_grade_detail['CourseRegistration']['published_course_id'] || $instructor_id == $instructor_id_for_checking || $this->department_id == $published_course_department['Section']['department_id'] || (empty($published_course_department['Section']['department_id'])
							&& $this->college_id == $published_course_department['Section']['college_id'])) &&
						(!isset($exam_grade_detail['CourseAdd']) || empty($exam_grade_detail['CourseAdd']) || $exam_grade_detail['CourseAdd']['id'] == "" || $instructor_id == $instructor_id_for_checking || $published_course_id == $exam_grade_detail['CourseAdd']['published_course_id'] || $this->department_id == $published_course_department['Section']['department_id'] || (empty($published_course_department['Section']['department_id']) && $this->college_id == $published_course_department['Section']['college_id'])) &&
						(($exam_grade_detail['CourseRegistration']['published_course_id'] != "" && $published_course_id == $exam_grade_detail['CourseRegistration']['published_course_id']) ||
							($exam_grade_detail['CourseAdd']['published_course_id'] != "" && $published_course_id == $exam_grade_detail['CourseAdd']['published_course_id']) ||
							(empty($makeup_exam_detail) || $makeup_exam_detail['MakeupExam']['published_course_id'] == $published_course_id)) &&
						$exam_grade_detail['ExamGrade']['department_approval'] == 1 &&
						$exam_grade_detail['ExamGrade']['registrar_approval'] == 1 &&
						$grade_change_deadline >= date('Y-m-d H:i:s') &&
						...
						if($this->request->data['ExamGradeChange'][$i]['result'] == "" || !is_numeric($this->request->data['ExamGradeChange'][$i]['result']) || $this->request->data['ExamGradeChange'][$i]['result'] < 0 || $this->request->data['ExamGradeChange'][$i]['result'] > 100 || trim($this->request->data['ExamGradeChange'][$i]['reason']) == "") {
							$this->Session->setFlash('<span></span>'.__('The system unable to process your grade change request due to incomplete data. Please make sure that you submited exam result, reason for change and the new grade is different from the previous one.'), 'default',array('class'=>'error-box error-message'));
						}
						else {
							...
							$this->request->data['ExamGradeChange'][$i]['initiated_by_department'] = ($instructor_id == $instructor_id_for_checking ? 0 : 1);
							if($last_grade != $this->request->data['ExamGradeChange'][$i]['grade']) {
								if($this->ExamResult->CourseRegistration->ExamGrade->ExamGradeChange->save($this->request->data['ExamGradeChange'][$i], array('validate' => false))) {
									...
									if(isset($exam_grade_detail['CourseRegistration']) && !empty($exam_grade_detail['CourseRegistration']) && $exam_grade_detail['CourseRegistration']['id'] != "")
										$student_detail = $this->ExamResult->CourseRegistration->find('first',
											array(
										...
										$student_detail = $this->ExamResult->CourseAdd->find('first',
											array(
									...
									if(isset($exam_grade_detail['CourseRegistration']['PublishedCourse']['CourseInstructorAssignment'][0]['Staff']['user_id']) && !empty($exam_grade_detail['CourseRegistration']['PublishedCourse']['CourseInstructorAssignment'][0]['Staff']['user_id'])) {
										...
										$auto_message['AutoMessage']['message'] = 'Grade change is initiated by the department for the course <u>'.$course_title.'</u> to <u>'.$student_detail['Student']['full_name'].' ('.$student_detail['Student']['studentnumber'].')</u> student.';
									...
									$this->Session->setFlash('<span></span>'.__('Grade change request for <u>'.($student_detail['Student']['first_name'].' '.$student_detail['Student']['middle_name'].' '.$student_detail['Student']['last_name'].' ('.$student_detail['Student']['studentnumber'].')').'</u> is sent to '.(!empty($student_detail['Student']['department_id']) ? 'department' : 'freshman program').' for approval.'), 'default',array('class'=>'success-box success-message'));
								}
								else {
									$this->Session->setFlash('<span></span>'.__('The system is unable to process your grade change request. Please try again.'), 'default',array('class'=>'error-box error-message'));
								}
								//debug($this->request->data['ExamGradeChange'][$i]);
							}
							else {
								$this->Session->setFlash('<span></span>'.__('There is no difference between the already recorded and new grade. Please make sure that you submitted a different grade.'), 'default',array('class'=>'error-box error-message'));
							}
						}
					}
					else {
						$this->Session->setFlash('<span></span>'.__('The system unable to process your grade change request. You are either accessing a different course or trying to send grade change request for non submitted or on process or rejected grade. Please try your request again within the grade change period of time.'), 'default',array('class'=>'error-box error-message'));
	...
	public function add($published_course_id = null) {
				//$this->ExamResult->CourseAdd->Student->getListOfDepartmentStudentsByYearLevel(10, 2, 1, 1, 1);
				//$this->ExamResult->CourseAdd->Student->getListOfDepartmentNonAssignedStudents(1, 'female');
				/*debug($this->ExamResult->CourseAdd->Student->find('first',
					array(
						'conditions' =>
						array(
							'Student.id' => 100
				...
				/*debug($this->ExamResult->CourseAdd->PublishedCourse->Course->find('first',
					array(
						'conditions' =>
						array(
							'Course.id' => 1
	...
	public function submit_freshman_grade_for_instructor($published_course_id = null) {
		...
		if(1) {
			...
			if(!$published_course_id && !empty($this->request->data)) {
				$this->__grade_change_request('college');
			...
			if($published_course_id == null && isset($this->request->data['ExamResult']['published_course_id']))
				$published_course_id = $this->request->data['ExamResult']['published_course_id'];
...
/**************  Save Exam Result, Preview Grade, or other buttons are clicked  ******************/
			...
			$this->Session->setFlash('<span></span>'.__('You need to have either college or 
			department role to access this area. Please contact your system administrator to 
			get college or department role.', true), 'default',array('class'=>'error-box error-message'));
	...
	public function submit_grade_for_instructor($published_course_id = null) {
		...
		if(1) {
			...
			//Check if change grade button is clicked
			if(!$published_course_id && !empty($this->request->data)) {
				$this->__grade_change_request('department');
			/* I do not know why I put this code but it is protecting me from saving preview 
			and performing other tasks and I disabled it now*/
			//if(isset($this->request->data['ExamResult']['published_course_id']))
...
/**************  Save Exam Result, Preview Grade, or other buttons are clicked  ******************/
			...
			if(empty($grade_scale) && !empty($published_course_id)) {
				...
				$published_course = $this->ExamResult->CourseRegistration->PublishedCourse->find('first', 
					array(
						'conditions' => array('PublishedCourse.id' => $published_course_id),
						'recursive' => -1
					)
			...
			$this->Session->setFlash('<span></span>'.__('You need to have department role to access this area. Please contact your system administrator to get department role.'), 'default',array('class'=>'error-box error-message'));
			return $this->redirect(array('controller' => 'dashboard', 'action' => 'index'));
		}	
	}
	...
	private function __exam_result_and_grade_mgt($published_course_id = null, $sourse = 'instructor') {
		if($sourse=="instructor" && empty($published_course_id) ){
			$selectedAcadamicYear = $this->ExamResult->ExamType->PublishedCourse->CourseInstructorAssignment->find('first',array('conditions'=>array(
				'CourseInstructorAssignment.staff_id in (select id from staffs where user_id="'.$this->Auth->user('id').'")'),
                 'fields'=>array(
                 	'CourseInstructorAssignment.academic_year',
                 	'CourseInstructorAssignment.academic_year'
		...
		$instructor_id = $this->ExamResult->ExamType->PublishedCourse->CourseInstructorAssignment->Staff->field('id',array('Staff.user_id' => $this->Auth->user('id')));
			...
			$exam_types = $this->ExamResult->ExamType->find('all', array('fields' => array('id', 'exam_name', 'percent', 'order'),'conditions' => array('ExamType.published_course_id' => $published_course_id),
				'contain' => array(),
				'order' => array('order ASC'),
				'recursive' => -1
			));
			$section_and_course_detail = $this->ExamResult->CourseRegistration->PublishedCourse->find('first', array('conditions' => array(
'PublishedCourse.id' => $published_course_id),'contain' => array('Section', 'Course',
...
/*******************************  Begin "Save Exam Result" button  ***********************************/
			...
			if(isset($this->request->data['saveExamResult']) || 
					...
					if(is_array($exam_result)) {
						...
						if(!isset($exam_result['result']) || 
							...
							if(isset($exam_result['id'])) {
								...
								$exam_result_detail = $this->ExamResult->find('first', 
									array(
								...
								if($exam_result_detail['ExamResult']['course_add'] == 0) {
									$course_grade_submited = $this->ExamResult->CourseRegistration->ExamGrade->find('first', 
										array(
											'conditions' => 
											array(
									...
									$other_exam_result_details = $this->ExamResult->find('all', 
										array(
											'conditions' => 
											array(
									...
									$course_grade_submited = $this->ExamResult->CourseAdd->ExamGrade->find('first', 
										array(
											'conditions' => 
											array(
...
-Check also if there is more than one result with that ID's exam type and (registration or add or makeup)
							...
							//New result entry validation
								...
								//Check if grade is already submitted
								if($exam_result['course_add'] == 0) {
									$course_grade_submited = $this->ExamResult->CourseRegistration->ExamGrade->find('count', 
										array(
											'conditions' => 
											array(
									...
									$course_grade_submited = $this->ExamResult->CourseAdd->ExamGrade->find('count', 
										array(
											'conditions' => 
											array(
...
/*************************** Begin "Preview Grade" button  **********************************/
			if(isset($this->request->data['previewExamGrade'])) {
				...
				$student_course_register_and_adds = $this->ExamResult->CourseRegistration->PublishedCourse->
				getStudentsTakingPublishedCourse($published_course_id);
				$course_required_scale=$this->ExamResult->CourseRegistration->PublishedCourse->
				isPublishedCourseRequiredScale($published_course_id);
				$students = $student_course_register_and_adds['register'];
				$student_adds = $student_course_register_and_adds['add'];
				$student_makeup = $student_course_register_and_adds['makeup'];
				$total_student_count = count($students) + count($student_adds) + count($student_makeup);
...
/*************************** Begin "Submit Grade" button  **********************************/
				...
				$student_course_register_and_adds = $this->ExamResult->CourseRegistration->PublishedCourse->getStudentsTakingPublishedCourse($published_course_id);
				$grade_scale = $this->ExamResult->CourseRegistration->PublishedCourse->getGradeScaleDetail($published_course_id);
				$grade_submit_status = $this->ExamResult->submitGrade($student_course_register_and_adds,
				 $students_course_in_progress, $grade_scale, $exam_types);
				$gradeStatistics=$this->ExamResult->CourseRegistration->ExamGrade->getLetterGradeStatistics($published_course_id);
				if(isset($grade_submit_status['error'])) {
					...
					$this->Session->setFlash('<span></span>'.__('Exam grade for '.(count($grade_submit_status['course_registration_add']) + count($grade_submit_status['makeup_exam'])).' student'.((count($grade_submit_status['course_registration_add']) + count($grade_submit_status['makeup_exam'])) > 1 ? 's' : '').' is successfully submitted to the department for approval. Till it get approved, you can cancel your grade submission to apply changes. Once it is approved, neither you can cancel the submitted grade nor edit exam result except for students with course "In Progress" status.'), 'default', array('class' => 'success-message success-box'));
				}
				else {
...
/************************* Begin "Cancel Exam grade Preview" button  ********************************/
				...
				return $this->redirect(array('action' => (strcasecmp($sourse, 'instructor') == 0 ? 'add' : (strcasecmp($sourse, 'college') == 0 ? 'submit_freshman_grade_for_instructor' : 'submit_grade_for_instructor')), $published_course_id));
			        ...
			        $publish_course_detail_info = $this->ExamResult->ExamType->PublishedCourse->find('first',
					array(
						'conditions' => 
						array(
				...
				//debug($publish_course_detail_info);
                               $filename="Grade-Sheet".$section_detail['name'].'-'
.$publish_course_detail_info['PublishedCourse']['academic_year'].'-'.$publish_course_detail_info['PublishedCourse']['semester'];
		...
		if(!empty($published_course_id)) {
			$published_course_detail = $this->ExamResult->CourseAdd->PublishedCourse->find('first',
				array(
					'conditions' =>
					array(
						'PublishedCourse.id' => $published_course_id
		...
		if(strcasecmp($sourse, 'instructor') == 0) {
			...
			if(empty($this->request->data)) {
				$published_course_detail = $this->ExamResult->CourseRegistration->PublishedCourse->find('first', 
					array(
						'conditions' => array('PublishedCourse.id' => $published_course_id),
						'recursive' => -1
					)
	...
	private function __data_entry_assignment($published_course_id=null,$sourse='instructor'){
	  if($sourse=="instructor" && empty($published_course_id)){
		$selectedAcadamicYear = $this->ExamResult->ExamType->PublishedCourse->CourseInstructorAssignment->find('first',array('conditions'=>array(
		'CourseInstructorAssignment.staff_id in (select id from staffs where user_id="'.$this->Auth->user('id').'")'),'fields'=>array(
		'CourseInstructorAssignment.academic_year',
		'CourseInstructorAssignment.academic_year'),
		 'order'=>array('CourseInstructorAssignment.academic_year DESC')
		));
		...
		{
			$section_and_course_detail = $this->ExamResult->CourseRegistration->PublishedCourse->find('first', array('conditions' => array(
			'PublishedCourse.id' => $published_course_id),'contain' => array('Section', 'Course','CourseInstructorAssignment'=>array('conditions'=>array('CourseInstructorAssignment.isprimary'=>true),
			'Staff'=>array('Title','Position')))));
			if(!empty($section_and_course_detail['Section'])) {
			$section_detail = $section_and_course_detail['Section'];
			}
...
/*******************************  Begin "Save Exam Result" button  ***********************************/
			...
			if(isset($this->request->data['saveExamResult']) || isset($this->request->data['previewExamGrade'])) {
				...
				if(isset($this->request->data['ResultEntryAssignment'])) {
					...
					if($entry_exam_published_course_id 
						!= $published_course_id) {
							$this->Session->setFlash('<span></span>'.__('Sorry, the system is unable to get some students for result entry. This usually happens when the department apply a change while you make exam result submission. Please re-submit your exam result entry.'), 'default', array('class' => 'error-message error-box'));
							$save_is_ok = false;
						}
			...
			if($save_is_ok) {
				...
				if(!empty($entry_exam_results)) {
					...
					if(!ClassRegistry::init('ResultEntryAssignment')->saveALL($entry_exam_results, array('validate' => false))) {
						$this->Session->setFlash('<span></span>'.__('Exam result entry/update is failed. Please re-enter the  exam result.'), 'default', array('class' => 'error-message error-box'));
...
/*************************** Begin "Preview Grade" button  **********************************/
		if(isset($this->request->data['previewExamGrade'])) {
				...
				*/
				...
				$student_course_register_and_adds = $this->ExamResult->CourseRegistration->PublishedCourse->
				getStudentsRequiresGradeEntryExamPublishedCourse($published_course_id);
				$course_required_scale=$this->ExamResult->CourseRegistration->PublishedCourse->
				isPublishedCourseRequiredScale($published_course_id);
...
/*************************** Begin "Submit Grade" button  **********************************/
				...
				$student_course_register_and_adds = $this->ExamResult->CourseRegistration->PublishedCourse->getStudentsRequiresGradeEntryExamPublishedCourse($published_course_id);
				$grade_scale = $this->ExamResult->CourseRegistration->PublishedCourse->getGradeScaleDetail($published_course_id);
				$grade_submit_status = $this->ExamResult->submitGradeEntryAssignment($student_course_register_and_adds,
				 $students_course_in_progress, $grade_scale);
				$gradeStatistics=$this->ExamResult->CourseRegistration->ExamGrade->getLetterGradeStatistics($published_course_id);
				if(isset($grade_submit_status['error'])) {
					$this->Session->setFlash('<span></span>'.__($grade_submit_status['error']), 'default', array('class' => 'success-message success-box'));
				}
...
/************************* Begin "Cancel Exam grade Preview" button  ********************************/
			...
			$total_student_count = count($students) + count($student_adds) + count($student_makeup);
				...
				$this->set(compact('selected_acadamic_year', 'selected_semester','gradeStatistics', 'grade_scale', 'published_course_detail','makeup_exam','gradeStatistics', 'exam_results', 'published_course_combo_id', 'publishedCourses', 'students', 'exam_types', 'student_adds', 'student_makeup', 'section_detail', 'course_detail', 'display_grade','filename','university',
				'publish_course_detail_info', 'grade_submission_status', 'view_only', 'days_available_for_grade_change', 'total_student_count'));
				$this->render('/Elements/marksheet_grade_xls');
			}
		...
		if(strcasecmp($sourse, 'instructor') == 0) {
			...
			if(empty($this->request->data)) {
				$published_course_detail = $this->ExamResult->CourseRegistration->PublishedCourse->find('first', array('conditions' => array('PublishedCourse.id' => $published_course_id),
				'recursive' => -1));
				$program_type_id = $published_course_detail['PublishedCourse']['program_type_id'];
				$program_id = $published_course_detail['PublishedCourse']['program_id'];
				$acadamic_year = $published_course_detail['PublishedCourse']['academic_year'];
		...
		$section_and_course_detail = $this->ExamResult->CourseRegistration->PublishedCourse->find('first', 
		array('conditions' => array('PublishedCourse.id' => $published_course_id),'contain' => array('Section', 'Course')));
		if(
		!empty($section_and_course_detail['Section'])) {
		$section_detail = $section_and_course_detail['Section'];
		}
		...
		if(!empty($publishedCourses))
			$publishedCourses = array
			("" => "---Select Course---") +$publishedCourses;
		$grade_scale = $this->ExamResult->CourseRegistration->PublishedCourse->getGradeScaleDetail($published_course_id);
	    $gradeStatistics=$this->ExamResult->CourseRegistration->ExamGrade->getLetterGradeStatistics($published_course_id);
...
$this->set(compact('selected_acadamic_year', 'selected_semester','gradeStatistics', 'grade_scale','lastGradeSubmissionDate', 'published_course_detail','course_assignment_detail', 'exam_results', 'published_course_combo_id', 'publishedCourses', 'students', 'exam_types', 'student_adds', 'student_makeup', 'section_detail', 'course_detail', 'display_grade', 'grade_submission_status', 'view_only', 'days_available_for_grade_change', 'total_student_count'));
	...
	private function __exam_result_and_grade_mgt_fx($published_course_id=null,$sourse='instructor'){
		...
		if(!$do_not_manipulate 
		&& !empty($this->request->data)) {
		$exam_types = $this->ExamResult->ExamType->find('all', array('fields' => array('id', 'exam_name', 'percent', 'order', 'mandatory'),'conditions' => array('ExamType.published_course_id' => $published_course_id),
...
/*************************** Begin "Preview Grade" button  **********************************/
			if(isset($this->request->data['previewExamGrade'])) {
				...
				$student_course_register_and_adds = $this->ExamResult->CourseRegistration->PublishedCourse->
				getStudentsTakingFxExamPublishedCourse($published_course_id);
				$course_required_scale=$this->ExamResult->CourseRegistration->PublishedCourse->
				isPublishedCourseRequiredScale($published_course_id);
				$students = $student_course_register_and_adds['register'];
				$student_adds = $student_course_register_and_adds['add'];
...
$this->set(compact('selected_acadamic_year', 'selected_semester','gradeStatistics', 'grade_scale','lastGradeSubmissionDate', 'published_course_detail','course_assignment_detail', 'exam_results', 'published_course_combo_id', 'publishedCourses', 'students', 'exam_types', 'student_adds', 'student_makeup', 'section_detail', 'course_detail', 'display_grade', 'grade_submission_status', 'view_only', 'days_available_for_grade_change', 'total_student_count'));
	...
	function get_exam_result_fx_entry_form($published_course_id = null) {
		...
		if($this->Auth->user('id')) {
			...
			if(!empty($published_course_id)) {
				...
				$section_and_course_detail = $this->ExamResult->CourseRegistration->PublishedCourse->find('first', array('conditions' => array(
				'PublishedCourse.id' => $published_course_id),'contain' => array('Section', 'Course','CourseInstructorAssignment'=>array(
				'conditions'=>array('CourseInstructorAssignment.isprimary'=>true),
				'Staff'=>array('Title','Position')))));
				$section_detail = $section_and_course_detail['Section'];
				$course_detail = $section_and_course_detail['Course'];
	...
	function cancel_grade_change_request($grade_change_id = null) {
		if($grade_change_id) {
			$last_grade_change = $this->ExamResult->CourseRegistration->ExamGrade->ExamGradeChange->find('first', 
				array(
			...
			if(($login_instructor_id != $instructor_id_for_checking &&
				...
				if($this->ExamResult->CourseRegistration->ExamGrade->ExamGradeChange->delete($grade_change_id)) {
						$this->Session->setFlash('<span></span>'.__('Exam grade change request for <u>'.($student_detail['Student']['first_name'].' '.$student_detail['Student']['middle_name'].' '.$student_detail['Student']['last_name'].' ('.$student_detail['Student']['studentnumber'].')').'</u> is successfully cancelled'), 'default', array('class' => 'success-message success-box'));
					}
					else
						$this->Session->setFlash('<span></span>'.__('Exam grade change request cancellation is failed. Please, try again.'), 'default', array('class' => 'error-message error-box'));
				}
	...
	/***
	*@Wonde
	*Generate Student Academic Status Using Command line whenever needed
	*/
	function _generate_student_academic_status ($section_id=null) {
	    
	    if (!empty($section_id)) {
            $getListOfPublishedCourses=$this->ExamResult->CourseRegistration->PublishedCourse->find('list',
        ...
        if(!empty($getListOfPublishedCourses)) {
	        ...
	        return "There is no published courses.";
	...
	function autoSaveResult(){
		...
		if(!empty($published_course_id)) {
		 ...
		 $login_instructor_id = $this->ExamResult->ExamType->PublishedCourse->CourseInstructorAssignment->Staff->field('id', array('Staff.user_id' => $this->Auth->user('id')));
			if($active_account == 1 
			&& $instructor_id_for_checking !=$login_instructor_id)
					$do_manipulate = false;
		}
	    debug($do_manipulate);
	    if($do_manipulate && isset($this->request->data['ExamResult'] ) && !empty($this->request->data['ExamResult'] ) && 
	    isset($exam_types) && !empty($exam_types)){
	    foreach($this->request->data['ExamResult'] 
as $key => $exam_result) {
          $save_is_ok=true;
          if(is_array($exam_result)){
			if(trim($exam_result['result']) != "") {
			...
			if(isset($exam_result['id'])) {
				$exam_type_id = 
				$this->ExamResult->field('exam_type_id',array('id' => $exam_result['id']));
			} else {
			$exam_type_id = $exam_result['exam_type_id'];
			}
			//Check if exam type is available
			...
			if($exam_percent == "" || 
			$exam_name == "" || $exam_type_id == "") {
				$save_is_ok = false;
			} else if(is_numeric($exam_result['result']) && $exam_result['result'] 
			> $exam_percent) {
				$save_is_ok = false;
			 } 
			 ...
			 if($save_is_ok){
			  		...
			  		debug($data);
					if(
					isset($data['ExamResult']['id']) 
					&& 
					!empty($data['ExamResult']['id'])){
					$alreadyRecored=
					$this->ExamResult->find('first',
					array('conditions'=>array('ExamResult.id'=>$data['ExamResult']['id']),
					'recursive'=>-1
					));
					} else {
					$alreadyRecored=$this->ExamResult->find('first',array('conditions'=>array('ExamResult.exam_type_id'=>$data['ExamResult']['exam_type_id'],'ExamResult.course_registration_id'=>$data['ExamResult']['course_registration_id'],
					'ExamResult.course_add'=>$data['ExamResult']['course_add']),
					'recursive'=>-1
					));
					}
					if(isset($alreadyRecored) && 
					!empty($alreadyRecored)){
...
$this->request->data['ExamResult']['year_level_id']);
	      ...
	      if(isset($this->request->data['rollback']) && !empty($this->request->data['rollback'])) {
		   ...
		   if(!empty($courseRegistrationAndGrade)) {
		        ...
		        if(ClassRegistry::init('ExamGrade')->saveAll($courseRegistrationAndGrade['ExamGrade'],array('validate'=>false))) {
				    //Notifications
						$this->Session->setFlash('<span></span>'.__('The instructor submittd grade rollback and will be processed by the instructor again.'), 'default',array('class'=>'success-box success-message'));
	    ...
	    if(!empty($this->department_ids)) {
            $departments=$this->ExamResult->CourseRegistration->Student->Department->find('list',array('conditions'=>array('Department.id'=>$this->department_ids)));
        
		} else if (!empty($this->college_id)) {
             $colleges=$this->ExamResult->CourseRegistration->Student->College->find('list',
		...
		if($this->Auth->user('id')) {
			...
			if(!empty($published_course_id)) {
				...
				$section_and_course_detail = $this->ExamResult->CourseRegistration->PublishedCourse->find('first', array(
					...
					'contain' => array('Section', 'Course'=>array('GradeType'=>array('Grade')))));
				
				$student_course_register_and_adds = $this->ExamResult->CourseRegistration->PublishedCourse->getStudentsTakingPublishedCourse($published_course_id);
		        
			}
			
			$this->set(compact('grade_submission_status', 'view_only','student_course_register_and_adds','section_and_course_detail'));
		}//End of login user cheking
...
$this->request->data['ExamResult']['year_level_id']);
	      ...
	      if(isset($this->request->data['saveGrade']) && !empty($this->request->data['saveGrade'])) {
			...
			foreach($this->request->data['ExamResult'] as $key => $student) {
			  ...
			  if(is_int($key) && $student['gp'] == 1 && $scale_id!=0 
&& !empty($student['grade'])) {
				debug($count);
...
$courseRegistrationAndGrade['ExamGrade'][$count]['registrar_reason']='Registrar Data Entry interface';
	...
	public function auto_grade_generation($published_course_id = null) {
			...
			if(!empty($this->request->data) 
			&& isset($this->request->data['listPublishedCourses'])) {
				$department_id = $this->request->data['ExamResult']['department_id'];
...
$this->request->data['ExamResult']['year_level_id']);
				...
				$courseRegistrationAndGrade=array();
				$count=0;
				foreach($publishedCourses 
				as $pk=>$pv){
				foreach($pv as $pid=>$pvi){
				debug($pid);
				//$courseRegistrationAndGrade=array();
			 	//$count=0;
             	$publishedC=$this->ExamResult->CourseRegistration->PublishedCourse->find('first',
             	array('conditions' => array('PublishedCourse.id' => $pid),
				...
				$count=0;
				//debug($student_course_register_and_adds);
				foreach($student_course_register_and_adds
				as $aadk=>$aadv){
...
$courseRegistrationAndGrade['ExamGrade'][$count]['registrar_reason']='Auto Generated for Testing';
				...
				if(!empty($courseRegistrationAndGrade)) {
					debug($courseRegistrationAndGrade);
					//saveAll 
					if(ClassRegistry::init('ExamGrade')->saveAll($courseRegistrationAndGrade['ExamGrade'],
