...
class CostSharingPaymentsController extends AppController {
	...
	public function add() {
	     	...
	     	isset($this->request->data['saveApplicablePayment'])) 
	     {
			...
			if ($this->CostSharingPayment->save($this->request->data)) {
				$this->Session->setFlash('<span></span>'.__('The cost sharing payment has been saved'),
				...
				$student_number=$this->CostSharingPayment->Student->field('studentnumber',
		...
		if (!empty($this->request->data) && isset($this->request->data['continue'])) {
			     ...
			     if (!empty($this->request->data['CostSharingPayment']['studentID'])) {
			            $check_id_is_valid=$this->CostSharingPayment->Student->
			            find('count',
			            ...
			            $studentIDs=1;
			             ...
			             if ($check_id_is_valid>0) {
			                 $everythingfine=true;
			                $student_id=$this->CostSharingPayment->Student->field('id',
			                array('studentnumber'=>trim($this->request->data['CostSharingPayment']['studentID'])));
			                $student_section_exam_status=$this->CostSharingPayment->Student->
	...
	function delete($id = null) {
		if (!$id) {
			$this->Session->setFlash('<span></span>'.__('Invalid id for cost sharing payment'),
			'default',array('class'=>'error-box error-message'));
			return $this->redirect(array('action'=>'index'));
		}
		if ($this->CostSharingPayment->delete($id)) {
			$this->Session->setFlash('<span></span>'.__('Cost sharing payment deleted'),
			'default',array('class'=>'success-box success-message'));
			return $this->redirect(array('action'=>'index'));
		}
		$this->Session->setFlash('<span></span>'.__('Cost sharing payment was not deleted'),
		'default',array('class'=>'error-box'));
		return $this->redirect(array('action' => 'index'));
	}
}
