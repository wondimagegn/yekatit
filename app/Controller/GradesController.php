...
class GradesController extends AppController {
	...
	function add() {
		if (!empty($this->request->data)) {
			...
			if ($this->Grade->save($this->request->data)) {
				$this->Session->setFlash('<span></span>'.__('The grade has been saved'),
				'default',array('class'=>'success-box success-message'));
				return $this->redirect(array('action' => 'add'));
			} else {
				$this->Session->setFlash('<span></span>'.__('The grade could not be saved. Please, try again.'),'default',array('class'=>'error-box error-message'));
			}
			*/
		}
	...
	function delete($id = null,$action_controller_id=null) {
		...
		if(!$this->Grade->exists()) {
			$this->Session->setFlash('<span></span>'.__('Invalid id for grade'), 'default', array('class' => 'error-message error-box'));
		...
		if($this->Grade->allowDelete($id)) {
		    if ($this->Grade->delete($id)) {
			    $this->Session->setFlash('<span></span>'.__('Grade deleted'),
			    'default',array('class'=>'success-box success-message'));
		...
		$this->Session->setFlash('<span></span>'.__('Grade represenation was not deleted. Students has already used this grade.'),
