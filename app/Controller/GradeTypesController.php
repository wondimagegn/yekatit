...
//     var $components =array('Security');
	...
	*/
	...
	function view($id = null) {
		if (!$id) {
			$this->Session->setFlash('<span></span>'.__('Invalid grade type'),
		...
		$gradeType=$this->GradeType->find('first',array('conditions'=>array('GradeType.id'=>$id),
		'contain'=>array('Course'=>array('fields'=>array('id','course_title','course_code','credit','course_detail_hours'),'Curriculum'=>array('Department'=>array('id','name'),'fields'=>array('id','name')),
		'CourseCategory'=>array('id','name')),'Grade'=>array('fields'=>array('id','grade','point_value','pass_grade','allow_repetition')))));
		...
		if (!empty($this->request->data)) {
			...
			if ($this->GradeType->Grade->checkGradeIsUnique($this->request->data)) {
			    if ($this->GradeType->saveAll($this->request->data,array('validate'=>'first'))) {
				    $this->Session->setFlash('<span></span>'.__('The grade type has been saved'),
				    'default',array('class'=>'success-box success-message'));
				    $this->redirect(array('action' => 'index'));
			    } else {
				    $this->Session->setFlash('<span></span>'.__('The grade type could not be saved. Please, try again.'),'default',array('class'=>'error-box error-message'));
			    }
	...
	function edit($id = null) {
		...
		if (empty($this->request->data)) {
			...
			$this->request->data = $this->GradeType->find('first',array('conditions'=>array('GradeType.id'=>$id),
			'contain'=>array('Grade')));
			$check_not_involved_in_grade_computing=$this->GradeType->
		is_grade_type_attached_to_course($id);
	...
	function delete($id = null,$action_controller_id=null) {
		...
		//TODO: CHeck grade type is not involved for any grade computing
		...
		if ($check_not_involved_in_grade_computing) {
		    if ($this->GradeType->delete($id)) {
			    $this->Session->setFlash('<span></span>'.__('Grade type deleted.'),
			    'default',array('class'=>'success-box success-message'));
		    ...
		    $this->Session->setFlash('<span></span>'.__('You can not delet this grade type, it is attached to courses.'),
		'default',array('class'=>'error-box error-message'));
		    $this->redirect(array('action' => 'index'));
		}
