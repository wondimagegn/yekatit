	...
	public function add() {
		...
		if($this->role_id == 6) {
			if (!empty($this->request->data)) {
				...
				if($this->request->data['MakeupExam']['course_registration_id'] != "0") {
					...
					if(strcasecmp($register_or_add[1], 'add') == 0) {
						...
						if(!(!empty($grade_history) && 
							(strcasecmp($grade_history[0]['ExamGrade']['grade'], 'I') == 0 ||
							 strcasecmp($grade_history[0]['ExamGrade']['grade'], 'NG') == 0 ||
							 strcasecmp($grade_history[0]['ExamGrade']['grade'], 'DO') == 0 ||
							 strcasecmp($grade_history[0]['ExamGrade']['grade'], 'W') == 0 ||
							  strcasecmp($grade_history[0]['ExamGrade']['grade'], 'F') == 0 ||
							  strcasecmp($grade_history[0]['ExamGrade']['grade'], 'D') == 0
							))
						) {
							$this->Session->setFlash('<span></span>'.__('Makeup exam is allowed for only NG,
							 I, DO or W grade.', true),'default',array('class'=>'error-box error-message'));
							$save_is_ok = false;
						}
					...
					//Duplication checking by course registration
						...
						if(!(!empty($grade_history) && 
							(strcasecmp($grade_history[0]['ExamGrade']['grade'], 'I') == 0 ||
							 strcasecmp($grade_history[0]['ExamGrade']['grade'], 'NG') == 0 ||
							 strcasecmp($grade_history[0]['ExamGrade']['grade'], 'DO') == 0 ||
							 strcasecmp($grade_history[0]['ExamGrade']['grade'], 'W') == 0 || strcasecmp($grade_history[0]['ExamGrade']['grade'], 'F') == 0 ||
							  strcasecmp($grade_history[0]['ExamGrade']['grade'], 'D') == 0
							))
						) {
							$this->Session->setFlash('<span></span>'.__('Makeup exam is allowed for only NG, I, DO or W grade.'),'default',array('class'=>'error-box error-message'));
							$save_is_ok = false;
						}
						/*
						else if($this->request->data['MakeupExam']['exam_published_course_id'] != "0") {
						
						//$this->request->data['MakeupExam']['exam_published_course_id'] != "0" && $published_course_id == $this->request->data['MakeupExam']['exam_published_course_id']
				...
				$student_sections = $this->MakeupExam->PublishedCourse->Section->allDepartmentSectionsOrganizedByProgramType($this->department_id, 1, $this->request->data['MakeupExam']['program_id']);
				...
				$exam_sections = $this->MakeupExam->PublishedCourse->Section->allDepartmentSectionsOrganizedByProgramType($department_id, 1, $program_id);
				...
				$student_sections = $this->MakeupExam->PublishedCourse->Section->allDepartmentSectionsOrganizedByProgramType($this->department_id, 1, 1);
				$student_section_id = "";
