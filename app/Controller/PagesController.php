...
class PagesController extends AppController
{
		...
		if (isset($this->request->data) && !empty($this->request->data['viewAcademicCalendar'])) {
			...
			if (empty($academicCalendars)) {
				$this->Session->setFlash('<span></span>' .
					__('There is no academic calendar defined in the system in the given criteria.'), 'default', array('class' => 'info-box info-message'));
			} else {
	...
	public function official_transcript_request()
	{
		...
		if ($this->request->is('post')) {
			...
			//$this->OfficialTranscriptRequest->set();
			$this->OfficialTranscriptRequest->set($this->request->data);
			if ($this->OfficialTranscriptRequest->saveAll($this->request->data)) {

				$this->Session->setFlash('<span></span>' . __("The official transcript request has been forwared to designated personnel.Your tracking number is $trackingnumber."), 'default', array('class' => 'success-box	success-message'));
				return $this->redirect(array('action' => 'official_request_tracking'));
			} else {
				$error = $this->OfficialTranscriptRequest->invalidFields();
				debug($error);

				$this->Session->setFlash('<span></span>' . __('The official transcript request could not be saved.'), 'default', array('class' => 'error-box	error-message'));
			}
		}
	...
	public function official_request_tracking()
	{
		if (isset($this->request->data['OfficialTranscriptRequest']) && !empty($this->request->data['OfficialTranscriptRequest']['trackingnumber'])) {
			...
			if (empty($request)) {
				$this->Session->setFlash('<span></span>' . __("The tracking number provides is invalid or request cancelled."), 'default', array('class' => 'success-box	success-message'));
			}
			$this->set(compact('request'));
		}
		$statuses = array(
			'request_verified' => 'Request Verified', 'request_cancelled' => 'Request Cancelled',
	...
	public function online_admission_tracking()
	{
		if (isset($this->request->data['OnlineApplicant']) && !empty($this->request->data['OnlineApplicant']['trackingnumber'])) {
			$request = $this->OnlineApplicant->find('first', array('conditions' => array('OnlineApplicant.applicationnumber' => trim($this->request->data['OnlineApplicant']['trackingnumber'])), 'contain' => array(
			...
			if (empty($request)) {
				$this->Session->setFlash('<span></span>' . __("The application number  is invalid or request cancelled."), 'default', array('class' => 'success-box	success-message'));
			}
			$this->set(compact('request'));
		}
	...
	public function admission()
	{
		...
		$academicCalendars = ClassRegistry::init('AcademicCalendar')->find('all', array('conditions' => array('AcademicCalendar.online_admission_end_date >=' => date('Y-m-d'))));
		//
        ...
        */
		if (
			$this->request->is('post') && isset($this->request->data['applyOnline'])
			&& !empty($this->request->data['applyOnline'])
		) {
			...
			if ($isAdmitted == 0) {
			    ...
			    cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:8px;border:0;margin:0;auto">';
					...
					try {
						if ($Email->send()) {
							$this->Session->setFlash('<span></span>' . __("Your online admission application request has been forwarded to designated personnel of the university for further processing. Your application number is  $applicationnumber and sent to " . $this->request->data['OnlineApplicant']['email'] . " address for tracking your application status. "), 'default', array('class' => 'success-box success-message'));
						} else {
							$this->Session->setFlash('<span></span>' . __("Your online application request has been forwared to designated personnel of the university for further processing. Your application  number is $applicationnumber and use it for tracking your application status. "), 'default', array('class' => 'success-box success-message'));
						}
					} catch (Exception $e) {
						$this->Session->setFlash('<span></span>' . __("Your online application request has been forwared to designated personnel of the university for further processing. Your application  number is $applicationnumber and use it for tracking your application status. "), 'default', array('class' => 'success-box success-message'));
	...
	public function check_graduate()
	{
		...
		if (
			...
			&& isset($this->request->data['continue'])
		) {

			$isStudentValid = ClassRegistry::init('Student')->find('count', array(
				'conditions' => array('Student.studentnumber' => trim($this->request->data['Page']['studentID'])),
				'recursive' => -1
			));
			debug($this->request->data);
			...
			if ($isStudentValid > 0) {
				$students = ClassRegistry::init('GraduateList')->Student->find('first', array(
					'conditions' => array('Student.studentnumber' =>
					trim($this->request->data['Page']['studentID'])),
					'contain' => array('GraduateList', 'Attachment', 'Program', 'Department', 'College', 'ProgramType', 'Curriculum' => array('fields' => array(
						'english_degree_nomenclature', 'amharic_degree_nomenclature', 'certificate_name', 'specialization_amharic_degree_nomenclature', 'specialization_english_degree_nomenclature'
					)))
				));
	...
	function get_department_combo($college_id)
	{
		...
		$academicCalendarss = ClassRegistry::init('AcademicCalendar')->find('all', array('conditions' => array('AcademicCalendar.online_admission_end_date >=' => date('Y-m-d'))));
		...
		foreach($academicCalendarss as $ack=>$acv){
			...
			$depIds = array_merge($depIds, $tmpIds);
