...
class XmlRpcController extends AppController { 
    ...
    {
        return $this->Student->Department->find('list', array('conditions' => array('Department.college_id' => $college_id, 'Department.active' => 1)));
    }

    function _college($id = null)
    {
        return $this->College->find('list', array('conditions' => array('College.id' => $id, 'College.active' => 1)));
    ...
    {
        $student_ids = $this->Student->find('list', array('conditions' => array('Student.first_name like ' => (trim($name)) . '%'), 'fields' => array('Student.id', 'Student.id')));
        return $student_ids;
    }
