...
$this->Paginator->paginate('Alumnus');  
	...
	public function add_baselinesurvey_onbehalf() {
	  ...
	  $student=ClassRegistry::init('Student')->find('first',array('conditions'=>array('Student.id'=>$student_id),'contain'=>array('GraduateList','User')));
		...
		debug($this->Auth->user('username'));
		if ($this->request->is('post') && !isset($this->request->data['continue'])) {
		    $this->request->data=$this->Alumnus->formatResponse($this->request->data);
			$this->Alumnus->create();
			if ($this->Alumnus->saveAll($this->request->data)) {
					 $this->Session->setFlash('<span></span>'.__('THANK YOU FOR YOUR PARTICIPATION!'),
	...
	if ($this->request->is('post') && 
		isset($this->request->data['applyOnline']) && !empty($this->request->data['applyOnline'])) {
			...
			if(ClassRegistry::init('AlumniMember')->save($data)) {
			    ...
			    Now you are officially Yekatit 12 Medical Hospital College alumni member. You will recieve information related to alumni events through your email. Your alumni number is ".$data['AlumniMember']['trackingnumber']." and provide this number whenever you need service from Y12HMC. 
		        ...
		        $Email->viewVars(array('message'=>$message));
		          ...
		          try{
						if($Email->send()) {
						$this->Session->setFlash('<span></span>'.__(" Now you are officially Yekatit 12 Medical Hospital College alumni member. You will recieve information related to alumni events through your email. "),'default',array('class'=>'success-box success-message'));
