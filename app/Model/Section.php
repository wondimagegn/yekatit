...
class Section extends AppModel
{
	...
	*/
	...
	function allDepartmentSectionsOrganizedByProgramTypeSuppExam($col_dep_id = "", $department = 1, $program_id = 1, $archive = 0, $include_split = false, $include_merge = false)
	{
		...
		// allow all program types id there is no restriction set on smis.php file
		$allowedProgramTypes = $this->ProgramType->find('list', array('fields' => array('ProgramType.id', 'ProgramType.id')));
		
		// check for defined program types and overwrite $allowedProgramTypes on conditons 
		$allowedProgramTypesOnly = Configure::read('program_types_to_allowed_for_supplementary_exam');

		if (STUDENTS_WITH_F_GRADE_ALLOWED_FOR_SUPPLEMENTARY_GRADE_SUBMISSION == 1 || STUDENTS_WITH_NG_GRADE_ALLOWED_FOR_SUPPLEMENTARY_GRADE_SUBMISSION == 1) {
			if (!empty($allowedProgramTypesOnly)) {
				$allowedProgramTypes = $allowedProgramTypesOnly;
			}
		} else if (!empty($allowedProgramTypesOnly) && ALWAYS_ENFORCE_ALLOWED_PROGRAM_TYPES_FOR_SUPPLEMENTARY_GRADE_SUBMISSION == 1) {
			$allowedProgramTypes = $allowedProgramTypesOnly;
		}
			...
			if ($department == 1) {
				$sections_data = $this->find('all', array(
					'conditions' => array(
						...
						'Section.archive' => $archiveSection,
						'Section.program_id' => $program_id,
						'Section.program_type_id' => $allowedProgramTypes,
						'Section.academicyear' => $currentAcademicYear
					),
					'contain' => array(
						'ProgramType', 
	...
	function getSectionStudents($section_id = null, $name = null, $include_archived_students = 0)
	{
		...
		if (!empty($include_archived_students)) {
			$include_archived_students = array(0, 1);
		}
		...
		if (!empty($name)) {
			...
			$students_in_section = $this->StudentsSection->find('all', array(
				'conditions' => array(
					'StudentsSection.section_id' => $section_id,
					'StudentsSection.archive' => $include_archived_students,
	...
	function getStudentSectionInGivenAcademicYear($academic_year = null, $student_id = null, $semester = null) 
	{
		$options = array();
		$sections = array();
		...
		if (!empty($student_id)) {
			...
			// check registration if semester is also passed and return the section
			if (!empty($semester) && !empty($academic_year)) {

				$checkForCourseRegistration = ClassRegistry::init('CourseRegistration')->find('first', array(
					'conditions' => array(
						'CourseRegistration.student_id' => $student_id,
				...
				if (!empty($checkForCourseRegistration)) {
					//debug($checkForCourseRegistration);
					if (!empty($checkForCourseRegistration['CourseRegistration']['section_id'])) {
						
						$options['conditions'][] = 'Section.id = ' . $checkForCourseRegistration['CourseRegistration']['section_id'] . '';
	...
	function getUpgradableSectionName($secId)
	{
		...
		if (!empty($sectionData)) {
			...
			if ($doesUpgradableYearLevelExist) {
				...
				$nextYearLevelName = $this->YearLevel->field('YearLevel.name', array('YearLevel.id' => $doesUpgradableYearLevelExist, 'YearLevel.department_id' => $sectionData['Section']['department_id']));

				$nextYearLevelNameNumeric = '';

				if (!empty($nextYearLevelName)) {
					$nextYearLevelNameNumeric = $nextYearLevelName[0];
				}

				$upgrade_section_name = ($prefix_selected_sectionname_character . (is_numeric($prefix_selected_sectionname_yearlevel) ? ($prefix_selected_sectionname_yearlevel + 1) : $prefix_selected_sectionname_yearlevel . $nextYearLevelNameNumeric) . ' ' . (trim($fixed_selected_sectionname)) . ' ' . (trim($variable_selected_sectionname)));
	...
	function getMostRepresntiveTakenCourse($sectionid)
	{
		...
		if ($sectionid) {
			...
			$student_previous_section = array();
	...
	function chceck_all_registered_added_courses_are_graded($student_id = null, $section_id = null, $check_for_invalid_grades = 0,  $from_student = '', $skip_f_grade = 0, $get_error_message = 0, $skip_I_grade = 0)
	{
					...
					foreach ($student['CourseRegistration'] as $key => $course_registration) {
						...
						if (!$this->Student->CourseRegistration->isCourseDroped($course_registration['id']) && $course_registration['PublishedCourse']['drop'] == 0) {
							...
							if (empty($grade_detail) /* && !$incomplete_grade  */) {
								...
								if ($skip_I_grade && (strcasecmp($grade_detail['ExamGrade']['grade'], 'I') == 0) && isset($grade_detail['ExamGrade']['course_registration_id']) && $course_registration['id'] == $grade_detail['ExamGrade']['course_registration_id'] && $grade_detail['ExamGrade']['id'] == $latestApprovedGrade['grade_id']) {
									continue;
								}
				...
				if (isset($student['CourseAdd']) && !empty($student['CourseAdd'])) {
					foreach ($student['CourseAdd'] as $key => $course_add) {
						...
						if (empty($grade_detail) /* && !$incomplete_grade */) {
							...
							if ($skip_I_grade && (strcasecmp($grade_detail['ExamGrade']['grade'], 'I') == 0) && isset($grade_detail['ExamGrade']['course_add_id']) && $course_add['id'] == $grade_detail['ExamGrade']['course_add_id'] && $grade_detail['ExamGrade']['id'] == $latestApprovedGrade['grade_id']) {
								continue;
							}
		...
		if (!empty($id) && is_numeric($id) && $id > 0) {
			...
			if ($all) {
				...
				if (!empty($section_detals)) {
					...
					$yearLevelName = (isset($section_detals['YearLevel']['id']) && !empty($section_detals['YearLevel']['id']) ? $section_detals['YearLevel']['name'] : ($section_detals['Section']['program_id'] == PROGRAM_REMEDIAL ? 'Remedial' : 'Pre/1st'));
