...
class Section extends AppModel
{
	...
	function checkUnique($data, $fieldName)
	{
		...
		if (isset($fieldName) && $this->hasField($fieldName)) {
			if (!empty($this->data['Section']['year_level_id'])) {
				$section = $this->find('first', array('conditions' => array(
	...
	function countsectionlessstudents(
		$collegeid = null,
		$role_id = null,
		$department_id = null,
		$year = null,
	...
	function getsectionlessstudentsummary($thisacademicyear = null, $college_id = null, $department_id = null, $role_id = null)
	{
		...
		if (!empty($thisacademicyear) && !empty($college_id)) {
				...
				foreach ($programtypes as $kpt => $vpt) {
					if (ROLE_DEPARTMENT == $role_id) {
                        ...
                        $students = $this->Student->find('all',array(
                   		...
                   		),'contain'=>array('AcceptedStudent','Section')));
                   		*/
						$students = $this->Student->find('list', array(
							'conditions' => array(
								...
								/*
                                     ...
                                     "OR"=>array(
			        	array('Student.curriculum_id'=>0),
			        	array('Student.curriculum_id'=>NULL),
			        	array('Student.curriculum_id'=>'null')
			        ),
			        */
						...
						/*
                          $students = $this->Student->find('all',array(
                                'conditions'=>array('AcceptedStudent.academicyear'=>$thisacademicyear,
                                    ...
                                    'Student.program_id'=>$kp,
                                    'Student.program_type_id'=>$kpt,
                                    "OR"=>array("Student.department_id is null","Student.department_id" => array(0,''))),'contain'=>array('AcceptedStudent','Section')));
                           */
						$students = $this->Student->find('list', array(
							'conditions' => array(
								//'AcceptedStudent.academicyear'=>$thisacademicyear,
								'Student.accepted_student_id in (select id from accepted_students where academicyear="' . $thisacademicyear . '" and college_id=' . $college_id . ')',
								'Student.college_id' => $college_id,
								'Student.program_id' => $kp,
								'Student.program_type_id' => $kpt,
					...
					if ($studentCountInSection == 0) {
						...
						$sectionless_student_count = ClassRegistry::init('StudentsSection')->find('count', array('conditions' => array(
							'StudentsSection.student_id' => $students, 'StudentsSection.archive' => 1,
			...
			if (ROLE_DEPARTMENT == $role_id) {
				$students = $this->Student->find('list', array(
					'conditions' => array(
						...
						'Student.department_id' => $department_id,
						'Student.accepted_student_id in (select id from accepted_students where academicyear="' . $thisacademicyear . '" )',
						"OR" => array('Student.curriculum_id' => array(0, 'null', 'NULL'))
					), 'fields' => array('Student.id', 'Student.id')
				));
			}
			$studentCountInSection = ClassRegistry::init('StudentsSection')->find('count', array('conditions' => array('StudentsSection.student_id' => $students)));
	...
	function getSectionlessStudentCurriculum(
		...
		foreach ($students as $k => $v) {
			$studentCountInSection = ClassRegistry::init('StudentsSection')->find('count', array('conditions' => array(
				'StudentsSection.student_id' => $v['Student']['id'],
				'StudentsSection.archive' => 0
			)));
			if (!$studentCountInSection) {
				$sectionless_student_curriculum_array[] =
					$v['Student']['curriculum_id'];
			}                /*
                $check_student_section = count($v['Section']);
	...
	function getSectionForAssignment($academicyear = null, $college_id = null, $department_id = null, $role_id = null, $selected_program = null, $selected_program_type = null, $yearlevel = null, $selected_curriculum = null)
	{
		...
		if (ROLE_COLLEGE != $role_id) {
			$conditions = array(
				...
				'Section.department_id' => $department_id, 'Section.program_type_id' => $program_type_id,
				'Section.year_level_id' => $yearlevel, 'Section.program_id' => $selected_program,
				'Section.archive' => 0
			);
		} else {
			$conditions = array(
				'Section.academicyear LIKE' => $academicyear . '%', 'Section.college_id' =>
				$college_id, 'Section.program_id' => $selected_program, 'Section.program_type_id' => $program_type_id,
				'Section.archive' => 0,
			...
			foreach ($studentssections as $ssk => $ssv) {
				if (count($ssv['Student']) > 0) {
					...
					if ($nonarchivekey != -1) {
						...
						$student_curriculum_id = $this->Student->field('Student.curriculum_id', array('Student.id' => $first_student_id));
						$sections_curriculum_name[$ssk] =  $this->Student->Curriculum->field('Curriculum.curriculum_detail', array('Curriculum.id' =>
	...
	function get_section_data($section_id = null)
	{
		if (!empty($section_id)) {
			$section_data = $this->find('all', array(
				...
				'fields' => array('Section.id'), 'contain' => array('Student' => array('fields' => array('Student.id')))
			));
	...
	function getSectionCurriculum($sectionId)
	{
		...
		if (!empty($sectionId)) {
			...
			$section_data = $this->find('first', array('conditions' => array('Section.id' => $sectionId), 'fields' => array('Section.id'), 'contain' => array('Student' => array(
				'fields' => array(
					'Student.id', 'Student.curriculum_id',
					'Student.accepted_student_id'
				),
				'conditions' => array('Student.id in (select student_id from course_registrations)'), 'limit' => 1
			))));
			...
			if (!empty($section_data['Student'][0]['id'])) {
				$findCurriculumHistorys = ClassRegistry::init('CurriculumAttachment')->find(
					'all',
					array('conditions' => array('CurriculumAttachment.student_id' => $section_data['Student'][0]['id']), 'recursive' => -1)
				);
			}

			if (count($findCurriculumHistorys) == 1 || empty($findCurriculumHistorys)) {
				...
				foreach ($findCurriculumHistorys as $k => $v) {
					...
					$findOtherStudents = ClassRegistry::init('Student')->find('first', array('conditions' => array(
	...
	function studentsection(
		...
		if ($role_id) {
			...
			if (ROLE_COLLEGE != $role_id) {
				...
				$conditions = array(
					'Section.college_id' => $college_id, 'Section.archive' => 0,
					'Section.program_id' => $selected_program,
					'Section.program_type_id' => $program_type_id,
					'Section.academicyear ' => $academic_year,
					"OR" => array("Section.department_id 
                              is null", "Section.department_id" => array(0, ''))
				);
			}
	...
	function studentsSectionById($sectionid = null)
	{
		if ($sectionid) {
			$students = $this->find('all', array(
				'conditions' => array('Section.id' => $sectionid, 'Section.id in (select section_id from students_sections where section_id=' . $sectionid . ' AND student_id is not null )'),
				'fields' => array(
					'Section.id', 'Section.name', 'Section.program_id',
					'Section.program_type_id', 'Section.college_id', 'Section.department_id', 'Section.academicyear'
				),
				'contain' => array(
					...
					'Student' => array(
						...
						'conditions' => array('Student.id in (select student_id from students_sections where archive=0  and section_id="' . $sectionid . '")'),
						'order' => array('Student.full_name'),
						/*'Course'=>array(
                                'fields'=>array(
                                    'Course.id'
						...
						'CourseAdd' => array('fields' => array(
							'id',
							'student_id', 'published_course_id'
						), 'PublishedCourse' => array('Course' => array('id')))
					)
	...
	function isSectionAssignedStudentsEqualToTotalNumberofAvaliableStudents($data = null, $sectionlesstotalstudents = null)
	{
		...
		if (!empty($data)) {
			...
			if ($sectionlesstotalstudents != $AssignedStudentSum) {
				$this->invalidate('section', 'The sum of assigned students in all section must be equal to 
                    the total number of students those are not assigned to any section which is ' . $sectionlesstotalstudents);
				return false;
			}
		}
		return true;
	...
	function isMoveAllowed($orginal_section_id = null, $student_id = null, $selected_section_move_id = null)
	{
		...
		if (!empty($studentYearLevel['year']) && $studentYearLevel['year'] != $new_section_detail['YearLevel']['name']) {
			$this->invalidate('move_not_allowed', '' . $student_full_name . ' will not able to move to section ' . $new_section_name . '. Because the target section is ' . $new_section_detail['YearLevel']['name'] . ' year while the student is ' . $studentYearLevel['year'] . ' year.');
			return false;
		}
		...
		$latest_semester_academic_year_published_course = $this->PublishedCourse->find('first', array(
			...
			'fields' => array(
				...
				'PublishedCourse.id', 'PublishedCourse.course_id', 'PublishedCourse.academic_year',
				'PublishedCourse.semester', 'PublishedCourse.section_id'
			),
		...
		if (!empty($latest_semester_academic_year_published_course['PublishedCourse']['academic_year']) && !empty($latest_semester_academic_year_published_course['PublishedCourse']['semester'])) {
			...
			$list_published_courses_students_registered = $this->PublishedCourse->find(
				'all',
				array(
					'conditions' => array(
					...
					'contain' => array('CourseRegistration' => array(
						...
						'fields' => array(
							'CourseRegistration.student_id', 'CourseRegistration.id',
			...
			foreach ($list_published_courses_students_registered as
				$list_published => $registered) {
			...
			$new_section_published_course = $this->PublishedCourse->find('all', array(
				'conditions' => array('PublishedCourse.semester' => $latest_semester_academic_year_published_course['PublishedCourse']['semester'], 'PublishedCourse.academic_year' => $latest_semester_academic_year_published_course['PublishedCourse']['academic_year'], 'PublishedCourse.section_id' => $selected_section_move_id),
				'fields' => array('PublishedCourse.id', 'PublishedCourse.course_id'),
				'recursive' => -1
			));
			// if the new section has not already published courses, and student has 
			// already registered for latest published course in his section,don't allow
			// move
			if (empty($new_section_published_course) && $registration_count_with_latest_publish > 0) {
				...
				$this->invalidate('move_not_allowed', '' . $student_full_name . ' will not able to move to section ' . $new_section_name . '.S/he has already registred for her/his course of the most recent academic year and semester in the current section.');
				return false;
			}
			// if the new section published course is not empty and students has not registered 
			// for his section published course allow move as long it has same curriculum.
			if (!empty($new_section_published_course) && $registration_count_with_latest_publish == 0) {
				return true;
			} else if (
				!empty($new_section_published_course)
				&& $registration_count_with_latest_publish > 0
			) {
				// allow but check if the courses he/she registred is same if
				// different dont allow.

				$own_section_published_course = $this->PublishedCourse->find('all', array(
					'conditions' => array('PublishedCourse.semester' => $latest_semester_academic_year_published_course['PublishedCourse']['semester'], 'PublishedCourse.academic_year' => $latest_semester_academic_year_published_course['PublishedCourse']['academic_year'], 'PublishedCourse.section_id' => $orginal_section_id),
					'fields' => array('PublishedCourse.id', 'PublishedCourse.course_id'), 'recursive' => -1
				));
				...
				if ($isEveryCourseBelongs) {
					...
					$getListOfRegistredCourses = $this->Student->CourseRegistration->find('all', array(
						'conditions' => array('CourseRegistration.student_id' => $student_id, 'CourseRegistration.published_course_id' => $previous_section_published_course_ids),
						'recursive' => -1, 'fields' => array('id', 'student_id', 'published_course_id')
					));
					...
					if ($check_grade_not_submitted > 0 && false) {
						...
						$this->invalidate('move_not_allowed', '' . $student_full_name . ' will not able to move to section ' . $new_section_name . '.Because one or more grade for semester for his/her section  has already submitted .');
						return false;
					} else {
					...
					// dont allow
					$this->invalidate('move_not_allowed', '' . $student_full_name . ' will not able to move to section ' . $new_section_name . '. Because the targeted section has different courses.');
					return false;
				}
			}
		} else {
	...
	function isSectionMoveAllowed($orginal_section_id = null, $student_id = null, $selected_section_move_id = null)
	{
		...
		if (!empty($latest_semester_academic_year_published_course['PublishedCourse']['academic_year']) && !empty($latest_semester_academic_year_published_course['PublishedCourse']['semester'])) {
			...
			if (empty($registered_published_course_ids)) {
				...
				$new_section_published_course = $this->PublishedCourse->find('all', array(
					'conditions' => array(
						//'PublishedCourse.semester'=>$latest_semester_academic_year_published_course['PublishedCourse']['semester'],
						'PublishedCourse.academic_year' => $latest_semester_academic_year_published_course['PublishedCourse']['academic_year'], 'PublishedCourse.section_id' => $selected_section_move_id
					),
					'fields' => array('PublishedCourse.id', 'PublishedCourse.course_id'),
					'recursive' => -1
				));
				...
				if (empty($new_section_published_course) && $registration_count_with_latest_publish > 0) {
					...
					$this->invalidate('move_not_allowed', 'The selected student will not able to move to section ' . $new_section_name . '. Because the targeted section has not published semester course but s/he has already registred for his/her course the section targeted for move is not registered for the courses for the given academic year and semester.');
					return false;
				}
				// if the new section published course is not empty and students has not registered 
				// for his section published course allow move as long it has same curriculum.
				if (!empty($new_section_published_course) && $registration_count_with_latest_publish == 0) {
					...
					$own_section_published_course = $this->PublishedCourse->find('all', array(
						'conditions' => array(
							//'PublishedCourse.semester'=>$latest_semester_academic_year_published_course['PublishedCourse']['semester'],
							'PublishedCourse.academic_year' => $latest_semester_academic_year_published_course['PublishedCourse']['academic_year'], 'PublishedCourse.section_id' => $orginal_section_id
						),
						'fields' => array('PublishedCourse.id', 'PublishedCourse.course_id'), 'recursive' => -1
					));
					...
					if ($isEveryCourseBelongs) {
						...
						$list_of_registered_courses_ids = array();
						$getListOfRegistredCourses = $this->Student->CourseRegistration->find('all', array(
							'conditions' => array('CourseRegistration.student_id' => $successMoves, 'CourseRegistration.published_course_id' => $previous_section_published_course_ids),
							'recursive' => -1, 'fields' => array('id', 'student_id', 'published_course_id')
						));
						$prepartedForUpdate = array();
						$counter = 0;
						...
						foreach ($getListOfRegistredCourses as $gk => $gc) {
							if (in_array($gc['CourseRegistration']['published_course_id'], $previous_section_published_course_ids)) {
								if (
									isset($gc['CourseRegistration']['id']) && !empty($gc['CourseRegistration']['id']) &&
									isset($gc['CourseRegistration']['published_course_id']) && !empty($gc['CourseRegistration']['published_course_id'])
								) {
									$prepartedForUpdate['CourseRegistration'][$counter]['id'] = $gc['CourseRegistration']['id'];
									$prepartedForUpdate['CourseRegistration'][$counter]['published_course_id'] = $new_section_publish_courses_ids[$gc['CourseRegistration']['published_course_id']];
									$prepartedForUpdate['CourseRegistration'][$counter]['section_id'] = $selected_section_move_id;
						...
						if ($check_grade_not_submitted > 0 && false) {
							...
							$this->invalidate('move_not_allowed', 'The selected student will not able to move to section ' . $new_section_name . '.Because one or more grade for semester for his/her section  has already submitted .');
							return false;
						} else {
							// allow move and update course registration.
						...
						// dont allow
						$this->invalidate('move_not_allowed', 'The selected student will not able to move to section ' . $new_section_name . '. Because the targeted section has different courses.');
						return false;
					}
				}
			}
		...
		if (!empty($unsuccessMoves)) {
			if (!empty($successMoves)) {
				$this->invalidate('move_not_allowed', '' . count($unsuccessMoves) . ' selected student will not able to move to section ' . $new_section_detail['Section']['name'] . '. Because the target section is ' . $new_section_detail['YearLevel']['name'] . ' year while the student is ' . $studentYearLevel['year'] . ' year. But ' . count($successMoves) . ' has moved to selected section successfully.');
				return $saveSection;
			} else if (!empty($unsuccessMoves)) {
				$this->invalidate('move_not_allowed', '' . count($unsuccessMoves) . ' selected student will not able to move to section ' . $new_section_detail['Section']['name'] . '. Because the target section is ' . $new_section_detail['YearLevel']['name'] . ' year while the student is ' . $studentYearLevel['year'] . ' year.');
				return $saveSection;
			}
		}
		return $saveSection;
	...
	function isSectionMoveAllowedM($orginal_section_id = null, $student_id = null, $selected_section_move_id = null)
	{
		...
		debug($successMoves);
		debug($orginal_section_published_course);
		$getListOfRegistredCourses = $this->CourseRegistration->find('all', array('conditions' => array(
			'CourseRegistration.student_id' => $successMoves,
			'CourseRegistration.published_course_id' => array_keys($orginal_section_published_course)
		), 'contain' => array('ExamGrade'), 'fields' => array('id', 'student_id', 'published_course_id', 'section_id')));

		$prepartedForUpdate = array();
		$counter = 0;
		debug($getListOfRegistredCourses);

		foreach ($getListOfRegistredCourses
			as $gk => $gc) {
			if (
				isset($gc['CourseRegistration']['published_course_id'])
				&& !empty($gc['CourseRegistration']['published_course_id'])
				&& empty($gc['ExamGrade'])
			) {
	...
	function saveSectionMove($studentLists, $orginal_section_id, $selected_section_move_id, $moveRegistration = array())
	{
		...
		if (!empty($sectionMoveSaveAllFormat)) {

			if ($this->StudentsSection->saveAll(
				$sectionMoveSaveAllFormat['StudentsSection'],
				...
				foreach ($sectionMoveSaveAllFormat['StudentsSection'] as $k => $v) {
					/*
						       if(!
ClassRegistry::init('ExamGrade')->isCourseGradeSubmitted($v['student_id'],$orginal_section_id)){
								$permanentDeleteSection[$counter]=$this->StudentsSection->field('StudentsSection.id',array('StudentsSection.student_id'=>$v,'StudentsSection.section_id'=>$orginal_section_id,'StudentsSection.archive'=>0));
							   } else {
                                 $archiveSection['StudentsSection'][$counter]['id']=$this->StudentsSection->field('StudentsSection.id',array('StudentsSection.student_id'=>$v,'StudentsSection.section_id'=>$orginal_section_id,'StudentsSection.archive'=>0));
				...
				if (!empty($archiveSection)) {
					$this->StudentsSection->saveAll(
						$archiveSection['StudentsSection'],
						array('validate' => false)
					);
				}
					...
					if (strcasecmp($section_row['Section']['program_id'], $p_id) == 0) {
						...
						foreach ($section_by_program as $Pt_id => $section_by_program_type) {
							if (strcasecmp($section_row['Section']['program_type_id'], $Pt_id) == 0) {
								$pt_found = true;
								$sections_organized[$p_id][$Pt_id][$section_row['Section']['id']] = $section_row['Section']['name'];
								break 2;
							}
						}
						if ($pt_found == false) {
							$sections_organized[$p_id][$section_row['ProgramType']['name']][$section_row['Section']['id']] = $section_row['Section']['name'];
						}
						break 1;
					}
				}
				if ($p_found == false) {
					$sections_organized[$section_row['Program']['name']][$section_row['ProgramType']['name']][$section_row['Section']['id']] = $section_row['Section']['name'];
				}
			}
		}
			...
			foreach ($sections_data as $sd_key => $section_row) {
				...
				foreach ($sections_organized as $Pt_name => $section_by_program_type) {
					if (strcasecmp($section_row['ProgramType']['name'], $Pt_name) == 0) {
						$pt_found = true;
						$sections_organized[$Pt_name][$section_row['Section']['id']] = $section_row['Section']['name'] . '(' . $section_row['Section']['academicyear'] . ')';
						break 1;
					}
				}
				if ($pt_found == false) {
					$sections_organized[$section_row['ProgramType']['name']][$section_row['Section']['id']] = $section_row['Section']['name'] . '(' . $section_row['Section']['academicyear'] . ')';
				}
			}
		}
	...
	function allStudents($section_id = "")
	{
		$students = $this->find(
			...
			array(
				...
				'contain' => array('Student' => array('order' => array(
					'first_name ASC', 'middle_name ASC',
					'last_name ASC'
				), 'fields' => array('id', 'first_name', 'middle_name', 'last_name', 'studentnumber')))
			)
		);
		...
		if (isset($students['Student'])) {
			foreach ($students['Student'] as $key => $student) {
				if (!ClassRegistry::init('GraduateList')->isGraduated($student['id'])) {
		...
		$students = $this->find(
			...
			array(
				...
				'contain' => array('Student' => array('order' => array(
					'first_name ASC', 'middle_name ASC',
					'last_name ASC'
				), 'fields' => array('id', 'first_name', 'middle_name', 'last_name', 'studentnumber', 'gender', 'full_name')))
			)
		);
	...
	{
		if (!empty($section_id)) {
			$students = $this->StudentsSection->find(
				'all',
				array('conditions' => array('StudentsSection.section_id' => $section_id, 'StudentsSection.archive' => 0))
			);
			return $students;
		}
	}

	function getSectionActiveStudentsId($section_id = null)
	{
		if (!empty($section_id)) {
			$students = $this->StudentsSection->find(
				'list',
	...
	function get_sectionless_students_last_sections($sectionlessStudents_ids = null)
	{
		...
		foreach ($sectionlessStudentsSection_ids as $sssik => $sssiv) {
			$sectionlessStudentsSection_details[$sssik] = $this->find('first', array(
				'conditions' => array('Section.id' => $sssiv['StudentsSection']['section_id']), 'fields' => array('Section.id', 'Section.name', 'Section.year_level_id', 'Section.academicyear'),
				'contain' => array(
					'YearLevel' => array(
						'fields' => array(
							'YearLevel.name'
	...
	{
		$exam_grade_detail = $this->PublishedCourse->CourseRegistration->ExamGrade->find(
			'first',
			array(
				'conditions' =>
				array(
				...
				'contain' => array('CourseAdd' => array('PublishedCourse' => array('Section')), 'CourseRegistration' => array('PublishedCourse' => array('Section')))
			)
		);
		$course = null;
	...
	function getSectionStudents($section_id = null, $name = null)
	{
		...
		if (!empty($name)) {
			$students_in_section = $this->StudentsSection->find(
				'all',
				array(
					'conditions' =>
					array(
 ...
 in (select id from students where first_name like "' . $name . '%")'
					),
	...
	function studentsAlreaydTakenCourse($sectionid = null)
	{
		...
		if ($sectionid) {
			...
			$student_section = $this->StudentsSection->find(
				'first',
				array(
					'conditions' =>
					array(
			...
			///////////////////////////////////////////////////////////////////////
			$student_sections = $this->StudentsSection->find(
				'all',
				array(
					'conditions' =>
					array(
			...
			$student_previous_section = $this->StudentsSection->find(
				'all',
				array('conditions' => array(
					'StudentsSection.student_id' => $student_idss, 'StudentsSection.archive' => 1
				), 'contain' => array())
			);
			...
			$students = $this->find('all', array(
				'conditions' => array('Section.id' => $previous_sections),
				'fields' => array('Section.id', 'Section.name'),
				'contain' => array(
					'Student' => array(
						'fields' => array(
	...
	{
		$count = $this->StudentsSection->find('count', array('conditions' => array('StudentsSection.section_id' => $section_id, 'StudentsSection.archive' => 0)));
		return $count;
	}
			...
			foreach ($year_levels as $year_level) {
				...
				if ((isset($yearLevel['YearLevel']['id']) || strcasecmp($dep_key, 'FP') == 0) && !empty($yearLevel['YearLevel']['id'])) {
					...
					foreach ($sections_t as $section_key => $section) {
						$examPeriod = $this->Program->ExamPeriod->find(
							'first',
							array(
								'conditions' =>
								array(
	...
	public function dropOutWithDrawAfterLastRegistrationNotReadmittedExcludeFromSectionless(
		...
		$last_registration_date = $this->Student->CourseRegistration->find(
			...
			array('conditions' => array(
				'CourseRegistration.student_id' => $student_id
			), 'order' => array('CourseRegistration.created DESC'), 'recursive' => -1)
		);
		if (!empty($last_registration_date['CourseRegistration'])) {
			$check_dropout = ClassRegistry::init('DropOut')->find(
				'count',
				array('conditions' => array(
		...
		if ($check_dropout > 0) {
			...
			if (!empty($last_registration_date)) {
				$check_withdraw = ClassRegistry::init('Clearance')->find(
					'count',
					array('conditions' => array(
			...
			// is dismissed and 
			if (!empty($last_registration_date['CourseRegistration'])) {
				$check_dismissal = ClassRegistry::init('Dismissal')->find(
					'count',
					array('conditions' => array(
						'Dismissal.student_id' => $student_id,
						'Dismissal.dismisal_date >= ' =>
						$last_registration_date['CourseRegistration']['created']
					))
				);
			}
	...
	function getStudentSectionInGivenAcademicYear(
		$academic_year,
		$student_id
	) {
		...
		$options['conditions'][] = 'Section.id  IN (SELECT section_id FROM students_sections
	...
	function getStudentSectionHistory($student_id)
	{
		...
		foreach ($sections as $k => &$v) {
			$archived = ClassRegistry::init('StudentsSection')->find(
				'first',
				array('conditions' => array(
	...
	function rearrangeSectionList(
		...
		if ($pre == 1) {
			$options['conditions'][] = 'Student.department_id is null and 
Student.college_id=' . $department_id . '';
		...
		$option1['conditions'][] = 'Student.id  IN (SELECT student_id FROM students_sections
		       where section_id in (' . join(',', $sections) . '))';
		...
		foreach ($sections as $k => $v) {
			...
			foreach ($studentOrganizedFakeSection[$lastStop] as $sk => $sv) {
				...
				$belongToSection = ClassRegistry::init('StudentsSection')->find(
					'count',
					array('conditions' => array(
						'StudentsSection.section_id' => $v, 'StudentsSection.student_id' => $sv,
						'StudentsSection.archive' => 0
					))
				);
				$ownSection = ClassRegistry::init('StudentsSection')->find(
					'first',
					array(
						'conditions' => array('StudentsSection.archive' => 0, 'StudentsSection.student_id' => $sv),
						'recursive' => -1, 'order' => 'StudentsSection.created DESC'
					)
				);
				if (!$belongToSection) {
	...
	function swampTheWholeStudentInSpecificBatch(
		$bathacademicYear,
		$academic_year,
		$semester,
		$department_id,
		...
		if ($pre == 1) {
			$options['conditions'][] = 'Student.department_id is null and 
Student.college_id=' . $department_id . '';
			$owsSection['conditions'][] = 'StudentsSection.section_id in 
(select id from sections where department_id is null and college_id=' . $department_id . '
and academicyear="' . $academic_year . '" and year_level_id=' . $year_level . ' and 
program_id=' . $program_id . ' and program_type_id=' . $program_type_id . ' )';
		} else {
			$options['conditions']['Section.department_id'] = $department_id;
			$owsSection['conditions'][] = 'StudentsSection.section_id in 
(select id from sections where department_id=' . $department_id . '
and academicyear="' . $academic_year . '" and year_level_id=' . $year_level . ' and 
program_id=' . $program_id . ' and program_type_id=' . $program_type_id . ' )';
		}
		...
		if ($pre == 1) {
			$option1['conditions'][] = 'Student.department_id is null and 
Student.college_id=' . $department_id . '';
		} else {
		...
		foreach ($sections as $k => $v) {
			...
			$sectionCoursePublication = $this->PublishedCourse->find(
				'all',
				array('conditions' => array(
			...
			foreach ($studentOrganizedFakeSection[$lastStop] as $sk => $sv) {
				$belongToSection = ClassRegistry::init('StudentsSection')->find(
					'count',
					array('conditions' => array('StudentsSection.section_id' => $v, 'StudentsSection.student_id' => $sv))
				);
				if (!$belongToSection) {
					//update the student section with the new one
					...
					if (isset($sv) && !empty($sv) && !empty($ownSection)) {
						$ownSectionPublishedCourse = $this->PublishedCourse->find(
							'all',
							array('conditions' => array('PublishedCourse.section_id' => $ownSection['StudentsSection']['section_id'], 'PublishedCourse.semester' => $semester), 'recursive' => -1)
						);
	...
	function swampStudentSection($student_detail, $previouse_section_id, $target_section_id)
	{
		...
		$previousSectionPublishedCourse = $this->PublishedCourse->find('all', array('conditions' => array('PublishedCourse.section_id' => $previousSection['Section']['id']), 'recursive' => -1));
		$targetSectionPublishedCourse = $this->PublishedCourse->find('all', array('conditions' => array('PublishedCourse.section_id' => $targetSection['Section']['id']), 'recursive' => -1));
		// allow movement if the target section and own section has similar course id and number of publication is same
		if (
			count($previousSectionPublishedCourse)
			== count($targetSectionPublishedCourse)
		) {
			// is the course similar
			if ($this->similarCourseInSection($previousSectionPublishedCourse, $targetSectionPublishedCourse)) {
				...
				$courseRegistrationUpdate['CourseRegistration'] = array_merge(
					...
					$this->publicationMappingWithCourseRegistration($previousSectionPublishedCourse, $target_section_id, $student_detail['Student']['id'])
				);
				// student section ready for update 
				$newSection = array();
				$newSection['id'] = $ownSection['StudentsSection']['id'];
	...
	function publicationMappingWithCourseRegistration($ownSectionPublishedCourse, $targetSectionId, $studentId)
	{
		...
		foreach ($ownSectionPublishedCourse as $k => $v) {
			$courseRegistration = $this->PublishedCourse->CourseRegistration->find(
				'first',
				array('conditions' => array(
			...
			$findTargetSectionPid = $this->PublishedCourse->find(
				'first',
				array('conditions' => array(
	...
	function automaticDownGradeSection(
		...
		if ($pre == 1) {
			$studentLists = ClassRegistry::init('Student')->find('list', array('conditions' => array('Student.college_id' => $department_college_id, 'Student.department_id is null', 'Student.admissionyear' => $admissionYear), 'fields' => array('Student.id', 'Student.id')));
		} else {
			$studentLists = ClassRegistry::init('Student')->find('list', array('conditions' => array('Student.department_id' => $department_college_id, 'Student.admissionyear' => $admissionYear), 'fields' => array('Student.id', 'Student.id')));
		}
		...
		foreach ($studentLists as $k => $student_id) {
			$findAllSections = ClassRegistry::init('StudentsSection')->find('list', array(
				...
				'order' => 'StudentsSection.created ASC'
			));
			if (!empty($findAllSections)) {
				$findCourseRegistration = $this->CourseRegistration->find('all', array(
					...
					'recursive' => -1, 'group' => array('CourseRegistration.academic_year DESC', 'CourseRegistration.semester')
				));
		...
		foreach ($selectedSectionIds as $secId => $Id) {
			if (!empty($Id)) {
				...
				if (!empty($precedingSection)) {
					...
					$currentStudentInSection = $this->StudentsSection->find('list', array('conditions' =>
					array(
						...
						if (!empty($currentStudentInSection)) {
							if ($this->StudentsSection->deleteAll(array('StudentsSection.id' => $currentStudentInSection), false)) {
								...
								if ($this->StudentsSection->updateAll(
									array('StudentsSection.archive' => 0),
	...
	function getPrecedingSection($secId)
	{
		...
		if (!empty($section)) {
			...
			$previousYearLevelOfSection = $this->YearLevel->find('first', array(
				'conditions' => array('YearLevel.department_id' => $section['Section']['department_id'], 'YearLevel.id <' => $section['Section']['year_level_id']),
				'recursive' => -1, 'order' => 'YearLevel.id DESC'
			));
			debug($this->getPrecedingSectionName($section['Section']['name']));
			$previousSection = $this->find('first', array('conditions' => array(
		...
		foreach ($selectedSectionIds as $key => $secId) {
			if (!empty($secId)) {
				...
				if (!empty($selectedSectionStudents)) {
					...
					$count = 0;
					foreach ($selectedSectionStudents
						as $sssk => $sssv) {
						$student_status = ClassRegistry::init('StudentExamStatus')->isStudentPassed($sssv['StudentsSection']['student_id']);
						if (
							$student_status == 4 ||
							$student_status == 2
						) {
					...
					if (isset($categorizeSelectedSectionStudents['upgradable']) && !empty($categorizeSelectedSectionStudents['upgradable'])) {
						...
						if ($this->StudentsSection->updateAll(
							array('StudentsSection.archive' => 1),
	...
	function findMeRepresentativeStudentInSection($student_detail, $currentAcademicYear, $studentAdmissionYear, $sectionInThisAC)
	{
		...
		do {
			...
			if (
				!empty($student_detail['Student']['department_id'])
				&& !empty($student_detail['Student']['department_id'])
			) {
	...
	function getStudentActiveSection($student_id)
	{
		$sectionDetail = array();
		if (!empty($student_id)) {
			...
			$students = $this->StudentsSection->find(
				'first',
				array('conditions' => array('StudentsSection.student_id' => $student_id, 'StudentsSection.archive' => 0), 'order' => array('StudentsSection.created DESC'))
			);

			if (!empty($students['StudentsSection'])) {
				$section = $this->find('first', array('conditions' => array('Section.id' =>
				$students['StudentsSection']['section_id']), 'contain' => array(
					'YearLevel',
					'Department', 'College', 'Program', 'ProgramType'
				)));
				return $section;
			}
		}
			...
			if (!empty($yearLevels)) {
				$student_previous_section = $this->StudentsSection->find(
					...
					array('conditions' => array(
						...
						'StudentsSection.section_id in (select id from sections 
where year_level_id in (' . join(',', $yearLevels) . ') )'

					), 'contain' => array())
				);
			} else {
				/*
                   $student_previous_section=$this->StudentsSection->find('all',
				array('conditions'=>array(
					'StudentsSection.student_id'=>$student_idss,
                   //'StudentsSection.archive' => 1,
			...
			if (!empty($student_previous_section)) {
				foreach ($student_previous_section as $pindex => $pvalue) {
					...
					$currentSectionYearLevelId = $this->field(
						'Section.year_level_id',
						array('Section.id' => $sectionid)
					);
					...
					if (
						$pvalue['StudentsSection']['section_id'] != $sectionid
						&& in_array($academicYear, $previousAcademicYears) &&
			...
			//half of the students will be consider to put on taken course list 
			foreach ($published_courses
				as $pk => $pv) {
				$majorityNumber = ($totalStudentsInSection / 2 + 1);
				debug($student_idss);
				debug($this->CourseRegistration->ExamGrade->is_grade_submitted($pv['PublishedCourse']['id'],$student_idss));
				debug($pv['PublishedCourse']['id']);
				debug($pv['PublishedCourse']['department_id']);
				debug($majorityNumber);
				debug($pv['PublishedCourse']['course_id']);
				if (
					$this->CourseRegistration->ExamGrade->is_grade_submitted($pv['PublishedCourse']['id'],$student_idss)
					>
					($majorityNumber) &&
					isset($pv['PublishedCourse']['course_id'])
				) {

					$section_already_taken_courses[$sectionid][] =
						$pv['PublishedCourse']['course_id'];
				}
			}
	...
	function updateCourseRegistrationAndSection(
		$admissionYear,
		$departmentId,
		$academicYear,
		$semester,
		...
		if ($pre == 1) {
			$studenLists = ClassRegistry::init('Student')->find('all', array('conditions' => array(
				'Student.college_id' => $departmentId, 'Student.admissionyear' => $AcademicYear->get_academicYearBegainingDate($admissionYear)
			), 'contain' => array('Student')));
		} else {
			$studenLists = ClassRegistry::init('Student')->find(
				'all',
				array(
					'conditions' => array(
		...
		foreach ($studenLists as $kkk => $vv) {
			$currentsections = ClassRegistry::init('StudentsSection')->find('first', array('conditions' => array(
			...
			$courseRegistration = ClassRegistry::init('CourseRegistration')->find('all', array('conditions' => array(
				'CourseRegistration.student_id' => $vv['Student']['id'], 'CourseRegistration.academic_year' => $academicYear,
				'CourseRegistration.semester' => $semester
			), 'contain' => array('PublishedCourse')));

			foreach ($courseRegistration as $k => $v) {
				$currentSectionPublished = ClassRegistry::init('PublishedCourse')->find(
					'first',
					array('conditions' => array(
						'PublishedCourse.section_id' => $currentsections['StudentsSection']['section_id'], 'PublishedCourse.academic_year' => $academicYear, 'PublishedCourse.semester' => $semester,
						'PublishedCourse.course_id' => $v['PublishedCourse']['course_id']
					), 'recursive' => -1)
				);
	...
	{
		$student_registration = ClassRegistry::init('CourseRegistration')->find(
			'first',
			array(
				'conditions' =>
				array(
	...
	{
		$yearAcademic = array();
		$stsec = $this->CourseRegistration->find(
			'first',
			array(
				'conditions' => array(
					'CourseRegistration.student_id' => $student_id,
		...
		if (!empty($stsec)) {
			...
			$studentSection = $this->find(
				'first',
				array(
					'conditions' =>
					array(
						'Section.id' =>
						$stsec['CourseRegistration']['section_id']
					),
					'contain' => array('YearLevel'),
		...
		$yearAcademic['academicyear'] =
			$studentSection['Section']['academicyear'];
	...
	public function autoSectionUpgrade($acadmicYear, $department_id = null)
	{
		...
		foreach ($sections as $section) {
			$sections_lastpublishedcourses_list[$section['Section']['id']] =
				$this->PublishedCourse->lastPublishedCoursesForSection($section['Section']['id']);
		}
