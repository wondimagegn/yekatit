...
class CourseRegistration extends AppModel {
	...
	*/
	function alreadyRegistred($semester=null,$academic_year=null,$student_id=null) {
	    ...
	    $already_registered=$this->find('count',array('conditions'=>
	    array('CourseRegistration.academic_year LIKE'=>$academic_year.'%','CourseRegistration.student_id'=>$student_id,'CourseRegistration.semester'=>$semester)));
	...
	*/ 
	function latestCourseRegistrationSemester ($academic_year=null,$student_id=null) {
				...
				if (!empty($academic_year) && !empty($student_id)) {
				      $latestSemester=$this->find('all',
				    array('fields'=>array('CourseRegistration.semester','CourseRegistration.academic_year'),'conditions'=>array('CourseRegistration.academic_year like '=>$academic_year.'%',
	...
	function futureAcademicYearCoursePublished() {
				...
				$latestSemester=$this->PublishedCourse->find('all',
				array('fields'=>array('PublishedCourse.academic_year')));
	...
	*/
	function TakenPrequisteCourse($course_id=null,$student_id=null) {
	        ...
	        /*
	        $courseregistrationdetail=$this->find('all',array('conditions'=>array(
	        'CourseRegistration.student_id'=>$student_id,
	...
	*/
	function latest_academic_year_semester ($academic_year=null) {
	   $ac_semester=array();
	   if ($academic_year) {
           $latest_semester=$this->PublishedCourse->find('first',
           array('conditions'=>array('PublishedCourse.academic_year 
           like '=>$academic_year.'%'),'fields'=>array("MAX(PublishedCourse.created)",'PublishedCourse.semester','PublishedCourse.academic_year'),
           'group'=>'PublishedCourse.semester',
           'order'=>"MAX(PublishedCourse.created) desc",'contain'=>array()));
         }
	...
	*/
	function latest_semester_of_section ($section_id=null,$current_academic_year=null) {
       ...
       $publishedcourses=$this->PublishedCourse->find('all',
       array('conditions'=>array('PublishedCourse.academic_year like '=>$current_academic_year.'%',
       ...
       if (!empty($published_course_ids)) {
        ...
        'conditions'=>array('PublishedCourse.academic_year like '=>$current_academic_year.'%',
        'PublishedCourse.id'=>$published_course_ids,
        'PublishedCourse.drop=0'),'recursive'=>-1));
	...
	*/
	function checkCourseIsPublishedForSection ($section_id=null,$current_academic_year=null) {
       ...
       $publishedcourses=$this->PublishedCourse->find('all',
       array('conditions'=>array('PublishedCourse.academic_year'=>$current_academic_year,
       'PublishedCourse.section_id'=>$section_id),'recursive'=>-1));
       ...
       foreach ($publishedcourses as $index=>$value) {
          		...
          		$grade_submitted=$this->ExamGrade->is_grade_submitted($value['PublishedCourse']['id']);
          		$registred_count = $this->find('count',array('conditions'=>
          		array('CourseRegistration.published_course_id'=>$value['PublishedCourse']['id'])));
       			if($grade_submitted) {
       ...
       if (!empty($published_course_ids)) {
        ...
        'PublishedCourse.section_id'=>$section_id,
        'PublishedCourse.id'=>$published_course_ids),'recursive'=>-1));
	...
	function checkMergingIsPossible($new_section_id=null,
	$section_ids=array(),$current_academic_year=null ) {
	   /*
	   ...
	   * 4. If grade is submitted to any of the published course of merge request section, deny
	   *    merge
	   * 
	   */
	    ...
	    if (!empty($section_ids)) {
	       ...
	       $sectionEarlierYearLevels = array();
	        ...
	        foreach ($section_ids as $k=>$v) {
	                 $students=$this->Student->StudentsSection->find('first',
	         ...
	         $studentsSectionHistory=$this->Student->find('all',
	           array('conditions'=>array('Student.id'=>$representativeStudentOfSection,
	           ...
	           'contain'=>array('StudentsSection')));
                ...
                foreach ($studentsSectionHistory as $k=>$v) {
                     ...
                     foreach ($v['StudentsSection'] as $sk=>$sv) {
                        if ($representativeSectionStudent[$v['Student']['id']] != 
                        $sv['section_id']) {
                         $EarlierSections[
                         $representativeSectionStudent[$v['Student']['id']]][]=$sv['section_id']; 
                        }
                     }    
                }
	           ...
	           foreach ($section_ids as $k=>$v) {
	                $yearLevelOfMergerSection=$this->Section->find('first', array('conditions'=>array(
	                'Section.id'=>$v),'contain'=>array('YearLevel')));    
			if(isset($EarlierSections[$v])){
	                foreach ($EarlierSections[$v] as $EK=>$EV) {
	                        // check the merged section id has higher year level 
	                   
	                     $yearLevelOfEarlierSection=$this->Section->find('first', 
	                     array('conditions'=>array('Section.id'=>$EV),'contain'=>array('YearLevel'))
	                     );
	                     // allow only recent year level of the section for merging 
	                     if (strcmp($yearLevelOfEarlierSection['YearLevel']['name'],
	                     $yearLevelOfMergerSection['YearLevel']['name'])>0) {
	                        
	                       // Merging of the selected 
	                   ...
	                   $coursesTakenThrughout[$v][$EV]=$this->PublishedCourse->find('all', array('conditions'=>array(
	                'PublishedCourse.section_id'=>$EV,'PublishedCourse.drop'=>0),
	                'fields'=>array('PublishedCourse.course_id'),'contain'=>array('Course')));                
	                        
	                }
	                  $coursesTakenThrughout[$v][$v]=$this->PublishedCourse->find('all', array('conditions'=>array(
	                'PublishedCourse.section_id'=>$v,'PublishedCourse.drop'=>0),
	                'fields'=>array('PublishedCourse.course_id'),'contain'=>array('Course')));  
		      }
			...
			if(isset($coursesTakenThrughout[$v])){  
	          ...
	          foreach ($section_ids as $k=>$v) {
	              ...
	              // use compare array, array_diff, to find out  takenCourseOrganizedByCourseId
	              // is differ
	               foreach ($section_ids as $kk=>$vv) {
			          if(isset($takenCourseOrganizedByCourseId[$v])){
				   $result = array_diff($takenCourseOrganizedByCourseId[$v],
				   $takenCourseOrganizedByCourseId[$vv]);
				   if (!empty($result)) {
				      return 3;
				   } 
				}   
	          ...
	          // Now the number of courses taken is similar, we need to check 
	          // the  published courses in given academic year. If grade submission of 
	          // any published course is started, deny the merge request
	          // If grade submission is not started, then merge the section , and 
	          // update, published_courses section_id , course_registration section_id, 
	          // Course_Instructor_Assignment section_id by the new merged section_id 
	         
	         
              foreach ($section_ids as $k=>$v) {
                    $tmp=$this->PublishedCourse->find('all',
                   array('conditions'=>array('PublishedCourse.academic_year'=>$current_academic_year,
                    ...
                    foreach ($tmp as $ppin=>$ppvalue) {
                  		$grade_submitted=$this->ExamGrade->is_grade_submitted($ppvalue['PublishedCourse']['id']);
               			if($grade_submitted) {
               				 ...
               				 return 4; // grade is submitted to one course so merging is not possible right now, you 
               				          // can merge them in the next year level 
               			} 
	...
	$newSectionId) {
	         ...
	         if (!empty($publishedCourseIds)) {
	          $publishedCourseCommaSeparted=  join(', ',$publishedCourseIds);
	           ...
	           $this->query("UPDATE  published_courses SET  
	           section_id = ".$newSectionId." WHERE id in (".$publishedCourseCommaSeparted.")");
	           
	            // update course registrations section with the new section  
	           $this->query("UPDATE course_registrations SET  
	           section_id = ".$newSectionId." WHERE published_course_id in (".$publishedCourseCommaSeparted.")");
	           
	             // update course_instructor_assignments section with the new section  
	           $this->query("UPDATE course_instructor_assignments SET  
	           section_id = ".$newSectionId." WHERE published_course_id in (".$publishedCourseCommaSeparted.")");
	...
	$new_section_id=null,$selected_sections_for_merge_ids=null,$current_academic_year=null) {
       
	   $publishedcourses=array();
       foreach ($selected_sections_for_merge_ids as $i=>$v) {
            $tmp=$this->PublishedCourse->find('all',
           array('conditions'=>array('PublishedCourse.academic_year like '=>$current_academic_year.'%',
           'PublishedCourse.section_id'=>$v),'recursive'=>-1));
            if (!empty($tmp)) {
       ...
       if (empty($publishedcourses)) {
                return 2; // allow merge happens for the first time or course not published.
       }
                    ...
                    foreach ($value as $ppin=>$ppvalue) {
                  		$grade_submitted=$this->ExamGrade->is_grade_submitted($ppvalue['PublishedCourse']['id']);
               			if($grade_submitted) {
               			    ...
               			    $published_course_ids_not_grade_submitted[$section_id][$ppvalue['PublishedCourse']['id']]=
               			    $ppvalue['PublishedCourse']['course_id'];
               			}
         ...
         if (!empty($published_course_ids_not_grade_submitted)) 
         {
               foreach ($selected_sections_for_merge_ids as $mid=>$msection_id) {
                     ...
                     if (!empty($published_course_ids_not_grade_submitted[$msection_id])) {
                              ...
                              foreach ($published_course_ids_not_grade_submitted[$msection_id] as 
                                $pub_course_id=>$course_id) {
                                  // compare with other section 
                                
                                  foreach ($published_course_ids_not_grade_submitted as 
                                        $sec_id=>$publish_course_ids) {
                                         if ($sec_id!=$msection_id) {
                                              foreach ($publish_course_ids as 
                                                    $other_published_id=>$other_course_id) {
                                                      if ($other_course_id == $course_id) {
                                                            $number_of_course_similar[$msection_id]
                                                            [$sec_id]=$course_id;
                                                            break 1;
                                                     }
                                                }
	...
	function getCourseRegistrationLatestGrade($course_registration_id = null) {
		...
		$latest_grade = "";
		foreach($grade_histories as $key => $grade_history) {
			if($grade_history['ExamGrade']['grade'] != $latest_grade &&
	...
	function getCourseRegistrationLatestGradeDetail($course_registration_id = null,$reg=1) {
		...
		foreach($grade_histories as $key => $grade_history) {
			if(strcasecmp($grade_history['type'], 'Register') == 0 ||
	...
	//Return grade detail for course registration only if it is approved by the department and college
	...
	function getCourseRegistrationLatestApprovedGradeDetail($course_registration_id = null) {
		...
		foreach($grade_histories as $key => $grade_history) {
			...
			if(((strcasecmp($grade_history['type'], 'Register') == 0 && !empty($grade_history['ExamGrade']) && $grade_history['ExamGrade']['department_approval'] == 1 && $grade_history['ExamGrade']['registrar_approval'] == 1) ||
				(strcasecmp($grade_history['type'], 'Change') == 0 && $grade_history['ExamGrade']['makeup_exam_result'] == null && $grade_history['ExamGrade']['department_approval'] == 1 && $grade_history['ExamGrade']['college_approval'] == 1 && $grade_history['ExamGrade']['registrar_approval'] == 1) ||
				(strcasecmp($grade_history['type'], 'Change') == 0 && $grade_history['ExamGrade']['makeup_exam_result'] != null && $grade_history['ExamGrade']['initiated_by_department'] == 0 && $grade_history['ExamGrade']['department_approval'] == 1 && $grade_history['ExamGrade']['registrar_approval'] == 1) ||
				(strcasecmp($grade_history['type'], 'Change') == 0 && $grade_history['ExamGrade']['makeup_exam_result'] != null && $grade_history['ExamGrade']['initiated_by_department'] == 1 && $grade_history['ExamGrade']['department_approval'] == 1 && $grade_history['ExamGrade']['registrar_approval'] == 1) || (strcasecmp($grade_history['type'], 'Change') == 0 && $grade_history['ExamGrade']['manual_ng_conversion'] == 1 )
	...
	function isCourseDroped($course_registration_id = null) {
		$course_registration_detail = $this->find('first',
			array(
				'conditions' =>
				array(
		...
		if(!empty($course_registration_detail['CourseDrop'])) {
			if($course_registration_detail['CourseDrop'][0]['forced'] == 1 || ($course_registration_detail['CourseDrop'][0]['department_approval'] == 1 && $course_registration_detail['CourseDrop'][0]['registrar_confirmation'] == 1))
				...
				/*
				The following code is commented out later to be completely removed as 
				the published course is updated when it is converted to drop 
				(a new record is not created). So we are going to check the published course itself.
				But it has not logical defect, as I saw it, if it is used as it is
				*/
				/*$drop_published_course = $this->PublishedCourse->find('first', 
					array(
						'conditions' =>
						array(
	...
	function alreadyRegistered ($student_id=null, $academic_year = null,$semester=null ) {
	        ...
	        $count = $this->find('count',array('conditions'=>array('CourseRegistration.student_id'=>$student_id,
	 ...
	 function not_allow_f_prerequisite ($publishedCourses=null,$student_id=null) {
			  ...
			  foreach ($publishedCourses as $index=>$value) {
			        ...
			        if (!empty($value['Course']['Prerequisite'])) {
			                 $passed_count=0;
			                 foreach ($value['Course']['Prerequisite'] as 
			                 $preindex=>$prevalue) {
...
*/
			                       ...
			                       if ($prevalue['co_requisite']==1) {
			                            ...
			                            $ready_registred_course_ids)) {
			                              $passed_count['passed']=$passed_count['passed']+1; 
			                                ...
			                                $pre_passed=$this->CourseDrop->prequisite_taken($student_id,
			                                $prevalue['prerequisite_course_id']); 
	...
	*/
	function registerSingleStudent($student_id=null, $academic_year=null,$semester=null) {
	    ...
	    $passed_or_failed=$this->Student->StudentExamStatus->
	    get_student_exam_status($student_id,$latest_academic_year);
	    if(empty($semester)) {
	    	$latestAcSemester= $this->getLastestStudentSemesterAndAcademicYear(
	   ...
	   /////////////////////////////////////////////////////////////////////////// 
	    ...
	    $get_student_acadamic_status=$this->Student
	        ->StudentExamStatus->getStudentAcadamicStatus($student_id,
	        $latest_academic_year,$latestSemester);

	       debug($get_student_acadamic_status);
	       $student_section= $this->Student->student_academic_detail(
	       $student_id,$latest_academic_year);
	       ...
	       if (!empty($student_section)) {
	            ...
	            'Course.tutorial_hours','Course.credit')))));
	             ...
	             $published_courses= $this->
	             getRegistrationType($published_courses,$student_id,$get_student_acadamic_status);
	  ...
	  */
	  function getLastestStudentSemesterAndAcademicYear ($student_id=null, 
	  $current_academic_year=null,$add_drop=0) {
	      ...
	      $list_of_academic_year_semester_student_attended = $this->ExamGrade->getListOfAyAndSemester($student_id);
			 ...
			 if (empty($list_of_academic_year_semester_student_attended)) {
			         ...
			         return $lastest_semester_academic_year;
			          ...
			          if(strcasecmp($current_academic_year,
			          $last_end_academic_year_semester['academic_year'])==0) {
			              if ($add_drop) {
			 
			                  $lastest_semester_academic_year['semester']=
			                  $last_end_academic_year_semester['semester'];
			                  $lastest_semester_academic_year['academic_year']=$current_academic_year;
			                  return $lastest_semester_academic_year;
			              } else {
			                  $next_semester=$this->Student->StudentExamStatus->
	           getNextSemster($current_academic_year,$last_end_academic_year_semester['semester']);
			          ...
			          } else if (strcasecmp($current_academic_year,
			          $last_end_academic_year_semester['academic_year'])>0) {
			                 //lastest published academic year and semester 
			                $lastest_semester_academic_year= $this->latest_academic_year_semester($current_academic_year);
			    return $lastest_semester_academic_year; 
			          }
			 }             
	  }

	 public function paySemesterAndAcademicYear($student_id,$current_academicYear=null){
		$currentAcSem=array();
		if(isset($student_id) && !empty($student_id)){
			$currentSection=$this->PublishedCourse->Section->find('first',array('conditions'=>array('Section.id in (select section_id from students_sections where archive=0 and student_id='.$student_id.') '),'recursive'=>-1));
			if(isset($currentSection) && !empty($currentSection)){
				$publishedCourses=$this->PublishedCourse->find('first',array('conditions'=>array(
'PublishedCourse.section_id'=>$currentSection['Section']['id']),
...
'recursive'=>-1));
		...
		if(isset($publishedCourses) && !empty($publishedCourses)){
			$currentAcSem['academic_year']=$publishedCourses['PublishedCourse']['academic_year'];
			$currentAcSem['semester']=$publishedCourses['PublishedCourse']['semester'];
		}
		if(empty($currentAcSem)){
			$currentAcSem['academic_year']=$current_academicYear;
			$currentAcSem['semester']="I";
		}
		return $currentAcSem;
	 }
	...
	/**
	* mass registration of selected sections
	*/
	function massRegisterStudent($section_id=null, $academic_yearr=null) {
	       ...
	       $published_courses=$this->PublishedCourse->find('all',
	        ...
	        foreach($students_list as $k=>$value) {
	           ...
	           $alreadyRegistered = $this->alreadyRegistered($value,
	           ...
	           if (!$alreadyRegistered) {
                        ...
                        /* $isThereFxInPrevAcademicStatus=$this->Student->
StudentExamStatus->checkFxPresenseInStatus($value);
		         */
                        if ($get_student_acadamic_status==1 || $get_student_acadamic_status==3) {
	                           $published= $this->
	                             getRegistrationType($published_courses,$value,
	        ...
	        foreach($validCourseRegistrationLists as $student_id=>$value) {
	              ...
	              foreach($value['register'] as $k=>$publishedvalue) {
	                ...
	                //registration checking
                       ...
                       if (!isset($publishedvalue['prequisite_taken_passsed']) && !isset($publishedvalue['exemption']) 
	               || 
                     (isset($publishedvalue['prequisite_taken_passsed']) && 
                     $publishedvalue['prequisite_taken_passsed']==1 )) {
                     
                         $formattedSaveAllRegistration['CourseRegistration'][$count]['published_course_id']
                         =$publishedvalue['PublishedCourse']['id'];
                          $formattedSaveAllRegistration['CourseRegistration'][$count]['course_id']
                         =$publishedvalue['PublishedCourse']['course_id'];
                         
                           $formattedSaveAllRegistration['CourseRegistration'][$count]['semester']
                         =$publishedvalue['PublishedCourse']['semester'];
                         
                            $formattedSaveAllRegistration['CourseRegistration'][$count]['academic_year']
                         =$publishedvalue['PublishedCourse']['academic_year'];
                         ...
                         =$publishedvalue['PublishedCourse']['section_id'];
                         
                         $formattedSaveAllRegistration['CourseRegistration'][$count]['year_level_id']
                         =$publishedvalue['PublishedCourse']['year_level_id'];
                     ...
                     && !isset($pv['exemption']))) {
                         ...
                         $formattedSaveAllRegistration['CourseRegistration'][$count]['type']
                         =11;
	        ...
	        if (
	        isset($formattedSaveAllRegistration['CourseRegistration']) && 
	        !empty($formattedSaveAllRegistration['CourseRegistration'])) {
                
	            if ($this->saveAll($formattedSaveAllRegistration['CourseRegistration'],
	                        array('validate'=>false))) {
	                     return 1; //registered successfully
	            } else {
	                     return 2;  // registeration not successful
	            } 
	       }
	       return 3;  // all students are registered except those who doesnt quality
	}
	...
	*/
	function courseRegistered($published_course_id=null,$semester=null,$academic_year=null,$student_id=null) {
	    ...
	    $already_registered=$this->find('count',array('conditions'=>
	    array('CourseRegistration.academic_year'=>$academic_year,
	    'CourseRegistration.student_id'=>$student_id,
    ...
    function getRegistrationStats($acadamic_year,$semester,
    $program_id = null, $program_type_id = null, $department_id = null,$type=null) {
		    ...
		    if (isset($acadamic_year) && isset($semester)) {
		       ...
		       where academic_year="'.$acadamic_year.'" and semester="'.$semester.'")';
					   ...
					   'StudentExamStatus'  => 
						...
						'CourseRegistration'  => 
							array(
								...
								'fields'=>array('CourseRegistration.academic_year',
...
'ProgramType' => 
	   ...
	   // debug($total_student);
		...
		foreach($students as $key => $student) {
			/*
			$section=ClassRegistry::init('Section')->getStudentSectionInGivenAcademicYear(
			$acadamic_year,$student['Student']['id']);
			*/
			$section=$student['CourseRegistration'][0]['Section'];
			// debug($student);
			if (!isset($section) || !isset($section['Program']['name']) 
		...
		if ($type['registered']==1) {  
          ...
          if (isset($section['YearLevel']['name']) && 
          !empty($section['YearLevel']['name']) && !empty($section['Department']['name'])
          && !empty($section['College']['name'])) {
               
               
                
                if (isset($attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']][$section['Department']['name']]['Registered']
                [$section['YearLevel']['name']]['total'])) {
                         $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']][$section['Department']['name']]['Registered'][$section['YearLevel']['name']]
          ['total']+=1;
          ...
          !empty($section['YearLevel']['name']) && !empty($section['College']['name'])) {
                ...
                [$section['College']['name']]['College'][$section['YearLevel']['name']]['total'])) {
                   $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']]['College']['Registered'][$section['YearLevel']['name']]
          ['total']+=1;
                } else {
                   $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']]['College']['Registered'][$section['YearLevel']['name']]
          ...
          !empty($section['YearLevel']['name'])) {
               ...
               if (isset($attraction_rate['University']['Registered'][$section['Program']['name']][
               $section['ProgramType']['name']][$section['YearLevel']['name']]['total'])) {
                
                 $attraction_rate['University']['Registered'][$section['Program']['name']]
               [$section['ProgramType']['name']][$section['YearLevel']['name']]['total']
               +=1;
          ...
          !empty($section['College']['name'])) {
                ...
                [$section['College']['name']]['Pre Engineering']['1st']['total'])) {
                     $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']]['Pre Engineering']['Registered']['1st']['total']+=1;
                } else {
                     $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']]['Pre Engineering']['Registered']['1st']['total']=1;
                }
              ...
              if (!isset($attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']]['Pre Engineering']['1st']['female_total'])) {
                 $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']]['Pre Engineering']['Registered']['1st']['female_total']=0;
              }
              
                  // male initialized 
              if (!isset($attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']]['Pre Engineering']['Registered']['1st']['male_total'])) {
                 $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']]['Pre Engineering']['Registered']['1st']['male_total']=0;
          ...
          !empty($section['College']['name'])) {
                if (isset($attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                  ...
                  $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                     [$section['College']['name']]['College']['Registered']['1st']['total']+=1;    
                } else {
                  $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                     [$section['College']['name']]['College']['Registered']['1st']['total']=1;    
                }
              ...
              // female initialized 
              if (!isset( $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                     [$section['College']['name']]['College']['Registered']['1st']['female_total'])) {
                  $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                     [$section['College']['name']]['College']['Registered']['1st']['female_total']=0;
              }
              
               // male initialized 
              if (!isset( $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                     [$section['College']['name']]['College']['Registered']['1st']['male_total'])) {
                  $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                     [$section['College']['name']]['College']['Registered']['1st']['male_total']=0;
               }
          ...
          if (empty($section['YearLevel']['name'])) {
              ...
              if (!isset($attraction_rate['University']['Registered'][$section['Program']['name']]
                  ...
                  $attraction_rate['University'][$section['Program']['name']]
                       [$section['ProgramType']['name']]['1st']['female_total']=0;
              }
		  ...
		  ///////////////////////////////initialization end ///////////////////////////////
		  {
		     ...
		     if (empty($section['department_id'])) {
			              ...
			              if (strcmp($student['Student']['gender'],'male')==0) {
			                  ...
			                  ['Pre Engineering']['Registered']['1st']['male_total']+=1;
		              
			              } else if (strcmp($student['Student']['gender'],'female')==0) {
			               
			                  $attraction_rate[$section['Program']['name']]
			                  [$section['ProgramType']['name']]
			                  [$section['College']['name']]['Pre Engineering']['Registered']['1st']['female_total']+=1;
	                ...
	                if (strcmp($student['Student']['gender'],'female')==0) {
                            if (isset($section['YearLevel']['name'])) {
                              $attraction_rate[$section['Program']['name']]
                              [$section['ProgramType']['name']][$section['College']['name']]
                              [$section['Department']['name']]['Registered']
	         ...
	         if (strcmp($student['Student']['gender'],'female')==0) {
                          ...
                          if (isset($section['YearLevel']['name'])) {
                            $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                            [$section['College']['name']]['College']['Registered']
                     [$section['YearLevel']['name']]
                      ['female_total']+=1;
                          } else {
                            if (!isset($section['YearLevel']['name'])) {
		               ...
		               if (isset($section['YearLevel']['name'])) {
		                   $attraction_rate['University']['Registered']
	                ...
	                //college level total male 
                      ...
                      ['male_total']+=1;
                             ...
                             if (!isset($section['YearLevel']['name'])) {
                               $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                               [$section['College']['name']]['College']['Registered']['1st']['male_total']+=1;
                             } 
                       }
     ...
     #############################END REGISTRATION###################################	
	    ...
	    #############################BEGIN DISMISSED###################################		
	 if (isset($student['StudentExamStatus'][0]['academic_status_id']) 
		   && $student['StudentExamStatus'][0]['academic_status_id']==4 && 
		   $type['dismissed']==1) {
          ...
          && !empty($section['College']['name'])) {
                ...
                if (isset($attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']][$section['Department']['name']]['Dismissed']
                [$section['YearLevel']['name']]['total'])) {
                         $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']][$section['Department']['name']]['Dismissed'][$section['YearLevel']['name']]
          ['total']+=1;
          ...
          !empty($section['YearLevel']['name']) && !empty($section['College']['name'])) {
                ...
                if (isset($attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']]['College']['Dismissed'][$section['YearLevel']['name']]['total'])) {
                   $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']]['College']['Dismissed'][$section['YearLevel']['name']]
          ['total']+=1;
                } else {
                   $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']]['College']['Dismissed'][$section['YearLevel']['name']]
          ...
          !empty($section['YearLevel']['name'])) {
               ...
               if (isset($attraction_rate['University']['Dismissed'][$section['Program']['name']][
               $section['ProgramType']['name']][$section['YearLevel']['name']]['total'])) {
                
                 $attraction_rate['University']['Dismissed'][$section['Program']['name']]
               [$section['ProgramType']['name']][$section['YearLevel']['name']]['total']
               +=1;
          ...
          !empty($section['College']['name'])) {
                if (isset($attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']]['Pre Engineering']['Dismissed']['1st']['total'])) {
                     $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']]['Pre Engineering']['Dismissed']['1st']['total']+=1;
                } else {
                     $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']]['Pre Engineering']['Dismissed']['1st']['total']=1;
                }
                 ...
                 // female initialized 
              if (!isset($attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']]['Pre Engineering']['Dismissed']['1st']['female_total'])) {
                 $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']]['Pre Engineering']['Dismissed']['1st']['female_total']=0;
              }
              
                  // male initialized 
              if (!isset($attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']]['Pre Engineering']['Dismissed']['1st']['male_total'])) {
                 $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']]['Pre Engineering']['Dismissed']['1st']['male_total']=0;
          ...
          !empty($section['College']['name'])) {
                if (isset($attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                     [$section['College']['name']]['College']['Dismissed']['1st']['total'])) {
                  $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                     [$section['College']['name']]['College']['Dismissed']['1st']['total']+=1;    
                } else {
                  $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                     [$section['College']['name']]['College']['Dismissed']['1st']['total']=1;    
                }
              ...
              // female initialized 
              if (!isset( $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                     [$section['College']['name']]['College']['Dismissed']['1st']['female_total'])) {
                  $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                     [$section['College']['name']]['College']['Dismissed']['1st']['female_total']=0;
              }
              
               // male initialized 
              if (!isset( $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                     [$section['College']['name']]['College']['Dismissed']['1st']['male_total'])) {
                  $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                     [$section['College']['name']]['College']['Dismissed']['1st']['male_total']=0;
               }
            //   debug($section['YearLevel']['name']);
          } 
		  ...
		  ///////////////////////////////initialization end ///////////////////////////////
		  {
		         ...
		         if (empty($section['department_id'])) {
			              ...
			              if (strcmp($student['Student']['gender'],'male')==0) {
			                  ...
			                  $attraction_rate[$section['Program']['name']]
			                  [$section['ProgramType']['name']][$section['College']['name']]
			                  ['Pre Engineering']['Dismissed']['1st']['male_total']+=1;
		              
			              } else if (strcmp($student['Student']['gender'],'female')==0) {
			               
			                  $attraction_rate[$section['Program']['name']]
			                  [$section['ProgramType']['name']]
			                  [$section['College']['name']]['Pre Engineering']['Dismissed']['1st']['female_total']+=1;
	                ...
	                if (strcmp($student['Student']['gender'],'female')==0) {
                            if (isset($section['YearLevel']['name'])) {
                              $attraction_rate[$section['Program']['name']]
                              [$section['ProgramType']['name']][$section['College']['name']]
                              [$section['Department']['name']]['Dismissed']
                             [$section['YearLevel']['name']]
                              ['female_total']+=1;
	         ...
	         if (strcmp($student['Student']['gender'],'female')==0) {
                          ...
                          if (isset($section['YearLevel']['name'])) {
                            $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                            [$section['College']['name']]['College']['Dismissed']
                     [$section['YearLevel']['name']]
                      ['female_total']+=1;
                          } else {
                            if (!isset($section['YearLevel']['name'])) {
		               ...
		               if (isset($section['YearLevel']['name'])) {
		                   $attraction_rate['University']['Dismissed']
                      [$section['Program']['name']][$section['ProgramType']['name']][$section['YearLevel']['name']]
                      ['female_total']+=1;
	                ...
	                //college level total male 
                      ...
                      ['male_total']+=1;
                             ...
                             if (!isset($section['YearLevel']['name'])) {
                               $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                               [$section['College']['name']]['College']['Dismissed']['1st']['male_total']+=1;
                             } 
                       }
	...
	function doesTheCourseRegistrationHaveWithdraw ($registrationId) {
 	       $registration_has_withdrawal=$this->find('count',array('conditions'=>array(
...
'CourseRegistration.id in (select course_registration_id from exam_grades where id in (select exam_grade_id from exam_grade_changes where grade="W")) '),
	                       'order'=>array('CourseRegistration.created DESC'),'contain'=>array('ExamGrade'=>array('ExamGradeChange'))));
	     return $registration_has_withdrawal;
 	}


    public function getMostRecentRegisteration($student_id) {
		$courseRegisteration=ClassRegistry::init('CourseRegistration')->find('first',
array('conditions'=>array('CourseRegistration.student_id'=>$student_id),
'order'=>'CourseRegistration.created DESC',
'contain'=>array('Section'=>array('YearLevel','Department','College'))));
        return $courseRegisteration;	      
	}
	public function getRegisteration($student_id,$academic_year,$semester) {
		$courseRegisteration=ClassRegistry::init('CourseRegistration')->find('first',
array('conditions'=>array('CourseRegistration.student_id'=>$student_id,'CourseRegistration.academic_year'=>$academic_year,
'CourseRegistration.semester'=>$semester),
'contain'=>array('Section'=>array('YearLevel','Department','College')),'order'=>'CourseRegistration.created DESC'));
        return $courseRegisteration;		  
	}

   function studentYearAndSemesterLevelByRegistration($student_id, $acadamic_year=null, $semester=null) {
        $studentStatusPattern = $this->Student->find('first',
			array(
				'conditions' =>
				array(
					'Student.id' => $student_id,
		...
		$pattern = $this->Student->ProgramType->StudentStatusPattern->getProgramTypePattern(
			$studentStatusPattern['Student']['program_id'], $studentStatusPattern['Student']['program_type_id'],
$studentStatusPattern['AcceptedStudent']['academicyear']);
		$student_registrations = $this->find('all',
			array(
				'conditions' =>
				array(
					'CourseRegistration.student_id' => $student_id,
   ...
   function listOfCoursesWithFx($department_id = null, $acadamic_year = null, $semester = null, $program_id = null, $program_type_id = null, $pre = 0,$onlySelectedByStudent=0) {
		...
		/*
		$publishedCourses=$this->find('all',
...
'CourseRegistration.academic_year'=> $acadamic_year,
'CourseRegistration.semester'=>$semester,
'CourseRegistration.published_course_id in (select id from published_courses where 
program_id="'.$program_id.'" and program_type_id="'.$program_type_id.'"  and academic_year="'.$acadamic_year.'" and semester="'.$semester.'"  and given_by_department_id="'.$department_id.'" )',
...
),'contain'=>array('PublishedCourse'=>array('Section','YearLevel','Course','Department'))));

$publishedCoursesAdds=ClassRegistry::init('CourseAdd')->find('all',
array('conditions'=>array(
...
'CourseAdd.published_course_id in (select id from published_courses where 
program_id="'.$program_id.'" and program_type_id="'.$program_type_id.'"  and academic_year="'.$acadamic_year.'" and semester="'.$semester.'"  and given_by_department_id="'.$department_id.'" )',
...
'CourseRegistration.semester'=>$semester,
'CourseRegistration.published_course_id in (select id from published_courses where 
program_id="'.$program_id.'" and program_type_id="'.$program_type_id.'"  and academic_year="'.$acadamic_year.'" and semester="'.$semester.'"  and college_id="'.$department_id.'"  and department_id is null )',
'CourseRegistration.id in (select course_registration_id from exam_grades where grade="Fx" and department_approval=1 and registrar_approval=1 )'
),'contain'=>array('PublishedCourse'=>array('Section','YearLevel','Course','Department'))));
...
),'contain'=>array('Student','PublishedCourse'=>array('Section','YearLevel','Course','Department'))));
...
'CourseAdd.id in (select course_add_id from exam_grades where grade="NG" and department_approval=1 and registrar_approval=1)'
	  ...
	  $publishedCoursesM = array_merge($publishedCourses, $publishedCoursesAdds);
       ...
       foreach($publishedCoursesM as $key => $published_course) {
           ...
           && !empty($published_course['CourseRegistration']) && $published_course['CourseRegistration']['id'] != ""){
				$grade = ClassRegistry::init('ExamGrade')->getApprovedGrade($published_course['CourseRegistration']['id'], 1);
		    	$course_reg_ids=ClassRegistry::init('CourseRegistration')->find('list',array('conditions'=>
		    	array(
		    	...
		    	$exam_grade_ids=ClassRegistry::init('ExamGrade')->find('list',array('conditions'=>
		    	array(
				...
				$course_reg_ids=ClassRegistry::init('CourseAdd')->find('list',array('conditions'=>
		    	array(
			...
			$isCheating=ClassRegistry::init('ExamGradeChange')->find('first',array('conditions'=>array('ExamGradeChange.cheating'=>1,
				'ExamGradeChange.exam_grade_id'=>$grade['grade_id'])));
				if(isset($exam_grade_ids) 
				&& !empty($exam_grade_ids)){
				$previousCheatingCount=ClassRegistry::init('ExamGradeChange')->find('count',array('conditions'=>array('ExamGradeChange.cheating'=>1,
           ...
           /*
            if(
            isset($published_course['CourseRegistration']['id'])){
...
'CourseRegistration.published_course_id in (select id from published_courses where 
program_id="'.$program_id.'" and program_type_id="'.$program_type_id.'"  and academic_year="'.$acadamic_year.'" and semester="'.$semester.'"  and department_id="'.$department_id.'" )',

'CourseRegistration.id in (select course_registration_id from exam_grades where department_approval=1 and registrar_approval=1 )'
...
*/
       ...
       foreach($publishedCourses as $key => $published_course) {
           ...
           /*
            if(isset($isCourseMissingEntry) 
            && !empty($isCourseMissingEntry)){
               $organized_Published_courses_by_sections[$published_course['PublishedCourse']['Section']['name']][$published_course['CourseRegistration']['published_course_id']] = $published_course['PublishedCourse']['Course']['course_title'].' ('.$published_course['PublishedCourse']['Course']['course_code'].')';
            }
         */
           if(isset($isCourseMissingEntry) 
...
'Student.department_id','Student.college_id',
...
'Department'=>array('fields'=>array('id','name')),'College'=>array('fields'=>array('id','name')),'Program'=>array('fields'=>array('id','name')),
            ...
            if (!empty($data['Student']['college_id'])) {
			 ...
			 if (!empty($data['Student']['department_id'])) {
              ...
              if (!empty($data['Student']['year_level_id']) &&!empty($data['Student']['department_id'])) {
			       ...
			       array('YearLevel.department_id'=>$data['Student']['department_id'],
			       'YearLevel.name'=>$data['Student']['year_level_id']));
				   
                    
             } 
             if (!empty($data['Student']['semester']) && !empty($data['Student']['academicyear']) && !empty($yearLevelId)) {
                     $search_conditions['conditions'][] = array(
                            'Student.id not in (select DISTINCT student_id 
from course_registrations where semester="'.$data['Student']['semester'].'" and academic_year="'.$data['Student']['academicyear'].'" and year_level_id="'.$yearLevelId.'") ');  
              } else {
              	$search_conditions['conditions'][] = array(
                            'Student.id not in (select DISTINCT student_id 
from course_registrations where semester="'.$data['Student']['semester'].'" and academic_year="'.$data['Student']['academicyear'].'") ');  
              }
            */ 
...
'ProgramType'=>array('fields'=>array('id','name')));
		...
		'Student.department_id is null');
             ...
             if (!empty($data['Student']['semester']) && !empty($data['Student']['academicyear']) && !empty($sectionId)) {
                     $search_conditions['conditions'][] = array(
                            'Student.id not in (select DISTINCT student_id 
from course_registrations where semester="'.$data['Student']['semester'].'" and academic_year="'.$data['Student']['academicyear'].'" and section_id='.$sectionId.') ');  
...
$yearLevelId==$studentSectionDetail['Section']['year_level_id']) {
                   $organized_students[$v['Program']['name'].'~'.
$v['ProgramType']['name'].'~'.$studentSectionDetail['YearLevel']['name'].'~'.$studentSectionDetail['Section']['name'].'~'.$studentSectionDetail['Section']['id']][]=$v;   
...
$v['ProgramType']['name'].'~'.$studentSectionDetail['YearLevel']['name'].'~'.$studentSectionDetail['Section']['name'].'~'.$studentSectionDetail['Section']['id']][]=$v;   
                    ...
                    $organized_students[$v['Program']['name'].'~'.
$v['ProgramType']['name'].'~'.'Pre/Fresh'.'~'.$studentSectionDetail['Section']['name'].'~'.$studentSectionDetail['Section']['id']][]=$v;   
				 } 
             }
	  ...
	  function registerAllSection($department_id, $program_type_id, $academicYear,
$semester){
		$academic_year['academic_year']=$academicYear;
        $academic_year['semester']=$semester;
           
		if(!empty($department_id) && !empty($program_type_id) && 
!empty($academicYear) && !empty($semester)) {
                $departmentsSection=$this->Section->find('list',
array('conditions'=>array('Section.department_id'=>$department_id,
'Section.program_type_id'=>$program_type_id,'Section.academicyear'=>$academic_year['academic_year'])));
	  ...
	  && !empty($department_id)){
	  $registeredNotInSection=$this->find('all',
	  array('conditions'=>array(
	  'CourseRegistration.section_id not in (select section_id from students_sections where student_id is not null)'),'contain'=>array(
	  'PublishedCourse'=>array('conditions'=>array('PublishedCourse.department_id'=>$department_id)))
	    ...
	    $registeredNotInSection=$this->find('all',
	  array('conditions'=>array(
	  'CourseRegistration.section_id not in (select section_id from students_sections where student_id is not null)'),'recursive'=>-1));
	  }
		//perform the update operation 
		debug(count($registeredNotInSection));	
			
		foreach($registeredNotInSection as $k=>$v){
			//check if student and section exists in 
			// table ?
			$sectionc = ClassRegistry::init('StudentsSection')->find('count',
array('conditions'=>array(
