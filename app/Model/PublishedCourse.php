				...
				foreach ($publishedcourses as $kkk => &$vvv) {
					if ($vvv['PublishedCourse']['section_id'] == $section_id) {
						if ($this->CourseRegistration->ExamGrade->is_grade_submitted($vvv['PublishedCourse']['id']) > 0) {
							...
							$vvv['PublishedCourse']['have_instructor_assignment'] = true;
						} else {
							...
							if ($this->CourseInstructorAssignment->find('count', array('conditions' => array('CourseInstructorAssignment.published_course_id' => $vvv['PublishedCourse']['id'])))) {
								$vvv['PublishedCourse']['have_instructor_assignment'] = true;
							}
						}
			...
			if (!empty($students_makeup_exam)) {
				foreach ($students_makeup_exam as $key => $student_makeup_exam) {
					...
					if (!empty($student_makeup_exam['CourseRegistration']) && $student_makeup_exam['CourseRegistration']['id'] != "") {
						...
						$students_makeup_exam[$key]['ExamGrade'] = (isset($student_makeup_exam['CourseAdd']['ExamGrade']) ? $student_makeup_exam['CourseAdd']['ExamGrade'] : array());
					}
			...
			if (!empty($published_courses)) {
				foreach ($published_courses as $key => $published_course) {
					if (isset($published_course['Course']['id'])) {
						$published_courses_list[$published_course['PublishedCourse']['id']] = $published_course['Course']['course_code_title'] . ', ' . ($last_ac_and_semester['PublishedCourse']['semester'] == 'I' ? '1st' : ($last_ac_and_semester['PublishedCourse']['semester'] == 'II' ? '2nd' : '3rd')) . ' semester,  ' . $last_ac_and_semester['PublishedCourse']['academic_year'];
					}
				}
			}
		}
	...
	function sectionPublishedCourses($section_id = null)
	{
		...
		if (!empty($section_id)) {
			...
			if (!empty($published_courses)) {
				foreach ($published_courses as $key => $published_course) {
					if (isset($published_course['Course']['id'])) {
						$published_courses_list[$published_course['PublishedCourse']['id']] = $published_course['Course']['course_code_title'] . ', ' . ($published_course['PublishedCourse']['semester'] == 'I' ? '1st' : ($published_course['PublishedCourse']['semester'] == 'II' ? '2nd' : '3rd')) . ' semester,  ' . $published_course['PublishedCourse']['academic_year'];
					}
				}
			}
		}
		...
		if (!empty($publishedCourseId)) {
			...
			if (!empty($pubList)) {
				...
				foreach ($pubList as $key => $value) {
					$publishedList[$value['PublishedCourse']['id']] = $value['Course']['course_code_title'] . $value['Section']['name'] . ' (' . (!empty($value['YearLevel']['name']) ? $value['YearLevel']['name'] : ($value['Section']['program_id'] == PROGRAM_REMEDIAL ? 'Remedial': 'Pre/1st')).', '. $value['Section']['academicyear']. ') ' . (isset($value['CourseInstructorAssignment'][0]) ? $value['CourseInstructorAssignment'][0]['Staff']['Title']['title'] . ' ' . $value['CourseInstructorAssignment'][0]['Staff']['full_name'] . ' (' . $value['CourseInstructorAssignment'][0]['Staff']['Department']['name'] . ')' : '');
				}
			}
		}
