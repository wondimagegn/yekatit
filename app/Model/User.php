...
class User extends AppModel {
    ...
    */
    ...
    /**
	 * Method to validate the number of users allowed to create under administrator 
    ...
    function checkNumberOfUserAccount($data=null) {
            ...
            if ($data['User']['role_id'] == ROLE_CLEARANCE) {
			  ...
			  $isMainAccountCreated=$this->find('first',array('conditions'=>array('User.is_admin'=>1,'User.role_id'=>$data['User']['role_id']),'contain'=>array('Role','Staff'=>array('conditions'=>array('Staff.department_id'=>$data['Staff'][0]['department_id'])))));
			 } elseif (ROLE_COLLEGE==$data['User']['role_id']) {
		   $isMainAccountCreated=$this->find('first',array('conditions'=>array('User.is_admin'=>1,
		'User.role_id'=>$data['User']['role_id']),'contain'=>array('Role','Staff'=>array('conditions'=>array('Staff.college_id'=>$data['Staff'][0]['college_id'])))));

			} else {
				$isMainAccountCreated=$this->find('first',array('conditions'=>array('User.is_admin'=>1,'User.role_id'=>$data['User']['role_id'],'User.active'=>1),'contain'=>array('Staff','Role')));
			}

				if(empty($isMainAccountCreated)) {
				 $isMainAccountCreated=$this->find('all',array('conditions'=>array('User.is_admin'=>0,
		'User.role_id'=>ROLE_SYSADMIN,'User.active'=>1),'limit'=>3,'contain'=>array('Staff','Role')));
			}
	  
	    if(!empty($isMainAccountCreated) && 
$data['User']['role_id']==ROLE_SYSADMIN && count($isMainAccountCreated)>2) {

	      $message= 'The selected role has already system administrator. You can not create more than 2 administrator for '.$isMainAccountCreated[0]['Role']['name'].' role. If the system administartor in the list are nomore system administrator, please deactive their account and create the new one. The system administrator in the system are :-
                         <ul>
 ...
 <li>'.$isMainAccountCreated[2]['User']['full_name'].'</li>
		...
		$message= 'The selected role has already system administrator. You can not create more than one administrator for '.$isMainAccountCreated['Role']['name'].' role. If the system administartor in the selected role is nomore system administrator, please deactive their account and create the new one.
                         <ul>
    ...
    /**
	 * Method to check given username is unique 
	 * 
   ...
   function parentNode() {
	...
	public function getUserDetails($user){
		$userRole=array();
		if($user!=null){
         $userRole=$this->find('first',
         	array('conditions' => array('User.id' => $user),'fields'=> array('User.id','User.role_id',
         		'User.username'),
         		'recursive'=>-1));
        }

		if($user!=null && $userRole['User']['role_id']==ROLE_STUDENT){   
	           $userdetails = $this->find('first', array(
                    'conditions' => array('User.id' => 
$user),
                    'fields'     => array('id','role_id','username'),
                    'contain'    => array(
                        'Role'       => array(
                            'fields'     => array(
                                'Role.id',
                ...
                return $userdetails;
	         } else if($user!=null && $userRole['User']['role_id']!=ROLE_STUDENT){
	         	  debug($user);
                  $userdetails = $this->find('first', array(
                    'conditions' => array('User.id' => 
$user),
                    'fields'     => array('User.id','User.role_id','User.username'),
                    'contain'    => array(
                        'Role'       => array(
                            'fields'     => array(
                                'Role.id',
   ...
   function elgibleToEdit($user_id=null,$deparment_college_ids=null) {
            $responsibility=$this->User->StaffAssigne->find('first',array('conditions'=>
            array('StaffAssigne.user_id'=>$user_id),'recursive'=>-1));
           
   }
	
   
       /**
     * Get the array with all user details 
     * @return Array
     */
     ...
     /**
     * Get list of users with their email address or username.
     **/    
    ...
    function getListOfUsers($role_id = null,$message=null){
            ...
            if (!$role_id) {
                return NULL;
                    ...
                    $conditions = array('User.role_id' => $role_id,
                            'User.active'=>1);
            }
    ...
    function getListOfUsersRole($role_id = null,$message=null,
    	$data=null)
    {
            $this->recursive = 0;
            $returnArray = array();
	...
	'Student.id NOT IN (SELECT graduate_lists.student_id from graduate_lists)',
     ...
     /**
    * a function which return firstname and lastname given user_id
    */
    ...
    function validate_department_college ($data=null) {
            ...
            if ($data['User']['role_id'] == ROLE_DEPARTMENT && empty($data['Staff'][0]['department_id'])) {
                      $this->invalidate('college_department',
	            'User with department role must have department. Please select department for the user. ');
                    return false;
            } else if ($data['User']['role_id'] == ROLE_COLLEGE && empty($data['Staff'][0]['college_id'])) {
                     $this->invalidate('college_department',
	            'User with college role must have college. Please select college for the user. ');
                    return false;
            }
         return true;           
    }
    ...
    function checkUserIsBelongsInYourAdmin ($user_id=null,$role_id=null,$department_id=null,$college_id=null) {
			   ...
			   if ($role_id == ROLE_SYSADMIN) {
					     ...
					     $user_role_ids = $this->Role->find('list',
array('conditions'=>array('Role.parent_id'=>$role_id),'fields'=>array('Role.id','Role.id')));
						
					     $is_account_belongs_to_admin=$this->find('count',
                         array('conditions'=>array('User.id'=>$user_id,
'User.role_id'=>$user_role_ids,'User.is_admin'=>1)));
                 ...
                 if ($role_id == ROLE_REGISTRAR) {
					     ...
					     $user_role_ids = $this->Role->find('list',
array('conditions'=>array('Role.parent_id'=>$role_id),
...
'User.role_id'=>$user_role_ids)));
                 ...
                 if ($role_id == ROLE_MEAL) {
                         $is_account_belongs_to_admin=$this->find('count',
                         array('conditions'=>array('User.id'=>$user_id,'User.role_id'=>$role_id)));
                 ...
                 if ($role_id == ROLE_DEPARTMENT) {
                        $staff_belongs_to_you=$this->Staff->find('count',
                         array('conditions'=>array('Staff.user_id'=>$user_id,'Staff.department_id'=>$department_id)));
                ...
                if ($role_id == ROLE_COLLEGE) {
                         $staff_belongs_to_you=$this->Staff->find('count',
                         array('conditions'=>array('Staff.user_id'=>$user_id,'Staff.college_id'=>$college_id)));
                         if ($staff_belongs_to_you==0) {
                            return false;
                         } else {
                             return true;
	...
	function doesItFullfillPasswordStrength($password = null, $securitysetting = null) {
	 ...
	 function getAllPermissions($userId) {
			   ...
			   $sqlAro2="SELECT id,parent_id,model,foreign_key,alias FROM aros as Aro WHERE 
			   Aro.id='".$aro1[0]['Aro']['parent_id']."' and model='Role'";
			   ...
			   if(isset($aro2[0]) && !empty($aro2[0])) {
				...
				if(empty($node)){
						//create node 
					    $parentNodeQuery="SELECT id,parent_id FROM aros as Aro 
					    where model='Role' and foreign_key='".$usersDetail['User']['role_id']."'";
					...
					$data['Aro']['model']="User";                                            $data['Aro']['foreign_key']=$usersDetail['User']['id'];                                                $data['Aro']['lft']=$lftMaxQueryResult[0]['Aro']['lft']+1;
					$data['Aro']['rght']=$lftMaxQueryResult[0]['Aro']['rght']+1;
         ...
         if($usersDetail['User']['role_id']==ROLE_STUDENT){
         	  $userDetail=$this->find('first', array('conditions'=>array(
            'User.id'=>$userId),
...
($userDetail['User']['role_id'] == ROLE_DEPARTMENT 
                ...
                $userDetail['User']['role_id'] == ROLE_REGISTRAR || $userDetail['User']['role_id'] > 10
                )) && strcmp($value['Aro']['model'], 'User') == 0 ) {
                ...
                WHERE Aco.id=p.aco_id
                  ...
                  LEFT JOIN
                    (
                  SELECT 'a' as mark2,aco_id,`_create`,`_read`, `_update`, `_delete`  FROM `aros_acos` AS `Permission` LEFT JOIN `aros` AS `Aro` ON (`Permission`.`aro_id` = `Aro`.`id`) LEFT JOIN `acos` AS `Aco` ON (`Permission`.`aco_id` = `Aco`.`id`) WHERE `Permission`.`aro_id` = $aroId
                 )p ON p.mark2=user_id.flag
                WHERE p._create=1 OR p._read=1 or p._update=1 or p._delete=1
                           ) tt
                                      LEFT JOIN
                           (
...
$userDetail['User']['role_id']==ROLE_STUDENT || $userDetail['User']['role_id'] == ROLE_SYSADMIN   || $userDetail['User']['role_id'] > 10 )) && strcmp($value['Aro']['model'], 'Role') == 0) {
        ...
        debug($value);               
		  ...
		  // debug($permissionAggregated);
            ...
            if (isset($permissionAggregated['UserLevelAllowed']) && !empty(
              $permissionAggregated['UserLevelAllowed'])) {
              foreach($permissionAggregated['UserLevelAllowed'] as $value){
						if(isset($permissionAggregated['RoleLevel']) && 
			!empty($permissionAggregated['RoleLevel'])) {
            ...
            if (isset($permissionAggregated['UserLevelDenied']) && 
!empty($permissionAggregated['UserLevelDenied'])) {
                    foreach($permissions as $index=> &$value){
                        if(in_array($value,$permissionAggregated['UserLevelDenied'])
                        || empty($value)) {
           ...
           //check the user is admin and give privilage to security modules
            ...
            if ($userDetail['User']['is_admin'] || 
$userDetail['User']['role_id']==ROLE_SYSADMIN) {
            
                if ($userDetail['User']['role_id']==ROLE_REGISTRAR) {
           ...
           foreach ($permissions as $in=>$controller) {
                ...
                //  debug($userDetail);
                  ...
                  isset($userDetail['AcceptedStudent'][0]['program_id'])
                  && $userDetail['AcceptedStudent'][0]['program_id']==PROGRAM_UNDEGRADUATE) {
                            $expireat=date("Y-m-d 00:00:01",strtotime("- 180 day"));
                             
                            if (isset($userDetail['AcceptedStudent']) && 
                            !empty($userDetail['AcceptedStudent']) && 
                            !empty($userDetail['AcceptedStudent'][0]['department_id'])
                            && $userDetail['AcceptedStudent'][0]['Placement_Approved_By_Department']
                            ==1 && $userDetail['AcceptedStudent'][0]['created']<$expireat ) {
                           // debug($tmController);
                            if (count($tmController)>2 && 
                            $tmController[0]=='controllers' && strcmp($tmController[1],'Preferences')==0
                            ) {
                               unset($permissions[$in]);
                                continue;
                  ...
                  */
                  ...
                  if ($userDetail['User']['role_id']==ROLE_STUDENT &&
                  $userDetail['Student'][0]['AcceptedStudent']['placementtype']=="REGISTRAR PLACED") {
                          
                            if (count($tmController)>2 && 
                            $tmController[0]=='controllers' && (
                            strcmp($tmController[1],'Preferences')==0 ||
                            strcmp($tmController[1],'AcceptedStudents')==0 
                            )
                            ) {
                  ...
                  } else if ($userDetail['User']['role_id']==ROLE_STUDENT 
                  &&  empty($userDetail['Student'][0]['AcceptedStudent']['placementtype'])) {
                            ...
                            && $userDetail['Student'][0]['AcceptedStudent']['Placement_Approved_By_Department']
                            ==1 && !empty($userDetail['Student'][0]['curriculum_id'])) {
                  ...
                  if (count($tmController)>2 && $tmController[0]=='controllers'
                  && $tmController[2]!='Acls') {
           ...
           $equivalentACL = Configure::read('ACL.equivalentACL');
			...
			if($checking[1] == '*' && isset($checking[0])) {
			  ...
			  if (isset($reformatePermission[$checking[0]]['action'])) {
			     ...
			     if (isset($controllerActions) && !empty($controllerActions)) {
			       ...
			       $cntrlaction=explode('/',$parent);
			        ...
			        if (isset($reformatePermission[$checking[0]]['action']) && 
			        is_array($reformatePermission[$checking[0]]['action'])) {
			 ...
			 if (isset($checking[0])) {
			   ...
			   if (isset($reformatePermission[$checking[0]]['action'])
			   && in_array($checking[1],$reformatePermission[$checking[0]]['action'])) {
   ...
   function my_array_merge(&$array1, $array2) {
    ...
    function regenerate_password_by_batch($department_college_id,$academicYear,$commonPassword,$pre=0) 
	{
		...
		if($academicYear!="all") {
		$options['conditions']['Student.admissionyear'] =$AcademicYear->get_academicYearBegainingDate($academicYear);
		}
		...
		if(!empty($student_password)) {
              if($this->saveAll($student_password['User'],
...
!empty($admissionYearConverted) && $pre==0) {
		$studentLists=ClassRegistry::init('Student')->find('all', array('conditions'=>array('Student.department_id'=>$department_college_id,'Student.admissionyear'=>$admissionYearConverted,'Student.program_type_id'=>$program_type,
'Student.user_id is null'),'recursive'=>-1));
		} else {
          $studentLists=ClassRegistry::init('Student')->find('all', array('conditions'=>array('Student.college_id'=>$department_college_id,'Student.department_id is null','Student.admissionyear'=>$admissionYearConverted,'Student.program_type_id'=>$program_type,
'Student.user_id is null'),'recursive'=>-1));
		}
		...
		foreach($studentLists as $kkk=>$vv) 
        {
		  ...
		  if(empty($vv['Student']['user_id'])) 
          {
			...
			$student_password['User']['role_id'] =3;
		...
		if(!empty($student_table_update)) {
              if(ClassRegistry::init('Student')->saveAll($student_table_update['Student'],
array('validate' => false))) {   
		...
		if(!empty($accepted_student_table_update)) {
              if(ClassRegistry::init('AcceptedStudent')->saveAll($accepted_student_table_update['AcceptedStudent'],
array('validate' => false))) {   
...
array('conditions'=>array('Student.phone_mobile'=>$mobilePhoneNumber),'contain'=>array('User')));
		if(!empty($studentDetail)) {
			 ...
			 $userDetails['User']['force_password_change']=2;    
                     ...
                     $staffDetail=ClassRegistry::init('Staff')->find('first',
array('conditions'=>array('Staff.phone_mobile'=>$mobilePhoneNumber),'contain'=>array('User')));
	    ...
	    return 'The phone number you attached not in our system.';
	}
	...
	public function afterSave($created, $options = array()) {
       ...
       if ($created === true) {
            $Event = new CakeEvent('Model.User.created', 
            	$this, array(
                'id' => $this->id,
        	...
        	$Event=new CakeEvent('Model.User.login',
        		$this,
        		array(
        			'data'=>$this->data[$this->alias],
    ...
    function synchronizeAros($findAccountNotInAro,$model='Staff')
    {
    	...
    	{
				$aros=ClassRegistry::init('Aros')->find('first',
    				array('order'=>array('Aros.rght DESC')));
    			$userDetail=$this->find('first',array('conditions'=>array('User.id'=>$avalue['User']['id']),'contain'=>array("$model")));
    			$arosRoles=ClassRegistry::init('Aros')->find('first',array('conditions'=>array('Aros.model'=>'Role'
    				,'Aros.parent_id is null','Aros.foreign_key'=>$avalue['User']['role_id'])));
    ...
    function getLogChangeDetail(
    $logDetail){
    	if($logDetail['model']=="ExamGrade"){
    		preg_match("/\(([^\)]*)\)/", $logDetail['description'], $aMatches);
		//debug($aMatches);
			$regId = $aMatches[1];
			if(isset($logDetail['foreign_key']) 
			&& !empty($logDetail['foreign_key'])){   
    		$examGradeDetail=ClassRegistry::init('ExamGrade')->find('first',array('conditions'=>
    		array(
    		'ExamGrade.id'=>$logDetail['foreign_key'],
    		),
    		'contain'=>array('CourseRegistration'=>array('Student','PublishedCourse'=>array('Course')),
    		...
    		if(isset($examGradeDetail) 
    		&& !empty($examGradeDetail)){
		...
		$regId = $aMatches[1];
    	 ...
    	 $examGradeDetail=ClassRegistry::init('CourseRegistration')->find('first',
    	 array('conditions'=>
    		array(
    	 ...
    	 array('conditions'=>
    		...
    		$examGradeDetail=ClassRegistry::init('CourseAdd')->find('first',
    	 array('conditions'=>
    		array(
    		...
    		&& !empty($examGradeDetail)){
    			...
    			!empty($logDetail)){
				 ...
				 $examGradeDetail=ClassRegistry::init('ExamGradeChange')->find('first',array('conditions'=>
					array(
    	  ...
    	  $courseChange=ClassRegistry::init('Course')->find('first',array('conditions'=>
					array(
    	...
    	$courseChange=ClassRegistry::init('Section')->find('first',array('conditions'=>
					array(
    	...
    	$courseChange=ClassRegistry::init('Student')->find('first',array('conditions'=>
					array(
    ...
    public function removeLog($months=12){
      ...
      public function getDepartmentAssignments($users_college){
    	$assignment=array();
    	if(!empty($users_college['Staff'])){
    		$assignment['staff_id']=$users_college['Staff'][0]['id'];
    		$assignment['college_id']=$users_college['Staff'][0]['college_id'];
    		$assignment['role_id']=$users_college['Role']['id'];
			$assignment['role_name']=$users_college['Role']['name'];

			if (isset($users_college['StaffAssigne']['department_id']) && !empty($users_college['StaffAssigne']['department_id'])) {
			  $assignment['department_id']= unserialize($users_college['StaffAssigne']['department_id']);
			  $assignment['college_id']=ClassRegistry::init('Department')->find('list',
			  	array('conditions'=>array('Department.id'=>$assignment['department_id']),
			  		'fields'=>array('Department.college_id','Department.college_id')));
			  
			} else {
				 $assignment['department_id']=$users_college['Staff'][0]['department_id'];
			}
		

			if (!empty($users_college['StaffAssigne']['program_id'])) {
				
					 $assignment['program_id']=unserialize($users_college['StaffAssigne']['program_id']);
			
			}

			if (!empty($users_college['StaffAssigne']['program_type_id'])) {
				$assignment['program_type_id']=unserialize($users_college['StaffAssigne']['program_type_id']);
			}

    	} else if(!empty($users_college['Student'][0])) {
    		$assignment['student_id']=$users_college['Student'][0]['id'];
    		$assignment['college_id']=$users_college['Staff'][0]['college_id'];
    		$assignment['role_id']=$users_college['Role']['id'];
			$assignment['role_name']=$users_college['Role']['name'];
			$assignment['department_id']=$users_college['Student'][0]['department_id'];
			$assignment['college_id']=$users_college['Student'][0]['college_id'];
			$assignment['program_id']=$users_college['Student'][0]['program_id'];
			$assignment['program_type_id']=$users_college['Student'][0]['program_type_id'];
    	}
    	return $assignment;      	
    }
