...
class AcceptedStudent extends AppModel {
    ...
    var $virtualFields = array(
        'full_name' => 'CONCAT(AcceptedStudent.first_name, " ",AcceptedStudent.middle_name," ",
                        AcceptedStudent.last_name)'
        );
	var $belongsTo = array(
		'Department' => array(
	...
	function getRecentAcceptedStudent($college_id=null,$academicyear=null) {
	        $recentAcceptedStudent = $this->find('all', array('conditions'=>array(
	        'AcceptedStudent.college_id'=>$college_id,'AcceptedStudent.academicyear'=>$academicyear),
	        'limit'=>100)); 
	        return  $recentAcceptedStudent;  
	}
	function countIdByDepartment($department_id=null,$year=null) {
			$count = $this->find("count", array(
				"conditions"=>array('AcceptedStudent.academicyear LIKE'=>$year.'%',
                'AcceptedStudent.department_id'=>$department_id,
				"NOT"=>array('AcceptedStudent.studentnumber'=>array('','null','NULL'))),
				'recursive'=>-1));
			return $count;				
	}
    function readAllById($id=null) {
		...
		$data=$this->find("first", array(
				...
				'contain'=>array('Program','ProgramType','College','Department')));
		return $data;
	}
	function countId($collegeid=null,
	$year=null,$program_id=null,$program_type_id=null) {
			$count = $this->find("count", array(
				"conditions"=>array('AcceptedStudent.academicyear LIKE'=>$year.'%',
	...
	/**
	* Method to check the student placement has been approved by departments
	@return boolean 
	*/
	...
	*/
	function checkPlacementSettingIsRecorded($academicyear=null,$college_id=null){
	    ...
	    $checkPlacementSettingIsRecord['placement_result_criteria']=ClassRegistry::
	    init('PlacementsResultsCriteria')->isPlacementResultRecorded($academicyear,$college_id);
	    
	    $checkPlacementSettingIsRecord['ReservedPlace']=ClassRegistry::
	    init('PlacementsResultsCriteria')->isReservedPlaceRecorded($academicyear,$college_id);
	    $checkPlacementSettingIsRecord['participating_departemnt']=ClassRegistry::
	    init('PlacementsResultsCriteria')->isParticipationgDepartmentRecorded($academicyear,$college_id);
	   
	    if(empty($checkPlacementSettingIsRecord['placement_result_criteria'])){
	         $this->invalidate('placement_result_criteria',
	        'Please record placement result criteria before 
	        running auto placement.');
            return false;
	    } elseif(empty($checkPlacementSettingIsRecord['ReservedPlace'])){
	          $this->invalidate('reserved_place',
	        'Please record reserved place for each department 
	        you want to participate in auto placement before running the auto placement.');
	         return false;
	    } elseif(empty($checkPlacementSettingIsRecord['participating_departemnt'])){
	        $this->invalidate('participating_department','Please record 
	        participating department you want in auto placement before running the auto
	         placement.');
	         return false;
	    } else {
	...
	*/
    function isPreferenceDeadlinePassed($academicyear=null,$college_id=null) {
        $checkPreferenceDeadline=ClassRegistry::init('PreferenceDeadline')->
        ...
        if($checkPreferenceDeadline){
                 $this->invalidate('preferencedeadline','The deadline for filling the preference is 
                 not passed. Please wait till the deadline is passed to run 
                 the auto placement.');
                 return false;
        }
        return true;
    ...
    /**
    * Total number of students for given academicyear and college, who doesnt assigned
    * department
    */
    ...
    if($college_id && $academicyear){
        ...
        $conditions['AND'] = array(array( "AcceptedStudent.academicyear LIKE " 
                                        ...
                                        $college_id,"AcceptedStudent.placementtype" =>null,
    ...
    /**
    *Sort out participationg department in the placement based on the demand by
    * privileage student most
    */
    
    function getListOfDepartmentRequesteByPrivilegageStudentMost_old($academicyear=null,
    $college_id=null){
    
       $regions=ClassRegistry::init('ParticipatingDepartment')->find("first",
       array("conditions"=>array('ParticipatingDepartment.academic_year LIKE'=>$academicyear.'%',
	    'ParticipatingDepartment.college_id'=>$college_id)));
	    
	    $prvilaged=null;
	      if(!empty($regions['ParticipatingDepartment']['developing_regions_id'])){
	    
             $prvilaged=array("OR"=> array("AcceptedStudent.region_id"=>array($regions['ParticipatingDepartment']['developing_regions_id']),"AcceptedStudent.sex"=>"female",
             "AcceptedStudent.disability <> null", "AcceptedStudent.disability <> ''"));
	    } else {
	       $prvilaged=array("OR"=> array("AcceptedStudent.sex"=>"female",
	        "AcceptedStudent.disability <> null", "AcceptedStudent.disability <> ''"));
	    }
	    ...
	    'conditions'=>array("Preference.academicyear LIKE"=>$academicyear.'%',
	    "Preference.college_id"=>$college_id,$prvilaged,
	    
	    "OR"=>array('AcceptedStudent.department_id is null ',
	    ...
	    'group'=>array('Preference.department_id','
	   ...
	   // need to clearification otherwise it is a mess, so need to comment out since
	   // top student can be assinged to their least privilaged. we need to sort departments 
	   // that don't have any quota for placement.
	    $departments_without_privilaged_quota=ClassRegistry::init('ParticipatingDepartment')->
	    find("all",array("fields"=>"ParticipatingDepartment.department_id","recursive"=>-1,
	    "conditions"=>array('ParticipatingDepartment.academic_year LIKE '=>$academicyear.'%',

	    'ParticipatingDepartment.college_id'=>$college_id,array("OR"=>array("ParticipatingDepartment.female"=>0,
	    "ParticipatingDepartment.female is null","ParticipatingDepartment.female = ''")),
	    array("OR"=>array("ParticipatingDepartment.disability"=>0,"ParticipatingDepartment.disability is null",
	    "ParticipatingDepartment.disability = ''")),
	    array("OR"=>array("ParticipatingDepartment.regions"=>0,"ParticipatingDepartment.regions is null",
	   ...
	   $participating_departments=ClassRegistry::init('ParticipatingDepartment')->find('all',
	   array("fields"=>"ParticipatingDepartment.department_id","recursive"=>-1,
	   "conditions"=>array('ParticipatingDepartment.academic_year LIKE '=>$academicyear.'%',
	    'ParticipatingDepartment.college_id'=>$college_id)));
     ...
     function getListOfDepartmentRequesteByPrivilegageStudentMost($academicyear=null,
    $college_id=null){
	    ...
	    $prefrenceMatrix=array();
	    foreach($prefrenceMatrixOfDepartments as $key=>$prefrenceMatrixOfDepartment){
	           $prefrenceMatrix[$prefrenceMatrixOfDepartment['Preference']['department_id']]
	           [$prefrenceMatrixOfDepartment['Preference']['preferences_order']]=
	           $prefrenceMatrixOfDepartment[0]['student_count'];
	           
	     }
        //department capacity
        $department_capacity=ClassRegistry::init('ParticipatingDepartment')->find("all",
        array("fields"=>array("ParticipatingDepartment.department_id","ParticipatingDepartment.number"),
        "recursive"=>-1,
        "conditions"=>array('ParticipatingDepartment.academic_year LIKE '=>$academicyear.'%',
	    'ParticipatingDepartment.college_id'=>$college_id)));
	   ...
	   // top student can be assinged to their least privilaged. we need to sort departments 
	   // that don't have any quota for placement.
	    $departments_without_privilaged_quota=ClassRegistry::init('ParticipatingDepartment')->find("all",array("fields"=>"ParticipatingDepartment.department_id","recursive"=>-1,"conditions"=>array('ParticipatingDepartment.academic_year LIKE '=>$academicyear.'%',
	    'ParticipatingDepartment.college_id'=>$college_id,array("OR"=>array("ParticipatingDepartment.female"=>0,"ParticipatingDepartment.female is null","ParticipatingDepartment.female = ''")),array("OR"=>array("ParticipatingDepartment.disability"=>0,"ParticipatingDepartment.disability is null","ParticipatingDepartment.disability = ''")),
	    array("OR"=>array("ParticipatingDepartment.regions"=>0,"ParticipatingDepartment.regions is null","ParticipatingDepartment.regions = ''")))));
	    //debug($departments_without_privilaged_quota);
	    $merged_department_order=array();
	   //if(!empty($departments_without_privilaged_quota)){
	        foreach($departments_without_privilaged_quota as $key=>$value){
   ...
   function checkAndAdjustAllocationWithAvailability($academicyear=null,
   $college_id=null,$resulttype=null,$department_id=null,$reservedQuotaNumber=array()){               
              ...
              foreach($reservedQuotaNumber as $result_critiera_id=>&$allocation_value){
                             ...
                             if($allocation_value['reservedquota']>$allocation_value['available']){
                                      $gap=$allocation_value['reservedquota']-$allocation_value['available'];
                                       ...
                                       foreach($reservedQuotaNumber as  
                                       $result_critiera_id1=>$allocation_value1){
                                       ...
                                       if ($gap_distribution_sum != $gap && $max_reserved_quota['max_index'] != 0){
                                        $reservedQuotaNumber[$max_reserved_quota['max_index']]['reservedquota']+=($gap - $gap_distribution_sum);
                                          ...
                                          foreach($reservedQuotaNumber as 
                                          $result_critiera_id3=>&$allocation_value3){
                                            if($allocation_value3['reservedquota'] == 0 
                                            && !$allocation_value3['adjusted'] && 
                                            $allocation_value3['available'] > 0){
                                              {
                                              $allocation_value3['reservedquota']=
                                              ($allocation_value3['available'] >= 
                                              ($gap - $gap_distribution_sum) ? 
                                              ($gap - $gap_distribution_sum) : 
                                              $allocation_value3['available']);
   ...
   */
   function checkAndAdjustPrivilagedQuota($academicyear=null,
   $college_id=null,$resulttype=null,$department_id=null,$adjusted_privilaged_quota=array(),$reservedQuotaNumber=array()){
                
                //get count of participating deparment for the given college and academic year
                $number_of_participating_department=ClassRegistry::init('ParticipatingDepartment')
                  ->find('count',array('conditions'=>array('ParticipatingDepartment.college_id'=>$college_id,'ParticipatingDepartment.academic_year'=>$academicyear)));
        ...
        foreach($adjusted_privilaged_quota as $privilage_type=>&$quota) {
                  ...
                  if(strcasecmp($privilage_type,"female")==0){
                    ...
                    /*
                    $privilagedcondition=array("OR"=>array("AcceptedStudent.disability IS NOT NULL",
                    "AcceptedStudent.disability"=>array('')));
                    */
                    $privilagedcondition="AcceptedStudent.disability IS NOT NULL";
                  } else {
                    $regions=ClassRegistry::init('ParticipatingDepartment')
                  ->find('first',array('conditions'=>array('ParticipatingDepartment.college_id'=>$college_id,'ParticipatingDepartment.academic_year'=>$academicyear)));
                   if(empty($regions['ParticipatingDepartment']['developing_regions_id']))
                     continue;
                  ...
                  // iterate each students availabilty against preference order for the given
                  // deparment_id
                  ...
                  if($number_of_participating_department && $quota){  
                      ...
                      for($i=1;$i<$number_of_participating_department;$i++){
                            $list_students_in_x_preference=$this->Preference->find(
                            'all',array('fields'=>array('Preference.accepted_student_id','Preference.department_id'),
                            'conditions'=>array('Preference.academicyear LIKE '=>$academicyear.'%',
                            'Preference.college_id'=>$college_id,'Preference.department_id'=>$department_id,'Preference.preferences_order'=>$i,$privilagedcondition,'AcceptedStudent.college_id'=>$college_id,'AcceptedStudent.academicyear LIKE '=>$academicyear.'%',array("OR"=>array('AcceptedStudent.department_id is null',"AcceptedStudent.department_id"=>array('',0))))));
                           ...
                           if($i == 1)
                             {
                                  $sum_available_students_privilaged += count($list_students_in_x_preference);
                                    // if there are enough students by their first 
                                    // preference for allocated quota for the department. 
                                  //no need to continue the loop if there are enough privilaged
                                  // students in system
                                  if($sum_available_students_privilaged >= 
                                   $quota){
                                     break;
                                  }
                                  continue;
                           ...
                           $list_of_departments_id=$this->find('all',
                           array('fields'=>array('DISTINCT AcceptedStudent.department_id'),
                           'conditions'=>array('AcceptedStudent.academicyear LIKE '=>$academicyear.'%',
                           'AcceptedStudent.college_id'=>$college_id,'AcceptedStudent.department_id is not null ','AcceptedStudent.department_id not '=>array('',0)),'recursive'=>-1));
                         ...
                         if(!empty($list_students_in_x_preference)){
                              foreach($list_students_in_x_preference as
                              &$student) {
                                   //check students back preferenc if they are not assigned.
                                   for($j=1; $j<$i; $j++){
                                         $department_id_accepted_student=$this->Preference->find('first',
                                         array('fields'=>array('Preference.department_id'),'conditions'=>array('Preference.accepted_student_id'
                                         =>$student['Preference']['accepted_student_id'],
                                         'Preference.preferences_order'=>$j)));
                                       
                                         // is her/his previous preference selected department was 
                                         // processed? 
                                         if(in_array($department_id_accepted_student['Preference']
                                         ['department_id'],$reformat_list_of_department_ids)===false){
...
///////////////////////COPIED FROM checkAndAdjustAllocationWithAvailability FUNCTION//////////////
    ...
    function privilagedStudentsFilterOut($academicyear=null,
   $college_id=null,$resulttype=null,$department_id=null,$adjusted_privilaged_quota=array(),$reservedQuotaNumber=array(),$placedStudents=array(),$privilage_type=null){
                ...
                $number_of_participating_department=ClassRegistry::init('ParticipatingDepartment')
                  ...
                  if($number_of_participating_department && 
                  $adjusted_privilaged_quota[$privilage_type]>0){  
                      ...
                      for($i=1;$i<$number_of_participating_department;$i++){
                           ...
                           if($i == 1)
                             {
                             //if the students is not in competitive list, please 
                             // consider me in the quota.
                             //debug($competitivly_assigned_students);
                             //debug($quota_assigned_students);
                             foreach($list_students_in_x_preference as $student){
                               if(in_array($student['Preference']['accepted_student_id'],
                               $competitivly_assigned_students)===false && 
                               in_array($student['Preference']['accepted_student_id'], 
                               $quota_assigned_students)===false){
                               $list_of_students_selected[] = $student['Preference']
                               ['accepted_student_id'];
                               ...
                               //debug($list_of_students_selected);
                                  ...
                                  //no need to continue the loop if there are enough privilaged
                                  // students in system
                                  if(count($list_of_students_selected) >= 
                                     $adjusted_privilaged_quota[$privilage_type]){
                                     break;
                                  }
                                  continue;
                           ...
                           if(!empty($list_of_departments_id)){
                                foreach($list_of_departments_id  as $key=>$value){
                                 $reformat_list_of_department_ids[]=$value['AcceptedStudent']
                                 ['department_id'];
                                }
                         ...
                         if(!empty($list_students_in_x_preference)){
                          ...
                          foreach($preliminary_students_filter as $student_id) {
                               if(in_array($student_id,
                               $competitivly_assigned_students)===false && 
                               in_array($student_id, $quota_assigned_students)===false)
   ...
   function runAutoParallelAssignmentAfterSeq($academicyear=null,
   $college_id=null,
   $resulttype=null){
   //find students who is assigned by quota
   $acceptedStudentPlacedByQuota=$this->find('all',array('conditions'=>array('AcceptedStudent.college_id'=>$college_id,
   'AcceptedStudent.academicyear like'=>$academicyear.'%',
  ...
  debug($acceptedStudentPlacedByQuota);
   //find students who is assigned by quota
   $acceptedStudents=$this->find('all',array('conditions'=>array('AcceptedStudent.college_id'=>$college_id,
   "AcceptedStudent.academicyear like "=>$academicyear.'%',
 ...
 "AcceptedStudent.program_type_id"=>PROGRAM_TYPE_REGULAR,
   ...
   foreach($acceptedStudents as $acceptedStudent) {
	...
	if(
	$this->saveAll($placement_cancelation_list)) {
			//run the sequential 
		$autoplacedstudents=$this->auto_parallel_assignment($academicyear,$college_id,$resulttype);
		if(isset($autoplacedstudents) 
		&& !empty($autoplacedstudents)){
   ...
   function auto_placement_algorithm($academicyear=null,$college_id=null,
   $resulttype=null, $high_proprity_for_high_result = null, $first_consider_first = null){
         //list of departments orderd by based on the demand by the privilaged students
        $departments= $this->getListOfDepartmentRequesteByPrivilegageStudentMost
        ($academicyear,$college_id,$resulttype);
        ...
        if(!empty($departments)){
                foreach($departments as $department_id =>$weight){
                 ...
                 $detail_of_participating_department=ClassRegistry::init('ParticipatingDepartment')
                  ->find('first',array('conditions'=>array('ParticipatingDepartment.college_id'=>$college_id,
                  'ParticipatingDepartment.academic_year'=>$academicyear,
                  'ParticipatingDepartment.department_id'=>$department_id)));

                  $adjusted_privilaged_quota['female']=
                  $detail_of_participating_department['ParticipatingDepartment']['female'];
                  ...
                  $reservedPlaceForDepartmentByGradeRange = ClassRegistry::init('PlacementsResultsCriteria')
                  ->reservedPlaceForDepartmentByGradeRange($academicyear,$college_id,$department_id);
                  ...
                  if(!empty($reservedPlaceForDepartmentByGradeRange)){
                      foreach($reservedPlaceForDepartmentByGradeRange as $key=>$grade_ranage_quota){
                          ...
                          $availablestudentInGivenRanage=$this->
                             availableStudentInGivenRangeAndQuota($academicyear,$college_id,
        $resulttype,$grade_ranage_quota);
              ...
              /// retreive and reformat end ===================
                      ...
                      //// adjusted privilaged  quota     
                        ...
                        $academicyear,$college_id,$resulttype,
                        $department_id,$adjusted_privilaged_quota,$reservedQuotaNumber);
                       
                        //// competitive quota adjustment
                        $ready_competitive_department_allocation=$this->checkAndAdjustAllocationWithAvailability($academicyear,$college_id,$resulttype,
                        $department_id, $pre_ready_normal_privilaged_department_allocation[0]);
...
//debug($department_id);
//debug($ready_competitive_department_allocation);
                        foreach($ready_competitive_department_allocation as 
                        $result_category_id=>$reserved_quota_adjusted){
                                if($reserved_quota_adjusted['reservedquota'] > 0 ){ 
                                $sortedStudentByPreferenceAndGrade=$this->
                                       sortOutStudentByPreference($college_id,
                                       $academicyear,$result_category_id,$resulttype,
                                       $department_id, $high_proprity_for_high_result, $first_consider_first);
                                //debug($department_id);
                        ...
                        foreach($pre_ready_normal_privilaged_department_allocation[1] as 
                        $privilage_type=>&$quota){
                         if($quota > 0){
                         $completed=false;
                         $privilaged_selected=$this->privilagedStudentsFilterOut($academicyear,
   $college_id,$resulttype,$department_id,$pre_ready_normal_privilaged_department_allocation[1],$ready_competitive_department_allocation,$placedStudents, $privilage_type);
                         //debug($department_id);
                         //debug($privilage_type);
...
///////////////////////COPIED FROM checkAndAdjustAllocationWithAvailability FUNCTION//////////////
                       ...
                       $max_reserved_quota = array('max_quota'=>0,'max_index'=>0); 
                       if($reserved_sum>0){
                       foreach($ready_competitive_department_allocation as $result_critiera_id2=>&
                       $allocation_value2){
                               $gap_distribution_sum += round($gap*
                               ($allocation_value2['reservedquota']/$reserved_sum));
                               $allocation_value2['reservedquota']+=round($gap*
                               ($allocation_value2['reservedquota']/$reserved_sum));
                               if($allocation_value2['reservedquota']>=$max_reserved_quota['max_quota']){
                                $max_reserved_quota['max_quota'] = $allocation_value2['reservedquota'];
                                $max_reserved_quota['max_index'] = $result_critiera_id2;
                       ...
                       // if there is no student any of assigned quota
                       if ($gap_distribution_sum != $gap && $max_reserved_quota['max_index'] != 0){
                        $ready_competitive_department_allocation[$max_reserved_quota['max_index']]
                        ['reservedquota']+=($gap - $gap_distribution_sum);      
                       }
...
//////////////////////////////////////////////////////////////////////////
                       ...
                       //debug($ready_competitive_department_allocation);
                       $ready_competitive_department_allocation=$this->checkAndAdjustAllocationWithAvailability($academicyear,$college_id,$resulttype,
                        $department_id, $ready_competitive_department_allocation);
                       //debug($ready_competitive_department_allocation);
                       break;
                       }
      ...
      //  $x=$this->runAutoParallelAssignmentAfterSeq($academicyear,$college_id,$resulttype);
         ...
         $placedstudent=$this->find('all',array('conditions'=>array('AcceptedStudent.college_id'
         =>$college_id,'AcceptedStudent.academicyear LIKE '=>$academicyear.'%','AcceptedStudent.placementtype'=>AUTO_PLACEMENT),'order'=>array('AcceptedStudent.department_id asc',$result_order_by)));
         $dep_id=array_keys($departments);
         $dep_name=$this->Department->find('list',array('conditions'=>array(
         ...
         foreach($dep_name as $dk=>$dv){
                 foreach($placedstudent as $k=>$v){
                  ...
                  $newly_placed_student['auto_summery'][$dv]['C']=$this->find('count',
                  array('conditions'=>array('AcceptedStudent.academicyear LIKE'=>
                  $academicyear.'%','AcceptedStudent.department_id'=>$dk,
                  'AcceptedStudent.college_id'=>$college_id,'AcceptedStudent.placement_based'=>'C')));
                  $newly_placed_student['auto_summery'][$dv]['Q']=$this->find('count',
                  array('conditions'=>array('AcceptedStudent.academicyear LIKE'=>
                  $academicyear.'%','AcceptedStudent.department_id'=>$dk,
                  'AcceptedStudent.college_id'=>$college_id,'AcceptedStudent.placement_based'=>'Q')));
          }
    ...
    function sortOutStudentByPreference($college_id=null,
        $academicyear=null,$result_category_id=null,$result_type=null,$department_id=null, 
        $high_proprity_for_high_result = null, $first_consider_first = null){
      
        $result_type_condition=null;
        $result_order_by=null;
      
        $resultcategoryvalue=ClassRegistry::init('PlacementsResultsCriteria')->
                  find('first',array('conditions'=>array('PlacementsResultsCriteria.id'=>
                  $result_category_id,
        ...
        $sort_out_students_result_category=$this->find('all',array('joins'=>array(array(
                        'table' => 'preferences',        
                        'alias' => 'Preference',        
                        'type' => 'LEFT',  
                        'conditions' => array('Preference.accepted_student_id = AcceptedStudent.id'))),
                        
                        'fields'=>array('AcceptedStudent.id','AcceptedStudent.EHEECE_total_results',
                        'AcceptedStudent.freshman_result', 'Preference.preferences_order'),
                        'recursive'=>-1,
                 ...
                 'AcceptedStudent.department_id '=>array('',0)),
                                        "AcceptedStudent.academicyear LIKE " 
                                        => $academicyear.'%',"AcceptedStudent.college_id" => 
                                        $college_id,"Preference.department_id"=>$department_id,$result_type_condition),
                                        'order'=>array('Preference.preferences_order asc',$result_order_by)));
        // students who doesnt complete their preference on time
        $sort_out_students_not_completed_category=$this->find('all',array('joins'=>array(array(
                        'table' => 'preferences',        
                        'alias' => 'Preference',        
                        'type' => 'LEFT',  
                        'conditions' => array('Preference.accepted_student_id = AcceptedStudent.id'))),
                 ...
                 'AcceptedStudent.department_id '=>array('',0)),
                                        "AcceptedStudent.academicyear LIKE " 
                                        => $academicyear.'%',"AcceptedStudent.college_id" => 
                                        $college_id,"AcceptedStudent.id not in (select accepted_student_id from preferences where preferences.department_id = ".$department_id.")",$result_type_condition),
                                        'order'=>array('Preference.preferences_order asc',$result_order_by)));
        ...
        //If enforce student other prferences as long as s/he has good result irespective of other chanse
        ...
        if($high_proprity_for_high_result || $first_consider_first) {
	        foreach($sort_out_students_result_category as $k => $v) {
		        if(!empty($v['Preference']['preferences_order']) && $v['Preference']['preferences_order'] > 1) {
			        $previous_preferences = ClassRegistry::init('Preference')->find('all', 
				        array(
					        'conditions' => 
					        array(
			        ...
			        foreach($previous_preferences as $pp_v) {
				        $placed_students_count = $this->find('count',
					        array(
						        'conditions' => 
						        array(
							        'AcceptedStudent.academicyear LIKE ' => $academicyear.'%',
    ...
    function getReservedQuota($departments=null,
          $academicyear=null,$college_id=null,$resulttype=null){
          $reservedQuotaNumber=array();
         ...
         if(!empty($departments)){
            foreach($departments as $department_id=>$weight){
                  ...
                  $reservedDepartmentNumber = ClassRegistry::init('PlacementsResultsCriteria')
                  ->reservedPlaceCategory($academicyear,$college_id,$department_id);
    ...
    function getPrivilagedQuota($departments=null,
          $academicyear=null,$college_id=null,$resultcategory=null){
    ...
    */
    function availableStudentInGivenRangeAndQuota ($academicyear=null,$college_id=null,
    $resulttype=null,$categoryvalue=null){
    ...
    function detect_privilaged_qutoa_presence($academicyear=null,$college_id=null){
       $is_quota_present=ClassRegistry::init('ParticipatingDepartment')->find("all",
       array("conditions"=>array('ParticipatingDepartment.academic_year LIKE'=>$academicyear.'%',
	    'ParticipatingDepartment.college_id'=>$college_id)));
	    $quota_sum=0;
	    foreach($is_quota_present as $qp=>$qv){
	        $quota_sum +=($qv['ParticipatingDepartment']['female']+
	        $qv['ParticipatingDepartment']['regions']+
	        $qv['ParticipatingDepartment']['disability']);
	    }
	    return  $quota_sum;
    }
    ...
    */
    function auto_parallel_assignment($academicyear=null,$college_id=null,$result_type=null){
        ///retrive  placement result criteria with reserved quota
          ...
          $list_of_result_category = ClassRegistry::init('PlacementsResultsCriteria')
          ->getListOfGradeCategory($academicyear,$college_id);
          //
          if(!empty($list_of_result_category)){
              $placedStudentsSave = array();
              $count = 0;
              foreach($list_of_result_category as $unwanted => 
              $placementsResultsCriteria){
              $students_without_preference = array();
              //debug($placementsResultsCriteria);
              $reserved_quota_by_department_draft=ClassRegistry::init('ReservedPlace')->find(
               'all',array('fields'=>array('ReservedPlace.id','ReservedPlace.participating_department_id',
               'ReservedPlace.number'),'conditions'=>array('ReservedPlace.college_id'=>$college_id,
               'ReservedPlace.academicyear LIKE'=>$academicyear.'%',
               'ReservedPlace.placements_results_criteria_id'=>
               $placementsResultsCriteria['PlacementsResultsCriteria']['id'])));
              ...
              //debug($reservedPlace);
              //incase user unable to clear the quota which is unncessary during parallel placement
            /*  
              $pd_value = ClassRegistry::init('ParticipatingDepartment')->find('first',array('conditions' =>array(
              'ParticipatingDepartment.college_id' => $college_id, 'ParticipatingDepartment.academic_year LIKE' => $academicyear.'%'),'recursive' => -1));
			  ...
			  */  
                ...
                $reserved_quota_by_department[$reservedPlace['ReservedPlace']
                ['participating_department_id']]['assigned'] = 0;
              }
              //debug($placementsResultsCriteria['PlacementsResultsCriteria']['id']);
              //debug($reserved_quota_by_department);
              //List of students who has a result for the current running grade range
              if($result_type)
                {
                $result_condition = array('AcceptedStudent.EHEECE_total_results >=' => 
                $placementsResultsCriteria['PlacementsResultsCriteria']['result_from'], 
                ...
                {
                $result_condition = array('AcceptedStudent.freshman_result >=' => $placementsResultsCriteria['PlacementsResultsCriteria']['result_from'], 'AcceptedStudent.freshman_result <=' => $placementsResultsCriteria['PlacementsResultsCriteria']['result_to']);
                $result_order = 'AcceptedStudent.freshman_result DESC';
                }
              $students_with_x_grade_range = $this->find('all', 
              array('fields' => array('AcceptedStudent.id'), 
              'conditions' => array('AcceptedStudent.academicyear LIKE'=>$academicyear.'%', 
              'AcceptedStudent.placementtype !="DIRECT PLACED"',
              
              'AcceptedStudent.college_id'=>$college_id, $result_condition), 'order' => array($result_order), 
              'recursive' => -1));
              //debug($placementsResultsCriteria);
              ...
              foreach($students_with_x_grade_range as $unwanted => $student){
                  ...
                  $student_prefrence = $this->Preference->find('all', 
                  array('fields'=>array('Preference.department_id','Preference.preferences_order'), 
               ...
               if(!empty($student_prefrence)){
                  ...
                  foreach($student_prefrence as $unwanted => $preference){
                    ...
                    if($reserved_quota_by_department[$preference['Preference']['department_id']]['assigned'] < 
                    $reserved_quota_by_department[$preference['Preference']['department_id']]['quota'])
                        {
              ...
              for($i = 0; $i < count($students_without_preference); $i++)
                  {
                  foreach($reserved_quota_by_department as $department_id => 
                  &$assigned_and_quota){
         ...
         =>$college_id,'AcceptedStudent.academicyear LIKE '=>$academicyear.'%','AcceptedStudent.placementtype'=>AUTO_PLACEMENT),'order'=>array('AcceptedStudent.department_id asc',$result_order_by)));
        //debug($placedstudent); 
         $departments=ClassRegistry::init('ParticipatingDepartment')->find("all",array('fields'=>'ParticipatingDepartment.department_id',"conditions"=>array('ParticipatingDepartment.academic_year LIKE'=>$academicyear.'%',
	    'ParticipatingDepartment.college_id'=>$college_id)));
         $dep_id=array();
        ...
        return   $newly_placed_student;          
                ...
                die("No result criteria recorded. Technical Detail:Model:AcceptedStudent
                ,Function: auto_parallel_assignment");
          ...
          /*$reservedQuotaNumber=array();
                  ...
                  if(!empty($reservedPlaceForDepartmentByGradeRange)){
                      foreach($reservedPlaceForDepartmentByGradeRange as $key=>$grade_ranage_quota){
                          ...
                          $availablestudentInGivenRanage=$this->
                             availableStudentInGivenRangeAndQuota($academicyear,$college_id,
        $result_type,$grade_ranage_quota);
                             $reservedQuotaNumber[$grade_ranage_quota["PlacementsResultsCriteria"]["id"]]['available']=$availablestudentInGivenRanage;
                  ...
                  debug($reservedQuotaNumber); 
                 */
                 /*$studentsOrderByGrade=null;
                 foreach($reservedQuotaNumber as $result_category_id => 
                 $result_category_place){
                        // find students in given grade range
                 }
                 */
    ...
    function check_program_type ($data=null,$role_id=null) {
            ...
            if($data['AcceptedStudent']['program_id']!=PROGRAM_UNDEGRADUATE){
                if (empty($data['AcceptedStudent']['department_id']) || $data['AcceptedStudent']['department_id']==0){
                       $this->invalidate('program',
	            'For post graduate  student, you need to select department.');
                     return false;
                }
            } 
            
            if (($data['AcceptedStudent']['program_type_id'] != PROGRAM_TYPE_REGULAR) && 
            empty($data['AcceptedStudent']['department_id'])) {
                 $this->invalidate('program',
	            'For non regular  student, you need to select department.');  
                return false;
            }
            return true;
        /*
        if($data['AcceptedStudent']['department_id']==null || $data['AcceptedStudent']['department_id']==0 ){
           
            if( $data['AcceptedStudent']['program_type_id'] ==  PROGRAM_TYPE_REGULAR
            && $data['AcceptedStudent']['program_id']==PROGRAM_UNDEGRADUATE){
               return true;
            } else {
               $this->invalidate('program',
            ...
            if($data['AcceptedStudent']['program_id']!=PROGRAM_UNDEGRADUATE){
               return true;
            } else if ($data['AcceptedStudent']['program_type_id'] != PROGRAM_TYPE_REGULAR) {
               return true;
            } else {
               $this->invalidate('program',
	            'You can not select department for regular undergraduate student, they will be placed by the college directly or auto to department.Please select no department');
	            return false;
            }
