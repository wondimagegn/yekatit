...
class ExamGrade extends AppModel
{
	...
	var $validate = array(
		...
		'makeup_exam_id' => array(
			'numeric' => array(
				'rule' => array('numeric'),
				'message' => 'Please select make up exam',
				//'allowEmpty' => false,
				//'required' => false,
				//'last' => false, // Stop validation after this rule
				//'on' => 'create', // Limit validation to 'create' or 'update' operations
	...
	function is_grade_submitted($published_course_ids = null,$student_lists=array())
	{
		...
		if (isset($student_lists) && !empty($student_lists)) {
			...
			debug($grade_submitted_registred_courses);
		} else {
...
*/
		...
		if (!empty($grade_submitted_registred_courses)) {
			...
			/*
                 $published_courses_student_registred_score_grade= ClassRegistry::
	    init('ExamResult')->find('count',
	...
	function getGradeSubmmissionDate($published_course_ids = null)
	{
		...
		if (!empty($grade_submitted_registred_courses)) {
			$published_courses_student_registred_score_grade = $this->find(
				...
				array(
					'conditions' =>
					array('ExamGrade.course_registration_id' => $grade_submitted_registred_courses),
					'recursive' => -1
				)
			);
	...
	/**
	 *Check grade is submitted for current section of student  given student id
	 *and section id return true if grade is submitted and return false if grade has
	 *not been submitted and allow hard deletion.
	 */
	function isCourseGradeSubmitted($student_id = null, $section_id = null)
	{
		...
		if (!empty($sections)) {
			...
			//find course regisration nuumber
			$courseRegistration = $this->CourseRegistration->find(
				'all',
				array(
					'conditions' => array(
		...
		if (!empty($sections)) {
			...
			if (!empty($list_course_registration_ids)) {
				...
				$gradeSubmitted = ClassRegistry::init('ExamResult')->find(
					'first',
					array(
						'conditions' => array('ExamResult.course_registration_id' =>
						$grade_submitted_add_courses),
						'recursive' => -1
					)
				);
				if ($gradeSubmitted > 0) {
	...
	{
		$grade_history = $this->find(
			'first',
			array(
				'conditions' => array('ExamGrade.id' => $grade_id),
		...
		if (!empty($grade_history['ExamGrade'])) {
			if ($grade_history['ExamGrade']) {
				if ($grade_history['ExamGrade']['department_approval'] == null)
					return "There is already a submitted grade which is waiting for department approval. Please first finalize the approval process for the already submitted grade before recording a new grade.";
				else if ($grade_history['ExamGrade']['department_approval'] == 1 && $grade_history['ExamGrade']['registrar_approval'] == null)
					return "There is already a submitted grade which is waiting for registrar approval. Please first let the registrar finalize the approval process for the already submitted grade before recording a new grade.";
			}
					...
					if ($examGradeChange['manual_ng_conversion'] != 1 && $examGradeChange['auto_ng_conversion'] != 1) {
						if ($examGradeChange['initiated_by_department'] != 1 && $examGradeChange['department_approval'] == null) {
							if ($examGradeChange['makeup_exam_result'] == null)
								return "There is already a submitted grade change request which is awaiting department approval. Please first finalize the approval process for the already submitted grade change request before recording a new grade.";
							else
								return "There is already a submitted makeup exam grade which is awaiting department approval. Please first finalize the approval process for the already submitted makeup exam grade before recording a new grade.";
							...
							if ($examGradeChange['initiated_by_department'] == 1 && $examGradeChange['makeup_exam_result'] != null)
								return "There is already a submitted supplementary exam grade which is awaiting registrar approval. Please first let the registrar finalize the approval process for the already submitted supplementary exam grade before recording a new grade.";
							else if ($examGradeChange['makeup_exam_result'] == null && $examGradeChange['college_approval'] == null)
								return "There is already a submitted grade which is awaiting college approval. Please first let the college finalize the approval process for the already submitted grade before recording a new grade.";
								...
								return "There is already a submitted grade which is awaiting registrar approval. Please first let the registrar finalize the approval process for the already submitted grade before recording a new grade.";
						}
					}
	...
	function getStudentCoursesAndFinalGrade($student_id, $acadamic_year, $semester, $include_exempted = 0)
	{
		...
		$course_registered = $this->CourseRegistration->find(
			'all',
			array(
				'conditions' =>
				array(
		...
		$course_added = $this->CourseAdd->find(
			'all',
			array(
				'conditions' =>
				array(
		...
		foreach ($course_added as $ca_key => $ca_value) {
			if (!($ca_value['PublishedCourse']['add'] == 1 ||
				($ca_value['CourseAdd']['department_approval'] == 1 &&
					$ca_value['CourseAdd']['registrar_confirmation'] == 1))) {
		...
		if (!empty($student_detail['Student']['curriculum_id'])) {
			...
			$courses_to_be_given = $this->CourseAdd->PublishedCourse->Course->find(
				'all',
				array(
					'conditions' =>
					array(
			...
			if ($include_exempted == 1) {
				$all_exempted_courses = $this->CourseAdd->Student->CourseExemption->find(
					'all',
					array(
						'conditions' =>
						array(
		...
		foreach ($course_registered as $key => $value) {
			if (!$this->CourseRegistration->isCourseDroped($value['CourseRegistration']['id']) && isset($value['PublishedCourse']['Course']['id'])) {
				...
				$matching_courses = ClassRegistry::init('EquivalentCourse')->validEquivalentCourse($value['PublishedCourse']['Course']['id'], $student_detail['Student']['curriculum_id']);
				$matching_courses[] = $value['PublishedCourse']['Course']['id'];
	...
	function getApprovedGrade($register_add_id = null, $registration = 1)
	{
		...
		if (!empty($grade_detail)) {
			$grade_change = $this->ExamGradeChange->find(
				'first',
				array(
					'conditions' =>
					array(
			...
			$grade_related_detail = $this->GradeScale->find(
				'first',
				array(
					'conditions' =>
					array(
			...
			if (isset($grade_related_detail['GradeScaleDetail'])) {
				foreach ($grade_related_detail['GradeScaleDetail'] as $key => $value) {
					...
					if (!empty($value['Grade'])) {
						...
						/*	$gradeTypeDetailsPF=ClassRegistry::init('Grade')->find('first',array('conditions'=>array('Grade.id'=>$value['grade_id']),
					'contain'=>array('GradeType')
					));
					debug($gradeTypeDetailsPF);
					if(isset($gradeTypeDetailsPF['GradeType']) && !empty($gradeTypeDetailsPF['GradeType'])){
	...
	function editableExamType($published_course_id)
	{
		...
		$registIds = $this->CourseRegistration->find(
			'list',
			array(
				'conditions' =>
				array(
	...
	function getExamType($register_add_id = null, $registration = 1)
	{
		$exam_result = array();
		if ($registration == 1) {
			$exam_result = $this->CourseRegistration->ExamResult->find(
				'all',
				array(
					'conditions' =>
					array(
	...
	function getStudentCopy($student_id = null, $academic_year = null, $semester = null)
	{
		...
		if ($pattern <= 1) {
			...
			$status_prepared = $this->CourseAdd->Student->StudentExamStatus->find(
				'count',
				array(
					'conditions' =>
					array(
			...
			if ($status_prepared == 0) {
				...
				//If there are lots of semester without status generation.
				//It is to avoid including other semester/s in the current pattern
		...
		$options['contain'] = array('PublishedCourse' => array('Course' => array('Curriculum', 'GradeType' => array('Grade'))));
		...
		$options['contain'] = array('PublishedCourse' => array('Course' => array('Curriculum')));
		...
		if (!empty($student_course_registrations)) {
			$section_detail = $this->CourseAdd->Student->Section->find(
				'first',
				array(
					'conditions' =>
					array(
		...
		} else if (!empty($student_course_adds)) {
			//TODO: Replace it with some kind of computation that figure out the section
		...
		$student_status = $this->CourseRegistration->Student->StudentExamStatus->find(
			'first',
			array(
				'conditions' =>
				array(
		...
		$all_student_status = $this->CourseRegistration->Student->StudentExamStatus->find(
			'all',
			array(
				'conditions' =>
				array(
		...
		foreach ($all_student_status as $st_key => $student_status2) {
			...
			if (
				strcasecmp($student_status2['StudentExamStatus']['academic_year'], $academic_year) == 0 &&
				strcasecmp($student_status2['StudentExamStatus']['semester'], $semester) == 0 || empty($student_status)
			) {
				break;
			} else {
	...
	function getMasterSheet($section_id = null, $academic_year = null, $semester = null)
	{
		...
		$students_in_section = $this->CourseRegistration->Student->Section->StudentsSection->find(
			'all',
			array(
				'conditions' =>
				array(
		...
		$students_in_section_ids = $this->CourseRegistration->Student->Section->StudentsSection->find(
			'list',
			array(
				'conditions' =>
				array(
		...
		debug($students_in_section_ids);
		$studentRegisteredCourseForSection = $this->CourseRegistration->find('list', array(
			'conditions' =>
			array('CourseRegistration.section_id' => $section_id),
			'recursive' => -1,
			'fields' => array(
		...
		debug($students_in_section);
		/* Get each student pattern, AY & Semester and list of courses s/he registered and add
			within the returned AY and semester */
		$registered_courses = array();
		$added_courses = array();
		...
		foreach($students_in_section as $key => $value) {
			$students_ids[] = $value['StudentsSection']['student_id'];
		}
		//Courses students in the section registered for
		$student_course_registrations = $this->CourseRegistration->find('all',
			array(
				'conditions' =>
				array(
					'CourseRegistration.student_id' => $students_ids,
		...
		$registered_courses = array();
		//debug($student_course_registrations);
		foreach($student_course_registrations as $key => $student_course_registration) {
			$r_index = count($registered_courses);
			foreach($registered_courses as $key2 => $registered_course)
				if($registered_course['id'] == $student_course_registration['PublishedCourse']['Course']['id'])
		...
		//debug($registered_courses);
		//Courses students in the section added for
		$student_course_adds = $this->CourseAdd->find('all',
			array(
				'conditions' =>
				array(
					'CourseAdd.student_id' => $students_ids,
		...
		foreach ($students_in_section as $key => $value) {
			...
			foreach ($all_student_status as $st_key => $student_status2) {
				if (
					strcasecmp($student_status2['StudentExamStatus']['academic_year'], $academic_year) == 0 &&
					strcasecmp($student_status2['StudentExamStatus']['semester'], $semester) == 0
				) {
					break;
				} else {
					$previous_credit_hour_sum += $student_status2['StudentExamStatus']['credit_hour_sum'];
			...
			foreach ($registered_courses as $key2 => $registered_course) {
				...
				$registration_id = $this->CourseRegistration->field(
					'id',
					array(
				...
				//debug($students_and_grades);

				//START: Getting repeated courses credit hour (if there is any)
				/*if(!empty($previous_ay_and_s_list)) {
					$previously_registered_courses = $this->CourseRegistration->getCourseRegistrations($value['StudentsSection']['student_id'], $previous_ay_and_s_list, $registered_course['course_id']);
					$previously_added_courses = $this->CourseAdd->getCourseAdds($value['StudentsSection']['student_id'], $previous_ay_and_s_list, $registered_course['course_id']);
					//If the student registered for the course
					if(!empty($previously_registered_courses)) {
						foreach($previously_registered_courses as $prc_key => $previously_registered_course) {
							$course_registration_grade_detail = $this->getApprovedGrade($previously_registered_course['CourseRegistration']['id'], 1);
							if(!empty($course_registration_grade_detail) && isset($course_registration_grade_detail['used_in_gpa']) && $course_registration_grade_detail['used_in_gpa'] == 1) {
								$deduct_credit += $previously_registered_course['PublishedCourse']['Course']['credit'];
								$deduct_gp += ($previously_registered_course['PublishedCourse']['Course']['credit']*$course_registration_grade_detail['point_value']);
							}
						}
					}
					//If the student add for the course
					if(!empty($previously_added_courses)) {
						foreach($previously_added_courses as $prc_key => $previously_added_course) {
							$course_add_grade_detail = $this->getApprovedGrade($previously_added_course['CourseAdd']['id'], 0);
							//debug($course_add_grade_detail);
							if(!empty($course_add_grade_detail) && isset($course_add_grade_detail['used_in_gpa']) && $course_add_grade_detail['used_in_gpa'] == 1) {
								$deduct_credit += $previously_added_course['PublishedCourse']['Course']['credit'];
								$deduct_gp += ($previously_added_course['PublishedCourse']['Course']['credit']*$course_add_grade_detail['point_value']);
							}
						}
					}
			...
			foreach ($added_courses as $key2 => $added_course) {
				...
				if (!empty($add_id)) {
					...
					//START: Getting repeated courses credit hour (if there is any)
					/*if(!empty($previous_ay_and_s_list)) {
						$previously_registered_courses = $this->CourseRegistration->getCourseRegistrations($value['StudentsSection']['student_id'], $previous_ay_and_s_list, $added_course['course_id']);
						$previously_added_courses = $this->CourseAdd->getCourseAdds($value['StudentsSection']['student_id'], $previous_ay_and_s_list, $added_course['course_id']);
						//If the student registered for the course
						if(!empty($previously_registered_courses)) {
							foreach($previously_registered_courses as $prc_key => $previously_registered_course) {
								$course_registration_grade_detail = $this->getApprovedGrade($previously_registered_course['CourseRegistration']['id'], 1);
								//debug($course_registration_grade_detail);
								if(!empty($course_registration_grade_detail) && isset($course_registration_grade_detail['used_in_gpa']) && $course_registration_grade_detail['used_in_gpa'] == 1) {
									$deduct_credit += $previously_registered_course['PublishedCourse']['Course']['credit'];
									$deduct_gp += ($previously_registered_course['PublishedCourse']['Course']['credit']*$course_registration_grade_detail['point_value']);
								}
	...
	function studentCopy($student_ids = array())
	{
		...
		foreach ($student_ids as $ky => $student_id) {
			...
			$first_registration = $this->CourseRegistration->find(
				'first',
				array(
					'conditions' =>
					array(
			...
			$last_registration = $this->CourseRegistration->find(
				'first',
				array(
					'conditions' =>
					array(
			...
			if ($first_ay_se['academic_year'] != null && $first_ay_se['semester'] != null) {
				...
				do {
					...
					$exam_status = $this->CourseRegistration->Student->StudentExamStatus->find(
						'first',
						array(
							'conditions' =>
							array(
	...
	function getListOfAyAndSemester($student_id = null, $upto_acadamic_year = null, $upto_first_semester = null)
	{
		...
		$first_added = $this->CourseAdd->find(
			'first',
			array(
				'conditions' =>
				array(
		...
		$first_registered = $this->CourseRegistration->find(
			'first',
			array(
				'conditions' =>
				array(
		...
		$academic_status_empty = $this->CourseAdd->Student->StudentExamStatus->find(
			'first',
			array(
				'conditions' =>
				array(
		...
		// introduced for regeneration bug when
		// there is no status history in the system

		if (
			empty($academic_status_empty) &&
			!empty($upto_acadamic_year) && !empty($upto_first_semester)
		) {
		...
		$last_added = $this->CourseAdd->find(
			'first',
			array(
				'conditions' =>
				array(
		...
		$last_registered = $this->CourseRegistration->find(
			'first',
			array(
				'conditions' =>
				array(
		...
		if (empty($first_added) && empty($first_registered))
			return array();
		/*** If the course registration comes first, then we need to start the
			  acdamic year and semester generation from the earliest possible time ***/
		else if (
		...
		if (
			...
			(((!isset($last_added['CourseAdd']) || empty($last_added['CourseAdd']))) ||
				(isset($last_added['CourseAdd']) && !empty($last_added['CourseAdd']) &&
					($last_added['CourseAdd']['created'] < $last_registered['CourseRegistration']['created'] || $last_added['CourseAdd']['academic_year'] < $last_registered['CourseRegistration']['academic_year'])))
		) {
		...
		foreach($ay_and_s_list as $k=>&$v) {
			$withdrawlAfterRegistration = $this->CourseRegistration->Student->Clearance->withDrawaAfterRegistration($student_id,
$v['academic_year'],$v['semester']);
	...
	function isRegistrationAddForFirstTime($id = null, $registration = 1, $include_equivalent = 1)
	{
		...
		$student_id = null;
		if ($registration == 1) {
			$regisration_detail = $this->CourseRegistration->find(
				'first',
				array(
					'conditions' =>
					array(
			...
			$course_id = $regisration_detail['PublishedCourse']['Course']['id'];
			$student_id = $regisration_detail['CourseRegistration']['student_id'];
			$studentDetail = $this->CourseRegistration->Student->find("first", array('conditions' => array('Student.id' => $student_id), 'recursive' => -1));
			$student_department['Student'] = $regisration_detail['Student'];
			$course_department['Course'] = $regisration_detail['PublishedCourse']['Course'];
		} else {
			debug($id);
			$add_detail = $this->CourseAdd->find(
				'first',
				array(
					'conditions' =>
					array(
			...
			$course_id = $add_detail['PublishedCourse']['Course']['id'];
			$student_id = $add_detail['CourseAdd']['student_id'];
			$studentDetail = $this->CourseAdd->Student->find("first", array('conditions' => array('Student.id' => $student_id), 'recursive' => -1));
			$student_department['Student'] = $add_detail['Student'];
			$course_department['Course'] = $add_detail['PublishedCourse']['Course'];
			debug($course_id);
		}
	...
	/*It returns list of courses department or freshman program is supposed to approve
	The code is directly copied from the approve_non_freshman_grade_submission action under
	exam_grade controller*/
	/*
	function getDepartmentNonApprovedCoursesList($dep_col_id = null, $department = 1){
		if($department == 1) {
			$published_course_list_student_registered=$this->CourseRegistration->
			PublishedCourse->find('list',
				array(
					'conditions' =>
					array(
						'PublishedCourse.drop' => 0,
						'PublishedCourse.department_id' => $dep_col_id,
			...
			PublishedCourse->find('list',
				array(
					...
					array(
						'PublishedCourse.drop' => 0,
						'PublishedCourse.college_id' => $dep_col_id,
						'(PublishedCourse.id in (select published_course_id from course_registrations) or
						PublishedCourse.id in (select published_course_id from course_adds))'
					),
					'fields'=>
		...
		$this->CourseRegistration->find('all',
			array(
				'fields' =>
				array(
				...
				array(
					...
					where department_approval is null)'
				),
				'contain' =>
				array(
					'PublishedCourse'=>
        ...
        //debug($merged_courses_student_registered_and_add);
        //debug($merged_courses_student_rejected_grade_registered_and_add);

			$courses_for_department_approval = array();
			$rejected_courses_for_department_approval = array();
			foreach($merged_courses_student_registered_and_add as $index=>$value) {
				foreach($courses_for_department_approval as $index2 => $value2) {
					if($value['PublishedCourse']['id'] == $value2['PublishedCourse']['id'])
						continue 2;
				}
				$courses_for_department_approval[] = $value;
			}
			foreach($merged_courses_student_rejected_grade_registered_and_add as $index=>$value) {
				foreach($rejected_courses_for_department_approval as $index2 => $value2) {
					if($value['PublishedCourse']['id'] == $value2['PublishedCourse']['id'])
						continue 2;
				}
				$rejected_courses_for_department_approval[] = $value;
			}
        $courses_for_approval['from_instructor'] = $courses_for_department_approval;
        $courses_for_approval['from_registrars'] = $rejected_courses_for_department_approval;

        return $courses_for_approval;
	}
	*/

	function getDepartmentNonApprovedCoursesList($department_college_id, $department = 1)
	{
		...
		if ($department == 1) {
			$resultsRegistration = $this->query(
				"
               SELECT exam_grades.id, exam_grades.course_registration_id,
               exam_grades.registrar_approval,exam_grades.department_approval,exam_grades.course_add_id,exam_grades.makeup_exam_id
               FROM exam_grades exam_grades
                INNER JOIN (
                    ...
                    GROUP BY course_registration_id) AS t2 ON ( exam_grades.id = t2.id AND
                    exam_grades.course_registration_id = t2.course_registration_id AND
                    exam_grades.created = t2.latest )

                WHERE exam_grades.registrar_approval=-1 OR exam_grades.department_approval IS NULL  AND
                exam_grades.course_registration_id
                    IN (
                        SELECT id
                        FROM course_registrations
                        WHERE published_course_id
                            IN (
                                SELECT id
                                FROM published_courses
                                WHERE department_id=" . $department_college_id . "
                              )
                    )
			...
			$resultsAdds = $this->query(
				"
                      SELECT exam_grades.id, exam_grades.course_registration_id,
                       exam_grades.registrar_approval,exam_grades.department_approval,
                       exam_grades.course_add_id,exam_grades.makeup_exam_id
               FROM exam_grades exam_grades
                INNER JOIN (
                    SELECT id, course_registration_id,course_add_id,MAX( created ) AS latest,
                    department_approval,registrar_approval
                    FROM exam_grades
                    WHERE registrar_approval=-1 OR department_approval IS NULL
                    GROUP BY course_add_id) AS t2 ON ( exam_grades.id = t2.id AND
                    exam_grades.course_add_id = t2.course_add_id AND
                    exam_grades.created = t2.latest )

                WHERE exam_grades.registrar_approval=-1 OR exam_grades.department_approval IS NULL  AND
                        exam_grades.course_add_id
                            IN (
                                SELECT id
                                FROM course_adds
                                WHERE published_course_id
         ...
         exam_grades.registrar_approval,exam_grades.department_approval,exam_grades.course_add_id,exam_grades.makeup_exam_id
               FROM exam_grades exam_grades
                INNER JOIN (
                    ...
                    FROM exam_grades
                    WHERE registrar_approval=-1 OR department_approval IS NULL
                    GROUP BY makeup_exam_id) AS t2 ON ( exam_grades.id = t2.id AND
                    exam_grades.makeup_exam_id = t2.makeup_exam_id AND
                    exam_grades.created = t2.latest )
        WHERE exam_grades.registrar_approval=-1 OR exam_grades.department_approval IS NULL  AND
        exam_grades.makeup_exam_id
            IN (
                SELECT id
                FROM makeup_exams
                WHERE published_course_id
		...
		if (!empty($registrationAddMakeupIDs['register'])) {
			$publication_ids_register = $this->CourseRegistration->find(
				'list',
				array(
		...
		if (!empty($registrationAddMakeupIDs['registerRejected'])) {
			$publication_ids_register_rejected = $this->CourseRegistration->find(
				'list',
				array(
					'fields' => array('CourseRegistration.published_course_id'),
		...
		if (!empty($registrationAddMakeupIDs['add'])) {
			$publication_ids_add = $this->CourseAdd->find(
				'list',
				array(
		...
		if (!empty($registrationAddMakeupIDs['addRejected'])) {
			$publication_ids_add_rejected = $this->CourseAdd->find(
				'list',
				array(
					'fields' => array('CourseAdd.published_course_id'),
		...
		if (!empty($registrationAddMakeupIDs['makeup'])) {
			$publication_ids_makeup = $this->MakeupExam->find(
				'list',
				array(
		...
		if (isset($publication_ids) && !empty($distinct_publication_ids)) {
			if ($department == 1) {
				$published_courses =
					$this->CourseRegistration->PublishedCourse->find(
						'all',
						array(
							'conditions' => array(
								'PublishedCourse.id' => $distinct_publication_ids,
								'PublishedCourse.department_id' => $department_college_id
							),
							'contain' => array(
	...
	/*
	*It returns list of courses from each department and freshman program that the registrar
	*is supposed to confirm the grade submission
	*/
	/*
	function getRegistrarNonApprovedCoursesList($department_ids = null, $college_ids = null) {
       //check to which department is assigned.
       $registrationAddMakeupIDs=array();
       ...
       if(!empty($department_ids)){
         ...
         $resultsRegistration = $this->query(
       "
       SELECT exam_grades.id, exam_grades.course_registration_id,  exam_grades.course_add_id,exam_grades.makeup_exam_id
       FROM exam_grades exam_grades
        INNER JOIN (
            SELECT id, course_registration_id, MAX( created ) AS latest, department_approval
            FROM exam_grades
            WHERE department_approval=1 AND exam_grades.registrar_approval IS NULL
            GROUP BY course_registration_id) AS t2 ON ( exam_grades.id = t2.id AND
            exam_grades.course_registration_id = t2.course_registration_id AND exam_grades.created = t2.latest )

        WHERE exam_grades.department_approval=1 AND exam_grades.registrar_approval IS NULL  AND
        exam_grades.course_registration_id
            IN (
                SELECT id
                FROM course_registrations
       ...
       FROM exam_grades exam_grades
        INNER JOIN (
            SELECT id,course_add_id, MAX( created ) AS latest,department_approval
            FROM exam_grades
            WHERE department_approval=1 AND exam_grades.registrar_approval IS NULL
            GROUP BY course_add_id) AS t2 ON ( exam_grades.id = t2.id AND
            exam_grades.course_add_id= t2.course_add_id AND exam_grades.created = t2.latest )
       ...
       FROM exam_grades exam_grades
        INNER JOIN (
            SELECT id, makeup_exam_id, MAX( created ) AS latest,department_approval
            FROM exam_grades
            WHERE department_approval=1 AND exam_grades.registrar_approval IS NULL
            GROUP BY makeup_exam_id) AS t2 ON ( exam_grades.id = t2.id AND
            exam_grades.makeup_exam_id= t2.makeup_exam_id AND exam_grades.created = t2.latest )
     ...
     //   $results = $resultsMakeup + $resultsAdds + $resultsRegistration;
            ...
            IN (
                ...
                WHERE published_course_id
                    IN (
                        SELECT id
                        FROM published_courses
                       WHERE college_id IN ($college_ids) and department_id is null
                      )
            )
      ...
      //  $results = $resultsMakeup + $resultsAdds + $resultsRegistration;
       ...
       if (!empty($results)) {
        ...
        $publication_ids=array();
        if (!empty($registrationAddMakeupIDs['register'])) {
             $publication_ids_register=$this->CourseRegistration->find('list',
		     array(
		       'fields'=>array('CourseRegistration.published_course_id'),
		       'conditions'=>array('CourseRegistration.id'=>$registrationAddMakeupIDs['register'])
		     )
        ...
        if (!empty($registrationAddMakeupIDs['add'])) {
           $publication_ids_add=$this->CourseAdd->find('list',
		     array(
		       'fields'=>array('CourseAdd.published_course_id'),
		       'conditions'=>array('CourseAdd.id'=>$registrationAddMakeupIDs['add'])
		     )
        ...
        if (!empty($registrationAddMakeupIDs['makeup'])) {
             $publication_ids_makeup=$this->MakeupExam->find('list',
		     array(
		       'fields'=>array('MakeupExam.published_course_id'),
		       'conditions'=>array('MakeupExam.id'=>$registrationAddMakeupIDs['makeup'])
		     )
        ...
        if (isset($publication_ids) && !empty($distinct_publication_ids)) {
             $published_courses=
            $this->CourseRegistration->PublishedCourse->find('all',
            array(
            'conditions'=>array('PublishedCourse.id'=>$distinct_publication_ids),
            'contain'=>array(
            'Program'=>array('id','name'),'ProgramType'=>array('id','name'),'Section'=>array('id','name'),
            'Department'=>array('id','name'),'YearLevel'=>array('id','name'),
            'CourseInstructorAssignment'=>array('fields'=>array('id','published_course_id','staff_id'),
            'Staff'=>array('id','full_name','user_id'),
            'conditions'=>array('CourseInstructorAssignment.isprimary'=>1)),
            'Course'=>array('id','course_title','course_code','course_detail_hours','credit'))
            ));
	...
	*/
	function getRegistrarNonApprovedCoursesList($department_ids = null, $college_ids = null, $program_id = null, $program_type_ids = null)
	{
		...
		WHERE exam_grades.registrar_approval IS NULL
		AND exam_grades.department_approval=1
		AND $queryPss
		AND exam_grades.course_registration_id NOT
		IN (
		...
		INNER JOIN published_courses published_courses ON course_registrations.published_course_id = published_courses.id
		WHERE exam_grades.registrar_approval is null
		AND exam_grades.department_reply=1
		AND $queryPss
		AND exam_grades.course_registration_id NOT
		IN (
		...
		AND exam_grades.department_approval=1
		AND $queryPss
		AND exam_grades.course_add_id NOT
		IN (
		...
		AND exam_grades.department_approval=1
		AND $queryPss
		AND exam_grades.makeup_exam_id NOT
		IN (
		...
		SELECT NULL
		FROM exam_grades exam_grades2
		WHERE exam_grades2.registrar_approval =1
		AND exam_grades2.department_approval =1
		AND exam_grades2.course_registration_id =exam_grades.course_registration_id
		GROUP BY exam_grades2.course_registration_id
		ORDER BY exam_grades2.id DESC
		)
		");
		$resultsRegistrationR=$this->query("SELECT DISTINCT exam_grades. *
		FROM exam_grades
		...
		WHERE exam_grades2.registrar_approval =1
		AND exam_grades2.department_approval =1
		AND exam_grades2.course_add_id =exam_grades.course_add_id
		GROUP BY exam_grades2.course_add_id
		ORDER BY exam_grades2.id DESC
		...
		WHERE exam_grades2.registrar_approval =1
		AND exam_grades2.department_approval =1
		AND exam_grades2.makeup_exam_id =exam_grades.makeup_exam_id
		GROUP BY exam_grades2.makeup_exam_id
		ORDER BY exam_grades2.id DESC
	...
	*/
	function getRegistrarNonApprovedPublishedCourseList(
		...
		if (!empty($department_ids)) {
			...
			/*
        $results = $this->query(
       "SELECT id, course_registration_id, course_add_id,makeup_exam_id
        FROM  `exam_grades`
        WHERE
        `department_approval`=1 AND `registrar_approval` IS NULL  AND
            ...
            IN (
                ...
                WHERE published_course_id
                    IN (
                        SELECT id
                        FROM published_courses
                        WHERE department_id IN ($dept_ids) and semester='".$semester."'
                        and program_id in (".implode(', ',$program_id).")  and
                        program_type_id in (".implode(', ',$program_type_id).")
       ...
       FROM exam_grades exam_grades
        INNER JOIN (
            SELECT id, course_registration_id, MAX( created ) AS latest, department_approval,registrar_approval
            FROM exam_grades
            WHERE department_approval=1 AND registrar_approval IS NULL
            GROUP BY course_registration_id) AS t2 ON ( exam_grades.id = t2.id AND
            exam_grades.course_registration_id = t2.course_registration_id AND exam_grades.created = t2.latest )
       ...
       FROM exam_grades exam_grades
        INNER JOIN (
            SELECT id,course_add_id, MAX( created ) AS latest,department_approval,registrar_approval
            FROM exam_grades
            WHERE department_approval=1 AND registrar_approval IS NULL
            GROUP BY course_add_id) AS t2 ON ( exam_grades.id = t2.id AND
            exam_grades.course_add_id= t2.course_add_id AND exam_grades.created = t2.latest )
       ...
       FROM exam_grades exam_grades
        INNER JOIN (
          SELECT id, makeup_exam_id, MAX( created ) AS latest,department_approval,registrar_approval
          FROM exam_grades
          WHERE department_approval=1 AND registrar_approval IS NULL
          GROUP BY makeup_exam_id) AS t2 ON ( exam_grades.id = t2.id AND
            exam_grades.makeup_exam_id= t2.makeup_exam_id AND exam_grades.created = t2.latest )
            ...
            IN (
                ...
                WHERE published_course_id
                    IN (
                        SELECT id
                        FROM published_courses
                        WHERE college_id IN ($college_ids) and department_id is null  and semester='".$semester."'
                        and program_id in (".implode(', ',$program_id).")  and
                        program_type_id in (".implode(', ',$program_type_id).")
                        and academic_year='".$academic_year."'
                      )
	...
	*/
	/*
	function getRejectedOrNonApprovedPublishedCourseList($department_college_id, $department=1) {
       ...
       if($department==1){
            ...
            $resultsRegistration = $this->query(
               ...
               FROM exam_grades exam_grades
                INNER JOIN (
                    SELECT id, course_registration_id, MAX( created ) AS latest,
                    department_approval,registrar_approval
                    FROM exam_grades
                    WHERE registrar_approval=-1 OR department_approval IS NULL
                    GROUP BY course_registration_id) AS t2 ON ( exam_grades.id = t2.id AND
                    ...
                    IN (
                        ...
                        WHERE published_course_id
                            IN (
                                SELECT id
                                FROM published_courses
                                WHERE given_by_department_id=".$department_college_id."
                              )
            ...
            $resultsAdds = $this->query(
                       "
                      SELECT exam_grades.id, exam_grades.course_registration_id,  exam_grades.course_add_id,
                      exam_grades.makeup_exam_id
               FROM exam_grades exam_grades
                INNER JOIN (
                    SELECT id, course_registration_id,course_add_id, MAX( created ) AS latest, department_approval,registrar_approval
                    ...
                    IN (
                        ...
                        WHERE published_course_id
                            IN (
                                SELECT id
                                FROM published_courses
                                WHERE college_id=".$department_college_id." AND department_id is null
                              )
                    )
       ...
       if (!empty($results)) {
        ...
        if (!empty($registrationAddMakeupIDs['register'])) {
		  ...
		  // $publication_ids=$publication_ids_register;
		  $publication_ids=$publication_ids+$publication_ids_register;
	...
	*/
	...
	function getRejectedOrNonApprovedPublishedCourseList($department_college_id, $department = 1)
	{
		...
		if (!empty($department_college_id) && $department == 1) {
			...
			$queryPss .= " and published_courses.college_id=$department_college_id and department_id is null ";
		}
		...
		");
			...
			/*
        $resultsRegistrationNR=$this->query("SELECT DISTINCT exam_grades. *,MAX(exam_grades2.id)
		FROM exam_grades
		INNER JOIN course_registrations course_registrations ON exam_grades.course_registration_id = course_registrations.id
		INNER JOIN published_courses published_courses ON course_registrations.published_course_id = published_courses.id
		LEFT JOIN exam_grades AS exam_grades2 ON exam_grades2.course_registration_id = exam_grades.course_registration_id AND exam_grades2.registrar_approval=1 AND exam_grades2.department_approval=1 AND exam_grades2.course_registration_id IS NOT NULL

		WHERE exam_grades.registrar_approval IS NULL
		AND exam_grades.department_approval IS NULL
		AND $queryPss
		AND exam_grades2.course_registration_id IS NULL
	...
	function getAddSemester($student_id = null, $current_academic_year = null)
	{
		...
		if (empty($first_added) && empty($last_added)) {
			return array();
		} else {  //getLastestStudentSemesterAndAcademicYear
	...
	function getTotalCreditAndPointDeduction($student_id = null, $all_ay_s_list = array())
	{
		...
		foreach ($all_ay_s_list as $ays_key => $ay_and_s) {
			...
			if (
				$ay_and_s['academic_year'] == "2017/18" &&
				$ay_and_s['semester'] == "I"
			) {
			...
			foreach ($course_and_grades as $key => $registered_added_course) {
				...
				*/
				...
				if ((strcasecmp($registered_added_course['grade'], 'W') != 0 || strcasecmp($registered_added_course['grade'], 'I') != 0) && $registered_added_course['used_in_gpa']) {
					//Begin: credit hour and grade point deduction sum

					if (
						$registered_added_course['repeated_new']
						== true
					) {
						...
						/** The returned AY and semester list is till the current round of AY
						and Semester. It is used to consider repeated courses within the same pattern AY and Semester **/
	...
	*/
	...
	function getStudentAllCoursesAndFinalGrade($student_id = null, $current_academic_year = null, $include_exempted = 0)
	{
		...
		if ($student_level['year'] == 1) {
			...
			$student_year_level = $student_year_level . 'th';
		}
		...
		foreach ($course_registered as $key => $value) {
			if (!$this->CourseRegistration->isCourseDroped($value['CourseRegistration']['id'])) {
				...
				//To determine if a student registered more than once for the same course
				/*
				$course_be_substitueds = $this->CourseRegistration->PublishedCourse->Course->CourseForSubstitued->find('all',
					array(
						'conditions' =>
						array(
							'CourseForSubstitued.course_for_substitued_id' => $value['PublishedCourse']['Course']['id']
						),
						'recursive' => -1
					)
				);
				foreach($course_be_substitueds as $key2 => $value2) {
					$matching_courses[] = $value2['CourseForSubstitued']['course_be_substitued_id'];
				}
				*/
				...
				/*
				$registration_freq = $this->CourseRegistration->find('all',
					array(
						'conditions' =>
						array(
							'CourseRegistration.student_id' => $student_id,
						...
						array(
							...
							array(
								'conditions' =>
								array(
									'PublishedCourse.course_id' => $matching_courses
								)
							)
						),
						'order' =>
				...
				$add_freq = $this->CourseAdd->find('all',
					array(
						'conditions' =>
						array(
							'CourseAdd.student_id' => $student_id,
				...
				*/
				...
				//If the student has multiple registration and/or add
					...
					/*
					if($register_and_add_freq[0]['id'] == $value['CourseRegistration']['id'] &&
						strcasecmp($register_and_add_freq[0]['type'] , 'register') == 0) {
						$courses_and_grades[$index]['repeated_old'] = true;
						$courses_and_grades[$index]['repeated_new'] = false;
					}
					else {

						$courses_and_grades[$index]['repeated_old'] = false;
						$courses_and_grades[$index]['repeated_new'] = true;
		...
		foreach ($course_added as $key => $value) {
				...
				array(
					...
					array(
						...
						array(
							'conditions' =>
							array(
								'PublishedCourse.course_id' => $value['PublishedCourse']['Course']['id']
							)
						)
					),
					'order' =>
			...
			*/
			...
			//If the student has multiple registration and/or add
				...
				$courses_and_grades[$index]['repeated_new'] = $rep['repeated_new'];
				/*
				if($register_and_add_freq[0]['id'] == $value['CourseAdd']['id'] &&
					strcasecmp($register_and_add_freq[0]['type'] , 'add') == 0) {
					$courses_and_grades[$index]['repeated_old'] = true;
					$courses_and_grades[$index]['repeated_new'] = false;
				}
				else {
	...
	///////////////////////////////////////////////////////////////////////////
	/*
	function getPublishedCourseIfExist($department_id, $academic_year, $semester,$program_id,$program_type_id,$studentDetail,
$admissionAcademicYear) {
	    //find the section and yearlevel of the student
            $student_section_id=null;
	    $student_section= ClassRegistry::init('StudentsSection')->find('first',array('conditions'=>array('StudentsSection.student_id'=>$studentDetail['Student']['id']),'order' => array('StudentsSection.created DESC')));
		...
		// Does the student has curriculum ?
	       if(!empty($studentDetail['Student']['curriculum_id']))
               {
		   // find student who have section, consider admission year of the students, given student number find out possible section fit
		   $students=$this->CourseRegistration->Student->find('list',array('conditions'=>array('Student.curriculum_id'=>$studentDetail['Student']['curriculum_id'],'Student.id !='=>$studentDetail['Student']['id'],'Student.admissionyear'=>$studentDetail['Student']['admissionyear']),'fields'=>array('Student.id','Student.curriculum_id')));
		  $student_section=ClassRegistry::init('StudentsSection')->find('first',array('conditions'=>array('StudentsSection.student_id'=>array_keys($students))));
		 // create section
	           if(!empty($student_section)) {
			unset($student_section['StudentsSection']['id']);
                        unset($student_section['StudentsSection']['student_id']);
		       $student_section['StudentsSection']['student_id'] = $studentDetail['Student']['id'];

			ClassRegistry::init('StudentsSection')->create();                	ClassRegistry::init('StudentsSection')->save($student_section);

			  $student_sections=ClassRegistry::init('StudentsSection')->find('first',array('conditions'=>array('StudentsSection.id'=>ClassRegistry::init('StudentsSection')->id)));
		...
		// the student has already section, check for upgrade ?
	       ...
	       $status_level=$this->CourseRegistration->Student->StudentExamStatus->studentYearAndSemesterLevelOfStatusDisplay($studentDetail['Student']['id'],$academic_year, $semester);
		...
		$options['conditions']['PublishedCourse.section_id']=$student_section_id;
		...
		$options['contain'] = array('Course'=>array('fields'=>array('id','course_title','course_code','credit'),'GradeType'=>array('Grade'=>array('fields'=>array('id','grade')))));

		$options['fields']=array(
'DISTINCT PublishedCourse.course_id','PublishedCourse.department_id',
'PublishedCourse.semester','PublishedCourse.program_id',
'PublishedCourse.program_type_id','PublishedCourse.id',
'PublishedCourse.section_id','PublishedCourse.year_level_id');

		$student_course_registrations['courses'] = $this->CourseRegistration->PublishedCourse->find('all', $options);
	foreach($student_course_registrations['courses']
as $k=>&$value) {
...
$value['PublishedCourse']['id']);
		    ...
		    if(isset($value['PublishedCourse']['grade_scale_id']) && $value['PublishedCourse']['grade_scale_id']!=0) {
		     $value['Course']['grade_scale_id']=$value['PublishedCourse']['grade_scale_id'];
		     } else {
$value['Course']['grade_scale_id']=ClassRegistry::init('GradeScale')->getGradeScaleId($value['Course']['grade_type_id'],$studentDetail);
		     }
		}
		//debug($student_course_registrations);
	...
	function getPublishedCourseIfExist(
		...
		if (isset($student_ay_and_s_lists) && !empty($student_ay_and_s_lists)) {
		...
		if (
			isset($student_ay_and_s_list)
			&& !empty($student_ay_and_s_list)
		) {
			foreach ($student_ay_and_s_list
				as $ay) {
				if (($ay['semester'] == $semester &&
		...
		if (!empty($student_ay_and_s)) {
		  ...
		  */
			$sectionfil = $this->CourseRegistration->find(
				'first',
			...
			if (isset($sectionfil) && !empty($sectionfil)) {
				$student_section = $this->CourseRegistration->Section->StudentsSection->find('first', array('conditions' => array(
					'StudentsSection.student_id' => $studentDetail['Student']['id'],
					'StudentsSection.section_id' => $sectionfil['Section']['id']
				)));
			} else {
				$student_section = $this->CourseRegistration->Section->StudentsSection->find('first', array('conditions' => array('StudentsSection.student_id' => $studentDetail['Student']['id']), 'order' => array('StudentsSection.section_id DESC')));
			}
			if (!empty($student_section)) {
				debug($academic_year);
				$section = $this->CourseRegistration->Section->find(
					'first',
					array('conditions' => array(
				...
				if ($section['Section']['academicyear'] != $academic_year) {
					$studentSectionnns = $this->CourseRegistration->Section->StudentsSection->find('first', array('conditions' => array(
						'StudentsSection.student_id' => $studentDetail['Student']['id'],
						'StudentsSection.section_id in (select id from sections where
...
*/
				...
				$section = $this->CourseRegistration->Section->find(
					...
					array('conditions' => array(
						...
						'Section.program_id' => $studentDetail['Student']['program_id'],
						'Section.program_type_id' => $studentDetail['Student']['program_type_id'],
						'Section.year_level_id' => $yearLevelId['YearLevel']['id'], 'Section.academicyear' => $student_ay_and_s['academic_year']
					), 'recursive' => -1)
				);
			}
			...
			if (!empty($academic_year) && !empty($semester)) {
				$section_id = $this->CourseRegistration->Section->StudentsSection->find(
					'first',
					array('conditions' => array(
						'StudentsSection.student_id' => $studentDetail['Student']['id'],
			...
			$options['contain'] = array('Course' => array('Prerequisite', 'fields' => array('id', 'course_title', 'course_code', 'credit'), 'GradeType' => array('Grade' => array('fields' => array('id', 'grade')))));
			...
			$options['fields'] = array(
				...
				'PublishedCourse.semester', 'PublishedCourse.program_id',
				'PublishedCourse.program_type_id', 'PublishedCourse.id',
				'PublishedCourse.section_id', 'PublishedCourse.grade_scale_id', 'PublishedCourse.year_level_id'
			);
			...
			foreach ($student_course_registrations['courses']
				...
				$registeredOrAddCourse = $this->CourseRegistration->find(
					'first',
					array(
						'conditions' => array(
							'CourseRegistration.student_id' => $studentDetail['Student']['id'],
				...
				if (isset($registeredOrAddCourse) && !empty($registeredOrAddCourse)) {
					...
					$registeredOrAddCourse = $this->CourseAdd->find(
						'first',
						array(
							'conditions' => array(
	...
	function isGradeSubmittedForPublishedCourseGivenStudentId($studentId, $published_course_ids)
	{
		...
		if (!empty($grade_submitted_registred_courses)) {
			$published_courses_student_registred_score_grade =
				$this->find('count', array('conditions' => array('ExamGrade.course_registration_id' => $grade_submitted_registred_courses)));
			if ($published_courses_student_registred_score_grade > 0) {
				return $published_courses_student_registred_score_grade;
			}
		}
	...
	function getCourseRepetation($course_reg_add_id, $student_id, $registered = 1)
	{
		...
		foreach ($course_registered as $key => $value) {
			if (!$this->CourseRegistration->isCourseDroped($value['CourseRegistration']['id'])) {
				...
				$matching_courses = ClassRegistry::init('EquivalentCourse')->validEquivalentCourse($value['PublishedCourse']['Course']['id'], $student_detail['Student']['curriculum_id'], 0);
				$matching_courses[] = $value['PublishedCourse']['Course']['id'];
				$register_and_add_freq = $this->getCourseFrequenceRegAdds($matching_courses, $student_detail['Student']['id']);
				debug($register_and_add_freq);
	...
	function gradeSubmittedForAYSem($student_id, $AY, $Sem)
	{
		...
		if (!empty($grade_submitted_registred_courses)) {
			...
			foreach ($grade_submitted_registred_courses as $k => $v) {
				...
				if (isset($grade_detail['grade'])) {
					if ($grade_detail['grade'] != 'W')
						return 1;
				}
			}
		}

		$grade_submitted_added_courses = $this->CourseAdd->find(
			'list',
			array(
				'conditions' => array(
					'CourseAdd.student_id' => $student_id,
		...
		foreach ($registration_and_course_add_merged as $k => $value) {
          ...
          $registration_freq = $this->CourseRegistration->find('all',
				array(
					...
					array(
						'CourseRegistration.student_id' => $student_id,
'CourseRegistration.published_course_id in (select id from published_courses where course_id in ('.join(',',$matching_courses).') )',
			...
			$add_freq = $this->CourseAdd->find('all',
				array(
					...
					array(
						...
						'CourseAdd.department_approval=1',
						'CourseAdd.registrar_confirmation=1',
'CourseAdd.published_course_id in (select id from published_courses where course_id in ('.join(',',$matching_courses).') )'
...
$register_and_add_freq[$m_index]['course_id'] = $value2['PublishedCourse']['course_id'];
					$register_and_add_freq[$m_index]['academic_year'] = $value2['CourseRegistration']['academic_year'];
				}
			}
			foreach($add_freq as $key2 => $value2) {
				if(!empty($value2['PublishedCourse']['id'])) {
						...
						if(isset($raaca) && !empty($raaca)) {
								  $register_and_add_course_registration[$value2['PublishedCourse']['course_id']]['add'][$value2['CourseAdd']['id']]=$raaca['grade'];
									...
									$register_and_add_freq[$m_index]['id'] = $value2['CourseAdd']['id'];
									$register_and_add_freq[$m_index]['type'] = 'add';
									$register_and_add_freq[$m_index]['academic_year'] = $value2['CourseAdd']['academic_year'];
				$register_and_add_freq[$m_index]['course_id'] = $value2['PublishedCourse']['course_id'];
				}
			}
			...
			for($i = 0 ; $i < count($register_and_add_freq); $i++) {
				for($j = $i+1; $j < count($register_and_add_freq); $j++) {
					if($register_and_add_freq[$i]['academic_year'] > $register_and_add_freq[$j]['academic_year']) {
						$tmp = $register_and_add_freq[$i];
						$register_and_add_freq[$i] = $register_and_add_freq[$j];
						$register_and_add_freq[$j] = $tmp;
					}
			...
			*/
			...
			//If the student has multiple registration and/or add
				...
				//6413
				/*
				if(isset($value['CourseAdd']['id']) && $register_and_add_freq[1]['id'] == $value['CourseAdd']['id'] &&
					strcasecmp($register_and_add_freq[1]['type'] , 'add') == 0) {
					$courses_and_grades[$acadamic_year]['repeated_old'] = true;
                    $courses_and_grades[$acadamic_year]['repeated_new'] = false;

				} else if (isset($value['CourseRegistration']['id']) && $register_and_add_freq[0]['id'] == $value['CourseRegistration']['id'] &&
					strcasecmp($register_and_add_freq[0]['type'] , 'register') == 0) {
                      $courses_and_grades[$acadamic_year]['repeated_old'] = true;
                    $courses_and_grades[$acadamic_year]['repeated_new'] = false;
debug($courses_and_grades);
					...
					$courses_and_grades[$acadamic_year]['repeated_old'] = false;
					$courses_and_grades[$acadamic_year]['repeated_new'] = true;

					//find the earlier grade of the course
					if(isset($register_and_add_course_registration
			[$register_and_add_freq[0]['course_id']]['add'])) {
					  foreach($register_and_add_course_registration
			[$register_and_add_freq[0]['course_id']]['add'] as
			$reg=>$valueg) {
		                 if(isset($value['CourseAdd']['id']) && !empty($value['CourseAdd']['id'])) {
							if($reg!=$value['CourseAdd']['id'] && $valueg=='W') {
							  $courses_and_grades[$acadamic_year]['repeated_old_grade']=$valueg;
					          $courses_and_grades[$acadamic_year]['repeated_new'] = false;
							}
					    }
					   }
				...
				// register
				if(isset($register_and_add_course_registration
		[$register_and_add_freq[0]['course_id']]['register'])) {
				  foreach($register_and_add_course_registration
		[$register_and_add_freq[0]['course_id']]['register'] as
		$reg=>$valueg) {

						  if(isset($value['CourseRegistration']['id']) &&
				$reg!=$value['CourseRegistration']['id'] && $valueg=='W') {
				   $courses_and_grades[$acadamic_year]['repeated_old_grade']=$valueg;
				   $courses_and_grades[$acadamic_year]['repeated_new'] = false;
				     }
				  }
		...
		foreach ($registrar_action_required_list as $key => $grade_detail) {
			if (empty($grade_detail['ExamGrade']['registrar_approval']) && $grade_detail['ExamGrade']['department_approval'] == 1)
				...
				if (isset($grade_detail['CourseRegistration']) && !empty($grade_detail['CourseRegistration']) && $grade_detail['CourseRegistration']['id'] != "") {
					if ((!empty($department_ids) && isset($grade_detail['CourseRegistration']['PublishedCourse']['Department']['id']) && in_array($grade_detail['CourseRegistration']['PublishedCourse']['Department']['id'], $department_ids)) ||
						(!empty($college_ids) && !empty($grade_detail['CourseRegistration']['PublishedCourse']['college_id']) && in_array($grade_detail['CourseRegistration']['PublishedCourse']['college_id'], $college_ids))
					) {
				...
				//Grade change for student course add
					...
					if ((!empty($department_ids) && isset($grade_detail['CourseAdd']['PublishedCourse']['Department']['id']) && in_array($grade_detail['CourseAdd']['PublishedCourse']['Department']['id'], $department_ids)) ||
						(!empty($college_ids) && !empty($grade_detail['CourseAdd']['PublishedCourse']['college_id']) && in_array($grade_detail['CourseAdd']['PublishedCourse']['college_id'], $college_ids))
					) {
	...
	{
		$mostRecentRegistration = $this->CourseRegistration->find(
			'first',
			array(
				'conditions' =>
				array(
				...
				'order' => array(
					'CourseRegistration.created DESC', 'CourseRegistration.academic_year DESC',
	...
	{
		$display = '';
		$display .= $studentDetail['Student']['first_name'] . ' ' .
			$studentDetail['Student']['last_name'] . '(' . $studentDetail['Student']['studentnumber'] . ') has scored :-';
	...
	public function getGradeDetailsForEmailNotification($exam_grade_id)
	{
		$details = $this->find(
			...
			array(
				'conditions' =>
				array('ExamGrade.id' => $exam_grade_id),
				'contain' => array(
					'CourseAdd' => array(
						'Student',
	...
	public function getLetterGradeStatistics($publishedCourseID)
	{
		...
		$publishedCourses_reg = $this->CourseRegistration->find('all', array(
			...
			'contain' => array('Student', 'ExamGrade', 'PublishedCourse' => array('Section', 'YearLevel', 'Program', 'ProgramType', 'Course', 'Department'))
		));
	...
	function repeatationLabeling($register_and_add_freq, $type = 'add', $add_reg_value, $student_detail, $course_id)
	{
		...
		foreach ($register_and_add_freq as $k) {
			/*
		 		if($k['id'] == $add_reg_value &&
					strcasecmp($k['type'] , $type) == 0 && $this->CourseRegistration->PublishedCourse->Course->isCourseTakenHaveRecentGrade($student_detail['Student']['id'],$course_id) && !$isLastRegistration) {
			...
			if (
				$k['id'] == $add_reg_value &&
				strcasecmp($k['type'], $type) == 0 && !$isLastRegistration
			) {
				$rep['repeated_old'] = true;
				$rep['repeated_new'] = false;
				//return $rep;
	...
	function isTheLastRegOrAdd($matching_courses, $student_id, $course_id, $add_reg_value)
	{
		...
		/*$registration = $this->CourseRegistration->find('first', array('conditions' => array(
			'CourseRegistration.student_id'=>$student_id,
			//'CourseRegistration.published_course_id in (select id from published_courses where course_id in ('.$courses_separated_by_coma.'))',
			'CourseRegistration.id not in (select course_registration_id from course_drops where registrar_confirmation=1 and department_approval=1)',),'contain' =>array('PublishedCourse'),'order'=>array(
			'CourseRegistration.created DESC')));
		$add = $this->CourseAdd->find('first',array(
		'conditions' =>array(
		'CourseAdd.student_id' => $student_id,
		...
		'PublishedCourse'),'order' =>array('CourseAdd.created DESC')));
		debug($registration);
		debug($add);
		if(!empty($registration) && $registration['PublishedCourse']['course_id']==$course_id){
			return true;
		} else if (!empty($add) && $add['PublishedCourse']['course_id']==$course_id){
		   return true;
		}
		*/
		return false;
	...
	function getCourseFrequenceRegAdds($matching_courses, $student_id)
	{
		...
		$register_and_add_freq = array();
		if (
			isset($matching_courses)
			&& !empty($courses_separated_by_coma)
		) {
			$registration_freq = $this->CourseRegistration->find('all', array('conditions' => array(
				'CourseRegistration.student_id' => $student_id,
				'CourseRegistration.published_course_id in (select id from published_courses where course_id in (' . $courses_separated_by_coma . '))', 'CourseRegistration.id not in (select course_registration_id from course_drops where registrar_confirmation=1 and department_approval=1)',
			), 'contain' => array('PublishedCourse'), 'order' => array(
				'CourseRegistration.created ASC'
			)));
			$add_freq = $this->CourseAdd->find('all', array(
				'conditions' => array(
					'CourseAdd.student_id' => $student_id, 'CourseAdd.department_approval=1',
					'CourseAdd.registrar_confirmation=1',
					'CourseAdd.published_course_id in(select id from published_courses where course_id in (' . $courses_separated_by_coma . '))'
				), 'contain' => array(
					'PublishedCourse'
		...
		foreach ($registration_freq as $key2 => $value2) {
			if (
				!empty($value2['PublishedCourse']['id'])
				&& !$this->CourseRegistration->isCourseDroped($value2['CourseRegistration']['id'])
			) {
				$m_index = count($register_and_add_freq);
		...
		for (
			$i = 0;
			$i < count($register_and_add_freq);
			$i++
		) {
			for (
				$j = $i + 1;
				$j < count($register_and_add_freq);
				$j++
			) {
				if (
					$register_and_add_freq[$i]['created'] >
					$register_and_add_freq[$j]['created']
				) {
					$tmp = $register_and_add_freq[$i];
					$register_and_add_freq[$i] = $register_and_add_freq[$j];
					$register_and_add_freq[$j] = $tmp;
	...
	function getApprovedThesisGrade($student_id)
	{
		...
		if (
			!empty($curr) && isset($curr['Student']['curriculum_id']) &&
			!empty($curr['Student']['curriculum_id'])
		) {
			$thesisCourse = $this->CourseRegistration->PublishedCourse->Course->find('list', array(
				'conditions' => array(
					'Course.thesis' => 1,
					'Course.curriculum_id' => $curr['Student']['curriculum_id']
				),
				'fields' => array('Course.id', 'Course.id'),

				//'contain'=>array('CourseRegistration','Course')
			));
			...
			if (!empty($thesisCourse)) {
				$registration = $this->CourseRegistration->find('first', array(
					...
					'order' => array('CourseRegistration.semester DESC'),
		...
		/*$registration = $this->CourseRegistration->find('first', array('conditions' => array(
			'CourseRegistration.student_id' => $student_id,
			'CourseRegistration.published_course_id in (select id from published_courses where thesis=1)','CourseRegistration.id not in (select course_registration_id from course_drops where registrar_confirmation=1 and department_approval=1)',),'contain' =>array('PublishedCourse'),'order'=>array(
			'CourseRegistration.created DESC')));
			*/
	...
	public function getListOfFXGradeChangeForStudentChoice($student_id, $academic_year = null, $semester = null, $department_id = null)
	{
		...
		if (isset($student_id) && !empty($student_id)) {
			$fx_grade_list_course_reg = $this->CourseRegistration->find(
				'all',
				array(
					'conditions' => array(
						'CourseRegistration.student_id' => $student_id,
					...
					'contain' => array('PublishedCourse' => array('Course', 'CourseInstructorAssignment'), 'Student', 'ExamGrade')
				)
			);
			//debug($fx_grade_list_course_reg);
			$fx_grade_list_course_add = $this->CourseAdd->find(
				'all',
				array(
					'conditions' => array(
						'CourseAdd.student_id' => $student_id,
						'CourseAdd.id in (select course_add_id from exam_grades where grade="Fx" and registrar_approval=1 and registrar_approval=1)'
						//'CourseAdd.id in (select course_add_id from exam_grades where grade="FX")'
					),
					'contain' => array('PublishedCourse' => array('Course'), 'Student', 'ExamGrade')
				)
			);
		...
		if (
			isset($fx_grade_list_course_reg) &&
			!empty($fx_grade_list_course_reg)
		) {
			foreach ($fx_grade_list_course_reg as
				$fk => $fv) {
				$grade = $this->getApprovedGrade($fv['CourseRegistration']['id'], 1);
				$latestGrade['LatestGradeDetail'] =
					$this->CourseRegistration->getCourseRegistrationLatestGradeDetail($fv['CourseRegistration']['id']);
...
$fv['Student']['fxgradesubmitted']=true;
		...
		if (
			isset($fx_grade_list_course_add) &&
			!empty($fx_grade_list_course_add)
		) {
	...
	function getListOfNGGrade(
		...
		foreach ($publishedCourseLists as $pk => $pv) {
			...
			foreach ($pv['CourseRegistration']
				...
				$autoChange = $this->find(
					...
					array(
						'conditions' => array(
							...
							'ExamGrade.grade'=>$gradeConverted,
							
							//'ExamGrade.id not in (select exam_grade_id from exam_grade_changes )'
						),
						'contain'=>array('ExamGradeChange')
					)
				);
				...
				if (
					isset($autoChange)
					&& !empty($autoChange) && empty($autoChange['ExamGradeChange'])
				) {
				...
				$autoChange = $this->find(
					...
					array(
						'conditions' => array(
							...
							'ExamGrade.course_add_id'=>$crv['id'],
							'ExamGrade.grade'=>$gradeConverted,
