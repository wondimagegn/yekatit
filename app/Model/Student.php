...
class Student extends AppModel {
	...
	public $validate = array(
		...
		'ecardnumber' => array(
            ...
            'unique'=>array(
				 'rule'=>'isUnique',
				 'message'=>'The ecardnumber  is used by someone. Please provided unique  ecardnumber.',
				 'on' => 'update',
			)
		),
		'phone_mobile' => array(
		...
		'card_number' => array(
			...
			'length'=>array(
				 ...
				 'message'=>'Card number you provide is greater than 15 characters. Please provided card number in less than or equal to 15 characters.',
				 'allowEmpty' => true,
				 'required' => false,
			...
			'unique'=>array(
				 ...
				 'message'=>'You have already used this card number. Please provided
				 unique card number.'
			)
		),
	...
	function countId($collegeid=null,$year=null) {
		$count = $this->find("count", array(
			"conditions"=>array('Student.admissionyear LIKE'=>$year.'%','Student.college_id'=>$collegeid,
			"NOT"=>array('studentnumber'=>array('','null','NULL')))));
		return $count;				
	}
	...
	function admittedMoreThanOneProgram($department_id=null){
       ...
       {		      
			...
			/*
			  $allAdmittedStudents=$this->find('all', array('fields' => array(
			  	'DISTINCT Student.studentnumber','Student.first_name', 'Student.middle_name',
		    	'Student.last_name','Student.studentnumber',
		    	'Student.department_id','Student.program_type_id',
		    	'Student.program_id','Count(*)'),
			  'conditions'=>array('Student.college_id'=>$college_id[1]),
		    	 'group' => array('Student.first_name','Student.middle_name',
		    	 	'Student.last_name'),
			  ...
			  */
		    	  $allAdmittedStudents = $this->query(
               "
              SELECT  Student.studentnumber , Student.first_name ,  Student.middle_name ,  Student.last_name , Student.department_id,Student.program_type_id,Student.program_id , COUNT( * ) 
FROM  students as Student 
WHERE  Student.college_id = ".$college_id[1]."
GROUP BY  Student.first_name, Student.middle_name, Student.last_name 
HAVING COUNT( * ) >1
                  "
                );
...
HAVING COUNT( * ) >1
		    ...
		    foreach ($allAdmittedStudents as $key => $value) {
		    	  $sameAdmitted=$this->find('all', 
		    	  	array('conditions'=>array('Student.first_name'=>$value['Student']['first_name'],
        ...
        */
        ...
        function student_academic_detail($id=null,$academic_year=null) {
              $this->bindModel(array('hasMany'=>array('StudentsSection'=>array(
              'conditions'=>array('StudentsSection.archive'=>0)))));
		                
				
            $student_section= $this->find('first',array('conditions'=>array('Student.id'=>$id),'fields'=>array('Student.studentnumber',
	    'Student.full_name','Student.curriculum_id','Student.department_id','Student.college_id',
	    'Student.program_id','Student.program_type_id'),
	                  ...
	                  'contain'=>array(
	                        ...
	                        'StudentExamStatus'=>array(
	                            'conditions'=>array('StudentExamStatus.academic_year LIKE '=>$academic_year.'%'),
	                            'AcademicStatus'=>array('fields'=>array('name'))
	                        ),
	                        'StudentsSection',
				),
	                        'Section'=>array('fields'=>array('Section.id','Section.name','Section.year_level_id'),'YearLevel'=>array('fields'=>array('name')
	                        )),
	                       
	                        'Course'=>array('fields'=>array('Course.id','Course.course_code',
	                        'Course.lecture_hours','Course.tutorial_hours','Course.credit'),
	                        'PublishedCourse'=>array('fields'=>array('course_id','semester'),
	                             
	                                ),
                        	        
	                        ),
	                         'CourseRegistration'=>array(
	                             'order'=>'CourseRegistration.semester asc',
                        	     'ExamGrade'=>array(
	                             'fields'=>array('id','grade','course_registration_id')),
	                             'PublishedCourse'=>array('Course'=>array('id','course_title','course_code',
	                             'credit','lecture_hours','tutorial_hours')
                        	 )
		...
		/*
	     foreach ($student_section['StudentsSection'] as $sect_index=>&$sect_value) {
 ...
 $student_section_tmp= ClassRegistry::init('Section')->find('first',array('conditions'=>array('Section.id'=>$student_section['StudentsSection'][0]['section_id'])));
	...
	function get_students_curriculum_for_section($thisacademicyear=null,$college_id=null,$department_id=null,$role_id=null,
			$selected_program=null, $selected_program_type=null) {
			//$this->recursive = 1;
			$program_type_id=$selected_program_type;
			$find_the_equvilaent_program_type=unserialize($this->ProgramType->
			field('ProgramType.equivalent_to_id',array(
			...
			$value=$this->find('all',array(
					'conditions'=>array('AcceptedStudent.academicyear'=>$thisacademicyear,
					...
					'fields'=>array('Student.id','Student.curriculum_id'),
	...
	function get_students_for_countsectionlessstudent($collegeid=null,$role_id=null,$department_id=null,$year=null,
        $selected_program=null,$selected_program_type=null,$selected_curriculum=null) {
		//$this->recursive = 1;
			if($selected_curriculum == null) {
			$selected_curriculum ="%";
		}
		//$program_type_id=arrya();
		$program_type_id=$selected_program_type;
		$find_the_equvilaent_program_type=unserialize($this->ProgramType->field('ProgramType.equivalent_to_id',array(
			'ProgramType.id'=>$selected_program_type)));
		if (!empty($find_the_equvilaent_program_type)) {
			$selected_program_type_array=array();
			$selected_program_type_array[] =$selected_program_type;
        ...
        if(ROLE_COLLEGE != $role_id ){
            ...
            $conditions = array('AcceptedStudent.academicyear'=> $year,
            //'Student.college_id'=>$collegeid,
             'Student.department_id'=>$department_id,'Student.program_id'=>$selected_program,
             'Student.program_type_id'=>$program_type_id,'Student.curriculum_id LIKE'=>$selected_curriculum); 
        } else {
            $conditions = array('AcceptedStudent.academicyear'=> $year,'Student.college_id'=>$collegeid,
            'Student.program_id'=>$selected_program,'Student.program_type_id'=>$program_type_id,
			//'Student.curriculum_id LIKE'=>$selected_curriculum,
            "OR"=>array("Student.department_id is null","Student.department_id" => array(0,'')));
        }
        $students = $this->find('all',array('conditions'=>$conditions,
             ...
             if (!empty($data['HighSchoolEducationBackground'])) {
		       ...
		       if (!empty($data['HigherEducationBackground'])) {
		            	    ...
		            	    foreach ($data['HigherEducationBackground'] as $hk=>&$hv) {
				                if (empty($bv['name']) && empty($hv['field_of_study']) &&
					                empty($hv['cgpa_at_graduation']) && 
					                empty($hv['cgpa_at_graduation'])) {
			  ...
			  if (!empty($data['EslceResult'])) {
 				  ...
 				  if(isset($ebv['exam_year'][0]) && 
isset($ebv['exam_year'][1])){
...
$ebv['exam_year']=$ebv['exam_year']['year'].'-'.$ebv['exam_year']['month'].'-'.'01';
		     ...
		     if (!empty($data['EheeceResult'])) {
			    ...
			    debug($data['EheeceResult']);
				    ...
				    if (empty($hbv['subject']) && empty($hbv['mark'])) {
						    unset($data['EheeceResult'][$hbk]);
				    } else {
					    $extra_eheeceResult_field=true;
				    }
				 if(isset($hbv['exam_year'][0]) && 
isset($hbv['exam_year'][1])){
...
$hbv['exam_year']=$hbv['exam_year']['year'].'-'.$hbv['exam_year']['month'].'-'.'01';
	...
	*/
	function calculateStudentLoad ($student_id=null,$semester=null,$academic_year=null) {
		   $credit_sum_added=0;
		  $credit_sum_registred=0;
		  $courseRegistration=ClassRegistry::init('CourseRegistration')->find('all',
		  	...
		  	'contain'=>array('PublishedCourse'=>array('Course'),'CourseDrop')));
		  ...
		  if (!empty($courseRegistration)) {
		  	...
		  	foreach ($courseRegistration as $key => $value) {
		  		...
		  		if($value['PublishedCourse']['drop'] == 0 
		  			&& !$this->CourseRegistration->isCourseDroped(
		  				$value['CourseRegistration']['id'])) {
				    if (strcasecmp($value['PublishedCourse']['semester'],$semester)==0 && 
				    	strcasecmp($value['PublishedCourse']['academic_year'],$academic_year)==0) {
				     $credit_sum_registred+=
				 $value['PublishedCourse']['Course']['credit'];
				     }
				}
		  	}
		  }
		  if (!empty($courseAdd)) {
	       ...
	       /*
	       $registered_and_added_courses = $this->find('first', 
				array(
			...
			$credit_sum_registred=0;
			foreach($registered_and_added_courses['CourseRegistration'] as $key => 
			$course_registration) {
				if($course_registration['PublishedCourse']['drop'] == 0 &&
					!$this->CourseRegistration->isCourseDroped($course_registration['id'])
				) {
				        if (strcasecmp($course_registration['PublishedCourse']['semester'],$semester)===0 && strcasecmp($course_registration['PublishedCourse']['academic_year'],$academic_year)===0) {
				              $credit_sum_registred+=$course_registration['PublishedCourse']['Course']['credit'];
				        }
			...
			foreach($registered_and_added_courses['CourseAdd'] as $key => $course_registration) {
				if($course_registration['PublishedCourse']['drop'] == 0) {
					if (strcasecmp($course_registration['PublishedCourse']['semester'],$semester)===0 && strcasecmp($course_registration['PublishedCourse']['academic_year'],$academic_year)===0) {
				              $credit_sum_added+=$course_registration['PublishedCourse']['Course']['credit'];
				        }
				}
			}
			*/
	...
	function maxCreditExcludingI($student_id=null,$semester=null,$academic_year=null) {
	     ...
	     if(($credit_sum_added+$credit_sum_registred)
	     >$creditSumofI){
	...
	*/
	function get_student_section ($student_id=null,$academic_year=null,$semester=null) {
	   $this->bindModel(array('hasMany'=>array('StudentsSection'=>array('conditions'=>array('StudentsSection.archive'=>0)))));
        ...
        $student_section= $this->find('first',array('conditions'=>array('Student.id'=>$student_id),'fields'=>array('Student.studentnumber',
	    'Student.full_name','Student.curriculum_id','Student.department_id','Student.college_id',
	    'Student.program_id','Student.program_type_id'),
	    ...
	    $student_latest_status=$this->StudentExamStatus->find('first',array('conditions'=>array('StudentExamStatus.student_id'=>$student_id,
	    'StudentExamStatus.academic_year like '=>$academic_year.'%',
	    'StudentExamStatus.semester'=>$semester),
'order'=>array('StudentExamStatus.academic_year DESC','StudentExamStatus.semester DESC'),'contain'=>array('AcademicStatus'=>array('id','name'))));
          if (empty($student_latest_status)) {
            	$previous_academic_year_semester=$this->StudentExamStatus->
	           getPreviousSemester($academic_year,$semester);  
	          
              $student_latest_status=$this->StudentExamStatus->find('first',array('conditions'=>array('StudentExamStatus.student_id'=>$student_id,
	    'StudentExamStatus.academic_year like '=>$previous_academic_year_semester['academic_year'].'%'),'order'=>'StudentExamStatus.created DESC ','contain'=>array('AcademicStatus'=>array('id','name'))));
          
          }	    
	    } else {
	      $student_latest_status=$this->StudentExamStatus->find('first',array('conditions'=>array('StudentExamStatus.student_id'=>$student_id),'order'=>'StudentExamStatus.created DESC ','contain'=>array('AcademicStatus'=>array('id','name'))));
	    }
	    ...
	    if (!empty($student_section)) {
	            ...
	            if (!empty($student_section['Section'])) {
	                foreach ($student_section['Section'] as $kk=>$vv) {
	                         ...
	                         if (!empty($vv['StudentsSection']) &&
	                             $vv['StudentsSection']['archive'] == 0) {
	...
	function get_student_details($student_id = ""){
		$students = $this->find('first',array('conditions'=>array('Student.id'=>$student_id),'contain'=>array('Department','College','Program','ProgramType')));
		return $students;
	}
	...
	*/
	function getListOfDepartmentStudentsByYearLevel($college_id = null, $department_id = null, $program_id = null, $program_type_id = null, $year_level_id = null, $plus_one = 1, $gender = null, $student_ids = null, $accepted_student_ids = null,$limit=100) {
		...
		if(!empty($year_level_id)) {
			if(is_numeric($year_level_id)){
				...
				$year_level_detail = ClassRegistry::init('YearLevel')->find('first', 
					array(
						'conditions' =>
						array(
	...
	function getStudentRegisteredAddDropCurriculumResult(
$student_id = "",$current_academic_year=null) {
		$courses = array();
		...
		if($student_id != "" && $student_id != 0) {
			...
			$studentAttachedCurriculumID=$this->field('curriculum_id',
			...
			foreach($registered_and_added_courses['CourseRegistration'] as $key => $course_registration) {
				if($course_registration['PublishedCourse']['drop'] == 0 &&
				   ...
				   if ($this->CourseRegistration->isCourseDroped($course_registration['id'])) {
					...
					$courses['Course Dropped'][$course_registration['id'].'~register']['credit'] 
					=$course_registration['PublishedCourse']['Course']['credit'];
	...
	$basic=$this->find('first',
			array('conditions'=>array('Student.id'=>$student_id),'contain'=>array('Attachment'=>array('conditions'=>array('Attachment.model'=>'Student')),'Curriculum'=>array('Department','Course'=>array('CourseCategory','Prerequisite'=>array('PrerequisiteCourse'),'GradeType','YearLevel'=>array('id','name')),'CourseCategory'),'Program','ProgramType','User',
			'Country','Region','City','Department','College')));
...
debug($student_ay_s_list);
    ...
    function getiProfileNotBuildList ($max_not_build_time=14) {
            ...
            $list=$this->find('count',
            array('conditions'=>array('Student.id NOT IN (SELECT student_id FROM graduate_lists)','Student.id NOT IN (SELECT student_id FROM contacts)',
            'Student.id NOT IN (SELECT foreign_key FROM attachments where model="Student")',
            'Student.modified <= '=>$not_build_for
            ),'contain'=>array('GraduateList','Contact','Attachment','Program','ProgramType',
            'Department','College')));
    ...
    function getStudentPassword($student_ids) {
        ...
        foreach($student_ids as $key => $student_id) 
        {
	 	  	...
	 	  	$student_details = $this->find('first',
		   array('conditions'=>array('Student.id'=>$student_id['student_id']),'contain'=>array('User')));

	      $student_password['User']['role_id']=ROLE_STUDENT;
	      $student_password['User']['username']=$student_details['Student']['studentnumber'];
	    $student_password['User']['password']=$student_id['hashed_password'];

               
          $student_password_data['User']['role_id']=ROLE_STUDENT;
	      $student_password_data['User']['username']=$student_details['Student']['studentnumber'];
	      ...
	      $student_password_data['User']['email']=$student_details['Student']['email'];
              $student_password_data['User']['last_password_change_date']=$student_details['Student']['created'];
    
			  if (isset($student_details['User']['id']) && 
!empty($student_details['User']['id'])) {
				  if (!empty($student_details['Student']['user_id']) && $student_details['Student']['studentnumber']==
		$student_details['User']['username']) {
			   ...
			   $checkFromUserTable=$this->User->find('first',
	array('conditions'=>array('User.username'=>$student_details['Student']['studentnumber'])));
		  ...
		  if(isset($student_password_data['User']) && 
!empty($student_password_data['User'])) {
			...
			$this->saveField('user_id', 
$student_details['Student']['user_id']);
		 ...
		 unset($student_password['User']);
          ...
          $student_details_new= $this->find('first',
               array('conditions'=>array('Student.id'=>$student_id['student_id']),'contain'=>array('College'=>array('Campus'),'Department','Program','ProgramType','User')));
    ...
    function listStudentByAdmissionYear($department_id=null,$college_id=null,$year=null,$name=null) {
        $list=array();
    ...
    function getAttrationRate($acadamic_year,$semester,
...
$year_level_id=null,$region_id=null,$sex=null) {
	        ...
	        if (isset($department_id ) && !empty($department_id)) {
	            ...
	            if(count($college_id) > 1) {
	                ...
	                $matchingYearIds=ClassRegistry::init('YearLevel')->find('list',
array('conditions'=>array('YearLevel.name'=>$year_level_id,
'YearLevel.department_id'=>$department_id),'fields'=>array('YearLevel.id','YearLevel.id')));
		...
		/*
		 if (isset($year_level_id ) && !empty($year_level_id)) {
	           $options['conditions']['Student.yearLevel'] =$year_level_id;
	        }
*/
...
!empty($region_id)) {
		...
		if (isset($acadamic_year) && isset($semester)) {
		       ...
		       $options['conditions'][] = 'Student.id  IN (SELECT student_id FROM course_registrations where academic_year="'.$acadamic_year.'" and 
semester="'.$semester.'" and year_level_id in ('.join(',',$matchingYearIds).'))';
				} else {
		...
		'CourseRegistration'  => 
			array(
				'conditions' => array(
					...
					'CourseRegistration.year_level_id'=>$matchingYearIds
       ...
       */
		   ...
		   $options['fields'] = array('Student.full_name', 'Student.first_name', 
		    'Student.middle_name', 'Student.last_name', 'Student.studentnumber', 'Student.admissionyear', 
		    'Student.gender');
	   ...
	   foreach($students as $key => $student) 
       {
		...
		if (!isset($student['CourseRegistration'][0]['Section']) || 
!isset($section['Program']['name']) 
		|| !isset($section['ProgramType']['name'])) {
        ...
        if (isset($section['YearLevel']['name']) && !empty($section['YearLevel']['name']) && !empty($section['Department']['name']) && !empty($section['College']['name'])) {
                ...
                if (isset($attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']][$section['Department']['name']]
                [$section['YearLevel']['name']]['total'])) {
                         $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']][$section['Department']['name']][$section['YearLevel']['name']]
          ['total']+=1;
                } else {
                     $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']][$section['Department']['name']][$section['YearLevel']['name']]
          ...
          if (isset($section['YearLevel']['name']) && 
          !empty($section['YearLevel']['name']) && !empty($section['College']['name'])) {
               
               
                
                if (isset($attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']]['College'][$section['YearLevel']['name']]['total'])) {
                   $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']]['College'][$section['YearLevel']['name']]
          ['total']+=1;
                } else {
                   $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']]['College'][$section['YearLevel']['name']]
          ...
          if (isset($section['YearLevel']['name']) && 
          !empty($section['YearLevel']['name'])) {
               
              
               if (isset($attraction_rate['University'][$section['Program']['name']][
               $section['ProgramType']['name']][$section['YearLevel']['name']]['total'])) {
                
                 $attraction_rate['University'][$section['Program']['name']]
               [$section['ProgramType']['name']][$section['YearLevel']['name']]['total']
               +=1;
              ...
              // female initialized 
              if (!isset($attraction_rate['University'][$section['Program']['name']]
               [$section['ProgramType']['name']][$section['YearLevel']['name']]['female_total'])) {
                 $attraction_rate['University'][$section['Program']['name']]
               [$section['ProgramType']['name']][$section['YearLevel']['name']]['female_total']=0;
              }
              
               // male initialized 
              if (!isset($attraction_rate['University'][$section['Program']['name']]
               [$section['ProgramType']['name']][$section['YearLevel']['name']]['male_total'])) {
                 $attraction_rate['University'][$section['Program']['name']]
               [$section['ProgramType']['name']][$section['YearLevel']['name']]['male_total']=0;
              }
          }
          ...
          !empty($section['College']['name'])) {
                 
               
                if (isset($attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']]['Pre Engineering']['1st']['total'])) {
                     $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']]['Pre Engineering']['1st']['total']+=1;
                } else {
                     $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']]['Pre Engineering']['1st']['total']=1;
                }
                 ...
                 // female initialized 
              if (!isset($attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']]['Pre Engineering']['1st']['female_total'])) {
                 $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']]['Pre Engineering']['1st']['female_total']=0;
              }
              
                  // male initialized 
              if (!isset($attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']]['Pre Engineering']['1st']['male_total'])) {
                 $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                [$section['College']['name']]['Pre Engineering']['1st']['male_total']=0;
          ...
          !empty($section['College']['name'])) {
                if (isset($attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                     [$section['College']['name']]['College']['1st']['total'])) {
                  $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                     [$section['College']['name']]['College']['1st']['total']+=1;    
                } else {
                  $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                     [$section['College']['name']]['College']['1st']['total']=1;    
                }
              ...
              // female initialized 
              if (!isset( $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                     [$section['College']['name']]['College']['1st']['female_total'])) {
                  $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                     [$section['College']['name']]['College']['1st']['female_total']=0;
              }
              
               // male initialized 
              if (!isset( $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                     [$section['College']['name']]['College']['1st']['male_total'])) {
                  $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                     [$section['College']['name']]['College']['1st']['male_total']=0;
               }
          ...
          if (empty($section['YearLevel']['name'])) {
              ...
              if (isset(
                       $attraction_rate['University'][$section['Program']['name']]
                       [$section['ProgramType']['name']]['1st']['total'])) {
                       
                       $attraction_rate['University'][$section['Program']['name']]
                       [$section['ProgramType']['name']]['1st']['total']+=1;
              ...
              // female initialized 
              if (!isset($attraction_rate['University'][$section['Program']['name']]
                       [$section['ProgramType']['name']]['1st']['female_total'])) {
                  $attraction_rate['University'][$section['Program']['name']]
                       [$section['ProgramType']['name']]['1st']['female_total']=1;
              }
              
               // male initialized 
              if (!isset($attraction_rate['University'][$section['Program']['name']]
                       [$section['ProgramType']['name']]['1st']['male_total'])) {
                  $attraction_rate['University'][$section['Program']['name']]
                       [$section['ProgramType']['name']]['1st']['male_total']=0;
              }
          }
          //////////initialization end ///////////////////////////////
          debug($student);
		 if (isset($student['StudentExamStatus'][0]['academic_status_id']) && $student['StudentExamStatus'][0]['academic_status_id']==4) {
		         ...
		         if (empty($section['Section']['department_id'])) {
			              ...
			              if (strcmp($student['Student']['gender'],'male')==0) {
			                  ...
			                  $attraction_rate[$section['Program']['name']]
			                  [$section['ProgramType']['name']][$section['College']['name']]
			                  ['Pre Engineering']['1st']['male_total']+=1;
		              
			              } else if (strcmp($student['Student']['gender'],'female')==0) {
			               
			                  $attraction_rate[$section['Program']['name']]
			                  [$section['ProgramType']['name']]
			                  [$section['College']['name']]['Pre Engineering']['1st']['female_total']+=1;
	         ...
	         if (strcmp($student['Student']['gender'],'female')==0) {
                     ...
                     [$section['YearLevel']['name']]
                      ['female_total']+=1;
		               ...
		               if (isset($section['YearLevel']['name'])) {
		                   $attraction_rate['University']
                      [$section['Program']['name']][$section['ProgramType']['name']][$section['YearLevel']['name']]
                      ['female_total']+=1;
	                ...
	                //college level total male 
                      ...
                      ['male_total']+=1;
                             ...
                             if (!isset($section['YearLevel']['name'])) {
                               $attraction_rate[$section['Program']['name']][$section['ProgramType']['name']]
                               [$section['College']['name']]['College']['1st']['male_total']+=1;
                             } 
                       }
                      ...
                      ['male_total']+=1;
		                   ...
		                   if (!isset($section['YearLevel']['name'])) {
		                      $attraction_rate['University']
                      [$section['Program']['name']][$section['ProgramType']['name']]['1st']
                      ['male_total']+=1; 
		                   } 
		             }   
    ...
    //	 debug($attraction_rate);
       ...
       function getStudentListName($admission_year, $program_id,$program_type_id,$department_id,$year_level_id=null,
           $studentNumber=null,$studentName=null) {
		$options['conditions']['Student.program_id'] = $program_id;
   ...
   function regenerate_academic_status_by_batch($department_college_id,$admissionAcademicYear=null,$statusAcademicYear=null,$semester=null,$all_college_dept=0, $pre=0,$program_id=null, $program_type_id=null) {
	  ...
	  $statusConditions['add']=array();
	   ...
	   if(!empty($semester)){
		...
		$options['conditions']=array('Student.id in (select student_id from course_registrations where 
			semester="'.$semester.'" and academic_year="'.$statusAcademicYear.'") ');

		debug($options);
	   }
	 
           $options['fields'] = array('DISTINCT Student.id','Student.curriculum_id', 'Student.full_name', 'Student.first_name', 'Student.middle_name', 'Student.last_name', 'Student.studentnumber', 'Student.admissionyear', 'Student.gender');
	  ...
	  $options['recursive']=-1;
          ...
          foreach($studentLists as $kkk=>$vv) {
                ...
                $course_registered=ClassRegistry::init('CourseRegistration')->find('all',array('conditions'=>$statusConditions['reg'],'order' => array('CourseRegistration.academic_year ASC',
'CourseRegistration.semester ASC'),'contain'=>array('PublishedCourse')));
               $course_added=ClassRegistry::init('CourseAdd')->find('all',
            array('conditions'=> $statusConditions['add'],
            'order' => array('CourseAdd.academic_year ASC',
            'CourseAdd.semester ASC'),'contain'=>array('PublishedCourse')));
	   
	      $statusgenerated=false;
	      foreach($course_registered as $rgk=>$rgv)
              {
			$checkIfStatusIsGenerated=ClassRegistry::init('StudentExamStatus')->find('count',array('conditions'=>array('StudentExamStatus.student_id'=>$vv['Student']['id'],'StudentExamStatus.academic_year'=>$rgv['PublishedCourse']['academic_year'],'StudentExamStatus.semester'=>$rgv['PublishedCourse']['semester'])));
			if(!$checkIfStatusIsGenerated) 
			{	
                          $checkTheStatusForward=ClassRegistry::init('StudentExamStatus')->find('first',array('conditions'=>array(
...
'StudentExamStatus.academic_year >'=>$rgv['PublishedCourse']['academic_year'])));
 ...
 ClassRegistry::init('StudentExamStatus')->deleteAll(array('StudentExamStatus.student_id'=>$vv['Student']['id'],'StudentExamStatus.academic_year >'=>$rgv['PublishedCourse']['academic_year']), false);
				 ...
				 */
                 ...
                 //$statusgenerated=ClassRegistry::init('StudentExamStatus')->updateAcdamicStatusByPublishedCourse($value['PublishedCourse']['id']);
	      ...
	      foreach($course_added as $adk=>$adv)
              {
		    $checkIfStatusIsGenerated=ClassRegistry::init('Student')->StudentExamStatus->find('count',array('conditions'=>array('StudentExamStatus.student_id'=>$vv['Student']['id'],'StudentExamStatus.academic_year'=>$adv['PublishedCourse']['academic_year'],'StudentExamStatus.semester'=>$adv['PublishedCourse']['semester'])));
		    if(!$checkIfStatusIsGenerated) 
                    {	
				 //does the student have forward status ?
		     $checkTheStatusForward=ClassRegistry::init('StudentExamStatus')->find('first',array('conditions'=>array('StudentExamStatus.student_id'=>$vv['Student']['id'],
'StudentExamStatus.academic_year >'=>$adv['PublishedCourse']['academic_year'])));
 ...
 ClassRegistry::init('StudentExamStatus')->deleteAll(array('StudentExamStatus.student_id'=>$vv['Student']['id']), false);
	...
	function update_academic_status_by_batch($department_college_id,$admissionAcademicYear=null,$statusAcademicYear=null,$semester=null,
$all_college_dept=0, $pre=0,$program_id=null,$program_type_id=null) 
	{
	   $options=array();
	   $statusConditions['reg']=array();
	   $statusConditions['add']=array();
	  ...
	  foreach($studenLists as $kkk=>$vv) {
	    $statusConditions['reg']['CourseRegistration.student_id']=$vv['Student']['id'];
            ...
            array('conditions'=>$statusConditions['reg'],'order' => array('CourseRegistration.academic_year ASC',
'CourseRegistration.semester ASC'),'recursive'=>-1,'fields'=>array('CourseRegistration.id','CourseRegistration.id')));
...
'fields'=>array('CourseAdd.id','CourseAdd.id'))); 
	...
	function updateAcademicStatus($startingFrom=2) 
	{
	    ...
	    $recent_grade_change_date_from = 
		date('Y-m-d H:i:s', mktime (date('H'), 
		date('i'), 
		date('s'), 
		date('n'), 
		date('j') - $days_after_gradechange_approved, 
		date('Y')));

	    $recent_grade_change_date_to = 
		date('Y-m-d H:i:s', mktime (date('H'), 
		date('i'), 
		date('s'), 
		date('n'), 
	    ...
	    $mostRecentGradeChange=ClassRegistry::init('ExamGradeChange')->find('all', 
			array(
				'conditions' => 
				array(
				...
				'contain' => array('ExamGrade' => array('CourseRegistration', 'CourseAdd','MakeupExam'))
			)
	   );
	...
	function updateMissingAcademicStatus($studentId) 
	{
		...
		foreach($acRange as $k=>$acv) {
		   ...
		   foreach($semesters as $v) {
			debug($v);
			$registrationList=ClassRegistry::init('CourseRegistration')->find('all',
array('conditions'=>array(
...
'recursive'=>-1,
'order'=>array('CourseRegistration.academic_year DESC',
'CourseRegistration.semester DESC')
            ...
            debug($acv);
			  ...
			  {
 $checkTheStatusForward=ClassRegistry::init('Student')->StudentExamStatus->find('first',array('conditions'=>array(
...
'StudentExamStatus.academic_year >'=>$value['CourseRegistration']['academic_year'])));
 ...
 ClassRegistry::init('StudentExamStatus')->deleteAll(array('StudentExamStatus.student_id'=>$checkTheStatusForward['StudentExamStatus']['student_id']), false);
			   	...
			   	$checkIfStatusIsGenerated=ClassRegistry::init('Student')->StudentExamStatus->find('count',array('conditions'=>array('StudentExamStatus.student_id'=>$value['CourseRegistration']['student_id'],
'StudentExamStatus.academic_year'=>$value['CourseRegistration']['academic_year'],
'StudentExamStatus.semester'=>$value['CourseRegistration']['semester'])));
	 ...
	 $statusgenerated=ClassRegistry::init('Student')->StudentExamStatus->updateAcdamicStatusByStudent(
$value['CourseRegistration']['student_id'],$value['CourseRegistration']['published_course_id']);
	   ...
	   $studentList=array();
		  ...
		  foreach($semesters as $value) {
			  ...
			  if(!empty($department_id)){
				  ...
				  $studentssections=$this->find('all',
				  	array('conditions'=>array('Student.department_id'=>$department_id,
				  	'Student.id in (select student_id from course_registrations where academic_year="'.$academicYear.'" and semester="'.$value.'")'),
				  	'recursive'=>-1));
				 
				  
			  } else {
			     $studentssections= $this->find('all',array('conditions'=>array('Student.id in (select student_id from course_registrations where academic_year="'.$academicYear.'" and semester="'.$value.'")'),'recursive'=>-1));
			  }

            if(!empty($studentssections)){
		       foreach($studentssections as $key => $student) {
                   //check if rank has already done and update z new one 
		        $rank=$this->StudentExamStatus->getACSemRank($student['Student']['id'],
		          	$academicYear,$value,$catvalue);
		       //   debug($rank);
		       //die;
                if(!empty($rank)) {
                   $rankDoneEarly=$this->StudentRank->find('first',
	array('conditions'=>array('StudentRank.student_id'=>
	$student['Student']['id'],'StudentRank.semester'=>$value,
    ...
    function getProfileNotBuildList ($max_not_located_time=14) {
	         $not_build_for = date('Y-m-d ',strtotime("-".$max_not_located_time." day "));
             $list=array();
		    //modified
            /*
            $list=$this->find('all',
            array('conditions'=>array('Student.id NOT IN (SELECT student_id FROM graduate_lists)',
	...
	$updateDepartmentFiled=array();
        $studenLists=ClassRegistry::init('DepartmentTransfer')->find('all',array('conditions'=>array(
'DepartmentTransfer.from_department_id is null'),'contain'=>array('Student')));
	    $count=0;
		foreach($studenLists as $kkk=>$vv) {
			debug($vv);
      ...
      function updateDepartmentInStudentTable() {
			$updateDepartmentFiled=array();
	        $studenLists=ClassRegistry::init('Student')->find('all',array('conditions'=>array('Student.department_id is null'),'contain'=>array('AcceptedStudent')));
		    $count=0;
			foreach($studenLists as $kkk=>$vv) {
     ...
     public function getDistributionStats($acadamic_year,$program_id = null, $program_type_id = null, $department_id = null,$sex='all',
     	$year_level_id=null,$region_id=null) 
     {
	   ...
	   foreach ($departments as $key => $value) {
	         foreach ($value['YearLevel'] as $ykey => $yvalue) {
                 ...
                 $distributionStatsByDept="SELECT count(DISTINCT s.studentnumber) FROM students AS s, students_sections AS stsec,sections AS sec,year_levels as yrl,departments as dpt WHERE  stsec.student_id=s.id AND yrl.id = sec.year_level_id and dpt.id=s.department_id and dpt.id=sec.department_id AND sec.id=stsec.section_id AND sec.id in (select section_id from students_sections) $query $internalQuery $genderQuery ";
              $distResultDept = $this->query($distributionStatsByDept); 
               $distributionByDepartmentYearLevel[$value['Department']['name']][
                        $yvalue['name']][strtolower($sex)]=$distResultDept[0][0]['count(DISTINCT s.studentnumber)']; 
                 ...
                 /*
                  $graph['data'][$value['Department']['id']]=$value['Department']['shortname'];
                  */
                  ...
                  $genderQuery.=' and s.gender="'.
                  $svalue.'"';
                 
                     $distributionStatsByDept="SELECT count(DISTINCT s.studentnumber) FROM students AS s, students_sections AS stsec,sections AS sec,year_levels as yrl,departments as dpt WHERE stsec.student_id=s.id AND yrl.id = sec.year_level_id and dpt.id=s.department_id and dpt.id=sec.department_id AND sec.id=stsec.section_id AND sec.id in (select section_id from students_sections) 
                     $query $internalQuery $genderQuery";
                      $distResult = $this->query($distributionStatsByDept); 
                       $distributionByDepartmentYearLevel[$value['Department']['name']][
                        $yvalue['name']][strtolower($svalue)]=$distResult[0][0]['count(DISTINCT s.studentnumber)'];
                     $graph['labels'][$value['Department']['id']]=$value['Department']['name'];
	  ...
	  return $distribution;
	  //return $distributionByDepartmentYearLevel;
    }	

    
     public function distributionStatsLetterGrade($acadamic_year,$semester,$program_id = null, $program_type_id = null, $department_id = null,$sex='all',
     	$year_level_id=null,$region_id=null) 
     {
       $regions=$this->Region->find('list');
       $query="";
       ...
       {		      
			...
			$departments=$this->Department->find('all',
	   	  ...
	   	  $departments=$this->Department->find('all',array('contain'=>array('College',
	   ...
	   foreach ($departments as $key => $value) {
	         foreach ($value['YearLevel'] as $ykey => $yvalue) {
              ...
              cr.published_course_id,count(cr.published_course_id)
...
";
              $distResultLetter=$this->query($distributionStatsByLetter);
                ...
                foreach ($distResultLetter as $dkey => $dvalue) {
                	     $publishedCourse=$this->CourseRegistration->PublishedCourse->find('first',
                	  	array('conditions'=>array('PublishedCourse.id'=>$dvalue['cr']['published_course_id']),
                            'contain'=>array('Course')
                	  		));
                      $distributionLetterGrade[$value['Department']['name']][$publishedCourse['Course']['course_title'].' '.$publishedCourse['Course']['course_code']][strtolower($sex)][
                        $dvalue['ex']['grade']]=$dvalue[0]['count(cr.published_course_id)']; 
                        /*
                        $graph['series'][$dvalue['s']['region_id']]=$regions[$dvalue['s']['region_id']];
                        
                        $graph['labels'][$value['Department']['id']]=$value['Department']['name'];
	                       $graph['data'][$dvalue['s']['region_id']][$value['Department']['id']]+=$distResult[0][0]['count(DISTINCT s.studentnumber)'];
	                       */ 
                   }
...
";
                ...
                foreach ($distResultLetter as $dkey => $dvalue) {
                      ...
                      $distributionLetterGrade[$value['Department']['name']][$publishedCourse['Course']['course_title'].' '.$publishedCourse['Course']['course_code']][strtolower($svalue)][
                        $dvalue['ex']['grade']]=$dvalue[0]['count(cr.published_course_id)'];
     ...
     public function listStudentByLetterGrade($acadamic_year,$semester,$program_id = null, $program_type_id = null, $department_id = null,$sex='all',
     	$year_level_id=null,$region_id=null,$grade) 
     {
     	//TODO:Preengineering
        $queryPS='';
       ...
       {		      
			...
			$queryPS .= ' and department_id in ('.join(',', $departments).')';
			 ...
			 $college_id=$college_ids[1];
                 ...
                 $queryPS .= ' and department_id='.
                 $department_id;
                 $queryST .= ' and department_id='.
                 $department_id;
                 debug($department_id);
			} 
	   } 
...
array('conditions'=>array('YearLevel.department_id in (select id from departments where college_id="'.$college_id.'")', 'YearLevel.name'=>$year_level_id),'fields'=>array('id','id')));
			...
			debug($year_level_id);
              $yearLevels=ClassRegistry::init('YearLevel')->find('list',
array('conditions'=>array('YearLevel.department_id'=>$department_id,'YearLevel.name'=>$year_level_id),
'fields'=>array('id','id')));
           $queryPS .= '
              and year_level_id in ('.join(',',$yearLevels).')';
		}
	    ...
	    $publishedCourses_reg=$this->CourseRegistration->find('all',array('conditions'=>array(
'CourseRegistration.academic_year'=> $acadamic_year,
'CourseRegistration.semester'=>$semester,
...
'contain'=>array('Student','ExamGrade','PublishedCourse'=>array('Section','YearLevel','Program','ProgramType','Course','Department'))));
      ...
      $publishedCourses_add=$this->CourseAdd->find('all',array('conditions'=>array(
...
'contain'=>array('Student','ExamGrade','PublishedCourse'=>array('Section','YearLevel','Program','ProgramType','Course','Department'))));
       ...
       foreach($publishedCourses as $key => $published_course) {
                ...
                $organized_Published_courses_by_sections[$published_course['PublishedCourse']['Program']['name'].'~'.$published_course['PublishedCourse']['ProgramType']['name'].'~'.$published_course['PublishedCourse']['Section']['name'].'~'.$published_course['PublishedCourse']['Course']['course_title'].' ('.$published_course['PublishedCourse']['Course']['course_code'].
                ')']['studentList'][] = $published_course['Student'];
               }
	     }
     ...
     {
       $publishedDetails=$this->CourseRegistration->PublishedCourse->find('first',array('conditions'=>array(
       'PublishedCourse.id'=>$published_course_id
       ),
       'recursive'=>-1
       ));
       if(isset($publishedDetails) 
       && !empty($publishedDetails)){
			   $studentRegisteredLists=$this->CourseRegistration->find('list',array('conditions'=>
			   array('CourseRegistration.published_course_id'=>$published_course_id),
			   'fields'=>array('CourseRegistration.student_id',
     ...
     public function getStudentNotRegisteredPublishourse($published_course_id) 
     {
       ...
       && !empty($publishedDetails)){
			   ...
			   array('conditions'=>array('Student.id'=>$notRegisteredStudentIds),
     ...
     public function cancelStudentFxAutomaticallyConvertedChange($acadamic_year,$semester,$program_id,
     $program_type_id, $department_id,$year_level_id='All') 
     {
     	//TODO:Preengineering
        $queryPS='';
        $queryST='id is not null ';
...
'contain'=>array('Student','ExamGrade','PublishedCourse'=>array('Section','YearLevel','Program','ProgramType','Course','Department'))));
       ...
       foreach($publishedCourses as $key => $published_course) {
               ...
               foreach($published_course['ExamGrade'] as $result){
				//delete grade 
				$examGradeChangeConverted=$this->CourseRegistration->ExamGrade->ExamGradeChange->find('first',
				array('conditions'=>array('ExamGradeChange.exam_grade_id'=>$result['id'],
     ...
     public function getDistributionStatsOfRegion($acadamic_year,$program_id = null, $program_type_id = null, $department_id = null,$sex='all',
     	$year_level_id=null,$region_id=null) 
     {
       $regions=$this->Region->find('list');
       $query="";
	         ...
	         foreach ($value['YearLevel'] as $ykey => $yvalue) {
              ...
              $distResultRegion=$this->query($distributionStatsByRegion);
                ...
                foreach ($distResultRegion as $dkey => $dvalue) {
                     ...
                     $distributionByRegionYearLevel[$value['Department']['name']][$regions[$dvalue['s']['region_id']]][strtolower($sex)][
                        $yvalue['name']]=$dvalue[0]['count(DISTINCT s.studentnumber)']; 
                        $graph['series'][$dvalue['s']['region_id']]=$regions[$dvalue['s']['region_id']];
                        $graph['labels'][$value['Department']['id']]=$value['Department']['name'];
	                       $graph['data'][$dvalue['s']['region_id']][$value['Department']['id']]+=$distResult[0][0]['count(DISTINCT s.studentnumber)'];  
              ...
              $internalQuery $genderQuery group by s.region_id order by s.region_id ASC ";
                       ...
                       foreach ($distResultRegion as $dkey => $dvalue) {
                           	 if($dvalue['s']['region_id']!=0){
                       	    $distributionByRegionYearLevel[$value['Department']['name']][$regions[$dvalue['s']['region_id']]][strtolower($svalue)][
                        $yvalue['name']]=$dvalue[0]['count(DISTINCT s.studentnumber)'];
    ...
    public function findAttrationRate($acadamic_year,$semester,$program_id = null, $program_type_id = null, $department_id = null,
     	$year_level_id=null,$region_id=null,$sex='all') {
        $query="";
        $queryR='';
	   ...
	   foreach ($departments as $key => $value) {
	    	...
	    	if(!empty($year_level_id)){
	   		...
	   		foreach ($yearLevel as $ykey => $yvalue) {
	            ...
	            $totalRegistred=$this->CourseRegistration->find('count',
	   			   	array(
	   			   		'conditions'=>
	   			   		array(
	   ...
	   $internalQuery='';
	   $internalR='';
	   // prengineering students 
	   $internalQuery .= ' and st.id in (select student_id from course_registrations where year_level_id is null and student_id !=0) ';
	   $totalRegistredPr=$this->CourseRegistration->find('count',array(
	   			   		'conditions'=>
	   			   		array('CourseRegistration.year_level_id is null',
                               'CourseRegistration.semester'=>$semester,
                                'CourseRegistration.academic_year'=>$acadamic_year,
	   ...
	   foreach ($attrResult as $akey => $avalue) {
			     $college_d=$this->College->find('first',
			     	array('conditions'=>array('College.id'=>$avalue['st']['college_id']),
			     		'recursive'=>-1
			     ));
			     $attritionRateByYearLevel[$programs[$avalue['st']['program_id']].'~'.$programTypes[$avalue['st']['program_type_id']]][$college_d['College']['name']]['Pre-engineering']['1st'][strtolower($avalue['st']['gender'])]=$avalue[0]['count(DISTINCT st.studentnumber)'];
			      $attritionRateByYearLevel[$programs[$avalue['st']['program_id']].'~'.$programTypes[$avalue['st']['program_type_id']]][$college_d['College']['name']]['Pre-engineering']['1st']["total"]=$totalRegistredPr;
	   }
      return $attritionRateByYearLevel;
    }
     public function getDistributionStatsOfStatus($acadamic_year,$semester,$program_id = null, $program_type_id = null, $department_id = null,$sex='all',
     	$year_level_id=null,$region_id=null) 
     {
	   ...
	   foreach ($departments as $key => $value) {
	         foreach ($value['YearLevel'] as $ykey => $yvalue) {
              ...
              $distributionStatsByStatus="SELECT count(DISTINCT s.studentnumber),stexam.academic_status_id FROM students AS s, students_sections AS stsec,sections AS sec,year_levels as yrl,departments as dpt,student_exam_statuses as stexam WHERE  stsec.student_id=s.id AND yrl.id = sec.year_level_id and dpt.id=s.department_id and dpt.id=sec.department_id AND sec.id=stsec.section_id AND stexam.student_id=s.id AND sec.id in (select section_id from students_sections) $query 
              $internalQuery $genderQuery group by stexam.academic_status_id";
              $distResultStatus=$this->query($distributionStatsByRegion);
                foreach ($distResultStatus as $dkey => $dvalue) {
                           	if(isset($dvalue['stexam']['academic_status_id'])){	
                       	    $distributionByStatusYearLevel[$value['Department']['name']][$academicStatus[$dvalue['stexam']['academic_status_id']]][strtolower($sex)][
                        $yvalue['name']]=$dvalue[0]['count(DISTINCT s.studentnumber)'];
               ...
               $distributionStatsByStatus="SELECT count(DISTINCT s.studentnumber),stexam.academic_status_id FROM students AS s, students_sections AS stsec,sections AS sec,year_levels as yrl,departments as dpt,student_exam_statuses as stexam WHERE  stsec.student_id=s.id AND yrl.id = sec.year_level_id and dpt.id=s.department_id and dpt.id=sec.department_id AND sec.id=stsec.section_id AND stexam.student_id=s.id AND sec.id in (select section_id from students_sections) $query 
              $internalQuery group by stexam.academic_status_id";

              	    $distResultStatus=$this->query($distributionStatsByStatus);
              	      foreach ($distResultStatus as $dkey => $dvalue) {
                           	if(isset($dvalue['stexam']['academic_status_id'])){
                       	    $distributionByStatusYearLevel[$value['Department']['name']][$academicStatus[$dvalue['stexam']['academic_status_id']]][strtolower($svalue)][
                        $yvalue['name']]=$dvalue[0]['count(DISTINCT s.studentnumber)'];
                              $graph['series'][$dvalue['stexam']['academic_status_id']]=$academicStatus[$dvalue['stexam']['academic_status_id']];
...
GROUP BY st1.academic_status_id , st.region_id";
              $distResultGraduate=$this->query($distributionStatsGraduate);
                foreach ($distResultGraduate as $dkey => $dvalue) {
                           	if(isset($dvalue['st1']['academic_status_id']) && 
                           		isset($dvalue['st']['region_id'])){	
                       	        $distributionByDepartmentGraduate[$value['Department']['name']][$regions[$dvalue['st']['region_id']]][strtolower($sex)]
                             [$academicStatus[
                        $dvalue['st1']['academic_status_id']]]=$dvalue[0]['count(DISTINCT st.studentnumber)'];   
                           }
                 }
...
GROUP BY st1.academic_status_id , st.region_id";
              	    $distResultGraduate=$this->query($distributionStatsGraduate);
              	      foreach ($distResultGraduate as $dkey => $dvalue) {
                           	if(isset($dvalue['st1']['academic_status_id']) && 
                           		isset($dvalue['st']['region_id'])){	
                       	        $distributionByDepartmentGraduate[$value['Department']['name']][$regions[$dvalue['st']['region_id']]][strtolower($svalue)]
                             [$academicStatus[
                        $dvalue['st1']['academic_status_id']]]=$dvalue[0]['count(DISTINCT st.studentnumber)'];   
                           }
                     }
    ...
    public function getNotRegisteredList($acadamic_year,$semester,$program_id = null, $program_type_id = null, $department_id = null,$sex='all',$year_level_id=null,$region_id=null) 
     {
     /*
	 ...
	 */
	 ...
	 $anotherPreviousSemester=ClassRegistry::init('StudentExamStatus')->getPreviousSemester($previousAcademicYear['academic_year'],
	 $previousAcademicYear['semester']);
	 ...
	 foreach($listOfPrevious as $v){
	  ...
	  return $activeList;
	  foreach($activeList as $k=>&$list){
	  	 foreach ($list as $ko=>$val) {
	  	    if($val['academic_status_id']!=4){
	  	    	$registeredCheck=$this->CourseRegistration->find('count',
	  	    	array('conditions'=>array('CourseRegistration.student_id'=>$val['id'],
	  	    	'CourseRegistration.semester'=>$acadamic_year,
	             	'CourseRegistration.academic_year'=> $semester,
	  	    	
	  	    	),
	  	    	'recursive'=>-1
	  	    	));
	  	    	$gradutionCheck=$this->GraduateList->find('count',
	  	    	array('conditions'=>array('GraduateList.student_id'=>$val['id']
	  	    	),'recursive'=>-1));
	  	    	
	  	    	if($registeredCheck || $gradutionCheck ){
	  	    		unset($activeList[$k]);
	  	    	}
	  	    }
	  	 }
	   ...
	   if (isset($region_id ) && !empty($region_id)) {     
		...
		debug($queryR);
	     if(!empty($semester) && !empty($acadamic_year)){
	     	...
	     	$previouseAcademicYear=ClassRegistry::init('StudentExamStatus')->getPreviousSemester($acadamic_year,$semester);
	          ...
	          as $ykey => $yvalue) {
	   		 ...
	   		 if(!empty($internalQuery)){
                  ...
                  foreach ($sexList as $skey => $svalue) {
                   $genderQuery.=' and s.gender="'.$svalue.'"';
                   debug($genderQuery);
                   $queryPreviousStatus=" stexam.academic_status_id !=4";  
...
print_r($getRegistrationListSQL);
	             	$resultGetRegistrationStatistics=$this->query($getRegistrationListSQL);
	             	debug($resultGetRegistrationStatistics);
	             ...
	             debug($studentRegisteredLists); $studentListRegistered=array_merge($studentListRegistered,$studentRegisteredLists);
                    $lastStatus=$this->StudentExamStatus->find('first',
                    	array('conditions'=>array('StudentExamStatus.student_id'=>$studentRegisteredLists,
                    		'StudentExamStatus.academic_year'=>$previouseAcademicYear['academic_year']
                    		),
                    		'order'=>array('StudentExamStatus.semester DESC')));
                    if(!empty($lastStatus)){
                    	 $queryPreviousStatus.=' and stexam.academic_year="'.$lastStatus['StudentExamStatus']['academic_year'].'" and stexam.semester="'.$lastStatus['StudentExamStatus']['semester'].'"';
                    	  $getPromotedStudentListSQL="SELECT  1 as a,group_concat(DISTINCT stsec.student_id SEPARATOR',') from students_sections AS stsec,sections AS sec where stsec.section_id=sec.id $internalQueryS and stsec.student_id not in (select student_id from graduate_lists where student_id is not null ) and stsec.student_id in (select s.id from students as s where  $query $genderQuery ) and stsec.student_id in (select stexam.student_id from student_exam_statuses as stexam where $queryPreviousStatus) and stsec.student_id not in ($commaSeparatedRegisteredList) ";

                 

                    } else {
                    	 
                    	  $internalQueryS.=' and sec.academicyear="'.$acadamic_year.'"';
              		  ...
              		  $resultPromotedStudentsStatistics=$this->query($getPromotedStudentListSQL);

                     
              		  $studentElegileLists=explode(',', $resultPromotedStudentsStatistics[0][0]["group_concat(DISTINCT stsec.student_id SEPARATOR',')"]);

              		  foreach ($studentElegileLists as $key => $stvalue) {
                        
              		  	 $checkRegistered=$this->CourseRegistration->find('count',
              		  	 	array('conditions'=>array('CourseRegistration.student_id'=>$stvalue,
              		  	 		'CourseRegistration.semester'=>$semester,
              		  	 		'CourseRegistration.academic_year'=>$acadamic_year)));
              		       if($checkRegistered==0) {
                              $studentDetail=$this->find('first',
     ...
     public function updateAllAcademicStatus($academic_year,$includingGraduate=0) 
	{
		...
		foreach($departments as $dptId => $dptValue) {
			  if($includingGraduate==1){
                       $students=$this->find('all',array('conditions'=>array('Student.department_id'=>$dptId,
			  ...
			  foreach($students as $sk=>$sv){
			  	    $isTheDeletionSuccessful=$this->StudentExamStatus->
deleteAll(array('StudentExamStatus.student_id'=>$sv['Student']['id']),false);
				   
			  	   $studentRegisteredLists=$this->CourseRegistration->find('all',
			  	   	array('conditions'=>array('CourseRegistration.student_id'=>$sv['Student']['id']),
			  	   		'order'=>array('CourseRegistration.academic_year ASC','CourseRegistration.semester ASC'),
			  	   		'recursive'=>-1,
			  	   		'group'=>array('CourseRegistration.semester','CourseRegistration.academic_year')));
	...
	function getListOfDepartmentStudentsByYearLevelAndSection($college_id = null,
	 $department_id = null, $program_id = null, $program_type_id = null, $year_level_id = null, $plus_one = 1, $gender = null, $student_ids = null, $accepted_student_ids = null,$section_id=0) {
		...
		if(!empty($section_id)){
				$studentList=ClassRegistry::init('StudentsSection')->find('list',
					array('conditions'=>array(
	...
	public function checkAdmissionTransaction($student_id){
			...
			$isRegistration=$this->CourseRegistration->find('count',array('conditions'=>
				array('CourseRegistration.student_id'=>$student_id)));
			$isBelongToSections=ClassRegistry::init('StudentsSection')->find('count',
				array('conditions'=>array('StudentsSection.student_id'=>$student_id)));

			if($isRegistration!=0 || 
				$isBelongToSections!=0 ){
					return 1;
			}
			return 0;
	}
	...
	function getActiveStudentStatistics(
	        $acadamic_year,
	         $semester,
	            $department_id,
		   $region_id=null,
	    ...
	    foreach ($departments as $key => $value) {
                   ...
                   foreach ($programs as $pid => $pvalue) {   	
	   				foreach ($programTypes as $ptypeId => $ptype) {
	   				  $total=0;
                      foreach ($sexList as $skey => $svalue) {
                      		$activeListStatistics[$pvalue][$value['College']['name']][$value['Department']['name']][$yvalue['name']][$ptype][$skey]=$this->find('count',
                      			array('conditions'=>array(
                   ...
                   foreach ($programs as $pid => $pvalue) {   	
                      ...
                      foreach ($sexList as $skey => $svalue) {
                      	...
                      	foreach ($ageRange as $agek => $agevalue) {
                      		...
                      		$activeListStatistics[$pvalue][$agek][$ptype][$skey]+=$this->find('count',
                      			array('conditions'=>array(
                      				'Student.department_id'=>$value['Department']['id'],
                                    'Student.program_id'=>$pid,
                                    'Student.program_type_id'=>$ptypeId,
		 ...
		 foreach($students as $stuval){
		   $sql= "SELECT count(*),borrowernumber FROM  borrowers as Borrower where branchcode='".$branchCode[$val['College']['id']]."' and categorycode='ST' and cardnumber='".$stuval['Student']['studentnumber']."'";
			...
			$file_headers = @get_headers($source);
      		if(isset($stuval['Student']['status_academic_year']) 
      		&& !empty($stuval['Student']['status_academic_year'])){
			    $newdate = strtotime(
			   '+3 months' , strtotime($AcademicYear->getAcademicYearBegainingDate($stuval['Student']['status_academic_year'],$stuval['Student']['status_semester']))) ;
			   
			   $dateExpired=date('Y-m-d' , $newdate);

			   } else {
			   $newdate = strtotime(
			   '+3 months' , strtotime(date('Y-m-d'))) ;
			   $dateExpired=date('Y-m-d' , $newdate);
			}
      		$result = $db->query($sql);
	...
	public function getRegisteredStudentList($acadamic_year,$semester,$program_id = null, $program_type_id = null, $department_id = null,$sex='all',$year_level_id=null,$region_id=null) {
       ...
       {		      
			$college_id = explode('~', $department_id);
			if(count($college_id) > 1) {
			$departments=$this->Department->find('all',array('conditions'=>array('Department.college_id'=>$college_id[1]),
			'contain'=>array('College','YearLevel')));
       ...
       as $key => $stvalue) {
           ...
           $checkRegistered=$this->CourseRegistration->find('all',array('conditions'=>array('CourseRegistration.student_id'=>$stvalue,
           'CourseRegistration.semester'=>$semester,
           'CourseRegistration.academic_year'=>$acadamic_year),'contain'=>array('PublishedCourse'=>array('Course'),
           'Section'
           )));
