    ...
    function getStaffCompletedHDPStatistics($acadamic_year=null,$department_id=null,$sex='all')
	{
		...
		if (isset($department_id) && !empty($department_id)) {
			...
			if (count($college_id) > 1) {
				$departments = $this->Staff->Department->find('all', array('conditions' => array('Department.college_id' => $college_id[1]), 'contain' => array('College', 'YearLevel')));
			} else {
				$departments = $this->Staff->Department->find('all', array('conditions' => array('Department.id' => $department_id), 'contain' => array('College', 'YearLevel')));
			}
		} else {
				...
				foreach ($completed as $ckey => $cvalue) {
					$check = $this->find('all', array(
						'conditions' => array(
							...
							'StaffStudy.leave_date >= ' => $acadamicYearBegDate,
