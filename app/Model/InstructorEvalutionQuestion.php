...
/**
 * InstructorEvalutionQuestion Model
 *
 ...
 */
	...
	public function totalObjectiveStudentQuestion($academic_year,$semester){
		 ...
		 $publishedCourse=ClassRegistry::init('CourseInstructorAssignment')->find('first',
		 array(
		 'contain'=>array('PublishedCourse'),
		 'conditions'=>array('CourseInstructorAssignment.semester'=>$semester,
		...
		// 'PublishedCourse.id in (select published_course_id from student_evalution_rates where published_course_id is not null and student_id is not null )'
		 ...
		 if(isset($publishedCourse) && !empty($publishedCourse)){
		   $t=ClassRegistry::init('StudentEvalutionRate')->find('all',array('conditions'=>array('StudentEvalutionRate.published_course_id'=>$publishedCourse['PublishedCourse']['id']),
        	...
        	foreach($t as $eq=>$ev){
        		...
        		$active=ClassRegistry::init('InstructorEvalutionQuestion')->find('first',
        	array('conditions'=>array(
       'InstructorEvalutionQuestion.id'=>$ev['StudentEvalutionRate']['instructor_evalution_question_id'], 		'InstructorEvalutionQuestion.type'=>'objective',
'InstructorEvalutionQuestion.for'=>'student'
        	...
        	if(count($totalDistinctQuestion['active']) >= count($totalDistinctQuestion['notactive'])){
			 ...
			 $totalObjectiveStudentQuestion=ClassRegistry::init('InstructorEvalutionQuestion')->find('count',
        	array('conditions'=>array(
...
'InstructorEvalutionQuestion.active'=>$active
	...
	public function totalObjectiveColleagueQuestion($academic_year,$semester,$staff_id){
	   $t=$this->find('list',
		...
		$ct=ClassRegistry::init('ColleagueEvalutionRate')->find('all',
		array('conditions'=>array(
    	...
    	return $count;    		
	/*
		$t=ClassRegistry::init('ColleagueEvalutionRate')->find('all',
		array('conditions'=>array(
		'ColleagueEvalutionRate.academic_year'=>$academic_year,
		'ColleagueEvalutionRate.semester'=>$semester,
		'ColleagueEvalutionRate.dept_head'=>0,
