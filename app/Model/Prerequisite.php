...
class Prerequisite extends AppModel {
	...
	function isCourseExist()
	{
		...
		if (!isset($this->data['Prerequisite']['prerequisite_course_id']) || strcasecmp($this->data['Prerequisite']['prerequisite_course_id'], 'none') == 0 || empty($this->data['Prerequisite']['prerequisite_course_id'])) {
			return true;
		}

		// check user has enter a valid course code
		$is_course_code_exist = $this->Course->find('count', array('conditions' => array('Course.course_code' => $this->data['Prerequisite']['prerequisite_course_id'])));
	...
	function deletePrerequisiteList ($course_id=null,$data=null)
	{
		...
		$deleteids = $this->find('list',array('conditions' => array('Prerequisite.course_id' => $course_id), 'fields' => 'id'));
		...
		if (!empty($deleteids)) {
			$this->deleteAll(array('Prerequisite.id' => $deleteids), false);
		}
	}
} ?>
