...
class ExitExam extends AppModel {
	...
	function getCourseCompletionCertificateMassPrint($student_ids = array(), $ctype = null) 
	{
		...
		if (!empty($student_ids)) {
			...
			foreach ($student_ids as $k => $student_id) {
				...
				if ($student_detail['Student']['program_id'] == PROGRAM_UNDEGRADUATE && $student_detail['Student']['graduated'] == 0 && empty($student_detail['GraduateList']['id']) && empty($student_detail['SenateList']['id']) && !empty($student_detail['ExitExam'][0]['id'])) {
					...
					if ($student_detail['Student']['program_id'] == PROGRAM_UNDEGRADUATE) {
						...
						if (!empty($approvedExitExamGrade)) {
							...
							$exitExamresult = $this->find('first', array(
								'conditions' => array(
					...
					$g_d = $temporary_degree['student_detail']['ExitExam']['exam_date'];
					...
					$exam_date = date('d F, Y',
						mktime(0, 0, 0,
							substr($temporary_degree['student_detail']['ExitExam']['exam_date'],5 ,2),
							substr($temporary_degree['student_detail']['ExitExam']['exam_date'],8 ,2),
							substr($temporary_degree['student_detail']['ExitExam']['exam_date'],0 ,4)
						)
					);


					if($temporary_degree['student_detail']['ExitExam']['exam_date']) {
						...
						$temporary_degree['student_detail']['graduated_date'] = $exam_date;
						//$temporary_degree['student_detail']['graduated_ethiopic_date'] = $this->getEthiopicGraduationDate($exam_date);
					} else {
		...
		//debug($temporary_degree_mass);
		//exit();
	    return $temporary_degree_mass;
	}

	function getListOfStudentsForCourseCompletionCertificatePrint($student_ids = array(), $program_id = PROGRAM_UNDEGRADUATE, $program_type_id = null, $department_id = null, $admission_year = '', $percentCompletedCredit = 95, $studentnumber = '')
	{
		/***
		...
		***/
		...
		if (isset($program_type_id) && !empty($program_type_id) /* && !is_array($program_type_id) */) {
			$exemptionMaximum = $this->query(
				"SELECT student_id, SUM(course_taken_credit) FROM  course_exemptions
				WHERE student_id in (select id from students where graduated = 0 and department_id in (". (!is_array($department_id) ? $department_id : join(', ', $department_id)). ") and program_id = $program_id and program_type_id in (". (!is_array($program_type_id) ? $program_type_id : join(', ', $program_type_id)). "))  
			...
			$exemptionMaximum = $this->query(
				"SELECT student_id, SUM(course_taken_credit) FROM  course_exemptions
				WHERE student_id in (select id from students where graduated = 0 and department_id in (". (!is_array($department_id) ? $department_id : join(', ', $department_id)). ") and program_id = $program_id) 
				and student_id  NOT IN (SELECT student_id FROM senate_lists where student_id is not null)
				and student_id NOT IN (SELECT student_id FROM graduate_lists where student_id is not null)
				GROUP BY student_id order by SUM(course_taken_credit)  DESC limit 1"
			);
		...
		if (isset($studentnumber) && !empty($studentnumber)) {
			...
			$studentLists = $this->query(
				...
				WHERE student_id IN (select id from students where graduated = 0 and department_id in (". (!is_array($department_id) ? $department_id : join(', ', $department_id)). ") and studentnumber LIKE $studentnumberQuoted and program_id = $program_id) 
			...
			$studentLists = $this->query(
				"SELECT student_id, SUM(credit_hour_sum) FROM  student_exam_statuses
				WHERE student_id IN (select id from students where graduated = 0 and id in (". join(', ', $student_ids). ")) 
				and student_id  NOT IN (SELECT student_id FROM senate_lists where student_id IN (". join(', ', $student_ids). "))
				and student_id NOT IN (SELECT student_id FROM graduate_lists where student_id IN (". join(', ', $student_ids). "))
				GROUP BY student_id HAVING SUM(credit_hour_sum) >= " . ($minimumPointofCurriculum['Curriculum']['minimum_credit_points'] - $exptionPoint - $notUsedInCGPACreditSum) * ($percentCompletedCredit) . ""
			);
		} else if (!empty($program_type_id) && !empty($department_id)) {
			$studentLists = $this->query(
				"SELECT student_id, SUM(credit_hour_sum) FROM  student_exam_statuses
				WHERE student_id in (select id from students where graduated = 0 and department_id in (". (!is_array($department_id) ? $department_id : join(', ', $department_id)). ") and program_id = $program_id and program_type_id in (". (!is_array($program_type_id) ? $program_type_id : join(', ', $program_type_id)). ")) 
				and student_id  NOT IN (SELECT student_id FROM senate_lists where student_id is not null)
			...
			foreach ($students as $key => $student) {
				...
				if (isset($student['Curriculum']['id'])) {
					...
					if ($credit_sum >= $student['Curriculum']['minimum_credit_points']) {
						...
						$filtered_students[$cid][$index]['ExitExam'] = $student['ExitExam'];
							...
							foreach ($student['CourseRegistration'] as $key => $course_registration) {
								if (!$this->Student->CourseRegistration->isCourseDroped($course_registration['id']) && $course_registration['PublishedCourse']['drop'] == 0) {
									...
									if (!empty($grade_detail) && isset($grade_detail['ExamGrade']['grade']) && !empty($grade_detail['ExamGrade']['grade'])) {
										...
										if (!$course_registration['PublishedCourse']['Course']['exit_exam']) {
									...
									} else if (!$course_registration['PublishedCourse']['Course']['exit_exam'] && !$invalid_grade && isset($grade_detail['ExamGrade']['grade']) && (strcasecmp($grade_detail['ExamGrade']['grade'], 'NG') == 0 || strcasecmp($grade_detail['ExamGrade']['grade'], 'DO') == 0 || strcasecmp($grade_detail['ExamGrade']['grade'], 'I') == 0 || strcasecmp($grade_detail['ExamGrade']['grade'], 'F') == 0 || strcasecmp($grade_detail['ExamGrade']['grade'], 'W') == 0 || strcasecmp($grade_detail['ExamGrade']['grade'], 'Fx') == 0 || strcasecmp($grade_detail['ExamGrade']['grade'], 'Fail') == 0)) {
										//debug($invalid_grade);
						...
						if (isset($student['CourseAdd']) && !empty($student['CourseAdd'])) {
							foreach ($student['CourseAdd'] as $key => $course_add) {
								...
								if (!empty($grade_detail) && isset($grade_detail['ExamGrade']['grade']) && !empty($grade_detail['ExamGrade']['grade'])) {
									...
									if (!$course_add['PublishedCourse']['Course']['exit_exam']) {
								...
								if (empty($grade_detail) && !$incomplete_grade) {
									...
									$incomplete_grade = true;
									$donot_consider_cgpa = true;
								} else if (!$course_add['PublishedCourse']['Course']['exit_exam'] && !$invalid_grade && isset($grade_detail['ExamGrade']['grade']) && (strcasecmp($grade_detail['ExamGrade']['grade'], 'NG') == 0 || strcasecmp($grade_detail['ExamGrade']['grade'], 'DO') == 0 || strcasecmp($grade_detail['ExamGrade']['grade'], 'I') == 0 || strcasecmp($grade_detail['ExamGrade']['grade'], 'F') == 0 || strcasecmp($grade_detail['ExamGrade']['grade'], 'W') == 0 || strcasecmp($grade_detail['ExamGrade']['grade'], 'Fx') == 0 || strcasecmp($grade_detail['ExamGrade']['grade'], 'Fail') == 0)) {
