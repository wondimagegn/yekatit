...
class Clearance extends AppModel {
	...
	function validateStudentClearance ($student_id=null) {
	    $check_student_returned_properties = $this->Student->TakenProperty->find('count',
	    array('conditions'=>array(
	     ...
	     if ($check_student_returned_properties>0) {
	            ...
	            $details=$this->Student->TakenProperty->find('all',
	            array('conditions'=>array(
	        'TakenProperty.returned '=> 0,
	        'TakenProperty.student_id'=>$student_id
	    )));
	...
	function checkDuplication ($data=null) {
		        ...
		        $lastregistredDate=ClassRegistry::init('CourseRegistration')->find('first',
		        array('conditions'=>array('CourseRegistration.student_id'=>
		        $data['Clearance']['student_id']),
		        'order'=>'CourseRegistration.created DESC','recursive'=>-1));
		
	           // allow multiple application in case of denied clearnce
	             $check1=$this->find('count',array('conditions'=>array(
	...
	function elegibleForReadmission($student_id = null,$academic_year=null) {
		...
		*/
		...
		$clearances = $this->find('all',
		array('conditions'=>array(
		'Clearance.student_id'=>$student_id,
		
		'Clearance.confirmed=1'),'recursive'=>-1,'order'=>'Clearance.request_date DESC'));
		
		if (empty($clearances)) {
		      $this->invalidate('error','You need to request clearance and should get 
		      approved clearance before applying for  readmission application.');
		     return 0; // not allow 
		} else {
		
		    // find the latest class attended and is s/he cleared ?        
		   debug($lastregistredDate['CourseRegistration']['created']);
		    $check_clearance = $this->find('count',
		    array('conditions'=>array(
		    'Clearance.student_id'=>$student_id,
		    'Clearance.confirmed=1',
		    'Clearance.request_date >= '=>$lastregistredDate['CourseRegistration']['created'])));
		    ...
		    if ($check_clearance) {
		          ...
		          if ($readmissionAchieved==5) {
		                ...
		                'You need to need to fill clearance/withdraw before applying for readmission.');
		                return 0; // not allow
		                 
		          } else if ($readmissionAchieved==3) {
		                $this->invalidate('error',
		                'You have not achieved minimum  readmission point for application. 
		                Please advice registrar for further information.');
                ...
                //	    
		         $this->invalidate('error','You need to reqeuest clearance/withdraw before 
		         applying  for readmission application.');
                  return 0; // not allow 
		    }	    
		}
	 }
    ...
    function readmissionPoint ($student_id=null,$year=null) {
		...
		*/
		 ...
		 $student_status = $this->Student->StudentExamStatus->find('first',
			    array(
				    'conditions' =>
				    array(
		...
		if (empty($student_status)) {
		    ...
		    $lastregistredDate=ClassRegistry::init('CourseRegistration')->find('first',
		    array('conditions'=>array('CourseRegistration.student_id'=>$student_id),'recursive'=>-1,
		    ...
		    'order'=>'Clearance.acceptance_date DESC'));
		     ...
		     if (empty($withdraw)) {
		      ...
		      $check_withdraw = $this->find('count',
		    array('conditions'=>array(
		    'Clearance.student_id'=>$student_id,
		    'Clearance.confirmed=1',
		    'Clearance.request_date >= '=>$lastregistredDate['CourseRegistration']['created']
    ...
    function latestCourseRegistrationSemesterYearLevel ($student_id=null) {
				...
				if (!empty($student_id)) {
				      $latestSemester=$this->Student->CourseRegistration->find('all',
				    array(
				    'fields'=>array(
	            ...
	            foreach ($latestSemester as $k=>$v) {
	                       ...
	                       if (strcasecmp($v['CourseRegistration']['semester'],
	                                $semester_year_level['semester'])>0) {
	                          ...
	                          if (!empty($student_detail['Student']['department_id'])) {
	                           ...
	                           $semester_year_level['year_level']=
	                           $this->Student->Department->YearLevel->field('name',
	                           array('YearLevel.department_id'=>
	                           $student_detail['Student']['department_id'],
	                           'YearLevel.id'=>$v['CourseRegistration']['year_level_id']));
	                           
	                          } else if (empty($student_detail['Student']['department_id']) &&
	                          !empty($student_detail['Student']['college_id'])) {
	...
	*/
    function isAchievedMinimumReadmissionPoint($student_id = null) {
		...
		foreach($academic_stands as $key => $academic_stand) {
			...
			if(in_array($student_detail['year_level'], $stand_year_levels) &&
				in_array($student_detail['semester'], $stand_semesters)) {
			
				if((substr($student_detail['admissionyear'], 0, 4) >= 
				$academic_stand['AcademicStand']['academic_year_from']) || 
				($academic_stand['AcademicStand']['applicable_for_all_current_student'] == 1 )) {
			...
			if(!empty($acadamic_rules)) {
				foreach($acadamic_rules as $key => $acadamic_rule) {
				    ...
				    if ($acadamic_rule['AcademicRule']['sgpa']!=0) {
					    if($student_status['StudentExamStatus']['sgpa']>
					    $acadamic_rule['AcademicRule']['sgpa']) {
						 
						    return 4;
						}
			       } else if ($acadamic_rule['AcademicRule']['cgpa']!=0) {
			           if($student_status['StudentExamStatus']['cgpa']>
					    $acadamic_rule['AcademicRule']['cgpa']) {
						    //return true;
						    return 4;
						}
			       }
	...
	function count_clearnce_request ($department_ids=null,$college_ids=null) {
	        $options = array (); 
 	...
 	function withDrawAfterLastStatusButNotReadmitted ($student_id = nul,$current_academicyear=null) {
 	    $last_status_date=$this->Student->StudentExamStatus->find('first',array('conditions'=>array('StudentExamStatus.student_id'=>$student_id),'order'=>array('StudentExamStatus.created DESC')));
	    ...
	    if(!empty($academic_year) && !empty($semester)) {
 	       $last_registration_date=$this->Student->CourseRegistration->find('first',array('conditions'=>array('CourseRegistration.student_id'=>$student_id,
...
,'CourseRegistration.id in (select course_registration_id from exam_grades where id in (select exam_grade_id from exam_grade_changes where grade="W")) '),
    ...
    $last_registration_date=$this->Student->CourseRegistration->find('first',array('conditions'=>array('CourseRegistration.student_id'=>$student_id,'CourseRegistration.id in (select course_registration_id from exam_grades where id in (select exam_grade_id from exam_grade_changes where grade="W")) '),
	                       'order'=>array('CourseRegistration.created DESC'),'contain'=>array('ExamGrade'=>array('ExamGradeChange'))));
