    ...
    function checkUnique($data, $fieldName) {
            ...
            */
            ...
            if(isset($fieldName) && $this->hasField($fieldName)) {
                ...
                if(!$valid){
                    $check=$this->find('count',array('conditions'=>array('grade_id'=>$this->data['GradeScaleDetail']['grade_id'],'point_value'=>$this->data['GradeScale']['grade_type_id'])));
    ...
    function field_comparison($check1, $operator, $field2) { 
	  ...
	  function check_scale_execlusiveness($data=null,$role=null) {
	           ...
	           if ($role == ROLE_COLLEGE) {
				$already_recorded_range=$this->find('all',array('conditions'=>array('model'=>'GradeScale.College','GradeScale.active'=>1,'GradeScale.foreign_key'=>$data['GradeScale']['foreign_key'],'GradeScale.program_id'=>$data['GradeScale']['program_id'])));
				} else if ($role == ROLE_DEPARTMENT) {
				  $already_recorded_range=$this->GradeScale->find('all',array('conditions'=>array('GradeScale.model'=>'Department','GradeScale.active'=>1,'GradeScale.foreign_key'=>$data['GradeScale']['foreign_key'],
				   ...
				   foreach($sr['GradeScaleDetail'] as $kkkk=>$vvvv){
					     ...
					     foreach ($data['GradeScaleDetail'] as $k=>$v) {
					         ...
					         if (!empty($v['minimum_result'])&&!empty($v['maximum_result'])) { 
					            ...
					            if( ($v['minimum_result']<=$vvvv['minimum_result'] 
					            && $vvvv['minimum_result'] <=$v['maximum_result']) ||
					            ($v['minimum_result']<=$vvvv['maximum_result'] && $vvvv['maximum_result']<=
					            $v['maximum_result']) || 
					            ($vvvv['minimum_result']<=$v['minimum_result'] && $v['minimum_result']<=
					            $vvvv['maximum_result'])  
					            ) {
					            
					              $this->invalidate('minimum_maximum_result',
					              'The given grade range is not unique. 
					              Please make sure that "Minimum result" and/or "Maximum" is  already recorded.');
						          return false;
					         }
	...
	*/
	function gradeRangeContinuty($data=null){
	    if (!empty($data)) {
	             ...
	             $grades=$this->Grade->find('all',array('conditions'=>array('Grade.grade_type_id'=>$data['GradeScale']['grade_type_id']),'order'=>'Grade.point_value desc','contain'=>array()));
...
//	             exit(1);
	             ...
	             foreach ($grades as $grade_key=>$grade_value) {
	                        ...
	                        $grade_scale=>$grade_scale_detail){   
	                           if (!empty($grade_scale_detail['maximum_result'])
	                           && !empty($grade_scale_detail['minimum_result'])) {          
	                             if ($grade_value['Grade']['id'] == $grade_scale_detail['grade_id']) {
	                                  if ($count==0) {
	                                       if ($grade_scale_detail['maximum_result']==100) {
	                                            $next_maximum=$grade_scale_detail['minimum_result'];                             $count++;
	                                            break 1;
	                                       } else {
	...
	*/
    function checkGradeIsUnique ($data=null) {
         if (!empty($data)) {
            ...
            $grades=$this->Grade->find('list',array('conditions'=>array('Grade.grade_type_id'=>$data['GradeScale']['grade_type_id']),'order'=>'Grade.point_value desc','contain'=>array(),'fields'=>array('id','grade')));
