...
<div class="smallheading"><?php echo __('Graduate List View'); ?></div>
<?php
...
'default' => false,'style'=>'width:70px'));
	...
	<tr>
		<td colspan="6">
		<?php echo $this->Form->submit(__('List Students'), array('name' => 'listStudentsForGraduateList',
...
padding:2px;
	...
	<tr>
			...
			<th class="actions" style="width:10%; text-align:center"><?php echo __('Action');?></th>
	</tr>
	<?php
	$i = 0;
	$count = 1;
...
echo $this->Html->link(
    $graduateList['Student']['full_name'],
    '#',
   array('class'=>'jsview','data-animation'=>"fade",
'data-reveal-id'=>'myModal','data-reveal-ajax'=>"/students/get_modal_box/".$graduateList['Student']['id'])
);
		...
		<td class="actions" style="background-color:white">
			...
			if($valid_deletion_time > date('Y-m-d')) {
				echo $this->Html->link(__('Delete'), array('action' => 'delete', $graduateList['GraduateList']['id']), null, sprintf(__('Are you sure you want to delete \'%s\' from the graduate list?', true	), $graduateList['Student']['full_name']));
			}
			else {
				echo '--';
			}
