...
<div class="smallheading"><?php echo __('Student Copy Printing');?></div>
<table class="fs13">
	<tr>
		...
		<td style="width:75%"><?php echo $this->Form->submit(__('Get Student Copy'), array('name' => 'continueStudentCopyPrint','class'=>'tiny radius button bg-blue', 'div' => false)); ?></td>
	</tr>
</table>
<?php
if(!empty($student_copy)) {
	...
	if(isset($student_copy['courses_taken']) && !empty($student_copy['courses_taken'])) {
		...
		echo $this->Form->submit(__('Display Student Copy'), array('name' => 'displayStudentCopyPrint','class'=>'tiny radius button bg-blue', 'div' => false));
	}
	//debug($student_copy);
}
?>
