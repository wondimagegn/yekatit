								...
								<div class="row">
									<div class="large-6 columns">
										...
										if (!empty($departments)) { ?>
											<?= $this->Form->input('department_id', array('id' => 'departmentId', 'label' => 'Department: ', 'style' => 'width:90%', 'type' => 'select', 'options' => $departments, 'default' => (isset($department_id) ? $department_id : false)));
										} else if (!empty($colleges)) { ?>
											<?= $this->Form->input('college_id', array('id' => 'collegeId', 'label' => 'College: ', 'style' => 'width:90%', 'type' => 'select', 'options' => $colleges, 'default' => (isset($college_id) ? $college_id : false)));
										} ?>
									</div>
									<div class="large-6 columns">
									</div>
...
<script>
	...
	$(function () {
		...
		$('#AcadamicYear, #Semester, #Program, #ProgramType, #departmentId, #collegeId').on('change', function () {
			if ($showSearchResults.length) {
				hideSearchResults();
			} else {
				hideSearchResults('');
