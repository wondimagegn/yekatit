...
array('id' => 'StudentNumber', 'class' => 'fs14','label' => false, 'type' => 'text')); ?>
        ...
        <td style="width:20%">
			<?php 
				echo $this->Form->input('password', array('id' => 'Password', 'class' => 'fs14','label' => false)); ?>
       </td>
    </tr>
	<tr>
		<td colspan="4">
...
if(isset($course['CourseRegistration']) && !empty($course['CourseRegistration'])) {
...
if(!empty($course['CourseRegistration']['id'])){
	echo $this->Form->hidden('CourseRegistration.'.$st_count.'.id', array('label' => false,
'value'=>$course['CourseRegistration']['id']));

}

if(!empty($course['CourseAdd']['id'])){
	echo $this->Form->hidden('CourseRegistration.'.$st_count.'.course_add_id', array('label' => false,
...
if(isset($grade_id) && !empty($grade_id)){
	echo $this->Form->hidden('CourseRegistration.'.$st_count.'.grade_id', array('label' => false,
'value'=>$grade_id));
$grade_id=null;
}
...
if(!isset($course['Grade']) && empty($course['Grade'])) {
		if(isset($course['PublishedCourse']['grade']['grade']) && !empty($course['PublishedCourse']['grade']['grade'])){   
		echo $this->Form->input('CourseRegistration.'.$st_count.'.grade', array('label' => false,'type'=>'select','options'=>$gradeList,'empty'=>'select',
		'value'=>$course['PublishedCourse']['grade']['grade']));
		} else {
		  echo $this->Form->input('CourseRegistration.'.$st_count.'.grade', array('label' => false,'type'=>'select','options'=>$gradeList,'empty'=>'select'));
		}
} else {
	 
echo $this->Form->input('CourseRegistration.'.$st_count.'.grade', array('label' => false,'type'=>'select','empty'=>'select','options'=>$gradeList,'value'=>$course['Grade']['grade']));
