	...
	<div class="box-body">
		<div class="row">
			<div class="large-12 columns">
				<?= $this->Form->create('CourseSubstitutionRequest', array('action'=> 'search')); ?>
				<?php
				if ($this->Session->read('Auth.User')['role_id'] != ROLE_STUDENT) { ?>
					<div style="margin-top: -15px;">
						<blockquote>
							<h6><i class="fa fa-info"></i> &nbsp; Important Note:</h6>
							<span style="text-align:justify;" class="fs14 text-gray">The student list you will get here depends on system wide setting which is set to only display course substitution requests which are <b style="text-decoration: underline;"><i> not more than <?= DAYS_BACK_COURSE_SUBSTITUTION ?> days  from today and those who are not graduated.</i></b></span>
						</blockquote>
						<hr>
						...
						<div id="ListPublishedCourse" style="display:<?= (!empty($turn_off_search) ? 'none' : 'display'); ?>">
							<fieldset style="padding-bottom: 5px;padding-top: 5px;">
								...
								<div class="row">
									<div class="large-3 columns">
										<?= $this->Form->input('Search.program_id', array('label' => 'Program: ', 'style' => 'width:90%',  'id' => 'program_id_1', 'empty' => ' ', 'options' => $programs)); ?>
									</div>
									<div class="large-3 columns">
										<?= $this->Form->input('Search.program_type_id', array('label' => 'Program Type: ', 'style' => 'width:90%', 'empty' => ' ', 'options' => $programTypes)); ?>
									</div>
									<div class="large-6 columns">
										<?php
										if (isset($colleges) && !empty($colleges)) {
											echo $this->Form->input('Search.college_id', array('label' => 'College: ', 'style' => 'width:90%', 'empty' => ' ', 'onchange' => 'getDepartment(1)', 'id' => 'college_id', 'default' => $default_college_id));
										} else if (isset($departments) && !empty($departments)) {
											echo $this->Form->input('Search.department_id', array('label' => 'Department: ', 'style' => 'width:90%', 'empty' => ' ', 'id' => 'department_id_1', 'default' => $default_department_id));
										} ?>
									</div>
								</div>
								<?php
								if (isset($departments) && !empty($departments) && $this->Session->read('Auth.User')['role_id'] != ROLE_STUDENT && $this->Session->read('Auth.User')['role_id'] != ROLE_REGISTRAR &&  $this->Session->read('Auth.User')['role_id'] != ROLE_COLLEGE &&  $this->Session->read('Auth.User')['role_id'] != ROLE_DEPARTMENT) { ?>
									<div class="row">
										<div class="large-6 columns">
											<?= $this->Form->input('Search.department_id', array('label' => 'Department: ', 'style' => 'width:90%', 'empty' => ' ', 'id' => 'department_id_1', 'default' => $default_department_id)); ?>
										</div>
										<div class="large-6 columns">
										</div>
									</div>
								...
								<div class="row">
									<div class="large-3 columns">
										<?= $this->Form->input('Search.studentnumber', array('label' => 'Student ID:', 'placeholder' => 'Leave this if not sure', 'default' => $studentnumber, 'style' => 'width:90%;')); ?>
									</div>
									<div class="large-3 columns">
										<?= $this->Form->input('Search.name', array('label' => 'Name:', 'placeholder' => 'First, Middle or Last name', 'default' => $name, 'style' => 'width:90%;')); ?>
									</div>
									<div class="large-3 columns">
										<?= $this->Form->input('Search.graduated', array('label' => 'Graduated: ', 'style' => 'width:90%', 'options' => array('0' => 'NO', '1' => 'Yes', '2' => 'All'), 'default' => '0')); ?>
									</div>
									<div class="large-3 columns">
										<?= $this->Form->input('Search.limit', array('id' => 'limit ', 'type' => 'number', 'min' => '100',  'max' => '1000', 'value' => $limit, 'step' => '100', 'label' => ' Limit: ', 'style' => 'width:80%;')); ?>
									</div>
								</div>
								<div class="row">
									<div class="large-3 columns">
							...
							<?= $this->Form->submit(__('Search'), array('name' => 'viewCourseAdds', 'class' => 'tiny radius button bg-blue', 'div' => false)); ?>
							<?php
