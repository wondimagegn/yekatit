...
form-data'
	...
	<fieldset>
		...
		<div id="basic_fields" style="display:block">	
		 <?php
	        ...
	        //echo $this->Html->image($media->url($student['Attachment'][0]['basename'],true), array('alt' => 'CakePHP'));
	  // echo $media->embed($media->file('s'.DS.$student['Attachment'][0]['dirname'].DS.$student['Attachment'][0]['basename']));

		        //echo $this->Form->hidden('Attachment.0.id');
		        //echo '<tr><div class="profile-picture"><td align="center">'.$media->embed($media->file('s'.DS.$this->data['Attachment']['dirname'].DS.$this->data['Attachment']['basename'])).'</td></div></tr>';
		        ...
		        echo '<tr><td>Username  :   '.$this->data['User']['username'].'</td></tr>';
	...
	<?php		
        ...
        echo "<div class=\"AddTab\">\n";
              ...
              echo $form->input('Address.0.zone/subcity');
              echo $form->input('Address.0.woreda');
              echo $form->input('Address.0.kebele');
              echo $form->input('Address.0.house_number');
              echo $form->input('Address.0.pobox',array('label'=>'P.O.Box'));
		echo '</div>'; // End add tab div
		?>
