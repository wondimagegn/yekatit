...
function updateCourseCategory() {
            ...
            //serialize form data
            var formData = $("#curriculum_id").val();
			$("#curriculum_id").attr('disabled', true);
			$("#course_category_id").attr('disabled', true);
            ...
            $.ajax({
                ...
                success: function(data,textStatus,xhr){
						$("#curriculum_id").attr('disabled', false);
			            $("#course_category_id").attr('disabled', false);
						$("#course_category_id").empty();
					    $("#course_category_id").append('<option></option>');
						
						$("#course_category_id").append(data);
					},
 	...
 	<div class="smallheading"><?php __('Course search');?></div>
	<?php
	   ...
	   echo '<table><tr><td>';
        ...
        echo '<tr><td>'.$this->Form->input('Search.year_level_id',array('empty'=>' ')).'</td></tr>';
        echo '<tr><td>'.$this->Form->input('Search.program_id',array('empty'=>' ')).'</td></tr>';
  		echo '</table>';
		echo '</td><td>';
		echo '<table>';
		echo '<tr><td>'.$this->Form->input('Search.curriculum_id',array('empty'=>' ',
		'onchange'=>'updateCourseCategory()','id'=>'curriculum_id','style'=>'width:200px')).'</td></tr>';
	    echo '<tr><td>'.$this->Form->input('Search.course_category_id',array('empty'=>' ',
	    'id'=>"course_category_id")).'</td></tr>';
...
<?php 
    if (!empty($courses)) {
?>
	<div class="smallheading"><?php __('Courses');?></div>
	
	<table cellpadding="0" cellspacing="0">
	<tr>
			<th>S.N<u>o</u></th>
			<th><?php echo $this->Paginator->sort('Course Title','course_title');?></th>
			<th><?php echo $this->Paginator->sort('Year','year_level_id');?></th>
			<th><?php echo $this->Paginator->sort('Course Code','course_code');?></th>
			<th><?php echo $this->Paginator->sort('Credit','credit');?></th>
		
			<th width="5%"><?php echo $this->Paginator->sort('L T L');?></th>
			<th><?php echo $this->Paginator->sort('course_category_id');?></th>
			<th><?php echo $this->Paginator->sort('curriculum_id');?></th>
			
			<th><?php echo $this->Paginator->sort('lecture_attendance_requirement');?></th>
			<th><?php echo $this->Paginator->sort('lab_attendance_requirement');?></th>
	...
	<tr<?php echo $class;?>>
		...
		<td> <?php echo $course['Course']['course_detail_hours']; ?>&nbsp; </td>
		...
		<td><?php echo $course['Course']['lecture_attendance_requirement']; ?>&nbsp;</td>
		<td><?php echo $course['Course']['lab_attendance_requirement']; ?>&nbsp;</td>
		<td>
						...
						$("#department_id_"+id).append(data);
							...
							$.ajax({
								...
								success: function(data,textStatus,xhr){
							            ...
							            var formUrl = '/curriculums/get_course_category_combo/'+subCat;
                                        $.ajax({
                                            type: 'get',
                                            url: formUrl,
                                            data: subCat,
...
<div class="smallheading"><?php __('List Courses'); ?></div>
	...
	<table class="fs13 small_padding">
		...
		'style'=>'width:250px','id'=>'curriculum_id_1','label'=>false));
			...
			<td style="width:13%"> Course Category:</td>
			<td style="width:37%">
			<?php 
