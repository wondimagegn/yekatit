...
//$menuoptimized=$this->Session->read('menuoptimized');

renameControllerTitle($menuoptimized, $rename_menu_title);
//debug($menu);
...
<?php
function renameControllerTitle(&$menuoptimized=null, $rename_menu_title=null) {
     if(!empty($menuoptimized)) {
            ...
            if(!empty($sub_menu['children']))
                renameControllerTitle($menuoptimized[$key]['children'], $rename_menu_title);
            }
          }
}
?>
