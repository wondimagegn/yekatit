...
<?php echo $this->Form->create('ClassRoomBlock'); ?>
	<div class="smallheading"><?php __('Assign/Edit Class Rooms Program Types'); ?></div>
	<table>
	<?php
			...
			if(!empty($fromadd_organized_classRooms_blocks_data)){
				...
				if(isset($fromadd_already_assign_class_rooms)) {
					...
					foreach($fromadd_already_assign_class_rooms as $aacrk=>$aacrv){
						...
						$this->Html->link(__('Delete', true), array('action' => 'delete_assign_program_program_type', $aacrv['ProgramProgramTypeClassRoom']['id']),null, sprintf(__('Are you sure you want to delete  %s?', true), $aacrv['ClassRoom']['room_code'])).
						"</td></tr>";
					}
					?></table><?php
				}
		...
		<!-- echo '<tr><td colspan="2"><table><tr><td>'.$this->Form->input('class_room_id',array('type'=>'checkbox',
			'options'=>$classRooms)).'</td></tr></table></td></tr>';-->
	</table>
