...
<?php echo $this->Form->create('DormitoryAssignment');?>
	<div class="smallheading"><?php __('Manually Add/Delete Students');?></div>
<?php if(isset($programs)) {?>
	...
	<?php 
		...
		echo '<tr><td colspan="6">'.$this->Form->end('Search').'</td></tr>'; 
	?>
	</table>
	...
	<tr<?php echo $class;?>>
		...
		<td class="actions">
			<!-- <?php echo $this->Html->link(__('View', true), array('action' => 'view', $dormitoryAssignment['DormitoryAssignment']['id'])); ?>
			<?php echo $this->Html->link(__('Edit', true), array('action' => 'edit', $dormitoryAssignment['DormitoryAssignment']['id'])); ?> -->
			<?php echo $this->Html->link(__('Delete', true), array('action' => 'delete', $dormitoryAssignment['DormitoryAssignment']['id']), null, sprintf(__('Are you sure you want to delete %s?', true), $dormitoryAssignment['Student']['full_name'])); ?>
		</td> 
	</tr>
<?php endforeach; 
	if(!empty($dormitory_id)){
	   echo '<tr><td colspan="12" style="text-align: center;" id="ajax_student">'.$this->Js->link(__('Add', true),'/dormitory_assignments/add_student_dormitory/'.$dormitory_id.'~'.$selected_program_id. '~'.$selected_program_type_id.'~'.$selected_gender.'~'.$selected_college_id.'~'.$selected_department_id.'~'. $selected_year_level_id.'~'.$dormitory_block_id, array('update'=>'#ajax_student','evalScripts'=>true)).
			'</td></tr>';
		}
?>
	</table>
