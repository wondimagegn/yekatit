...
App::import('Vendor','tcpdf/tcpdf');
    ...
    if(!empty($student_copy['Department']) && !empty($student_copy['Department']['id'])) {
		 $pdf->MultiCell(92, 7, $student_copy['Department']['name'].' Department', 0, 'C', false, 0, 1, 22);
    }
    else {
    ...
    $pdf->SetFont($fontPath, 'U', 12, '', false);
    if(!empty($student_copy['Department']) && !empty($student_copy['Department']['id'])) {
		 $pdf->MultiCell(85, 7, $student_copy['Department']['amharic_name'].' ዲፓርትመንት', 0, 'C', false, 0, 120, 22);
    }
    else {
    ...
    $pdf->MultiCell(15, 7, '19', 0, 'C', false, 0, 42, 27);
    if((!empty($student_copy['Department']) && !empty($student_copy['Department']['id']) && 
    ...
    $pdf->MultiCell(157, 7, 'አርባ ምንጭ፡ ኢትዮጵያ', 0, 'C', false, 0, 27, 31);
    ...
    $pdf->MultiCell(157, 7, 'Arba Minch, Ethiopia', 0, 'C', false, 0, 27, 36);
    //Footer
