...
<div class="smallheading"><?php __('Update Student Details'); ?></div>
            ...
            echo "<div class=\"AddTab\">\n";
                ...
                echo "<tr><td><table><tbody>";
                   echo '<tr><td colspan=2><strong>Demographic Information</strong></td></tr>';
                    /*
                      echo "<tr><td style='padding-left:95px;'> First Name: ".$this->data['Student']['first_name']."</td></tr>";
			          echo "<tr><td style='padding-left:95px;'> Middle Name: ".$this->data['Student']['middle_name'] ."</td></tr>";
			          echo "<tr><td style='padding-left:95px;'> Last Name: ".$this->data['Student']['last_name']."</td></tr>";
		           */
		          
		          echo '<tr><td>'.$this->Form->input('first_name',array('label'=>'First name in English',
		          'readOnly'=>true)).'</td></tr>';
		          echo '<tr><td>'.$this->Form->input('middle_name',array('label'=>'Middle name in English',
		          'readOnly'=>true)).'</td></tr>';
		          echo '<tr><td>'.$this->Form->input('last_name',array('label'=>'Last name in English',
		          'readOnly'=>true)).'</td></tr>';
		        
		               
		          if (empty($this->data['Student']['amharic_first_name'])) {
		            
		             echo '<tr><td>'.$this->Form->input('amharic_first_name',array('label'=>'First name in Amharic ',
		             array('id'=>'AmharicText',
		             'onkeypress'=>"return AmharicPhoneticKeyPress(event,this);"))).'</td></tr>';  
		             
		            
		          } else {
		                echo "<tr><td style='padding-left:95px;'> First Name: ".$this->data['Student']['amharic_first_name']."</td></tr>";
		          } 
		          if (empty($this->data['Student']['amharic_middle_name'])) {
		               echo '<tr><td>'.$this->Form->input('amharic_middle_name',array('label'=>'Middle name in  Amharic',
		               array('id'=>'AmharicTextMiddleName','onkeypress'=>"return AmharicPhoneticKeyPress(event,this);")))
		               .'</td></tr>';
		          } else {
		                echo "<tr><td style='padding-left:95px;'> Middle Name: ".$this->data['Student']['amharic_middle_name']."</td></tr>";
		          } 
		          
		          if (empty($this->data['Student']['amharic_last_name'])) {
		              
		                echo '<tr><td>'.$this->Form->input('amharic_last_name',array('label'=>'Last name in  Amharic',
		                array('id'=>'AmharicTextLastName','onkeypress'=>"return AmharicPhoneticKeyPress(event,this);"))).'</td></tr>';
		          } else {
		                echo "<tr><td style='padding-left:95px;'> Middle Name: ".$this->data['Student']['amharic_last_name']."</td></tr>";
		          } 
		          
		          echo "<tr><td> ".$this->Html->link(__('Change Name', true),
		           array('action' => 'name_change', $this->data['Student']['id']))."</td></tr>";
		         
		       
		       
		         
		        echo '<tr><td>'. $this->Form->input('estimated_grad_date',array(
		         'minYear' => date('Y'), 'maxYear' =>date('Y')+Configure::read('Calendar.expectedGraduationInFuture'), 'orderYear' => 'desc'
		        )).'</td></tr>';
		        echo '<tr><td>'. $this->Form->input('gender',array('label'=>'Gender','type'=>'select',
		        ...
		        // echo '<tr><td>'. $this->Form->input('studentnumber',array('label'=>'Student Identification Number')).'</td></tr>';
		         echo '<tr><td style="padding-left:95px;"> Student Number: <strong>'.$this->data['Student']['studentnumber'].'</strong></td></tr>';
		         
		         //$this->Form->input('studentnumber',array('label'=>'Student Identification Number'))
		        ...
		        echo '<tr><td>'.$this->Form->input('birthdate',array('label'=>'Birth date',
		        'minYear' => date('Y')-Configure::read('Calendar.birthdayInPast'), 'maxYear' => date('Y')-14, 'orderYear' => 'desc')).'</td></tr>';
		        // echo $this->Form->input('order_date_from', array('label' => false, 'type' => 'date', 'dateFormat' => 'MDY', 'minYear' => Configure::read('Calendar.applicationStartYear'), 'maxYear' => date('Y'), 'orderYear' => 'desc'));
                    ...
                    foreach($this->data['Attachment'] as $ak=>$av){
                       ...
                       if(!empty($av['dirname']) && !empty($av['basename']) && strcasecmp($av['dirname'],'img')==0){
                          ...
                          // echo $media->embed($media->file($av['dirname'].DS.$av['basename']));
                           ...
                           echo '<td>'.$this->Html->link(__('Delete Picture', true), array('controller'=>'attachments','action' => 'delete', $av['id'],$action_controller_id), null, sprintf(__('Are you sure you want to delete picture ?', true))).'</td>';
                           echo '</tr>';
		       ...
		       // echo '<tr><td>'.$this->data['User']['password'].'</td></tr>';
	  ...
	  <div id="education_background" style="display:none">	
		...
		<INPUT type="button" value="Delete Row" onclick="deleteRow('high_school_education')" />
			...
			<?php 
		        ...
		        ?>
		          <table><tr><td colspan=6>
		<INPUT type="button" value="Add Row" onclick="addRow('higher_education_background','HigherEducationBackground',6,'<?php echo  $higher_all_fields; ?>')" />
		<INPUT type="button" value="Delete Row" onclick="deleteRow('higher_education_background')" />
		        ...
		        echo "<table>";
                   ...
                   $yearoptions=array();
					...
					$this->data['EheeceResult'][$bk]['exam_year']:''))."</td></tr>";
		...
		<INPUT type="button" value="Delete Row" onclick="deleteRow('eheece_result')" />
					...
					$this->data['EslceResult'][$bk]['exam_year']:''))."</td></tr>";
