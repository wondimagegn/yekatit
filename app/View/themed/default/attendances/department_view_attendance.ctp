...
$(document).ready(function () {
	$("#PublishedCourse").change(function(){
		...
		window.location.replace("/attendances/<?php echo $this->action; ?>/"+(pc_id != 0 ? pc_id : ''));
	});
});	
...
<div class="attendances <?php echo $this->action; ?>" style="margin-bottom:0px">
<?php echo $this->Form->create('Attendance');?>
<div class="smallheading"><?php __('View instructor\'s attendance');?></div>
...
<table class="fs14" style="margin-bottom:0px">
	...
	<tr>
		<td colspan="4">
		<?php echo $this->Form->submit(__('List Published Courses', true), array('name' => 'listPublishedCourses', 'div' => false)); ?>
		</td>
	</tr>
</table>
</div>
...
<?php
echo $this->Form->submit(__('View Attendance', true), array('name' => 'viewCourseAttendance', 'div' => false));
}
?>
