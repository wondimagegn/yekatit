...
<div class="smallheading"><?php __('Manual Student Assignment and Cancellation'); ?></div>
...
<table cellpadding="0" cellspacing="0">
	...
	<?php 
		if(!empty($selected_meal_hall_id) && !empty($selected_academicyear)){
		   ...
		   echo '<tr><td colspan="12" style="text-align: center;" id="ajax_student">'.$this->Js->link(__('Add', true),'/meal_hall_assignments/add_student_meal_hall/'.$selected_meal_hall_id.'~'.$selected_program_type_id.'~'.$selected_college_id.'~'.$selected_department_id.'~'. $selected_year_level_id.'~'.$formatted_academicyear, array('update'=>'#ajax_student','evalScripts'=>true)).'</td></tr>';
		}
	?>
	<tr>
	...
	<tr<?php echo $class;?>>
		...
		<td><?php echo $this->Format->short_date($mealHallAssignment['MealHallAssignment']['created']); ?>&nbsp;</td>
		 <td class="actions">
			<!-- <?php echo $this->Html->link(__('View', true), array('action' => 'view', $mealHallAssignment['MealHallAssignment']['id'])); ?>
			<?php echo $this->Html->link(__('Edit', true), array('action' => 'edit', $mealHallAssignment['MealHallAssignment']['id'])); ?> -->
			<?php echo $this->Html->link(__('Delete', true), array('action' => 'delete', $mealHallAssignment['MealHallAssignment']['id']), null, sprintf(__('Are you sure you want to delete  %s?', true), $mealHallAssignment['Student']['full_name'])); ?>
		</td> 
	</tr>
<?php endforeach; ?>
	</table>
