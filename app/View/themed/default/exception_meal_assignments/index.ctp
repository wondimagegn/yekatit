...
<?php if (isset($exceptionMealAssignments) && !empty($exceptionMealAssignments)) { ?>
	<h2><?php __('Exception Meal Assignments');?></h2>
	...
	<tr<?php echo $class;?>>
		...
		<td class="actions">
			...
			<?php echo $this->Html->link(__('Delete', true), array('action' => 'delete', $exceptionMealAssignment['ExceptionMealAssignment']['id']), null, sprintf(__('Are you sure you want to delete  %s is from exception meal ?', true), $exceptionMealAssignment['Student']['full_name'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
