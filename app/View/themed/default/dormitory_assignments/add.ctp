...
<div class="smallheading"><?php __('Students Dormitory Assignment'); ?></div>
...
<table style="border: #CCC dashed 1px">
 <?php echo '<tr><td colspan="2" class="smallheading">Dormitory unassigned students Lists</td></tr>';
 	...
 	foreach($admitted_unassigned_students as $admitted_unassigned_student){
		echo "<tr><td>".$count++."</td>";
 		echo "<td>".$this->Form->checkbox('Student.Selected.'.$admitted_unassigned_student['Student']['id']). "&nbsp;&nbsp;&nbsp;".$admitted_unassigned_student['Student']['full_name'].' ( ID:'.$admitted_unassigned_student['Student']['studentnumber'].')'."</td></tr>";
 	}
 	
 	foreach($non_admitted_unassigned_students as $non_admitted_unassigned_student){
 		 echo "<tr><td>".$count++."</td>";
 		 echo "<td>".$this->Form->checkbox('AcceptedStudent.Selected.'.$non_admitted_unassigned_student['AcceptedStudent']['id']). "&nbsp;&nbsp;&nbsp;".$non_admitted_unassigned_student['AcceptedStudent']['full_name'].' ( ID:'.$non_admitted_unassigned_student['AcceptedStudent']['studentnumber'].')'."</td></tr>";
...
<?php 	
       ...
       echo '<tr><td>'. $this->Form->Submit('Assign',array('name'=>'assign','div'=>false, 'onClick'=>'return confirmDormitoriesAssignment()')).'</td></tr>';
?>
</table>
</td></tr></table>
