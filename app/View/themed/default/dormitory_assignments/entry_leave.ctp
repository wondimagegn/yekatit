...
<?php echo $this->Form->create('DormitoryAssignment');?>
	<div class="smallheading"><?php __('students Entry/Leave Management');?></div>
	...
	<?php 
        ...
        echo '<td class="font">'.$this->Form->input('Search.dormitory',array('label' => false, 'id'=>'ajax_dormitory','type'=>'select', 'style'=>'width:150px','options'=>$dormitories, 'empty'=>"All",'selected'=>isset($this->data['Search']['dormitory']) ? $this->data['Search']['dormitory'] : '' )).'</td></tr>';
        		
		echo '<tr><td colspan="4">'.$this->Form->Submit('Search',array('name'=>'search','div'=>false)).'</td></tr>'; 
	?>
	</table>
	<?php } ?>
...
?>
	...
	<tr<?php echo $class;?>>
		...
		<!-- <td class="actions">
			...
			<?php echo $this->Html->link(__('Delete', true), array('action' => 'delete', $dormitoryAssignment['DormitoryAssignment']['id']), null, sprintf(__('Are you sure you want to delete # %s?', true), $dormitoryAssignment['DormitoryAssignment']['id'])); ?>
		</td> -->
	</tr>
<?php endforeach; 
 echo '<tr><td colspan="11">'. $this->Form->Submit('Update Received Date/Leave Date',array('name'=>'update','div'=>false)).'</td></tr>'; 
?>
	</table>
	<p>
	<?php
