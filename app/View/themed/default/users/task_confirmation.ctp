<div class="smallheading">Task Confirmation</div>
<div>
	...
	foreach($task_confirmation_request_status as $value) {
		...
		<tr>
			...
			if($value['Vote']['confirmation'] == 0 && $value['Vote']['created'] >= $valid_date_from) {
				echo $this->Html->link(__('Cancel', true), array('action' => 'cancel_task_confirmation', $value['Vote']['id']), null, sprintf(__('Are you sure you want to cancel "%s" request for "'.$value['ApplicableOn']['first_name'].' '.$value['ApplicableOn']['middle_name'].' '.$value['ApplicableOn']['last_name'].' ('.$value['ApplicableOn']['username'].')"?', true), $value['Vote']['task']));
			}
			else
				echo '---';
			?>
...
if(!empty($tasks_for_confirmation)) {
	...
	foreach($tasks_for_confirmation as $value) {
		...
		<tr>
			...
			$valid_date_from = date("Y-m-d H:i:s", mktime(date("H")-72, date("i"), date("s"), date("n"), date("j"), date("Y")));
			if($value['Vote']['confirmation'] == 0 && $value['Vote']['created'] >= $valid_date_from) {
				echo $this->Html->link(__('Accept', true), array('action' => 'confirm_task', $value['Vote']['id']), null, sprintf(__('Are you sure you want to accept "%s" request for "'.$value['ApplicableOn']['first_name'].' '.$value['ApplicableOn']['middle_name'].' '.$value['ApplicableOn']['last_name'].' ('.$value['ApplicableOn']['username'].')"?', true), $value['Vote']['task']));
			}
			else
				echo '---';
			?>
