...
echo "</table>";
	 ...
	 <td>  <table>
       ...
       <table>	
        ...
        ?> 
		   <?php __('Course Exemption Request Waiting Decision',true); ?>
		    <?php 
		    } else if ($role_id ==ROLE_REGISTRAR) {
		    ?>
		       <?php 
		       __('Course Exemption Approved by Department waiting registrar confirmation.',true); 
		       ?>
		        <?php 
		    }
		?>
	...
	<?php
		...
		echo '<tr><td>Request Date : <strong>'.$this->Format->humanize_date($this->data['CourseExemption']['request_date']).'</strong></td></tr>';
		...
		echo '<tr><td>Course Title:<strong>'.$this->data['Course']['course_title'].'</strong></td></tr>';
		echo '<tr><td>Course Code:<strong>'.$this->data['Course']['course_code'].'</strong></td></tr>';
		echo '<tr><td>Course Credit:<strong>'.$this->data['Course']['credit'].'</strong></td></tr>';
		echo '<tr><td colspan=2><strong> By </strong></td></tr>';
		echo '<tr><td>Course Title:<strong>'.$this->data['CourseExemption']['taken_course_title'].'</strong></td></tr>';
		echo '<tr><td>Course Code:<strong>'.$this->data['CourseExemption']['taken_course_code'].'</strong></td></tr>';
		echo '<tr><td> Course Credit:<strong>'.$this->data['CourseExemption']['course_taken_credit'].'</strong></td></tr>';
		  
			if (isset($this->data['Attachment']) && count($this->data['Attachment'])>0) { 
			    echo '<tr><td>';
			    echo 'PDF file uploaded on: '.$this->Format->humanize_date($this->data['Attachment'][0]['created']). '<br/> '; 
			  echo "<a href=".$this->Media->url($this->data['Attachment'][0]['dirname'].DS.$this->data['Attachment'][0]['basename'],true)." target=_blank'>View Attachment</a>";
		      echo '</td></tr>';
		    }
