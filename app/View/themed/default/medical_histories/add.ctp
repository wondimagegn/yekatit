...
<?php echo $this->Form->create('MedicalHistory');?>
	<div class="smallheading"><?php __('Add Student Medical Histories');?></div>
	<div class="font"><?php __('Search parameters');?> </div>
...
?>
	...
	<tr>
	    ...
	    <th style="border-right: #CCC solid 1px" class="actions"><?php __('Actions');?></th> 
	</tr>
	<tr>
		...
		<td style="border-right: #CCC solid 1px" class="actions">
			<?php echo $this->Html->link(__('View', true), array('controller'=>'students','action' => 'view', $students['Student']['id'])); ?>
		</td> 
	</tr>
<?php
?>
...
<?php 
	if(isset($student_id)){
		...
		echo '<tr><td colspan="2">'.$this->Form->Submit('Submit',array('name'=>'submit','div'=>false, 'onClick' => 'return confirmCardNumberchange()')).'</td></tr>'; 
	}
?>
	</table>
			...
			<tr>
				...
				<td style="border-right: #CCC solid 1px" class="actions">
					...
					if($user_id == $medicalHistory['MedicalHistory']['user_id'] && $deadline >= date('Y-m-d H:i:s')){
						echo $this->Html->link(__('Edit', true), array('action' => 'edit', $medicalHistory['MedicalHistory']['id'])); 
					}
					?>
				</td> 
			</tr>
