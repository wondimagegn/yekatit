...
/*
	function mass_add () {
	    ...
	    if (!empty($this->data) && isset($this->params['form']['continue'])) {
		    ...
		    switch($this->data) {
			        ...
			        case empty($this->data['Student']['department_id']) :
			         $this->Session->setFlash('<span></span> '.__('Please select department you want to add courses for mass students.', true),'default',array('class'=>'error-box error-message'));  
			         break; 
			        case empty($this->data['Student']['academic_year']) :
			         $this->Session->setFlash('<span></span> '.__('Please select academic year you  want to add  courses for mass students.', true),'default',array('class'=>'error-box error-message'));  
			         break; 
			          case empty($this->data['Student']['year_level_id']) :
			         $this->Session->setFlash('<span></span> '.__('Please select year level you  want to add  courses for mass students.', true),'default',array('class'=>'error-box error-message'));  
			         break; 
			        case empty($this->data['Student']['program_id']) :
			         $this->Session->setFlash('<span></span> '.__('Please select the program you want to add courses for mass students. Please, try again.', true),'default',array('class'=>'error-box error-message'));  
			         break; 
			         case empty($this->data['Student']['program_type_id']) :
			         $this->Session->setFlash('<span></span> '.__('Please select the program type you want to add courses for mass students. Please, try again.', true),'default',array('class'=>'error-box error-message'));  
			         break;    
	      ...
	      if ($everythingfine) {
	         ...
	         $this->data['Student']['program_type_id']);
	         
	         //debug($this->data['Student']['semester']);
	         ...
	         'conditions'=>array('PublishedCourse.department_id'=>$this->data['Student']['department_id'],
	         ...
	         'PublishedCourse.academic_year like '=>$this->data['Student']['academic_year'].'%'
	         ),
	         ...
	         $program=$this->CourseAdd->PublishedCourse->Program->field('name',array('id'=>$this->data['Student']['program_id']));
	         $programType=$this->CourseAdd->PublishedCourse->ProgramType->field('name',array('id'=>$this->data['Student']['program_type_id']));
	        if(empty($published_courses)) {
	         ...
	         'tutorial_hours','credit'))));
	             ...
	             if(!empty($section_ids)) {
	              ...
	              array('conditions'=>array('Student.department_id'=>$this->data['Student']['department_id'],
	              'Student.id'=>$student_index,'Student.program_type_id'=>$program_type_id,'Student.program_id'=>$this->data['Student']['program_id'],
		        ...
		        $this->Session->setFlash('<span></span>'.__('There is no section who need mass add for the selected criteria.', true),'default',array('class'=>'error-box error-message'));
		        } else {
	    ...
	    if (!empty($this->data) && isset($this->params['form']['massadd'])) {
	          
	           //check for duplicate entry
	                $already_added_courses=array();
	                $selected_courses_add=array();
	                unset($this->data['Student']);
	                $count=0;
	                foreach ($this->data['CourseAdd'] as $cdd=>$cdv) {
		           ...
		           if (count($already_added_courses)==count($this->data['CourseAdd'])) {
		                  $this->Session->setFlash('<span></span>'.__('All the selected courses has already  added for the selected sections. You do not need to add it again', true),'default',array('class'=>'error-box error-message'));
	                     $this->redirect(array('action'=>'index'));
		            } else {
		                  unset($this->data);
		                  $this->data=$already_added_courses;
		            }
			       //saveAll
			       $this->set($this->data);
			           ...
			           array('validate'=>'first'))) {
				                $this->Session->setFlash('<span></span>'.__('The course add has been saved', true),'default',
				                ...
				                $this->Session->setFlash('<span></span>'.__('The course add could not be saved. Please, try again.', true),
				                'default',array('class'=>'error-box error-message'));
			           }
