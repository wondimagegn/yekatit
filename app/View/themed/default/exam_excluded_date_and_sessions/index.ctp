...
<?php echo $this->Form->create('ExamExcludedDateAndSession');?>
	<div class="smallheading"><?php __('Exam Excluded Date And Sessions');?></div>
...
<div class="smallheading"><?php echo $examPeriod['YearLevel']['name'] .' Year Exam Periods from '. $this->Format->short_date($examPeriod['ExamPeriod']['start_date']).' to '.$this->Format->short_date($examPeriod['ExamPeriod']['end_date']).' Exam Excluded Date and Sessions are:'; ?></div>
	...
	<tr>
		...
		<td class="actions">
			<?php echo $this->Html->link(__('Delete', true), array('action' => 'delete', $examExcludedDateAndSession['ExamExcludedDateAndSession']['id']), null, sprintf(__('Are you sure you want to delete?', true), $examExcludedDateAndSession['ExamExcludedDateAndSession']['id'])); ?>
		</td>
	</tr>
<?php }?>
	</table>
