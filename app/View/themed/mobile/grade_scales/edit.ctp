...
<?php echo $this->Form->create('GradeScale');?>
	<fieldset>
		<legend class="smallheading"><?php __('Set Grade Scale'); ?></legend>
	<table>
	  <td><table>
	    <?php
	
		   /* echo '<tr><td>'.$this->Form->input('grade_type_id',array('id'=>'grade_type_id',
		    'onchange' => 'updateSubCategory(1)')).'</td></tr>';*/
		    
		    echo $this->Form->hidden('id',array('value'=>$this->data['GradeScale']['id']));
	        ...
	        <table id="grade_scale_details">
	            <tr><th>S.No</th><th>Grade</th><th>Minimum Result</th><th>Maximum Result</th><th>Action</th></tr>
		            ...
		            array( 'value'=>isset($this->data['GradeScaleDetail'][$bk]['maximum_result'])?$this->data['GradeScaleDetail'][$bk]['maximum_result']:'','label'=>false,'div'=>false,'size'=>4));
		                    echo "</td><td>";
		                     echo $this->Html->link(__('Delete', true), array('controller' => 'grade_scale_details', 'action' => 'delete', $bv['grade_id']), null, sprintf(__('Are you sure you want to delete # %s?', true), $bv['grade_id']));  
		                      echo "</td></tr>";
		                      $count++;
		                  }
		         } else {
