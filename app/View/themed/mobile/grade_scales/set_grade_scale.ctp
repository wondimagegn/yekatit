...
<strong> Important Note: </strong> 
 This tool will help you to set grade scale for those department whose grade scale not delegated.
</p>
...
<table class="fs13 small_padding">
		...
		<tr>
			...
			array('name' => 'continue',  'div' => false)); ?></td>
		</tr>
	</table>
</div>	
<?php 
...
<?php 
    if (isset($turn_off_search)) {
        if(isset($onlyfresh) && !empty($onlyfresh)) {
            echo '<div class="info-box info-message">';
            echo "<span></span>You have delegated scale setting to departments for both undergraduate 
            and post graduate program but you can define scale for non department assigned or pre/freshman program.";
            
            echo '</div>';
        }
?>	
	<div class="smallheading"><?php __('Set Grade Scale'); ?></div>
	<table>
	  <td><table>
	    <?php
	
		    echo '<tr><td>';
		    echo $this->Form->input('name',array('label'=>'Scale Name'));
		    if ($role_id == ROLE_REGISTRAR ) {  
		        echo $this->Form->hidden('Search.college_id',array('value'=>$college_id));
		    }
	        ...
	        <table id="grade_scale_details">
	            <tr><th>S.No</th><th>Grade</th><th>Minimum Result</th><th>Maximum Result</th></tr>
	                ...
	                <?php 
	                   ...
	                   if (!empty($this->data['GradeScaleDetail'])) {
		                    $count=1;
		                    
		                      foreach ($this->data['GradeScaleDetail'] as $bk=>$bv) {
		                      echo "<tr><td>".$count."</td><td>".$this->Form->input('GradeScaleDetail.'.$bk.'.grade_id',
		            array(
		            'options'=>$grades,'type'=>'select','label'=>false,'selected'=>!empty($this->data['GradeScaleDetail'][$bk]['grade_id'])?$this->data['GradeScaleDetail'][$bk]['grade_id'] :'','id'=>"grade_id_".$count))."</td><td>".$this->Form->input('GradeScaleDetail.'.$bk.'.minimum_result',
		            array(
		            'value'=>isset($this->data['GradeScaleDetail'][$bk]['minimum_result'])?$this->data['GradeScaleDetail'][$bk]['minimum_result']:'','label'=>false,'div'=>false,'size'=>4))."</td><td>".$this->Form->input('GradeScaleDetail.'.$bk.'.maximum_result',
		            array( 'value'=>isset($this->data['GradeScaleDetail'][$bk]['maximum_result'])?$this->data['GradeScaleDetail'][$bk]['maximum_result']:'','label'=>false,'div'=>false,'size'=>4));
		                      echo "</td></tr>";
		                      $count++;
		                  }
		         } else {
		            ...
		            array(
		            'options'=>$grades,'type'=>'select','label'=>false,'selected'=>!empty($this->data['GradeScaleDetail'][0]['grade_id'])? $this->data['GradeScaleDetail'][0]['grade_id']:'','id'=>'grade_id_1'))."</td><td>".$this->Form->input('GradeScaleDetail.0.minimum_result',
		            array(
		            'value'=>isset($this->data['GradeScaleDetail'][0]['minimum_result'])?$this->data['GradeScaleDetail'][0]['minimum_result']:'','label'=>false,'size'=>4))."</td><td>".$this->Form->input('GradeScaleDetail.0.maximum_result',
		            array( 'value'=>isset($this->data['GradeScaleDetail'][0]['maximum_result'])?$this->data['GradeScaleDetail'][0]['maximum_result']:'','label'=>false,'size'=>4));
		                      echo "</td></tr>";
...
<?php 
 echo $this->Form->submit(__('Submit', true), 
			array('name' => 'saveIt',  'div' => false)); ?>
			
<?php 
}
?>
<div class="gradeScales index">
		<h2><?php __('Already recorded grade scales');?></h2>
	...
	<tr<?php echo $class;?>>
		...
		<td class="actions">
		    <?php 
		        $action_controller_id='set_grade_scale~gradeScales';
			...
			<?php 
			 ...
			 echo $this->Html->link(__('Delete', true), array('action' => 'delete', $gradeScale['GradeScale']['id'],
			  $action_controller_id), null, sprintf(__('Are you sure you want to delete # %s?', true), 
			  $gradeScale['GradeScale']['id']));
			 
			 ?> 
			<?php 
			/* echo $this->Html->link(__('Delete', true), array('action' => 'delete', $gradeScale['GradeScale']['id'],$action_controller_id), null, sprintf(__('Are you sure you want to delete # %s?', true), $gradeScale['GradeScale']['id']));
			*/
			 ?>
