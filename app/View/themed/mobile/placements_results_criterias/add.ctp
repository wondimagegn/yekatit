...
<!---
 ...
 /*
  $(function() {
    ...
    $("#result_to").observe(function (element) {
        // Your callback code, gets called when the field's value changes
         //alert('Change observed! new value: ' + this.value );
        $('#result_to_count').load('/acceptedStudents/count_result/'+$(this).val()+
        "/<?php echo $this->data['PlacementsResultsCriteria']['prepartory_result'];?>");
        
    });
    $("#result_from").observe(function (element) {
        // Your callback code, gets called when the field's value changes
           $('#result_from_count').load('/acceptedStudents/count_result/'+$(this).val());
    });
...
$check_auto_placement_already_run_not_allow_adding_or_edit>0) {
 ...
 'name'=>'addresultcategory'));
    ...
    // TODO:graph
                ...
                //echo '<div class="smallheading">Result Category Graph</div>';
            
                //echo "<img src='/placements_results_criterias/result_category_graph'></img>"; 
...
$check_auto_placement_already_run_not_allow_adding_or_edit>0) {
                         ...
                         echo $this->Html->link(__('Delete', true), array('action' => ''),array('onclick'=>'return false',
			           'style'=>'color:gray')); 
                    } else {
                       echo $this->Html->link(__('Delete', true), array('action' => 'delete', $v['PlacementsResultsCriteria']['id'],
					$academicyear,$result_type), 
			null, sprintf(__('Are you sure you want to delete \'%s\'?', true), $v['PlacementsResultsCriteria']['name'])); 
                    }
