...
$(document).ready(function () {
	$("#Section").change(function(){
		...
		window.location.replace("/students/<?php echo $this->action; ?>/"+s_id+"/"+$("#SemesterSelected").val());
	    */
	    
	    var s_id = $("#Section").val();
		window.location.replace("/students/<?php echo $this->action; ?>/"+s_id);
...
<div class="smallheading"><?php __('Issue/Reset Student Password');?></div>
<div onclick="toggleViewFullId('ListSection')"><?php 
	if (!empty($sections)) {
		echo $html->image('plus2.gif', array('id' => 'ListSectionImg')); 
		?><span style="font-size:10px; vertical-align:top; font-weight:bold" id="ListSectionTxt">Display Filter</span><?php
		}
	else {
		echo $html->image('minus2.gif', array('id' => 'ListSectionImg')); 
...
<table cellspacing="0" cellpadding="0" class="fs14">
	...
	<tr>
		<td colspan="6">
		<?php echo $this->Form->submit(__('Get Students', true), array('name' => 'listSections', 'div' => false)); ?>
		</td>
	</tr>
</table>
</div>
...
if(isset($students_in_section) && empty($students_in_section)) {
	...
	?>
	<p class="fs13">Please select student/s for whom you want to issue/reset password.
	The password should be changed during first login by student.</p>
	<table>
		<tr>
			<th style="width:10%"><?php echo $this->Form->input('select_all', 
			array('type' => 'checkbox', 'id' => 'SelectAll', 'div' => false,
			 'label' => false, 'onchange' => 'check_uncheck(this.id)')); ?>Select All</th>
	...
	<?php echo $this->Form->submit(__('Get Student Password', true), array('name' => 'issueStudentPassword', 
	'div' => false)); ?>
	<?php
