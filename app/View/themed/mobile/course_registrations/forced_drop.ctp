               ...
               // echo $this->Form->hidden('CourseRegistration.'.$count.'.course_id',array('value'=>$pv['course_id']));
                ...
                // echo "<td>".$form->checkbox('CourseRegistration.drop.' . $pv['id'])."</td>"; 
                 ...
                 echo "<tr class='linethough'><td>".++$count."</td><td>".$form->checkbox('CourseRegistration.drop.' . $pv['CourseRegistration']['id'],array('disabled'=>in_array($pv['CourseRegistration']['id'],$already_dropped)?true:false))."</td><td>".$pv['PublishedCourse']['Course']['course_title']."</td>";
                   ...
                   echo "<tr><td>".++$count."</td><td>".$form->checkbox('CourseRegistration.drop.' . $pv['CourseRegistration']['id'])."</td><td>".$pv['PublishedCourse']['Course']['course_title']."</td>";
                 }
 ...
 // debug($students);
  ...
  <table cellpadding="0" cellspacing="0">
	...
	<tr<?php echo $class;?>>
		...
		<td class="actions">
			<?php echo $this->Html->link(__('Drop Course', true), array('action' => 'add', $student['Student']['id'])); ?>
