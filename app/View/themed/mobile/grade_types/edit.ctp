...
<div class="gradeTypes form">
  <?php 
    ...
    ?>
		...
		<div  class="info-box info-message"><span></span><?php __('The grade type can not be editted because it is attached to courses.'); ?></div>
    <table>
	<tr>
	...
	<td style="width:5%">
	<?php
	  if ($this->data['GradeType']['used_in_gpa']==1) {
	    echo 'Yes';
	  } else {
	    echo 'No';
	  }
	...
	<?php
	  ...
	  if ($this->data['GradeType']['scale_required']==1) {
	    echo 'Yes';
	  } else {
	    echo 'No';
	  }
	...
	<td>
	  ...
	  Pass Grade</th></tr>
	               ...
	               if (!empty($this->data['Grade'])) {
		                    $count=1;
		                      foreach ($this->data['Grade'] as $bk=>$bv) {
  ...
  ?>
		...
		<div  class="smallheading"><?php __('Edit Grade Type'); ?></div>
	<table>
	...
	<td>
	  <table id="grade"><tr><th>S.No</th><th>Grade</th><th>Point Value</th><th>Pass Grade</th><th>Action</th></tr>
		                    ...
		                    $count=1;
		                      foreach ($this->data['Grade'] as $bk=>$bv) {
		                       ...
		                       if (isset($bv['id']) && !empty($bv['id'])) {
		                            ...
		                            array('value'=>isset($this->data['Grade'][$bk]['id']) && !empty($this->data['Grade'][$bk]['id'])?$this->data['Grade'][$bk]['id']:''));
		            ...
		            array(
		            'value'=>isset($this->data['Grade'][$bk]['grade'])?$this->data['Grade'][$bk]['grade']:'','label'=>false,'div'=>false,'size'=>4)).
		            "</td><td>".$this->Form->input('Grade.'.$bk.'.point_value',
		            array(
		            'value'=>isset($this->data['Grade'][$bk]['point_value'])?$this->data['Grade'][$bk]['point_value']:'','label'=>false,'div'=>false,'size'=>4))."</td><td>".$this->Form->input('Grade.'.$bk.'.pass_grade',
		            array( 'value'=>isset($this->data['Grade'][$bk]['pass_grade'])?$this->data['Grade'][$bk]['pass_grade']:'','label'=>false,'div'=>false)).'</td>';
		                      echo "<td>".$this->Html->link(__('Delete', true), array('controller'=>'grades','action' => 'delete', $bv['id'], $action_controller_id), null, sprintf(__('Are you sure you want to delete # %s?', true), $bv['id'])); 
		                      echo "</td></tr>";
		                      $count++;
		                  }
		         } else {
		                if (isset($this->data['Grade'][0]['id']) && !empty($this->data['Grade'][0]['id'])) {
		                   echo $this->Form->hidden('Grade.0.id');
		                }
		                echo "<tr><td>1</td><td>".$this->Form->input('Grade.0.grade',
		            array(
		            'value'=>isset($this->data['Grade'][0]['grade'])?$this->data['Grade'][0]['grade']:'','label'=>false,'div'=>false,'size'=>4)).
		            "</td><td>".$this->Form->input('Grade.0.point_value',
		            array(
		            'value'=>isset($this->data['Grade'][0]['point_value'])?$this->data['Grade'][0]['point_value']:'','label'=>false,'div'=>false,'size'=>4))."</td><td>".$this->Form->input('Grade.0.pass_grade',
		            array( 'value'=>isset($this->data['Grade'][0]['pass_grade'])?$this->data['Grade'][0]['pass_grade']:'','label'=>false,'div'=>false));
		                      echo "</td></tr>";
		         }
		         ?>
