...
<div class="smallheading"><?php __('Apply Readmission On Behalf Student '); ?></div>
...
<table cellspacing="0" cellpadding="0" class="fs13">
	...
	<tr>
		<td colspan="6">
		<?php echo $this->Form->submit(__('List Students', true), array('name' => 'listStudentsForReadmission', 
		'div' => false)); ?>
		</td>
	</tr>
</table>
...
foreach($students_for_readmission_list as $c_id => $students) {
	...
	foreach($students as $key => $student) {
		...
		<tr style="color:<?php echo (empty($student['criteria']['error']) ? 'green' : 'red'); ?>">
			...
			<td style="background-color:white"><?php echo $this->Html->link(__($student['Student']['full_name'], 
			true), array('controller' => 'students', 'action' => 'view', $student['Student']['id']), 
...
echo $this->Form->submit(__('Add Student to Readmission List', true), 
array('name' => 'addStudentToReadmissionList', 'div' => false));
}
else if(isset($this->data) && empty($students_for_readmission_list)) {
