...
<div class="smallheading"><?php __('Add Class Room Course Constraints'); ?></div>
	   ...
	   echo '<td>'. $this->Form->input('class_rooms', array('id'=>'ajax_class_room', 'onchange'=>'updateconstraints()','type'=>'select','options'=>$classRooms,'selected'=>isset($selected_class_rooms)?$selected_class_rooms:"",'empty'=>'---Select Class Rooms ---')).'</td></tr>';
		...
		if(isset($classRoomCourseConstraints)) {
			...
			foreach($classRoomCourseConstraints as $classRoomCourseConstraint){
				...
				"</td><td style='border-right: #CCC solid 1px'>".
				//$this->Html->link(__('View', true), array('action' => 'view', $classRoomCourseConstraint['ClassRoomCourseConstraint']['id'])).'&nbsp;&nbsp;&nbsp;'. 
				$this->Html->link(__('Delete', true), array('action' => 'delete', $classRoomCourseConstraint['ClassRoomCourseConstraint']['id'],"fromadd"),null, sprintf(__('Are you sure you want to delete?', true), $classRoomCourseConstraint['ClassRoomCourseConstraint']['id'],"fromadd")).
				"</td></tr>";
			}
			echo "</table>";
		}
