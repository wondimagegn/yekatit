...
<p class="smallheading"><?php __('Course Schedule Cancellation'); ?> .</p>
<p class='fs16'>
    <strong>Important Note:</strong> This tool will help you to cancel auto generated schedule, the cancellation is batch cancellation, the cancellation will be possible if instructor has not started taking attendance. 
 </p>
<table cellspacing="0" cellpadding="0" class="fs14">
		...
		<tr>
		<td colspan='4'><?php echo $this->Form->Submit('Continue',array('name'=>'search','div'=>false)); 
		?></td>
		</tr>
</table>
<?php 
 if(!empty($section_course_schedule)) {
	...
	foreach($section_course_schedule as $scsk=>$scsv){
		...
		if (!empty($section_unscheduled_courses[$scsk])) {
		    echo '<tr>';
		      echo '<th colspan=4>Unscheduled Course Of '.$scsv[0]['Section']['name'].' section.</th>';
		    echo '</tr>';
		    ...
		    foreach ($section_unscheduled_courses[$scsk] as $ddd=>$ddv)  {
		                    ...
		                    echo '</td>';
		                    echo '<td>'.$this->Js->link(__('Schedule ', true),
								    '/course_schedules/manual_schedule_unscheduled/'.$ddv['UnschedulePublishedCourse']['published_course_id'],
