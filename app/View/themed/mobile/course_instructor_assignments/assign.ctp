...
<p class="smallheading"><?php __('Instructor Assignment For Course'); ?> .</p>
<table cellspacing="0" cellpadding="0" class="fs14">
		...
		<tr>
		<td colspan='4'><?php echo $this->Form->Submit('Search',array('name'=>'search','div'=>false)); 
		?></td>
		</tr>
</table>
...
<table cellpadding="0" cellspacing="0">
	<?php 
      /* echo '<tr><td colspan=2>'.$this->Form->input('academicyear',array('label' => 'Academic Year','type'=>'select',
		'options'=>$acyear_array_data,'empty'=>"--Select Academic Year--")).'</td></tr>';
        echo '<tr><td width="250PX">'. $this->Form->input('program_id',array('empty'=>"--Select Program--")).'</td>'; 
        echo '<td width="400PX">'. $this->Form->input('program_type_id',array('empty'=>"--Select Program Type--")).'</td></tr>'; 
        if(ROLE_COLLEGE != $role_id ) {  
            echo '<td width="250PX">'. $this->Form->input('year_level_id',array('empty'=>"--Selecte year Level--")).'</td>'; 
          	echo '<td>'.$this->Form->input('semester',array('options'=>array('I'=>'I','II'=>'II',
            'III'=>'III'),'empty'=>'--select semester--')).'</td></tr>'; 
        } else {
		echo '<td colspan=2>'.$this->Form->input('semester',array('options'=>array('I'=>'I','II'=>'II',
            'III'=>'III'),'empty'=>'--select semester--')).'</td></tr>'; 
           }
        echo '<tr><td colspan="2">'. $this->Form->Submit('Search',array('name'=>'search','div'=>false)).'</td></tr>'; 
	*/
	?> 
</table>
<?php 
		...
		foreach($sections_array as $sak=>$sav){
			...
			foreach($sav as $sk=>$sv){
				...
				if(is_string($sk)){
					 ...
					 //echo '<tr><td colspan="5" style="text-align: center;"> Assigned Instructor(S)</td></tr>';
					...
					if($sv['grade_submitted'] == 0){
						...
						/*echo $this->Form->hidden('CourseInstructorAssignment.'.$index.'.course_split_section_id',
							array('value'=>$sv['course_split_section_id']));
						echo $this->Form->hidden('CourseInstructorAssignment.'.$index.'.section_id',
							array('value'=>$sv['section_id']));
						echo $this->Form->hidden('CourseInstructorAssignment.'.$index.'.published_course_id',
							array('value'=>$sv['published_course_id']));
						echo $this->Form->hidden('CourseInstructorAssignment.'.$index.'.course_id',
							array('value'=>$sv['course_id']));
						echo $this->Form->hidden('CourseInstructorAssignment.'.$index.'.course_id',
							array('value'=>$sv['course_id']));
						echo $this->Form->hidden('CourseInstructorAssignment.'.$index.'.isprimary',
							array('value'=>1));
						echo $this->Form->hidden('CourseInstructorAssignment.'.$index.'.course_title',
							array('value'=>$sv['course_title']));
						*/
						echo '<tr><td colspan="4" id="ajax_instructor_'.$index.'"></td></tr>';
						/*
						echo '<td style="border-right: #CCC solid 1px">'.$this->Js->link('Assign', array('page' => 
							$index),array('id'=>'assign_'.$index)).'</td></tr>';
						echo $this->Js->get("#assign_".$index)->event('click', 
							$this->Js->request(array(
								'controller'=>'course_instructor_assignments',
								'action'=>'assign_instructor'
								,$index), array(
								'update'=>"#ajax_instructor_".$index,
								'async' => true,
								'method' => 'post',
						...
						echo $this->Js->get("#ajax_college_".$index)->event('change', 
							$this->Js->request(array(
								'controller'=>'course_instructor_assignments',
								'action'=>'get_department',$index), array(
								'update'=>"#ajax_department_".$index,
								'async' => true,
								'method' => 'post',
								'dataExpression'=>true,
								'data'=> $this->Js->serializeForm(array(
					...
					if($sv['grade_submitted'] == 0){
						...
						*/
						echo '<tr><td colspan="4" id="ajax_instructor_'.($index).'"></td></tr>';
						/*echo '<td style="border-right: #CCC solid 1px">'.$this->Js->link('Assign', array('page' => 
							$index+1),array('id'=>'assign_'.$index)).'</td></tr>';
						echo $this->Js->get("#assign_".$index)->event('click', 
							$this->Js->request(array(
								'controller'=>'course_instructor_assignments',
								'action'=>'assign_instructor'
					...
					echo '<tr><td colspan="2">';
					 ...
					 echo '<table style="border: #CCC dashed 2px">';
						...
						*/	
						echo '<tr><td colspan="4" id="ajax_instructor_'.$index.'"></td></tr>';
				/*
				echo '<td style="border-right: #CCC solid 1px">'.$this->Js->link('Assign', array('page' => $index),
					array('id'=>'assign_'.$index)).'</td></tr>';
				echo $this->Js->get("#assign_".$index)->event('click', 
							$this->Js->request(array(
								'controller'=>'course_instructor_assignments',
								'action'=>'assign_instructor'
