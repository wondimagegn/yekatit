...
<div class="smallheading"><?php __('Add/Edit Exam Room Number Of Invigilator'); ?></div>
...
?>
	...
	<?php 
		...
		echo "<tr><td>".$this->Form->Submit('Submit', array('name'=>'submit','div'=>false))."</td></tr>";
	...
	<?php 
		if(!empty($already_recorded_exam_room_number_of_invigilators)) {
			...
			foreach($already_recorded_exam_room_number_of_invigilators as $examRoomNumberOfInvigilator){
				...
				"</td><td style='border-right: #CCC solid 1px'>".
			 	$this->Html->link(__('Edit', true), array('action' => 'edit', $examRoomNumberOfInvigilator['ExamRoomNumberOfInvigilator']['id']))."&nbsp;&nbsp;&nbsp;".
			 	$this->Html->link(__('Delete', true), array('action' => 'delete', $examRoomNumberOfInvigilator['ExamRoomNumberOfInvigilator']['id'],'fromadd'), null, sprintf(__('Are you sure you want to delete # %s?', true), $examRoomNumberOfInvigilator['ExamRoomNumberOfInvigilator']['id'],'fromadd')).
				"</td></tr>";
			}
		}
	echo "</table>";
