...
var totalRow = <?php if(!empty($this->data)) echo (count($this->data['ExamType'])); else if(!empty($exam_types)) echo (count($exam_types)); else echo 2; ?>;
...
<div class="smallheading"><?php __('Course Exam Setup Management'); ?></div>
	...
	<tr>
		...
		'id' => 'AcadamicYear', 'label' => false, 'style' => 'width:100px',
		 'type' => 'select', 'options' => $acyear_array_data, 
		  'default'=>isset($this->data['ExamType']['acadamic_year'])?$this->data['ExamType']['acadamic_year']:
		...
		'id' => 'Semester', 'label' => false, 'style' => 'width:100px', 'type' => 'select',
		 'options' => array('I' => 'I', 'II' => 'II', 'III' => 'III'),
		  'default' => isset($this->data['ExamType']['semester'])?$this->data['ExamType']['semester']:
...
if(!empty($published_course_combo_id)) {
	...
	<table cellspacing="0" cellpadding="0" id="exam_setup" style="margin-bottom:5px">
		...
		if(empty($this->data)) {
			...
			$count = 1;
			foreach($this->data['ExamType'] as $key => $examType) {//debug($examType['mandatory']);
