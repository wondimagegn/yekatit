...
<div class="smallheading"><?php __('Add/Edit Instructor Number Of Exam Constraint'); ?></div>
...
?>
	...
	<?php 
		...
		echo "<tr><td colspan='2'>".$this->Form->Submit('Submit', array('name'=>'submit', 'div'=>false))."</td></tr>";
	...
	foreach($instructorNumberOfExamConstraints as $ineck=>$inecv){
			...
			if(isset($inecv['StaffForExam'])){
				...
				echo "<td style='border-right: #CCC solid 1px'>".$this->Html->link(__('Delete', true), array('action' => 'delete', $inecv['InstructorNumberOfExamConstraint']['id'],"fromadd"),null, sprintf(__('Are you sure you want to delete?', true), $inecv['InstructorNumberOfExamConstraint']['id'],"fromadd"))."</td></tr>";
