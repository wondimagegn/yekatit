...
<div class="actions">
	...
	<ul>
		...
		<li><?php echo $this->Html->link(__('List User Role Maps', true), array('controller' => 'user_role_maps', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New User Role Map', true), array('controller' => 'user_role_maps', 'action' => 'add')); ?> </li>
...
<div class="related">
	...
	<?php if (!empty($role['RoleModuleMap'])):?>
	...
	<?php
		$i = 0;
		foreach ($role['RoleModuleMap'] as $roleModuleMap):
			$class = null;
			if ($i++ % 2 == 0) {
				$class = ' class="altrow"';
			}
...
<div class="related">
	<h3><?php __('Related User Role Maps');?></h3>
	<?php if (!empty($role['UserRoleMap'])):?>
	<table cellpadding = "0" cellspacing = "0">
	<tr>
		<th><?php __('Id'); ?></th>
	...
	<?php
		$i = 0;
		foreach ($role['UserRoleMap'] as $userRoleMap):
			$class = null;
			if ($i++ % 2 == 0) {
				$class = ' class="altrow"';
			}
		?>
		<tr<?php echo $class;?>>
			<td><?php echo $userRoleMap['id'];?></td>
			<td><?php echo $userRoleMap['user_id'];?></td>
			<td><?php echo $userRoleMap['role_id'];?></td>
			<td class="actions">
				<?php echo $this->Html->link(__('View', true), array('controller' => 'user_role_maps', 'action' => 'view', $userRoleMap['id'])); ?>
				<?php echo $this->Html->link(__('Edit', true), array('controller' => 'user_role_maps', 'action' => 'edit', $userRoleMap['id'])); ?>
				<?php echo $this->Html->link(__('Delete', true), array('controller' => 'user_role_maps', 'action' => 'delete', $userRoleMap['id']), null, sprintf(__('Are you sure you want to delete # %s?', true), $userRoleMap['id'])); ?>
			</td>
		</tr>
	<?php endforeach; ?>
	</table>
