...
<div class="smallheading"><?php __('Add Exam Excluded Date And Session'); ?></div>
...
<?php
	if(isset($examExcludedDateAndSessions)) {
		...
		foreach($examExcludedDateAndSessions as $eedsk=>$eedsv){
			...
			$this->Html->link(__('Delete', true), array('action' => 'delete', $eedsv['ExamExcludedDateAndSession']['id'],"fromadd"),null, sprintf(__('Are you sure you want to delete # %s?', true), $eedsv['ExamExcludedDateAndSession']['id'],"fromadd")).
			"</td></tr>";
		}
		?></table><?php
	} 
