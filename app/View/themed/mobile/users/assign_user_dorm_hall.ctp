...
<?php echo $this->Form->create('User');?>
    ...
    <div class="headerfont"><?php __('Assign Responsibility'); ?></div>
...
?></td></tr>
    ...
    <?php 
        ...
        echo "<table><tr><td>Check the college the user is responsible for</td></tr><tr><td>".$this->Form->input('StaffAssigne.college_id',array('multiple' => 'checkbox', 'value' => isset($college_id) ? $college_id :'', 'selected' =>isset($this->data['StaffAssigne']['college_id'])?$this->data['StaffAssigne']['college_id']:'','div'=>false))."
                   </td></tr></table>";
    ?>
		               ...
		               if (isset($college_department[$college_id]) && count($college_department[$college_id])>0) {
                        ...
                        echo "<table><tbody>";
                         if (!empty($college_department[$college_id])){
                              ...
                              //checked=in_array($department_id,$this->data['Staff'][0]['assigned_to'] ? 'checked':'unchecked'.'
                             ...
                             // debug(in_array($department_id,$this->data['Staff'][0]['assigned_to']));
                               //debug($this->data);
                               echo '<tr><td ><input type="checkbox"  name="data[StaffAssigne][department_id][]" value='.$department_id.' id="StaffAssigneDepartmentId'.$department_id.'">'.$department_name.'</td></tr>';
                               }
                             }
