...
<div class="users form">
    ...
    <div class="smallheading"><?php __('Assign Dormitory Block Level Responsibility'); ?></div>
    ...
    <?php 
            ...
            echo '<tr><td>'.$this->Form->input('dormitory_block_id',array('type' => 'select', 
            'multiple' => 'checkbox','div'=>'input select','label'=>false)).'</td></tr>';
            echo '<tr><td>'.$this->Form->end(__('Assign', true)).'</td></tr>';
            echo '<tr>';
            echo '<td>';
                  ...
                  foreach ($alreadyAssignedBlocks as $campus=>$assignment) {
                     echo '<tr><td colspan=4 style="text-align:center">'.$campus.'</td></tr>';
                        ...
                        foreach ($assignment as $as=>$av) {
                           ...
                           echo $this->Html->link(__('Unassign', true), array('action' => 'assign_user_dorm_block', $av['User']['id'],$av['UserDormAssignment']['id']), null, sprintf(__('Are you sure you want to unassigned  %s from '.$av['DormitoryBlock']['block_name'].'('.$av['DormitoryBlock']['type'].')'.' dormitory block ?', true), $av['User']['full_name'])); 
                           echo '</td></tr>';
                        }
                  }
                  echo '</table>';
