...
<?php echo $this->Form->create('Course');?>
		...
		<legend class="smallheading"><?php __(' Edit Course '.$this->data['Course']['course_code_title'].'. '); ?></legend>
	<table>
		<tr><td>
		<?php 
		echo "<table><tbody>";
		...
		<TABLE id="prerequisite" border="1">
			<?php 
		     ...
		     if (isset($this->data['Prerequisite']) && count($this->data['Prerequisite'])>0 ) {
		                ...
		                echo "&nbsp;&nbsp;&nbsp;".$this->Html->link(__('Delete', true), array('action' => 'deleteChildren', $cv['id'],$action_model_id), null, sprintf(__('Are you sure you want to delete # %s?', true), $cv['id']));  
	...
	<table><tr><td colspan=3>
	   ...
	   <TR><td>No.</td><td style="width:125px;">ISBN</td><td style="width:125px;">Title</td><td style="width:125px">Publisher</td>
		...
		<?php 
		    if (!empty($this->data['Book'] )) {
		        $count=1;
		        foreach ($this->data['Book'] as $bk=>$bv) {
		            if (!empty($bv['id'])) {
		            ...
		            array(
		            'value'=>isset($this->data['Book'][$bk]['ISBN'])?$this->data['Book'][$bk]['ISBN']:'','size'=>13,'label'=>'','div'=>false)).'</td><td>'.
		            $this->Form->input('Book.'.$bk.'.title',
		            array(
		            'value'=>isset($this->data['Book'][$bk]['title'])?$this->data['Book'][$bk]['title']:'','size'=>13,'label'=>'','div'=>false)).'</td><td>'.
					$this->Form->input('Book.'.$bk.'.publisher',
		            array(
		            'value'=>isset($this->data['Book'][$bk]['publisher'])?$this->data['Book'][$bk]['publisher']:'','size'=>13,'label'=>'','div'=>false)).'</td><td>'.
		            $this->Form->input('Book.'.$bk.'.edition',
		            array(
		            'value'=>isset($this->data['Book'][$bk]['edition'])?$this->data['Book'][$bk]['edition']:'','size'=>13,'label'=>'','div'=>false)).'</td><td>'.
					$this->Form->input('Book.'.$bk.'.author',
		            array(
		            'value'=>isset($this->data['Book'][$bk]['author'])?$this->data['Book'][$bk]['author']:'','size'=>13,'label'=>'','div'=>false)).'</td><td>'.
		            $this->Form->input('Book.'.$bk.'.place_of_publication',
		            array(
		            'value'=>isset($this->data['Book'][$bk]['place_of_publication'])?$this->data['Book'][$bk]['place_of_publication']:'','size'=>13,'label'=>'','div'=>false)).'</td><td>'.
					$this->Form->input('Book.'.$bk.'.year_of_publication',
		            array(
		            'value'=>isset($this->data['Book'][$bk]['year_of_publication'])?$this->data['Book'][$bk]['year_of_publication']:'','type'=>'text','size'=>13,'label'=>'','div'=>false)).'</td>';
		              
		                       if(!empty($action_book_id)) {
		                
		                echo "&nbsp;&nbsp;&nbsp;".$this->Html->link(__('Delete', true), 
		                array('action' => 'deleteChildren', $bv['id'],$action_book_id), null, 
		                sprintf(__('Are you sure you want to delete # %s?', true), $bv['id']));  
	...
	<?php 
		...
		<?php 
		    if (!empty($this->data['Journal'] )) {
		        $count=1;
		        foreach ($this->data['Journal'] as $jk=>$jv) {
				
					  if (!empty($jv['id'])) {
		                       	echo $this->Form->hidden('Journal.'.$jk.'.id',
		                       	array('value'=>$this->data['Journal'][$jk]['id']));    
		                        $action_journal_id='edit~Journal~'.$jv['course_id'];
		                              
		              }
		            echo "<tr><td>".$count."</td><td>".$this->Form->input('Journal.'.$jk.'.journal_title',
		            array(
		            'value'=>isset($this->data['Journal'][$jk]['journal_title'])?$this->data['Journal'][$jk]['journal_title']:'','size'=>10,'label'=>'','div'=>false)).'</td><td>'.
		            $this->Form->input('Journal.'.$jk.'.article_title',
		            array(
		            'value'=>isset($this->data['Journal'][$jk]['article_title'])?$this->data['Journal'][$jk]['article_title']:'','size'=>10,'label'=>'','div'=>false)).'</td><td>'.
					$this->Form->input('Journal.'.$jk.'.author',
		            array(
		            'value'=>isset($this->data['Journal'][$jk]['author'])?$this->data['Journal'][$jk]['author']:'','size'=>10,'label'=>'','div'=>false)).'</td><td>'.
		            $this->Form->input('Journal.'.$jk.'.url_address',
		            array(
		            'value'=>isset($this->data['Journal'][$jk]['url_address'])?$this->data['Journal'][$jk]['url_address']:'','size'=>10,'label'=>'','div'=>false)).'</td><td>'.
					$this->Form->input('Journal.'.$jk.'.volume',
		            array(
		            'value'=>isset($this->data['Journal'][$jk]['volume'])?$this->data['Journal'][$jk]['volume']:'','size'=>10,'label'=>'','div'=>false)).'</td><td>'.
		            $this->Form->input('Journal.'.$jk.'.issue',
		            array(
		            'value'=>isset($this->data['Journal'][$jk]['issue'])?$this->data['Journal'][$jk]['issue']:'','size'=>10,'label'=>'','div'=>false)).'</td><td>'.
					$this->Form->input('Journal.'.$jk.'.page_number',
		            array(
		            'value'=>isset($this->data['Journal'][$jk]['page_number'])?$this->data['Journal'][$jk]['page_number']:'','size'=>10,'label'=>'','div'=>false)).'</td><td>'.
					$this->Form->input('Journal.'.$jk.'.ISBN',
		            array(
		            'value'=>isset($this->data['Journal'][$jk]['ISBN'])?$this->data['Journal'][$jk]['ISBN']:'','type'=>'text','size'=>10,'label'=>'','div'=>false)).'</td>';
		            
		               if(!empty($action_journal_id)) {
		                
		                echo "&nbsp;&nbsp;&nbsp;".$this->Html->link(__('Delete', true), 
		                array('action' => 'deleteChildren', $jv['id'],$action_journal_id), null, 
		                sprintf(__('Are you sure you want to delete # %s?', true), $jv['id']));  
	...
	<?php 
		...
		<?php 
		    if (!empty($this->data['Weblink'] )) {
		        $count=1;
		        foreach ($this->data['Weblink'] as $wk=>$wv) {
		            ...
		            array(
		            'value'=>isset($this->data['Weblink'][$wk]['title'])?$this->data['Weblink'][$wk]['title']:'','size'=>30,'label'=>'','div'=>false)).'</td><td>'.
		            $this->Form->input('Weblink.'.$wk.'.url_address',
		            array(
		            'value'=>isset($this->data['Weblink'][$wk]['url_address'])?$this->data['Weblink'][$wk]['url_address']:'','size'=>30,'label'=>'','div'=>false)).'</td><td>'.
					$this->Form->input('Weblink.'.$wk.'.author',
		            array(
		            'value'=>isset($this->data['Weblink'][$wk]['author'])?$this->data['Weblink'][$wk]['author']:'','size'=>30,'label'=>'','div'=>false)).'</td><td>'.
					$this->Form->input('Weblink.'.$wk.'.year',
		            array(
		            'value'=>isset($this->data['Weblink'][$wk]['year'])?$this->data['Weblink'][$wk]['year']:'','type'=>'text','size'=>30,'label'=>'','div'=>false)).'</td>';
		               ...
		               if(!empty($action_weblink_id)) {
		                    ...
		                    sprintf(__('Are you sure you want to delete # %s?', true), $wv['id']));  
