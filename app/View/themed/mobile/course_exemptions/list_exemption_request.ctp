...
<?php if (!empty($courseExemptions)) { ?>
	<div class="smallheading"><?php __('List of course exemptions request');?></div>
	...
	<tr<?php echo $class;?>>
		...
		<td><?php 
		    ...
		    /*
		echo $courseExemption['CourseExemption']['department_accept_reject'] == 1 ? 'Accepted': (!empty($courseExemption['CourseExemption']['department_approve_by']) ? 'Denied':'Waiting Department Approval')*/; ?>&nbsp;</td>
		...
		<?php 
		 ...
		 if ($courseExemption['CourseExemption']['registrar_confirm_deny']==1) {
		    ...
		    echo 'Denied';
		 } else if ($courseExemption['CourseExemption']['registrar_confirm_deny']==null) {
		        if ($courseExemption['CourseExemption']['department_accept_reject']==0) {
		            echo '--';
		        } else {
		             echo 'Waiting Registrar Confirmation';
		        }
		 }
		...
		*/
		...
		<td class="actions">
			...
			if ($role_id == ROLE_STUDENT) {
			   echo $this->Html->link(__('Cancel Request', true), array('action' => 'delete', $courseExemption['CourseExemption']['id']), null, sprintf(__('Are you sure you want to cancel # %s? course exemption request.', true),  $courseExemption['Course']['course_code_title'])); 
			}
			?>
			<?php 
			if ($role_id != ROLE_STUDENT) {
