...
<div class="smallheading"><?php __('Add Student to Senate List'); ?></div>
...
<table class="fs13">
	...
	<tr>
		<td>Approval Date:</td>
		<td><?php echo $this->Form->input('approved_date', array('label' => false, 'minYear' => $yFrom, 'maxYear' => $yTo)); ?></td>
	</tr>
</table>
<hr />
<style>
	...
	foreach($students as $key => $student) {
		...
		<tr style="color:<?php echo (empty($student['disqualification']) ? 'green' : 'red'); ?>">
			...
			?>
				...
				id="<?php echo $count; ?>"><?php echo $html->image('plus2.gif', 
			...
			<td style="background-color:white"><?php echo $this->Html->link(__($student['Student']['full_name'], true), array('controller' => 'students', 'action' => 'view', $student['Student']['id']), array('target' => '_blank', 'style' =>  'font-weight:normal; color:'.(empty($student['disqualification']) ? 'green' : 'red'))); ?></td>
...
echo $this->Form->submit(__('Add Student to Senate List', true), array('name' => 'addStudentToSenateList', 'div' => false));
}
else if(isset($this->data) && empty($students_for_senate_list)) {
