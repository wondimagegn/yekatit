...
<?php echo $this->Form->create('AcademicStand');?>
    ...
    <?php 
        if (isset($this->data['AcademicStand'])) {
            debug($this->data['AcademicStand']);
        }
    ?>
	<!-- <fieldset> -->
		<div class="smallheading"><?php __('Add Academic Stand'); ?></div>
		<table border=2>
		<?php 
		...
		echo $this->Form->input('academic_year_from',array('id'=>'academicyear',
            ...
            'empty'=>"--Select Academic Year--",
              ...
              'selected'=>isset($this->data['AcademicStand']['academic_year_from'])?$this->data['AcademicStand']['academic_year_from']:
	   ...
	   <!--
	   <TR><td>No.</td><td>SGPA</td><td>Operator</td><td>CGPA</td><td>Operator</td>
	    <td>Two Conecutive Warnings</td><td>Operator</td><td>Probation followed by Warning</td></TR>
	    -->
		</table>
		...
		<?php 
		  /* if (isset($this->data['CourseCategory']) && count($this->data['CourseCategory'])>0 ) {
		        ...
		        foreach ($this->data['CourseCategory'] as $ck=>$cv) {
		            echo "<tr><td>".$count."</td><td>".$this->Form->input('CourseCategory.'.$ck.'.name',
		            array(
		            'value'=>isset($this->data['CourseCategory'][$ck]['name'])?$this->data['CourseCategory'][$ck]['name']:'','size'=>25,'label'=>false,'div'=>false)).'</td><td>'.
		            $this->Form->input('CourseCategory.'.$ck.'.total_credit',
		            array(
		            'value'=>isset($this->data['CourseCategory'][$ck]['total_credit'])?$this->data['CourseCategory'][$ck]['total_credit']:'','size'=>13,'label'=>false,'div'=>false)).'</td></tr>';
		          $count++;
		        }
		    }
		    */
		  if (isset($this->data['AcademicRule']) && count($this->data['AcademicRule'])>1 ) {
		         $count=1;
		         foreach ($this->data['AcademicRule'] as $ar=>$av) {
					...
					$this->data['AcademicRule'][$ar]['scmo']:'','options'=>$arithmetic_operator,'type'=>'select','div'=>false,'label'=>false))."</td>";
					  echo "<td>".$this->Form->input('AcademicRule.'.$ar.'.sgpa',array(
		            'value'=>isset($this->data['AcademicRule'][$ar]['sgpa'])?
					$this->data['AcademicRule'][$ar]['sgpa']:'','label'=>false,'size'=>4,'div'=>false))."</td>";
					echo "<td>".$this->Form->input('AcademicRule.'.$ar.'.operatorI',
		            array('name'=>"data[AcademicRule][$ar][operatorI]",
		            'value'=>isset($this->data['AcademicRule'][$ar]['operatorI'])?
					$this->data['AcademicRule'][$ar]['operatorI']:'','options'=>$logical_operator,'type'=>'select','div'=>false,'label'=>false,'empty'=>'--select--'))."</td>";
					...
					$this->data['AcademicRule'][$ar]['cgpa']:'','label'=>false,'size'=>4,'div'=>false))."</td>";
					/*
					...
					*/
					...
					$this->data['AcademicRule'][$ar]['pfw']:'','label'=>false,
