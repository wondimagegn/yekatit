...
<?php echo $this->Form->create('CourseExamGapConstraint');?>
	<div class="smallheading"><?php __('Add Course Exam Gap Constraint'); ?></div>
...
<table cellpadding="0" cellspacing="0">
	<?php 
        ...
        echo '<tr><td colspan="6">'. $this->Form->Submit('Search',array('name'=>'search','div'=>false, 'style'=>'width:150PX')).'</td></tr>'; 
	?> 
</table>
<?php 
	if (isset($sections_array) && !empty($sections_array)) { 
		...
		echo '<tr><td colspan="4">'.$this->Form->Submit('Submit',array('div'=>false,'name'=>'submit')).'</td></tr>';
		echo '</table>';	
	}
		//echo '</table>';
		if(isset($courseExamGapConstraints) && !empty($courseExamGapConstraints)) {
			...
			foreach($courseExamGapConstraints as $courseExamGapConstraint){
				...
				$this->Html->link(__('Delete', true), array('action' => 'delete', $courseExamGapConstraint['CourseExamGapConstraint']['id'],"fromadd"),null, sprintf(__('Are you sure you want to delete?', true), $courseExamGapConstraint['CourseExamGapConstraint']['id'],"fromadd")).
				"</td></tr>";
			}
			echo "</table>";
		}
