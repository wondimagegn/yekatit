	...
	//if(!$makeup_exam) {
		...
		<tr>
			...
			if($grade_view_only) {
				...
				if(!empty($student['ExamGradeChange']) && $student['ExamGradeChange'][0]['department_approval'] != -1) {
					...
					if($display_grade || $view_only) {
						...
						if(!isset($this->data))// || $display_grade)
			...
			//If it is non-makeup exams (add and registered)
				...
				if(isset($this->data['ExamResult'][$i]['result'])) {
					if(isset($this->data) && !$display_grade && !$view_only) {
						 $total_100 = "";
						for(; $i <= ((($st_count-1)*count($exam_types))+count($exam_types)); $i++) {
							 if($this->data['ExamResult'][$i]['result'] != "" && is_numeric($this->data['ExamResult'][$i]['result']))
								  $total_100 += $this->data['ExamResult'][$i]['result'];
						}
					}
				}
			...
			<td id="G_<?php echo ++$in_progress; ?>">
				<?php
					...
					//If the grade is from the database (makeup)
					else if(isset($student['ExamGradeChange']) && !empty($student['ExamGradeChange']) && $student['ExamGradeChange'][0]['department_approval'] != -1)
						echo $student['ExamGradeChange'][0]['grade'];*/
					//If the course is on progress (Neither generated or saved)
					else
						echo '**';
			...
			<td>
				...
				//If it is grde form the database (saved)
				/*if(($grade_submission_status['grade_submited'] && !$display_grade) || $view_only) {
					if(isset($student['MakeupExam'])) {
...
*/
				...
				/* //Now it is improved with the above code
				if(isset($student['MakeupExam'])) {
					if(!isset($student['ExamGradeChange']) || empty($student['ExamGradeChange']))
						echo '<p class="on-process">Grade not submitted</p>';
					else if($student['ExamGradeChange']['0']['department_approval'] == null)
						echo '<p class="on-process">Pending for department approval</p>';
					else if($student['ExamGradeChange']['0']['department_approval'] == -1)
						echo '<p class="rejected">Grade is rejected by the department</p>';
					else
						{
						if($student['ExamGradeChange']['0']['registrar_approval'] == null)
							echo '<p class="on-process">Approved by department, pending for registrar approval</p>';
						else if($student['ExamGradeChange']['0']['registrar_approval'] == -1)
							echo '<p class="rejected">Approved by department, but rejected by registrar</p>';
						else
							echo '<p class="accepted">Accepted</p>';
						}
				}
...
//debug($student['ExamGradeHistory']);
						...
						(isset($student['MakeupExam']) && !empty($student['ExamGradeChange']) && ($student['ExamGradeChange'][0]['registrar_approval'] == 1 || $student['ExamGradeChange'][0]['makeup_exam_result'] == null)))
							...
							if(!$student['AnyExamGradeIsOnProcess'] && $grade_change_deadline >= date('Y-m-d H:i:s')) {
								echo '<p style="text-align:left" class="fs13 accepted">Exam grade change deadline: '.$this->Format->humanize_date($grade_change_deadline).'</p>';
								?>
								<table>
									<tr>
										<th colspan="2">
										<?php
										echo '<div style="font-weight:bold; font-size:14px">Grade Change Request</div>';
										?>
										</th>
									</tr>
									<tr>
										<td style="font-weight:bold">Exam Result (out of 100%):</td>
										<td><?php 
										//debug($student);
									...
									<tr>
										<td style="font-weight:bold">New Grade:</td>
										<td id="GradeChangeResult_grade_<?php echo $st_count; ?>">---</td>
									</tr>
									<tr>
										<td style="font-weight:bold">Reason for Change:</td>
										<td>
										<?php
										echo $this->Form->input('ExamGradeChange.'.$st_count.'.reason', array('type' => 'textarea', 'label' => false, 'cols' => 35, 'rows' => 5, 'id' => 'GradeChangeResult_reason_'.$st_count));
										?>
										</td>
									</tr>
									<tr>
										<td colspan="2"><?php echo $this->Form->submit(__('Send Grade Change Request to '.$approver_c.'', true), array('name' => 'grade_change_request_'.$st_count, 'onclick' => 'return submitGrdeChangeRequest(\'GradeChangeResult\', '.$st_count.')')); ?></td>
									</tr>
								</table>
								<?php
							}
							else if($grade_change_deadline < date('Y-m-d H:i:s')){
								echo '<p style="text-align:center" class="fs13 rejected">Exam grade change deadline was '.$this->Format->humanize_date($grade_change_deadline).'</p>';
							}
							else if($student['AnyExamGradeIsOnProcess']) {
								echo '<p style="text-align:center" class="fs13 on-process">Exam grade is on process to request grade change.</p>';
							}
						}
						else if($grade_history_count > 1){
							$last_grade_change = $student['ExamGradeHistory'][$grade_history_count-1];
							...
							if((strcasecmp($this->action, 'add') != 0 && 
								...
								(strcasecmp($this->action, 'add') == 0 && 
								$last_grade_change['ExamGrade']['initiated_by_department'] == 0 &&
								$last_grade_change['ExamGrade']['manual_ng_conversion'] == 0 &&
								$last_grade_change['ExamGrade']['auto_ng_conversion'] == 0 &&
								$last_grade_change['ExamGrade']['department_approval'] == null &&
								$last_grade_change['ExamGrade']['makeup_exam_result'] == null)) {
								echo 'You have grade change request placed by you. You can '.$this->Html->link(__('Cancel', true), array('action' => 'cancel_grade_change_request', $last_grade_change['ExamGrade']['id']), null, sprintf(__('Are you sure you want to cancel your grde change request for %s?', true), $student['Student']['first_name'].' '.$student['Student']['middle_name'].' '.$student['Student']['last_name'].' ('.$student['Student']['studentnumber'].')')).' the request before it is processed.';
							}
