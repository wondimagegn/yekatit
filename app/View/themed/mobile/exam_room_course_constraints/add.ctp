...
<div class="smallheading"><?php __('Add/Edit Exam Room Course Constraints'); ?></div>
...
<?php 
	if (isset($sections_array)) { 
		...
		echo '<tr><td colspan="6">'.$this->Form->Submit('Submit',array('div'=>false,'name'=>'submit')).'</td></tr>';
		echo '</table>';
		...
		if(isset($examRoomCourseConstraints)) {
			...
			foreach($examRoomCourseConstraints as $examRoomCourseConstraint){
				...
				"</td><td style='border-right: #CCC solid 1px'>".
				$this->Html->link(__('Delete', true), array('action' => 'delete', $examRoomCourseConstraint['ExamRoomCourseConstraint']['id'],"fromadd"),null, sprintf(__('Are you sure you want to delete?', true), $examRoomCourseConstraint['ExamRoomCourseConstraint']['id'],"fromadd")).
				"</td></tr>";
			}
			echo "</table>";
		}
