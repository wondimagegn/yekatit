...
// create new PDF document
    ...
    */
    ...
    <table style="width:100%; font-size:'.$font_size.'px">
    	<tr>
    		<td style="text-align:center; font-weight:bold">'.(!empty($student_copy['student_detail']['University']) ? strtoupper($student_copy['student_detail']['University']['University']['name']) : 'AMBO UNIVERSITY').'</td>
    	</tr>
    	<tr>
...
<td><table style="padding:'.$course_justification.'px; margin:0px; background-image:url(\'/img/sc-diagonal-line.gif\'); background-position:left top; background-attachment:scroll; background-repeat:no-repeat">
		<tr>
			...
			<td style="width:36px; border-bottom:0px solid #000000">&nbsp;Co.No.</td>
			<td style="width:205px; border-bottom:0px solid #000000">&nbsp;Course Title</td>
		...
		for($j = $i*$no_of_semester*2; $j < (($i*$no_of_semester*2)+$no_of_semester) && $j < count($student_copy['courses_taken']); $j++) {
			...
			foreach($student_copy['courses_taken'][$j]['courses_and_grades'] as $key => $courses_and_grade) {
				...
				if(isset($courses_and_grade['point_value']))
					...
					$full_gr_pts = false;
				$course_code = str_replace (' ', '', $courses_and_grade['course_code']);
				$student_copy_content .= '
				<tr>
					<td style="text-align:center">'.strtoupper(substr($course_code, 0, strpos($course_code, '-'))).'</td>
					<td style="text-align:center">'.(substr($course_code, -(strpos($course_code, '-')-1))).'</td>
...
<td><table style="padding:'.$course_justification.'px; height:100%">
		<tr>
			...
			<td style="width:9%; border-bottom:0px solid #000000">&nbsp;Co.No.</td>
