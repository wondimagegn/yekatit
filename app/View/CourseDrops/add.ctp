...
<p class="fs16">
                    ...
                    This tool will help you to perform course drop on behalf of  students 
...
if(isset($coursesDrop)){
 ...
 if (isset($student_lists) && !isset($no_display) && !empty($student_lists)) {
  ...
  ?>
  <div class="smallheading"> List of students who have registered for semester  <?php 
  echo $semester.'/'.$current_academic_year ?> academic year   </div>
  <table cellpadding="0" cellspacing="0">
	...
	<tr<?php echo $class;?>>
		...
		<td>
			<?php echo $student['Student']['ProgramType']['name']; ?>
		</td>
		...
		<td class="actions">
			<?php 
			   // debug($student);
			/*
			echo $this->Html->link(__('Drop Course'), array('action' => 'add',
			 $student['Student']['id']));
			 
			 */
			 	echo $this->Html->link(__('Drop Course'), array('action' => 'add',0,
			 $student['CourseRegistration']['id']));
			  ?>
