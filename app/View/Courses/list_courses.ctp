...
<div class="box">
	...
	<table class="fs13 small_padding">
		...
		<tr>
			<td colspan="4"><?php echo $this->Form->submit(__('Get Courses'), 
			array('name' => 'search', 'id' => 'getCourses','class'=>'tiny radius button bg-blue','div' => false)); ?></td>
		</tr>
	</table>
<?php 
...
<table cellpadding="0" cellspacing="0" style="border: #CCC solid 1px">
	<tr>
			...
			<th style="border-right: #CCC solid 1px;width:50px;"><?php echo $this->Paginator->sort('credit','ECTS');?></th>
			<th style="border-right: #CCC solid 1px;width:100px;"> L T L</th>
			<th style="border-right: #CCC solid 1px"><?php echo $this->Paginator->sort('course_category_id','Module/Course Category');?></th>
			
			<th style="border-right: #CCC solid 1px;"><?php echo 'Prerequisite';?></th>
			<th class="actions" style="border-right: #CCC solid 1px"><?php echo __('Actions');?></th>
	...
	<tr<?php echo $class;?>>
		...
		<td style="border-right: #CCC solid 1px; width: 120px;">
			 ...
			 if(isset($course['Prerequisite']) && !empty($course['Prerequisite'])) {
				...
				foreach($course['Prerequisite'] as $k=>$v ) {
					echo '<li>'.$v['PrerequisiteCourse']['course_title'].'</li>';
				}
			    echo '</ul>';
			  } else {
					echo 'none';
		...
		<td class="actions" style="border-right: #CCC solid 1px">
				<?php echo $this->Html->link(__(''), array('action' => 'view', $course['Course']['id']),array('class'=>'fontello-eye','title'=>'View')); ?>
		<?php
		if($role_id == ROLE_DEPARTMENT) {
		?>
				<?php echo $this->Html->link(__(''), array('action' => 'edit', $course['Course']['id']),array('class'=>'fontello-pencil','title'=>'Edit')); ?>
				<?php echo $this->Html->link(__(''), array('action' => 'delete', $course['Course']['id']), array('class'=>'fontello-trash-1','title'=>'Delete'), sprintf(__('Are you sure you want to delete # %s?'), $course['Course']['id'])); ?>
			</td>
		<?php
		}
		?>
