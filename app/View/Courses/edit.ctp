...
array('novalidate' => true));?>
     <?php if (isset($this->request->data['Course']['course_code_title'])) {  ?>
		<div class="smallheading"><?php echo __(' Edit Course '.$this->request->data['Course']['course_code_title'].'. '); ?></div>
		<?php 
		} else {
		?>
	        	<div class="smallheading"><?php echo __(' Edit Course '.$course_code_title.' '); ?></div>	
		<?php 
	...
	<table>
	    ...
	    if ($editCreditDetail ==0 ) {
		    ...
		    echo "<tr><td style='padding-left:150px'> Course Title: ".$this->request->data['Course']['course_title']."</td></tr>";
		    echo "<tr><td style='padding-left:150px'> Course Code: ".$this->request->data['Course']['course_code']."</td></tr>";
		...
		if ($editCredit ==0 ) {
		    echo "<tr><td>".$this->Form->input('credit',array('label'=>$creditname))."</td></tr>";
		       ...
		       echo "<tr><td style='padding-left:100px'>".$creditname.': '.$this->request->data['Course']['credit']."</td></tr>";
		                    ...
		                    echo '<td>';
		                     ...
		                     echo '</td>';
		                       ...
		                       if(!empty($action_model_id)) {
		                        ...
		                        sprintf(__('Are you sure you want to delete # %s?'), $cv['id'])).'</td>';  
		   ...
		   foreach ($this->request->data['Book'] as $book_index=>$book_value){
		            ...
		            echo '</td>';
		              if(!empty($action_book_id)) {
		                ...
		                sprintf(__('Are you sure you want to delete # %s?'), $book_value['id'])).'</td>';  
		...
		if (isset($this->request->data['Journal']) && count($this->request->data['Journal'])>0) {
		   ...
		   foreach ($this->request->data['Journal'] as $journal_index=>$journal_value){
		              if (!empty($journal_value['id'])) {
		                       	...
		                       	array('value'=>$this->request->data['Journal'][$journal_index]['id']));    
		            ...
		            echo '</td>';
		              if(!empty($action_journal_id)) {
		                ...
		                sprintf(__('Are you sure you want to delete # %s?'), $journal_value['id']))."</td>";  
		   ...
		   foreach ($this->request->data['Weblink'] as $weblink_index=>$web_value){
		            ...
		            echo '</td>';
		             if(!empty($action_weblink_id)) {
		                    ...
		                    sprintf(__('Are you sure you want to delete # %s?'), $web_value['url_address'])).'</td>';  
