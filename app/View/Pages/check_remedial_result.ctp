...
<div class="box">
	<div class="box-header bg-transparent">
		<div class="box-title" style="margin-top: 10px;"><i class="fontello-check-outline" style="font-size: larger; font-weight: bold;"></i>
			<span style="font-size: medium; font-weight: bold; margin-top: 20px;"> <?= __('Check Remedial Result'); ?></span>
		</div>
	</div>
	<div class="box-body">
		<div class="row">
			<div class="large-12 columns">
				...
				if (!isset($this->data['Page']) || (isset($this->data['Page']) && isset($resultFound) && empty($resultFound))) { ?>
					<fieldset style="padding-bottom: 5px; padding-top: 15px;">
						<div class="row">
							...
							<div class="large-4 columns">
								<?= $this->Form->input('search_key', array('id' => 'SearchKey', 'size' => MAXIMUM_STUDENT_ID_NUMBER_LENGTH_DB, 'placeholder' => 'MoE Admission Number or AMU Student ID', 'label' => 'MoE Admission Number or AMU Student ID: ', 'value' => (!isset($this->data['Page']) ? '' : (isset($searchKeyProvided) && !empty($searchKeyProvided) ? $searchKeyProvided : '')), 'autocomplete' => 'off', 'required', 'maxlength' => MAXIMUM_STUDENT_ID_NUMBER_LENGTH_DB)); ?>
								<div style="margin-bottom: 50px;"></div>
							</div>
							...
							<div class="large-12 columns">
								<hr>
								<?= $this->Form->Submit(__('Check Result', true), array('class' => 'tiny radius button bg-blue btn-primary', 'name' => 'continue', 'id' => 'continue', 'div' => false)); ?>
							</div>
							<?= $this->Form->end(); ?>
						</div>
					</fieldset>
				...
				if (isset($resultFound) && !empty($resultFound)) { ?>
					<div class="large-12 columns">
						<?= $this->element('student_remedial_result_check'); ?>
						<br>
					</div>
					<hr>
					<?php
