...
<tr><td colspan=2 class="smallheading"> Unpublish or delete courses from the publish list.</td></tr>
        <?php 
            echo '<tr><td>'.$this->Form->input('PublishedCourse.academic_year',array(
                        ...
                        'empty'=>"--Select Academic Year--",'selected'=>isset($this->request->data['PublishedCourse']['academic_year'])?$this->request->data['PublishedCourse']['academic_year']:$defaultacademicyear)).'</td><td>'. $this->Form->input('PublishedCourse.semester',array('options'=>array('I'=>'I'),'empty'=>'--select semester--')).'</td></tr>';

		    echo '<tr>';
				echo '<td>'.$this->Form->input('PublishedCourse.program_id').'</td>';
				echo '<td>'.$this->Form->input('PublishedCourse.program_type_id').'</td>';
	...
	<?php 
            ...
            ?>
	<tr><td colspan="2"><?php echo $this->Form->submit('Continue',array('name'=>'getsection','div'=>'false','class'=>'tiny radius button bg-blue')); ?> </td>	
</tr></table>
            ...
            <tr>
                <td style='padding:0'> <?php 
                 // echo $this->Form->submit('Unpublished Selected',array('name'=>'unpublishselected','div'=>'false'));?></td>
                   <td style='padding:0'> <?php 
                  echo $this->Form->submit('Delete Selected',array('name'=>'deleteselected','div'=>'false',
'class'=>'tiny radius button bg-blue'));?></td>
                   <td style='padding:0'> <?php 
                  echo $this->Form->submit('Publish As Drop Selected',array('name'=>'dropselected','div'=>'false',
'class'=>'tiny radius button bg-blue'));?></td>
