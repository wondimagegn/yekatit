...
<?php echo $this->Form->create('Curriculum',array('type'=>'file',
...
<script  type="text/javascript">    
     function updateSum(tableID) {
		...
		for(i = 1; i < document.getElementById(tableID).rows.length; 
i++) {
	 ...
	 function addRow(tableID,model,no_of_fields,all_fields) {
			...
			for(var i=1;i<=no_of_fields;i++) {
				...
				if (elementArray[i-1] == "total_credit") {
				   ...
				   element.onblur = function () {return updateSum('course_categories');};
...
<div class="box">
     <div class="box-body">
       <div class="row">
		<div class="large-12 columns">
			 <div class="row">
			       ...
			       <div class="large-6 columns">
					...
					echo $this->Form->input('from_curriculum',array('label'=>'Copy Curriculum From','options'=>$earlierCurriculums,'empty'=>'---Select Curriculum---','style'=>'width:200px'));
			echo $this->Form->Submit('Copy',array('name'=>'copyCurriculum',
		...
		echo '<tr><td>'.$this->Form->input('year_introduced',array('style'=>'width:80px;')).'</td></tr>';
		echo '<tr><td>'.$this->Form->input('type_credit',array('empty'=>'--select credit type--',
		'options'=>array('ECTS Credit Point'=>'ECTS Credit Point','Credit'=>'Credit')
,'id'=>'CreditType','onchange'=>'updateCreditLable("CreditType")')).'</td></tr>';
...
echo '<tr><td>'.$this->Form->input('minimum_credit_points',
array('id'=>'minimum_credit_points')).'</td></tr>';
...
?>		
		...
		echo $this->Form->input('CourseCategory.'.$ck.'.code',array('label'=>false,
					 ...
					 array('size'=>5,'label'=>false,'id'=>'t'.$count,
'onBlur'=>'updateSum("course_categories");','div'=>false)).'</td>';
					
					echo '<td>'.$this->Form->input('CourseCategory.'.$ck.'.mandatory_credit',
					array('label'=>false,
'id'=>'m'.$count,
'onBlur'=>'updateSum("course_categories");','size'=>5)).'</td></TR>';
		            ?>
			      <?php 
		    ...
		    ?>
				...
				<?php 
					 ...
					 array('size'=>5,'id'=>'t1',
'onBlur'=>'updateSum("course_categories");','label'=>false,'div'=>false)).'</td>';
					
					echo '<td>'.$this->Form->input('CourseCategory.0.mandatory_credit',
					array('label'=>false,
'size'=>5,'id'=>'m1','onBlur'=>'updateSum("course_categories");')).'</td>';
		            ?>
 ...
 <td width="116px">
		...
		<table style="border:0px;"><tr><td colspan=3>
		...
		<div class="large-12 columns">
<?php 
echo $this->Form->end(array('label'=>__('Submit', true),
'class'=>'tiny radius button bg-blue','name'=>'saveCurriculum','div'=>false));
?>
	       </div>
