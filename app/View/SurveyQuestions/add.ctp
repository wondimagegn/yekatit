     ...
     <div class="box-body">
       <div class="row">    
			...
			<div class="large-6 columns">
					...
					array('id'=>'ansryn'));
					echo '<div id="RequireRemarkText">'.$this->Form->input('require_remark_text',array('id'=>'rrt')).'</div>';
...
<table cellspacing="0" cellpadding="0" id="surveyanswersetting" style="margin-bottom:5px">
		<tr>
			...
			<th style="width:35%">Answer(English)</th>
			<th style="width:35%">Answer(Amharic)</th>
		...
		if(empty($this->request->data)) {
			if(empty($survey_question_answers)) {
				...
				<tr id="SurveyQuestionAnswer_1">
					...
					<td><?php echo $this->Form->input('SurveyQuestionAnswer.1.order', array('maxlength' => '2','type' => 'number', 'label' => false, 'maxlength' => '2', 
					'style' => 'width:40px'));?></td>
			...
			<tr id="SurveyQuestionAnswer_<?php echo $count; ?>">
				...
				<td><?php echo $this->Form->input('SurveyQuestionAnswer.'.$key.'.answer_amharic', array('label' => false));?></td>
				<td><?php echo $this->Form->input('SurveyQuestionAnswer.'.$key.'.order', array('maxlength' => '2', 'type' => 'number', 'label' => false, 'maxlength' => '2',
				'style' => 'width:40px'));?></td>
	...
	<p><input type="button" value="Add Row" onclick="addRow('surveyanswersetting', 'SurveyQuestionAnswer', 4, '<?php echo $all_answer_setup_detail; ?>')" /></p>
       ...
       <div class="row">
       		<div class="large-12 columns">
						...
						echo $this->Form->Submit('Submit',array('name'=>'search','div'=>false,
...
/*
    if($("#ansryn").is(":checked")){
    			...
    			$('#rrt').attr('checked', 'checked');    
				$('#RequireRemarkText').show(); 
				 
	} else { 
				$('#rrt').attr('checked', false);       
				$('#RequireRemarkText').hide(); 
				    
	} 
			
    
      $("#ansryn").click(function(){ 
		if($("#ansryn").is(":checked")){
			$('#rrt').attr('checked', 'checked');    
			$('#RequireRemarkText').show(); 
