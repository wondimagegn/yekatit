...
?>
	<h3 class="box-title"><i class="fontello-folder"></i>
		          <span>Dispatched courses instructor assignment is not done!</span>
    </h3>
						<table class="small_padding">
							<?php
							if(empty($dispatched_course_list)) {
								echo '<tr><td style="border:0px solid #ffffff"><p style="font-size:12px">
			There is no other department who dispatched courses to assign instructor from your department.</p></td></tr>';
							}
							else {
							   	echo '<tr><td style="border:0px solid #ffffff">
							   	<p style="font-size:16px;font-weight:bold"> List of courses dispatched for instructor assignment.</p></td></tr>';
									...
									<tr>
										...
										;
										   echo '<br/> <strong>Dispatched To Department: </strong>'.
										    ...
										    echo '<strong>Course: </strong> <a href="/course_instructor_assignments/assign_course_instructor/'.$dc['PublishedCourse']['id'].'">'.$dc['Course']['course_title'].'('.$dc['Course']['course_code'].' )</a> <br/>';
...
<?php 
//Latest dispatched courses 
if(!empty($latest_assigned_courses)) 
{
?>
		<?php 
		 if(empty($latest_assigned_courses)) {
		    echo '<p>Currently you do not have assigned courses.</p>';
		 } else {
