...
class FormatHelper extends AppHelper {
	...
	{
		return date("F d, Y h:i:s A", mktime (
			substr($date,11 ,2), 
			substr($date,14 ,2), 
			substr($date,17 ,2), 
			substr($date,5 ,2), 
	...
	{
		return date("M d, y h:i:s A", mktime (
			substr($date,11 ,2), 
			substr($date,14 ,2), 
			substr($date,17 ,2), 
			substr($date,5 ,2), 
	   ...
	   // return false;
		if (!empty($submenu)) {
			...
			if (strcasecmp($keys[0], '0') != 0) {
				if ((strcasecmp($submenu['url']['controller'], $controller) == 0 || strcasecmp(Inflector::variable($submenu['url']['controller']), $controller) == 0) && strcasecmp($submenu['url']['action'], $action) == 0) {
					return true;
				} else if (!empty($submenu['children'])) {
