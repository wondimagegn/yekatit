				...
				<tbody>
					...
					foreach ($list as $ko => $val) {
						...
						<tr class='jsView' data-animation="fade" data-reveal-id="myModal" data-reveal-ajax="/students/get_modal_box/<?= $val['Student']['id']; ?>">
							...
							<td class="center" style="mso-number-format:'@'; white-space: nowrap;"><?= ((isset($val['GraduateList']['graduate_date']) && !empty($val['GraduateList']['graduate_date'])) ? $this->Time->format("M j, Y", $val['GraduateList']['graduate_date'], NULL, NULL) : '---'); ?></td>
