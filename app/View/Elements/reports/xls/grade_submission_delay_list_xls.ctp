                        ...
                        foreach ($courseList as $rkey => $rvalue) {
                            foreach ($rvalue as $mn => $ym) { ?>
                                <tr>
                                    ...
                                    <td style="mso-number-format:'@'; white-space: nowrap; <?= (!empty($ym['CourseInstructorAssignment']['grade_submission_deadline']) ? ($ym['CourseInstructorAssignment']['grade_submission_deadline'] > date('Y-m-d') ? 'color:green' : 'color:red') : 'color:gray'); ?>">
