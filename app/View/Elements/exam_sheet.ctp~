...
if(!isset($total_student_count))
 ...
 ?>
<script>
var col = Number(<?php echo count($exam_types);?>); 
var rows = Number(<?php echo $total_student_count;?>);
...
function moveByKeyPress(e) 
{
   ...
   try 
   {
	...
	if(liveValue != "" && isNaN(liveValue)) {
			...
			alert('The maximum value of "'
			+$(e.target).attr("data-type")+'" exam result is '+$(e.target).attr("data-percent")+'.');
				$('#'+$(e.target).attr("id")).focus(); 
	} else if (liveValue != "" 
	&& parseFloat(liveValue) < 0) {
			alert('The minimum value of "'
...
<table class="grade_list" onkeypress="javascript:moveByKeyPress();">
	<tr>
		<th style="width:2%">&nbsp;</th>
...
$input_options['tabindex'] = (($total_student_count * ($et_count - 1)) + $st_count);
		echo $this->Form->input('ExamResult.'.$count.'.result', $input_options);
					...
					//New exam result entry
						...
						echo $this->Form->input('ExamResult.'.$count.'.result', array('tabindex' => (($total_student_count * ($et_count - 1)) + $st_count), 'type' => 'text', 'label' => false, 'maxlength' =>'5', 'style' => 'width:50px', 'id' => 'result_'.$st_count.'_'.$et_count,
						...
						(isset($student['MakeupExam']) && !empty($student['ExamGradeChange']) && ($student['ExamGradeChange'][0]['registrar_approval'] == 1 || $student['ExamGradeChange'][0]['makeup_exam_result'] == null)))
							...
							if(!$student['AnyExamGradeIsOnProcess'] && $grade_change_deadline >= date('Y-m-d H:i:s')) {
								...
								<table>
									...
									<tr>
										<td colspan="2"><?php echo $this->Form->submit(__('Send Grade Change Request to'.$approver_c.''), array('name' => 'grade_change_request_'.$st_count,'class'=>'tiny radius button bg-blue', 'onclick' => 'return submitGrdeChangeRequest(\'GradeChangeResult\', '.$st_count.')')); ?></td>
									</tr>
								</table>
								<?php
							}
