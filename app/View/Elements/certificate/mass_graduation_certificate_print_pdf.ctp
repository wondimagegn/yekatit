...
'.$graduation_certificate['student_detail']['University']['University']['fax'].'
    ...
    $pdf->MultiCell(100, 7, 'Serial: ', 0, 'L', false, 0, 215, 65);
    $pdf->SetFont($fontPath, 'U', 12, '', false);
    $pdf->MultiCell(100, 7, ' '.$graduation_certificate['student_detail']['Student']['code'] , 0, 'L', false, 0, 228, 65);
...
*/
    ...
    $pdf->Image($_SERVER['DOCUMENT_ROOT'].'/app/webroot/img/centered-line-certificate.gif', 13, 70, 270, 5, '', '', '', false, 300, '', false, false, 0);
...
text-decoration:underline; width:42%;font-weight:bold; line-height:5px; font-size:'.($graduation_certificate_template['GraduationCertificate']['en_title_font_size']*5).'px">'.$graduation_certificate_template['GraduationCertificate']['english_title'].'</td>
    ...
    */
    ...
    $pdf->write2DBarcode(BASE_URL_HTTPS.'pages/check_graduate/'.str_replace('/','-',
            $graduation_certificate_template['student_detail']['Student']['studentnumber']), 'QRCODE,H', 260, 165, 20, 20, $style, 'N');
    $pdf->SetFont($fontPath, '', 8, '', true);
    $pdf->Text(259, 176, $graduation_certificate_template['student_detail']['Student']['studentnumber']);
