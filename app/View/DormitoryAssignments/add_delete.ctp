...
<?php echo $this->Form->create('DormitoryAssignment');?>
	<div class="smallheading"><?php echo __('Manually Add/Delete Students');?></div>
<?php if(isset($programs)) {?>
	<div class="info-box info-message"><font color=RED><u>Beaware:</u></font><br/> -To add student manaually to a given dormitory, you have to selected specific dormitory and at least one other parameter from below optional search parameters. </div>
	<div class="font"><?php echo __('Optional search parameters');?> </div>
	
	<table cellpadding="0" cellspacing="0">
	<?php 
		echo '<tr><td class="font"> Program</td>'; 
        echo '<td>'. $this->Form->input('program_id',array('label'=>false,'selected'=>isset($selected_program_id)?$selected_program_id:"",'style'=>'width:150px','empty'=>"All")).'</td>'; 
        ...
        echo '<td>'. $this->Form->input('gender',array('label'=>false,'id'=>'ajax_gender', 'options'=>array('male'=>'Male','female'=>'Female'),'onchange'=>'getDormitoryBlock()','selected'=>isset($selected_gender)?$selected_gender:"", 'style'=>'width:150px','empty'=>"All")).'</td></tr>'; 
        echo '<tr><td class="font"> College</td>'; 
		echo '<td>'. $this->Form->input('college_id',array('label'=>false,'id'=>'ajax_college_id', 'onchange'=>'getDepartment()','style'=>'width:150px','selected'=>isset($selected_college_id)?$selected_college_id:"", 'empty'=>"All")).'</td>'; 
		echo '<td class="font"> Department</td>'; 
		echo '<td>'. $this->Form->input('department_id',array('label'=>false,'id'=>'ajax_department_id', 'onchange'=>'getYearLevel()','options'=>$departments, 'style'=>'width:150px','selected'=>isset($selected_department_id)?$selected_department_id:"",'empty'=>'All')).'</td>';
		echo '<td class="font"> Year Level</td>'; 
        echo '<td>'. $this->Form->input('year_level_id',array('label'=>false, 'id'=>'ajax_year_level_id','options'=>$yearLevels,'style'=>'width:150px','selected'=>isset($selected_year_level_id)?$selected_year_level_id:"",'empty'=>'All')).'</td></tr>'; 
        echo '<tr><td class="font"> Dormitory Block</td>'; 
        echo '<td class="font">'.$this->Form->input('dormitory_block',array('label' => false, 'id'=>'ajax_dormitory_block','onchange'=>'getDormitory()','style'=>'width:150px','type'=>'select', 'options'=>$fine_formatted_dormitories,'selected'=>isset($dormitory_block_id)?$dormitory_block_id:"", 'empty'=>"All")).'</td>';
        echo '<td class="font"> Dormitory</td>'; 
        echo '<td class="font">'.$this->Form->input('dormitory',array('label' => false,'id'=>'ajax_dormitory','type'=>'select', 'options'=>$dormitories, 'selected'=>isset($dormitory_id)?$dormitory_id:"",'empty'=>"All")).'</td></tr>';
        	
		echo '<tr><td colspan="6">'.$this->Form->submit(__('Search'), array('div' => false,'class'=>'tiny radius button bg-blue')).'</td></tr>'; 
	?>
	</table>
	...
	<tr<?php echo $class;?>>
		...
		<td class="actions">
			...
			<?php echo $this->Html->link(__('Delete'), array('action' => 'delete', $dormitoryAssignment['DormitoryAssignment']['id']), null, sprintf(__('Are you sure you want to delete %s?'), $dormitoryAssignment['Student']['full_name'])); ?>
		</td> 
	</tr>
<?php endforeach; 
	if(!empty($dormitory_id)){
		...
		/*
	   echo '<tr><td colspan="12" style="text-align: center;" id="ajax_student">'.$this->Js->link(__('Add'),
	   '/dormitory_assignments/add_student_dormitory/'.$dormitory_id.'~'.$selected_program_id. '~'.$selected_program_type_id.'~'.$selected_gender.'~'.$selected_college_id.'~'.$selected_department_id.'~'. $selected_year_level_id.'~'.$dormitory_block_id, array('update'=>'#ajax_student','evalScripts'=>true)).
			'</td></tr>';
			*/
			 echo '<tr><td colspan="12" style="text-align: center;" id="ajax_student">'.$this->Html->link('Add','#',
   array('data-animation'=>"fade",
'data-reveal-id'=>'myModalDormAssignment',
'data-reveal-ajax'=>'/dormitory_assignments/add_student_dormitory/'.$dormitory_id.'~'.$selected_program_id. '~'.$selected_program_type_id.'~'.$selected_gender.'~'.$selected_college_id.'~'.$selected_department_id.'~'. $selected_year_level_id.'~'.$dormitory_block_id)).'</td></tr>';
...
<div class="box">
     <div class="box-body">
            <div class="row">
            	<div class="large-12 columns">
            		<div id="myModalDormAssignment" class="reveal-modal" data-reveal>
