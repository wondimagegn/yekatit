            ...
            <div class="large-12 columns">
                ...
                <div id="show_search_results">
                    ...
                    if (isset($organized_published_course_by_section) && !empty($organized_published_course_by_section)) { ?>
                        ...
                        <h6 class="fs13 text-gray">Please select mass added courses to cancel</h6>
                                ...
                                <div style="overflow-x:auto;">
                                    <table id='fieldsForm' cellpadding="0" cellspacing="0" class="table">
                                        <thead>
                                            ...
                                            <!-- <tr>
                                                <th colspan=6>Section: <?php //echo $sections[$section_id]; ?></td>
                                            </tr> -->
                                            <tr>
                                        ...
                                        <tbody>
                                            ...
                                            foreach ($coursss as $kc => $vc) { 
                                                if ($vc['grade_submitted']) {
                                                    ...
                                                    $grade_not_submitted_counter++;
                                                } ?>
                                                <tr>
                                                    <td class="center"><?= ($vc['grade_submitted'] ? '**' : '<div style="margin-left: 30%;">' . $this->Form->checkbox('PublishedCourse.' . $section_id . '.' . $vc['PublishedCourse']['id'], array('class' => 'listOfPublishedCourse', 'id' => $count)) . '</div>'); ?></td>
                                        ...
                                        <?php
                                        if ($grade_submitted_counter) { ?>
                                            <tfoot>
                                                <tr>
                                                    <td class="center">**</td>
                                                    <td colspan=5 class="vcenter" style="font-weight: normal;">Those courses are not allowed for cancellation since one or more students has got grade.</td>
                                                </tr>
                                            </tfoot>
                                            <?php
                                        } ?>
                        ...
                        if (/* $grade_submitted_counter != $publish_counter */ $grade_not_submitted_counter) { ?>
                            <hr>
                            <?= $this->Form->submit('Cancel Selected Mass Adds', array('name' => 'cancelmassadd', 'id' => 'SubmitID', 'div' => 'false', 'class' => 'tiny radius button bg-blue')); ?>
                            <?php
                        } 
                    } ?>
...
<script type='text/javascript'>
	...
	$('#getsection').click(function(event) {
        ...
        if (!search_button_clicked && formIsValid) {
            ...
            $('#cancelmassadd').attr('disabled', true);
    ...
    $('#cancelmassadd').click(function(event) {
        ...
        if (form_being_submitted) {
            alert('Canceling selected Mass Add requests, please wait a moment...');
            $('#cancelmassadd').attr('disabled', true);
            $('#getsection').attr('disabled', true);
            return false;
        }
        ...
        if (!form_being_submitted && isValid) {
            $('#cancelmassadd').val('Canceling Selected Mass Adds...');
            $('#getsection').attr('disabled', true);
