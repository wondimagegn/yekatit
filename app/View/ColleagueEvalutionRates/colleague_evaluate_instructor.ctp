...
<?php
	...
	'label' => false, 'type' => 'select', 'options' => $colleagueLists, 'onchange' => 'toggleFields("Staff")',
...
if(isset($instructorEvalutionQuestionsObjective) && !empty($instructorEvalutionQuestionsObjective) && !empty($colleagueLists)) {
	...
	<div class='fs16'>
               <p class="fs16">
              Listed below are statements, which describe aspects of your colleague’s behavior. Please rate him/her on each of these items by circling the appropriate coded response category.  Your ratings should be based on a comparison between the particular individual and the other members of the department. If you feel that you cannot rate him on a particular item or that the item is not applicable to his work, then mark the response category NA.
               </p>
		      <u id="nameOfStaff"></u> 
		      is waiting your evaluation for <?php echo $this->request->data['Search']['acadamic_year'].'/'.$this->request->data['Search']['semester']; ?>. Please make sure that the submitted evaluation is correct as <strong> your evaluation will be used for improving quality of eduation</strong>. 
