									...
									if (!empty($autoplacedstudents)) {
										...
										if (!empty($autoplacedstudents)) {
											...
											if (!isset($minute_number)) {
												echo "<table><tbody><tr><td style='width:30%'>" . $this->Form->input(
													'minute_number',
													array('label' => 'Minute Number')
												) . "</td><td>" . $this->Form->Submit(__('Approve'), array(
													'div' => false,
													'name' => 'approve', 'class' => 'tiny radius button bg-blue'
												)) . "</td></tr></tbody></table>";
											} else {
