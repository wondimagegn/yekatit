<?php
    ...
    $logo_path = $this->Media->file($university['Attachment'][$logo_index]['dirname'].
    DS.$university['Attachment'][$logo_index]['basename']);
    ...
    $pdf->MultiCell(92, 7, ($university['University']['name']), 0, 'C', false, 0, 1, 12);
    $pdf->SetFont($fontPath, '', 13, '', false);
   /* 
   if(isset( $staff_details['Staff'][0]['College']['name'])) {
    $pdf->MultiCell(92, 7, $staff_details['Staff'][0]['College']['name'], 0, 'C', false, 0, 1, 18);

   } else {
	 $pdf->MultiCell(92, 7, '', 0, 'C', false, 0, 1, 18);
   }
	
    $pdf->SetFont($fontPath, 'U', 12, '', false);
    */
    if(!empty($staff_details['Staff'][0]['Department']) && !empty($staff_details['Staff'][0]['Department']['id'])) {
		 $pdf->MultiCell(92, 7, $staff_details['Staff'][0]['Department']['name'].' Department', 0, 'C', false, 0, 1, 32);
    }
    else {
		 $pdf->MultiCell(92, 7, '', 0, 'C', false, 0, 1, 32);
    }
    $fontPath = $pdf->addTTFfont($_SERVER['DOCUMENT_ROOT'].'/app/Vendor/tcpdf/fonts/jiret.ttf');
    $pdf->SetFont($fontPath, '', 20, '', true);
    $pdf->MultiCell(85, 7, strtoupper($university['University']['amharic_name']), 0, 'C', false, 0, 120, 11);
	...
	*/
    ...
    if(!empty($staff_details['Staff'][0]['Department']) && !empty($staff_details['Staff'][0]['Department']['id'])) {
		 $pdf->MultiCell(85, 7, 
$staff_details['Staff'][0]['Department']['amharic_name'].' ዲፓርትመንት', 0, 'C', false, 0, 120, 32);
    }
    else {
	$pdf->MultiCell(85, 7, '', 0, 'C', false, 0, 120, 32);
    }
    ...
    $pdf->MultiCell(15, 7, '21', 0, 'C', false, 0, 42, 27);
    if((!empty($staff_details['Staff'][0]['Department']) && !empty($staff_details['Staff'][0]['Department']['id']) && 
    !empty($staff_details['Staff'][0]['Department']['phone'])) || 
    	(empty($staff_details['Staff'][0]['Department']) && !empty($staff_details['Staff'][0]['Department']['id']) && 
    	!empty($staff_details['Staff'][0]['College']['phone']))
    	) {
	 	$pdf->Image($_SERVER['DOCUMENT_ROOT'].'/app/webroot/img/phone_icon.png', '140', '26', 7, 7, 'PNG', '', '', true, 300, '');
    	if((!empty($staff_details['Staff'][0]['Department']) && !empty($staff_details['Staff'][0]['Department']['id']))) {
    		$pdf->MultiCell(100, 7, 
$staff_details['Staff'][0]['Department']['phone'], 0, 'L', false, 0, 146, 27);
    	}
		else {
			$pdf->MultiCell(100, 7, $staff_details['Staff'][0]['College']['phone'], 0, 'L', false, 0, 146, 27);
		}
	 }
    $fontPath = $pdf->addTTFfont($_SERVER['DOCUMENT_ROOT'].'/app/Vendor/tcpdf/fonts/bookman_old_style.ttf');
    $pdf->SetFont($fontPath, '', 12, '', false);
    ...
    $pdf->MultiCell(157, 7, 'አዲስ አበባ, ኢትዮጵያ', 0, 'C', false, 0, 27, 31);
...
if(!empty($staff_details['User']['last_login'])) { 
  ...
  $welcomeFirstTime= '
        <td colspan="4"> Welcome to '.$university['University']['name'].' and its exciting world of knowledge. All academic related transaction is handled by our student information system. Inorder to access 
        the portal('.BASE_URL.'), use the account below: </td>';
} 


 $staff_copy_html = '<table style="width:100%" border="0" cellpadding="0" cellspacing="0" >
  
    <tr>
        <td colspan="4"> Dear '.
$staff_details['Staff'][0]['Title']['title'].' ' .$staff_details['Staff'][0]['full_name'].', </td>
    </tr>
     <tr>
        <td colspan="4"> &nbsp;</td>
    </tr>
	...
	<tr>
		...
		<td>'.$staff_details['User']['username'].'</td>
	...
	<tr>
        <td colspan="4">NOTE: In the first login you will be forced to chanage your password so change the password accordingly to access the portal.
        </td>
    </tr>
     <tr>
        <td colspan="4"> &nbsp;</td>
    ...
    <tr>
        <td colspan="4">Always remember your password and you are advised to keep it secure and secret particularly if using a shared computer.
        </td>
    </tr>
