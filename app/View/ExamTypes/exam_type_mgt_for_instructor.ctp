<script language="javascript">
    ...
    var secondaryInstructors = <?= json_encode(!empty($secondary_instructors) ? $secondary_instructors : array()); ?>;

    function buildStaffSelect(name) {
        ...
        for (var staffId in secondaryInstructors) {
            if (!secondaryInstructors.hasOwnProperty(staffId)) {
                continue;
            }
            var option = document.createElement("option");
            option.value = staffId;
            option.text = secondaryInstructors[staffId];
    ...
    function addRow(tableID, model, no_of_fields, all_fields) {
        ...
        for (var j = 1; j <= no_of_fields; j++) {
            ...
            var fieldName = elementArray[j - 1];
            var element = null;
            ...
            if (fieldName != 'staff_id' && fieldName != 'edit') {
                element.name = "data[" + model + "][" + rowCount + "][" + fieldName + "]";
            }
