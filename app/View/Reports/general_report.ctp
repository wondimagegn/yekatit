...
style="display:<?php echo (!empty($student_lists)  || !empty($resultBy) || !empty($attrationRate) || !empty($dismissedList) || !empty($resultBy) || !empty($activeList) ||!empty($gradeChangeLists) || !empty($gradeSubmissionDelay) || !empty($distributionStatistics) || !empty($top) || !empty($studentList) || !empty($delayedGradeSubmissionReportList) ||
!empty($distributionStatisticsStatus) ||
!empty($distributionStatsGraduate) || !empty($graduateRateToEntry['distributionGraduateEntry']) || !empty($notRegisteredList) || !empty($distributionStatsLetterGrade['distributionLetterGrade'])  ? 'none' : 'display'); ?>">
...
<span style="font-size:11px"></span></td>
	...
	<tr class="academicStatus byAcademicStatus notVisibleOnAttritionRate"  style="<?php isset($showFromToBlock) && !empty($showFromToBlock) ? 'display:block;':'display:none;' ?>">
...
&& $this->data['Report']['report_type']=="academic_status_range") {
	?>
    <tr class="academicStatus"  style="<?php isset($showFromToBlock) && !empty($showFromToBlock) ? 'display:block;':'display:none;' ?>">
...
<?php 
  ...
  } else if (isset($delayedGradeSubmissionReportList)
  	&& !empty($delayedGradeSubmissionReportList)) {
  ...
  } else if (isset($notAssignedCourseeList)
  ...
  } else if(!empty($activeList)) {
  	echo $this->element('reports/active_list');
  } else if(!empty($notRegisteredList)) {
  		echo $this->element('reports/not_registered_list');
  } else if(!empty($registeredList)){
  	echo $this->element('reports/registered_list');
  } else if(isset($graduateRateToEntry['distributionGraduateEntry']) && 
  	!empty($graduateRateToEntry['distributionGraduateEntry'])){
  	debug($graduateRateToEntry['distributionGraduateEntry']);
