				...
				<div class="large-12 columns">
					...
					<div class="tabs-content edumix-tab-horz">
						<div class="content active" id="basic_data" style="padding-left: 0px; padding-right: 0px;">
							<div class="row">
								<div class="large-12 columns">
									...
									$tinMandatory = ($ethiopianStudent && (FORCE_ALL_STUDENTS_TO_FILL_TIN_NUMBER == 1 /* || (isset($isGraduatingClassStudent) && $isGraduatingClassStudent) */) ? true : false);
									...
									$faidaMandatory = ((FORCE_ALL_STUDENTS_TO_FILL_FAYDA_NUMBERS == 1 || (isset($isGraduatingClassStudent) && $isGraduatingClassStudent)) && $studentDetail['Student']['graduated'] == 0  ? true : false);
												...
												<td style="background-color: white;">
													...
													if (($ethiopianStudent || $tinMandatory) /* || !empty($studentDetail['Student']['fayda_identification_number']) || !empty($studentDetail['Student']['fayda_alias_number']) */) { ?>
														<div class="large-12 columns">
															<hr>
															<br>
															<?= $this->Form->input('fayda_alias_number', array('id' => 'faydaFan', /* 'required' => (FORCE_ALL_STUDENTS_TO_FILL_FAYDA_NUMBERS == 1 ? true : false), */ 'type' => 'text', 'label' => 'Fayda FAN Number (16 digit) : &nbsp;<span class="rejected">* (Fill out this very carefully!)</span>', 'style' => 'width:100%;', 'placeholder' => 'Check the FRONT SIDE of your Fayda ID for FAN.', 'onBlur' => 'checkFaydaFan(this)')); ?>
															<br>
															<br>
														</div>
														<div class="large-12 columns">
															<?= $this->Form->input('fayda_identification_number', array('id' => 'faydaFin', /* 'required' => (FORCE_ALL_STUDENTS_TO_FILL_FAYDA_NUMBERS == 1 ? true : false), */ 'type' => 'text', 'label' => 'Fayda FIN Number (12 digit) : &nbsp;<span class="rejected">* (Fill out this very carefully!)</span>', 'style' => 'width:100%;', 'placeholder' => 'Check the BACK SIDE of Student Fayda ID for FIN.', 'onBlur' => 'checkFaydaFin(this)')); ?>
															<br>
															<hr>
														</div>
													...
													<?php
													if ($this->Session->read('Auth.User')['role_id'] == ROLE_REGISTRAR || $this->Session->read('Auth.User')['role_id'] == ROLE_ALUMNI) { ?>
														<div class="large-12 columns">
															<hr>
						...
						<div class="content" id="education_background" style="padding-left: 0px; padding-right: 0px;">
							...
							<hr> -->
							<?php
							if ((($this->Session->read('Auth.User')['role_id'] == ROLE_REGISTRAR || $this->Session->read('Auth.User')['role_id'] == ROLE_ALUMNI) && $studentDetail['Program']['id'] == PROGRAM_UNDEGRADUATE) || (!empty($this->data['HighSchoolEducationBackground']))) { ?>
							...
							if ((($this->Session->read('Auth.User')['role_id'] == ROLE_REGISTRAR || $this->Session->read('Auth.User')['role_id'] == ROLE_ALUMNI) && ($studentDetail['Program']['id'] == PROGRAM_POST_GRADUATE || $studentDetail['Program']['id'] == PROGRAM_PhD )) || (!empty($this->data['HigherEducationBackground']))) { ?>
							...
							<div class="row">
								...
								if ((($this->Session->read('Auth.User')['role_id'] == ROLE_REGISTRAR || $this->Session->read('Auth.User')['role_id'] == ROLE_ALUMNI) && $studentDetail['Program']['id'] == PROGRAM_UNDEGRADUATE && ALLOW_ESLCE_RESULTS_TO_BE_FILLED_FOR_UNDER_GRADUATE_STUDENTS == 1) || (!empty($this->data['EslceResult']))) { 
								...
								if ((/* $ethiopianStudent &&  */($this->Session->read('Auth.User')['role_id'] == ROLE_REGISTRAR || $this->Session->read('Auth.User')['role_id'] == ROLE_ALUMNI) && ($studentDetail['Program']['id'] == PROGRAM_UNDEGRADUATE)) || (isset($this->data['EheeceResult'][0]['subject']) && !empty($this->data['EheeceResult'][0]['subject']))) { 
					...
					<?= (($this->Session->read('Auth.User')['role_id'] == ROLE_REGISTRAR || $this->Session->read('Auth.User')['role_id'] == ROLE_ALUMNI) ? $this->Form->end(array('label' => 'Update Student Detail', /* 'disabled', */ 'name' => 'updateStudentDetail', 'id' => 'updateStudentDetail', 'class' => 'tiny radius button bg-blue')) : ''); ?>
...
<script type="text/javascript">
	...
	function checkFaydaFin(obj) {
		...
		if ((obj.value !== '' && !isNaN(obj.value) && obj.value.length !== 12) || (faidaMandatory && obj.value == '')) {
			...
			message.innerText = 'Please check the front side of Student Fayda ID for a valid FIN, which 12 digits long.';
	...
	function checkFaydaFan(obj) {
		...
		if ((obj.value !== '' && !isNaN(obj.value) && obj.value.length !== 16) || (faidaMandatory && obj.value == '')) {
			...
			message.innerText = 'Please check the front side of Student Fayda ID for a valid FAN, which 16 digits long.';
			message.style.position = 'absolute';
			message.style.backgroundColor = '#f8d7da';
			message.style.color = '#721c24';
			message.style.border = '1px solid #f5c6cb';
