...
<div class="smallheading"><?php echo __('Regenerate Student Academic Status');?></div>
...
<table cellspacing="0" cellpadding="0" class="fs14">
   ...
   <tr>
		<td>Semester:</td>
		<td><?php echo $this->Form->input('semester', array('options'=>array('I'=>'I','II'=>'II','III'=>'III'))); ?></td>
		
		<td>Status AcademicYear:</td>
		<td><?php echo $this->Form->input('status_acadamic_year', array('id' => 'AcadamicYear', 
		'label' => false, 'class' => 'fs14', 'style' => 'width:125px', 'type' => 'select',
		 'options' => $acyear_array_data, 
		...
		<td><?php echo $this->Form->input('name', array('id' => 'name', 'class' => 'fs14', 'label' => false)); ?></td>
	        ...
	        ?>
	         ...
	         <td>Department:</td>
	        <?php 
	        echo '<td>'.$this->Form->input('department_id',array('label'=>'false',
	         'class' => 'fs14', 'label' => false, 'type' => 'select', 'options' => $departments,
		  'default' => (isset($department_id) ? $department_id : false))).'</td>'; 
	        } else if (!empty($colleges)) {
	        ?>
	          <td>College:</td>
	        <?php 
	             // echo $this->Form->input('Student.college_id',array('label'=>'College','style'=>'width:200px'));    
	             echo '<td>'.$this->Form->input('college_id',array('label'=>'false',
	         'class' => 'fs14', 'label' => false, 'type' => 'select', 'options' => $colleges,
		  'default' => (isset($college_id) ? $college_id : false))).'</td>'; 
	...
	<tr>
		<td colspan="3">
		<?php echo $this->Form->submit(__('Get Students'), array('name' => 'listSections','class'=>'tiny radius button bg-blue', 'div' => false,'id'=>'listSections')); ?>
		</td>
		...
		<br/>
		  <strong>Note:</strong> Very expensive process, not recommended to do it via interface.
		*/ ?> 
...
'options' => $sections, 
'selected' => (!empty($section_id) && $section_id!=0 ? $section_id : 0)));
?>
		</td>
	</tr>
</table>
...
if(isset($students_in_section) && empty($students_in_section)) {
	...
	?>
	<p class="fs13">Please select student/s for whom you want to regenerate academic status.</p>
	<table>
		<tr>
		...
		foreach($students_in_section as $key => $student) {
			...
			<tr>
				<td><?php 
					echo $this->Form->input('StudentStatusPattern.'.$st_count.'.gp', array('type' => 'checkbox','class'=>'checkbox1','label' => false, 'id' => 'StudentSelection'.$st_count));
					echo $this->Form->input('StudentStatusPattern.'.$st_count.'.student_id', array('type' => 'hidden', 'value' => $student['Student']['id']));
				?></td>
	...
	<?php echo $this->Form->submit(__('Regenerate Student Status'), array('name' => 'regenerateStatus', 
...
*/
	$("#Section").change(function(){
		...
		window.location.replace("/student_status_patterns/<?php echo $this->request->action; ?>/"+s_id);
	    
	});
    
});
/*
 window.location.replace("/student_status_patterns/<?php echo $this->request->action; ?>/");
*/
