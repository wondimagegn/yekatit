...
'default'=>!isset($this->request->data['Search']['academic_year']) ? $defaultacademicyear:$this->request->data['Search']['academic_year'])).'</td></tr>';
...
echo '<span>'.$this->Form->submit('Search',array('class'=>'tiny radius button bg-blue',
...
if (!empty($courseRegistrations)) {   
    ...
    <?php 
     ...
     ?>
       <div class="smallheading"><?php echo __('Course registration between '.$this->Format->short_date($from).' and '.$this->Format->short_date($to));?></div>
       <?php 
   ...
   ?>
	<div class="smallheading"><?php echo __('Course registration Lists');?></div>
<?php 
      }
      ?>
